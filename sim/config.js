...
// CAS-2702: EMBATE (STRIKE_COMMIT_SURGE, EVO#120 DARK) — PROFUNDIZA la familia COMPOSICIÓN-DE-INTENCIÓN abierta por #118 AGGRO-FOCUS con una SUB-PARTICIÓN de la manada ENGANCHADA por SUB-ESTADO de combate. #118 AGGRO-FOCUS mide QUÉ FRACCIÓN de la manada VIVA está fijada/enganchada en el héroe (targeting); EMBATE mide, DENTRO de esa manada YA enganchada, qué FRACCIÓN está en pleno ATAQUE COMPROMETIDO {windup, strike} vs meramente merodeando en aggro {chase, recover, shield}. MÉTRICA = strikeCommit SC∈[0,1] = |{mobs en windup|strike}| / |{mobs aggro-ENGANCHADOS}| sobre la manada VIVA en radio, minMobs3 (sobre el DENOMINADOR ENGANCHADO), band hi/mid/low. SC≈1 ⇒ toda la manada enganchada está mid-golpe (lluvia de telegraphs simultáneos); SC≈0 ⇒ la manada te persigue/rodea pero NINGUNO ha comprometido el golpe (todos chase/shield). Es una FRACCIÓN AGREGADA server-auth sobre el DENOMINADOR ENGANCHADO (NO el vivo — ésa es la CLAVE ⊥#118), INTENSIVA (invariante al conteo), DETERMINISTA, lee el MISMO FSM de IA server-auth (e.state) que #118. 🔑 SUB-ESTADO server-auth: engaged=aggroEngaged(e) {chase,windup,strike,recover,shield} (reusa el predicado de #118); committed={windup,strike} (los dos sub-estados de ATAQUE con hitbox activa/telegraph — MISMO FSM que WINDUP_SURGE #75 / RECOVER_SURGE #76 leen, replicado). Los NO-comprometidos {chase,recover,shield} cuentan en el DENOMINADOR (están enganchados) pero NUNCA en el numerador (aún no han comprometido el golpe). Un ESCALAR de COMPOSICIÓN-DE-INTENCIÓN de 2º NIVEL (fracción-DENTRO-de-la-fracción) — ORTOGONAL a las 61 flags. Camino al 62º flag (serializa tras #119 JERK_DIR_SURGE LIVE&served e0f65ada9e43/815, base HEAD e5de531).
//  · PRE-FLIGHT ⊥#118 AGGRO-FOCUS (LA CRÍTICA — DENOMINADORES DISTINTOS ⇒ disocian en el ratio): AGGRO-FOCUS F=enganchados/VIVOS (fracción de targeting sobre la manada VIVA); EMBATE SC=comprometidos/ENGANCHADOS (fracción de compromiso-de-golpe DENTRO de la enganchada). 4-CUAD: (a) manada TODA-enganchada en `chase` ⇒ focus HIGH (todos vivos enganchados) PERO strikeCommit 0 (0 de los enganchados en windup|strike); (b) 3-mob pack TODO en `strike` ⇒ focus HIGH (3/3 vivos enganchados) Y strikeCommit HIGH (3/3 enganchados golpeando) — co-mueven aquí; (c) manada mitad-enganchada donde los enganchados TODOS golpean: p.ej. 2 strike + 2 idle ⇒ focus 0.5 (2 enganchados/4 vivos) PERO strikeCommit 1.0 (2/2 enganchados golpeando) ⇒ **focus BAJO / strikeCommit ALTO**; (d) manada dispersa/idle ⇒ focus 0 Y strikeCommit 0 (denominador enganchado=0 ⇒ indefinido ⇒ 0). ⇒ focus mapea a strikeCommit 0 (a) Y 1 (c) ⇒ strikeCommit NO es función de focus: DISOCIAN en el RATIO por el DENOMINADOR (vivos vs enganchados). Una manada con MÁXIMO focus (toda enganchada en chase) tiene strikeCommit=0.
//  · ⊥#67 FRENZY/#94 SWIFT (composición de ESTADO, NO kill-rate/ventana temporal — strikeCommit puede ser 100% con 0 kills, p.ej. toda la manada whiffeando/errando el golpe simultáneo). ⊥ 2º-orden CINÉTICO #117 ACCEL/#119 JERK-SPLIT (sub-estado de combate ≠ churn de movimiento: una manada puede regatear salvajemente [jerk alto] 0%-comprometida [toda en chase] ⇒ strikeCommit0/jerk2; y quedarse QUIETA golpeando [jerk≈0, windup/strike estacionarios] 100%-comprometida ⇒ strikeCommit2/jerk0). ⊥ velocidad-1er-orden #110/#111/#112/#116 (fracción de sub-estado ⊥ rumbo/rapidez/convergencia/órbita: un embiste-frontal-coordinado en `chase` ⇒ converge2/strikeCommit0; una manada en windup estacionaria ⇒ converge0[sin traslación]/strikeCommit2). ⊥ ATRIBUTO #115/#88/#106 (sub-estado dinámico ≠ template estático). ⊥ GEOM/POSICIÓN #107/#108/#109/#113/#114 (un anillo ESTACIONARIO todo en windup ⇒ encircle2 Y strikeCommit2 CO-OCURREN pero mismo anillo en chase ⇒ encircle2/strikeCommit0 ⇒ la posición NO determina el sub-estado). ⊥#87 PACKHARVEST (FRACCIÓN invariante al conteo: 2-de-2 y 5-de-5 enganchados golpeando ⇒ MISMO SC=1). ⊥ dynamic-state #83/#85/#86: el frost-slow #85 escala velocidad/timing NO el sub-estado del FSM ⇒ un pack slowed en windup SIGUE en windup ⇒ strikeCommit INDEPENDIENTE del slow (mide el RATIO de compromiso per-enganchado, no la duración). Canal FRESCO `strikeCommitFind` — NINGUNA de las 61 flags #59-#119 lo usa; recurso TRANSITORIO NUEVO h.strikeCommitBounty, STATELESS (fuera del save allowlist + worldFingerprint), sub-cap propio strikeCommitBountyCap, badge ↯.
export const STRIKE_COMMIT_SURGE = {
  enabled: true,            // DARK (EVO#120, 62º flag pendiente). enabled:false ⇒ byte-neutral OFF (funciones de lectura PURAS gated + seam rama muerta + _strikeCommitPre=0 const inerte + badge NO despachado + h.strikeCommitBounty NUNCA creado). SNAPSHOT PURO (sin captura per-tick / sin update-gate, como #118 AGGRO-FOCUS — lee e.state directo) ⇒ 0 estado nuevo. Flip config-only false→true tras CEO Gate 2/2.
  channel: "strikeCommitFind", // canal FRESCO (fichas de embate por rematar en medio de una manada enganchada mid-golpe COMPROMETIDO — la FRACCIÓN en windup|strike DENTRO de la enganchada) — NINGUNA de las 61 flags #59-#119 lo usa. Distinto por CONSTRUCCIÓN de aggroFocusFind #118 (fracción sobre VIVOS): EMBATE = la sub-fracción de compromiso-de-golpe sobre el DENOMINADOR ENGANCHADO. Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio de CONSIDERACIÓN (px) de la vecindad VIVA: SC se computa sobre los mobs VIVOS dentro del radio del héroe (enganchados = denominador, comprometidos = numerador). Mirror #87/…/#119 radius. CEO balance knob.
  minMobs: 3,                // nº MÍNIMO de mobs aggro-ENGANCHADOS en radio (DENOMINADOR) para que la fracción esté DEFINIDA: con <3 enganchados es degenerada ⇒ 0. ≥3 enganchados ⇒ una manada genuina en aggro. Con el filtro de muertos (la víctima ya tiene e.dead=true en el TOP ⇒ NO cuenta) ⇒ ANTI-AUTO-CONTEO. CEO balance knob.
  midCommit: 0.34,           // umbral de SC para la banda PRESSING/embate parcial (peso 1): SC≥0.34 (≈1/3 de la manada enganchada comprometiendo el golpe). Adimensional ∈[0,1]. CEO balance knob.
  hiCommit: 0.67,            // umbral de SC para la banda ONSLAUGHT/embate coordinado (peso 2): SC≥0.67 (≈2/3+ de la manada enganchada mid-golpe simultáneo). SC<midCommit (manada enganchada pero merodeando/sin comprometer) ⇒ 0. Umbrales sobre SC adimensional ∈[0,1]. CEO balance knob.
  weights: { onslaught: 2, pressing: 1 },   // PESO de un remate por la BANDA de FRACCIÓN de COMPROMISO de la manada ENGANCHADA VIVA en radio: ONSLAUGHT/embate coordinado (SC ≥hiCommit) pesa 2; PRESSING/embate parcial (≥midCommit) pesa 1; sin comprometer (<midCommit) ⇒ 0. Reward = rematar en medio de una lluvia de golpes comprometidos, no una manada que te persigue sin atacar. CEO balance knobs.
  strikeCommitBountyCap: 2,  // SUB-CAP DURO PROPIO de las fichas de embate por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.onslaught)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE FRACCIÓN DE COMPROMISO (score∈{0,1,2}). Más manada enganchada mid-golpe = tier MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (manada enganchada sin comprometer/solitario) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate en PRESSING/embate parcial, SC≥midCommit): +1 ficha de embate por kill. "remataste con ~1/3 de la manada enganchada comprometiendo el golpe".
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate en pleno ONSLAUGHT/embate coordinado, SC≥hiCommit): +2 fichas por kill. "remataste bajo una lluvia de golpes comprometidos simultáneos". CEO balance knobs.
  ],
};

// CAS-2706: DISPERSIÓN DE OBJETIVOS (TARGET_SPREAD_SURGE, EVO#121 DARK) — AÑADE la dimensión de DISTRIBUCIÓN-DE-OBJETIVOS a la familia COMPOSICIÓN-DE-INTENCIÓN (#118 AGGRO-FOCUS = fracción del pack fijada en el HÉROE; #120 EMBATE = fracción del enganchado mid-golpe COMPROMETIDO). DISPERSIÓN mide qué tan UNIFORMEMENTE reparte la manada aggro-ENGANCHADA su targeting sobre TODOS los jugadores de la party VIVA en radio. MÉTRICA = spread S∈[0,1] = ENTROPÍA de SHANNON NORMALIZADA de la distribución de objetivos H(targets)/ln(nPlayers): por cada jugador j de la party se cuenta cⱼ = #mobs-enganchados que targetean a j, N=Σcⱼ, pⱼ=cⱼ/N, H=−Σpⱼ·ln(pⱼ), S=H/ln(P) con P=#jugadores VIVOS en radio. S≈1 ⇒ la manada reparte su aggro UNIFORME sobre toda la party (todos igual de amenazados); S≈0 ⇒ toda la manada CONCENTRA su aggro en UN solo jugador (foco único, sea el héroe o no). minMobs3 (sobre el DENOMINADOR ENGANCHADO), minPlayers2 (spread INDEFINIDO con <2 jugadores). 🔑 INTRÍNSECAMENTE MULTIJUGADOR: single-player ⇒ P=1 ⇒ ln(1)=0 ⇒ degenerado ⇒ S=0 (colapso LIMPIO). Server-authoritative sobre la aggro-table COMPARTIDA (lee el aiTarget/aggro-ref de cada mob — NO un guess de cliente); en single-player el ÚNICO jugador es el héroe ⇒ todo el aggro concentra en él ⇒ S=0. FRACCIÓN AGREGADA INTENSIVA (invariante al conteo de mobs), DETERMINISTA, mismo aggro-table para todo cliente. Un ESCALAR de COMPOSICIÓN-DE-INTENCIÓN sobre la DISTRIBUCIÓN de objetivos — ORTOGONAL a las 62 flags. Camino al 63º flag (serializa tras #120 STRIKE_COMMIT_SURGE LIVE&served 17b956823d40/815, base HEAD ff88293).
//  · PRE-FLIGHT ⊥#118 AGGRO-FOCUS (LA CRÍTICA — DISTRIBUCIÓN vs NIVEL-sobre-el-héroe ⇒ disocian): AGGRO-FOCUS F = fracción del pack targeteando al HÉROE específicamente; DISPERSIÓN S = cuán UNIFORME reparte el pack su aggro sobre TODA la party. 4-CUAD (índice 0 = héroe): (a) pack 100%-enganchado TODO sobre el héroe ⇒ counts=[N,0,0,0] ⇒ focus MAX(=1) / spread 0 (concentrado en 1 = héroe); (b) pack repartido UNIFORME sobre 4 jugadores ⇒ counts=[k,k,k,k] ⇒ H=ln4 ⇒ spread MAX(=1) / focus BAJO (=1/4); (c) pack TODO sobre UN jugador NO-héroe ⇒ counts=[0,N,0,0] ⇒ focus 0 / spread 0 (concentrado, sólo que NO en el héroe — AMBOS bajos por RAZONES DISTINTAS); (d) pack idle ⇒ N=0 ⇒ ambos 0. ⇒ focus mapea a spread 0 en (a) [focus1] Y (c) [focus0] Y a spread MAX en (b) [focus0.25] ⇒ spread NO es función de focus: DISOCIAN. El cuadrante (c) "todo-sobre-uno-no-héroe" es la clave: AMBOS bajos pero focus=0 por no-héroe, spread=0 por concentración; y (b) "reparto-uniforme" spread MAX mientras focus es sólo 1/nPlayers.
//  · ⊥#120 STRIKE-COMMITMENT (DISTRIBUCIÓN-de-objetivo vs SUB-ESTADO-de-combate): spread es sobre a QUIÉN apuntan; EMBATE sobre en qué SUB-ESTADO {windup|strike} están. Un pack 100%-comprometido TODO-sobre-héroe ⇒ strikeCommit MAX / spread 0; un pack 0%-comprometido (todo chase) repartido uniforme sobre 4 jugadores ⇒ strikeCommit 0 / spread MAX ⇒ INDEPENDIENTES.
//  · ⊥#67 FRENZY/#94 SWIFT (una RATIO de distribución, NO kill-rate/ventana temporal — spread MAX con 0 kills). ⊥ 2º-orden CINÉTICO #117 ACCEL/#119 JERK-SPLIT (distribución de objetivos ≠ churn de movimiento). ⊥ velocidad-1er-orden #110/#111/#112/#116 (independiente de rumbo/rapidez/convergencia/órbita). ⊥ ATRIBUTO #115/#88/#106 (distribución dinámica ≠ template estático). ⊥ GEOM/POSICIÓN #107/#108/#109/#113/#114 (a QUIÉN apuntan ≠ dónde están: un anillo puede concentrar en 1 jugador [spread0] o repartir sobre la party [spreadMAX] con la MISMA geometría). ⊥#87 PACKHARVEST (ENTROPÍA NORMALIZADA invariante al conteo: 2-y-2 y 5-y-5 ⇒ MISMO S=1). ⊥ dynamic-state #83/#85/#86: el frost-slow #85 escala velocidad/timing NO a QUIÉN targetea ⇒ spread INDEPENDIENTE del slow. 🔑 requiere ≥2 jugadores para estar DEFINIDO ⇒ single-player degenera a 0 LIMPIO; métrica BIEN-DEFINIDA multijugador (verificada vía spreadProbe LUT pura de entropía + driveSpread party sintética). Canal FRESCO `targetSpreadFind` — NINGUNA de las 62 flags #59-#120 lo usa; recurso TRANSITORIO NUEVO h.targetSpreadBounty, STATELESS (fuera del save allowlist + worldFingerprint), sub-cap propio targetSpreadBountyCap, badge ⋔.
export const TARGET_SPREAD_SURGE = {
  enabled: true,             // LIVE (EVO#121, 63º flag). CAS-2709 config-only flip false→true tras CEO Gate 2/2 (DARK QA CAS-2707 PASS 18/18). enabled:false ⇒ byte-neutral OFF (funciones de lectura PURAS gated + seam rama muerta + _targetSpreadPre=0 const inerte + badge NO despachado + h.targetSpreadBounty NUNCA creado). SNAPSHOT PURO (sin captura per-tick, lee la aggro-table directo, como #118 AGGRO-FOCUS / #120 EMBATE) ⇒ 0 estado nuevo. Flip config-only false→true tras CEO Gate 2/2.
  channel: "targetSpreadFind", // canal FRESCO (fichas de dispersión por rematar en medio de una manada que REPARTE su aggro sobre TODA la party — la ENTROPÍA de la distribución de objetivos) — NINGUNA de las 62 flags #59-#120 lo usa. Distinto por CONSTRUCCIÓN de aggroFocusFind #118 (fracción sobre el HÉROE) / strikeCommitFind #120 (sub-estado): DISPERSIÓN = la UNIFORMIDAD del reparto de objetivos sobre los jugadores. Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio de CONSIDERACIÓN (px) de la vecindad VIVA: S se computa sobre los mobs VIVOS ENGANCHADOS y los jugadores VIVOS dentro del radio del héroe. Mirror #87/…/#120 radius. CEO balance knob.
  minMobs: 3,                // nº MÍNIMO de mobs aggro-ENGANCHADOS en radio (DENOMINADOR N) para que la distribución esté DEFINIDA: con <3 es degenerada ⇒ 0. ≥3 ⇒ una manada genuina en aggro. Con el filtro de muertos (la víctima ya tiene e.dead=true en el TOP ⇒ NO cuenta) ⇒ ANTI-AUTO-CONTEO. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores VIVOS en radio (P) para que la entropía normalizada esté DEFINIDA: con <2 (single-player) ⇒ ln(P)=0 ⇒ degenerado ⇒ 0 (colapso single-player LIMPIO). ≥2 ⇒ party genuina ⇒ distribución bien-definida. 🔑 la dispersión es INTRÍNSECAMENTE multijugador. CEO balance knob.
  midSpread: 0.34,           // umbral de S para la banda DIVIDED/reparto parcial (peso 1): S≥0.34 (≈1/3 de la entropía máxima — el aggro empieza a repartirse). Adimensional ∈[0,1]. CEO balance knob.
  hiSpread: 0.67,            // umbral de S para la banda SCATTERED/reparto amplio (peso 2): S≥0.67 (≈2/3+ de la entropía máxima — aggro muy repartido sobre la party). S<midSpread (aggro concentrado en 1-2 jugadores) ⇒ 0. Umbrales sobre S adimensional ∈[0,1]. CEO balance knob.
  weights: { scattered: 2, divided: 1 },   // PESO de un remate por la BANDA de ENTROPÍA de la distribución de objetivos de la manada ENGANCHADA VIVA en radio: SCATTERED/reparto amplio (S ≥hiSpread) pesa 2; DIVIDED/reparto parcial (≥midSpread) pesa 1; concentrado en 1 jugador (<midSpread) ⇒ 0. Reward = rematar en medio de una manada cuyo aggro está REPARTIDO sobre toda la party, no una que fija todo su fuego en un solo blanco. CEO balance knobs.
  targetSpreadBountyCap: 2,  // SUB-CAP DURO PROPIO de las fichas de dispersión por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.scattered)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE ENTROPÍA DE DISTRIBUCIÓN (score∈{0,1,2}). Aggro más repartido = tier MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (aggro concentrado/single-player) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate en DIVIDED/reparto parcial, S≥midSpread): +1 ficha de dispersión por kill. "remataste con el aggro de la manada empezando a repartirse".
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate en pleno SCATTERED/reparto amplio, S≥hiSpread): +2 fichas por kill. "remataste con la manada repartiendo su aggro sobre toda la party". CEO balance knobs.
  ],
};

// CAS-2712: VAIVÉN DE OBJETIVOS (AGGRO_SWITCH_SURGE, EVO#122 DARK) — AÑADE la dimensión TEMPORAL/CHURN a la familia COMPOSICIÓN-DE-INTENCIÓN (#118 AGGRO-FOCUS = fracción sobre el HÉROE, un NIVEL; #120 EMBATE = sub-estado de combate; #121 REPARTO = ENTROPÍA de la distribución de objetivos, un SNAPSHOT). VAIVÉN mide qué tan VOLÁTIL es el targeting de la manada ENGANCHADA a lo largo de una VENTANA reciente: la FRACCIÓN de mobs enganchados cuyo objetivo (aiTarget) CAMBIÓ respecto a hace W ticks. MÉTRICA = switch V∈[0,1] = (#mobs enganchados con e._swHas cuyo aiTarget ACTUAL ≠ su aiTarget de hace W ticks) / (#mobs enganchados con historia). W = window ticks; cada mob enganchado mantiene un ring-buffer server-auth de su propio aiTarget (e._swR, poblado por switchTick tras updateEnemies). V≈1 ⇒ la manada re-apunta frenéticamente (churn ALTO, targeting volátil); V≈0 ⇒ la manada mantiene su objetivo fijo (churn 0, sea cual sea la FORMA actual de la distribución). minMobs3 (enganchados con historia), minPlayers2 (el switch sólo es significativo con ≥2 opciones de objetivo). 🔑 INTRÍNSECAMENTE MULTIJUGADOR: single-player ⇒ el ÚNICO objetivo es el héroe ⇒ el aiTarget NUNCA cambia ⇒ V=0 (colapso LIMPIO); cold-start ⇒ ring-buffer no lleno (e._swHas=false) ⇒ ese mob NO cuenta ⇒ con <minMobs con historia ⇒ 0. Server-authoritative sobre la aggro-table history COMPARTIDA (compara el aiTarget ACTUAL de cada mob con un ring-buffer server-auth de su propio aiTarget PREVIO — NO un guess de cliente). CAPTURA TEMPORAL como #117 ACCEL / #119 JERK-SPLIT: historia per-mob TRANSITORIA (enemigos NUNCA serializados ⇒ fuera del save + worldFingerprint), poblada SÓLO cuando enabled ⇒ 2-cliente evoluciona idéntico (0-desync). FRACCIÓN AGREGADA INTENSIVA (invariante al conteo), DETERMINISTA. Un ESCALAR de COMPOSICIÓN-DE-INTENCIÓN sobre el CHURN TEMPORAL del targeting — ORTOGONAL a las 63 flags. Camino al 64º flag (serializa tras #121 TARGET_SPREAD_SURGE LIVE&served 3767e248c042/815, base HEAD 03e60db7).
//  · PRE-FLIGHT ⊥#121 REPARTO (LA CRÍTICA — CHURN-TEMPORAL vs FORMA-SNAPSHOT ⇒ disocian): REPARTO S = qué tan UNIFORME es la distribución de objetivos AHORA (un snapshot estático de a QUIÉN apuntan); VAIVÉN V = cuánto CAMBIA esa distribución en el tiempo (el churn, sin importar su forma actual). 4-CUAD: (a) pack repartido UNIFORME sobre 4 Y estable ⇒ spread ALTO / switch 0; (b) pack TODO-sobre-uno pero ROTANDO cuál cada tick ⇒ spread ≈0 (concentrado cada tick) / switch ALTO; (c) pack repartido uniforme Y re-rolleando objetivos ⇒ spread ALTO / switch ALTO; (d) pack fijo TODO-sobre-héroe sin cambiar ⇒ spread 0 / switch 0. ⇒ spread mapea a switch 0 en (a) [spread ALTO] Y (d) [spread 0], y a switch ALTO en (b) [spread ≈0] Y (c) [spread ALTO] ⇒ switch NO es función de spread: DISOCIAN. Los cuadrantes CLAVE: (b) "concentrado-pero-rotando" (spread bajo, switch alto) y (a) "uniforme-pero-estable" (spread alto, switch bajo).
//  · ⊥#118 AGGRO-FOCUS (NIVEL-estático vs TASA-de-cambio): FOCUS = fracción sobre el héroe (un NIVEL); VAIVÉN = tasa de CAMBIO de objetivo (temporal). Un pack 100%-sobre-el-héroe SIN cambiar ⇒ focus ALTO / switch 0; un pack re-apuntando constantemente lejos-y-de-vuelta al héroe ⇒ focus mid / switch ALTO ⇒ INDEPENDIENTES.
//  · ⊥#120 EMBATE (sub-estado de combate vs identidad-de-objetivo): COMPROMISO = sub-estado {windup|strike}; VAIVÉN = churn de la IDENTIDAD del objetivo. Un pack 100%-mid-golpe TODO fijo ⇒ commit ALTO / switch 0; un pack 0%-comprometido re-apuntando frenético ⇒ commit 0 / switch ALTO.
//  · ⊥ 2º-orden CINÉTICO #117 ACCEL/#119 JERK-SPLIT (IMPORTANTE — ambos son métricas de "cambio"): ACCEL/JERK = el churn del MOVIMIENTO (velocidad/rumbo en el tiempo); VAIVÉN = el churn de la IDENTIDAD del OBJETIVO (a quién apuntan en el tiempo). Un pack acelerando duro SIN cambiar de objetivo ⇒ accel ALTO / switch 0; un pack a velocidad CONSTANTE re-apuntando cada tick ⇒ accel 0 / switch ALTO ⇒ cambio-de-identidad ≠ cambio-de-movimiento. (Historia per-mob SEPARADA: #117 usa e._accPm, #119 e._jrkPm, #122 e._swR.)
//  · ⊥#67 FRENZY/#94 SWIFT (una RATIO de churn-de-objetivo, NO kill-rate/ventana-de-kill/buff-de-velocidad — switch MAX con 0 kills). ⊥ velocidad-1er-orden #110/#111/#112/#116 (independiente de rumbo/rapidez/convergencia/órbita). ⊥ ATRIBUTO #115/#88/#106 (churn dinámico ≠ template estático). ⊥ GEOM/POSICIÓN #107/#108/#109/#113/#114 (a QUIÉN apuntan-en-el-tiempo ≠ dónde están). ⊥#87 PACKHARVEST (FRACCIÓN invariante al conteo). ⊥ dynamic-state #83/#85/#86: el frost-slow #85 escala velocidad/timing NO a QUIÉN targetea ⇒ switch INDEPENDIENTE del slow. 🔑 requiere ≥2 jugadores Y ≥2 ticks de historia para estar DEFINIDO ⇒ single-player degenera a 0 LIMPIO (aiTarget SIEMPRE el héroe ⇒ nunca cambia); cold-start degenera a 0 LIMPIO (buffer no lleno); métrica BIEN-DEFINIDA multijugador (verificada vía switchProbe LUT pura + driveSwitch party sintética con SCHEDULE de objetivos step-por-tick). Canal FRESCO `aggroSwitchFind` — NINGUNA de las 63 flags #59-#121 lo usa; recurso TRANSITORIO NUEVO h.aggroSwitchBounty, STATELESS (fuera del save allowlist + worldFingerprint), sub-cap propio aggroSwitchBountyCap, badge ⇄.
export const AGGRO_SWITCH_SURGE = {
  enabled: true,             // LIVE (EVO#122, 64º flag CAS-2714). enabled:false ⇒ byte-neutral OFF (switchTick NUNCA corre + funciones de lectura PURAS gated + seam rama muerta + _aggroSwitchPre=0 const inerte + badge NO despachado + h.aggroSwitchBounty NUNCA creado + historia per-mob e._swR NUNCA creada). CAPTURA TEMPORAL (ring-buffer per-mob del aiTarget, como #117 ACCEL / #119 JERK-SPLIT). Flip config-only false→true tras CEO Gate 2/2.
  channel: "aggroSwitchFind", // canal FRESCO (fichas de vaivén por rematar en medio de una manada que RE-APUNTA frenéticamente su aggro — el CHURN TEMPORAL del targeting) — NINGUNA de las 63 flags #59-#121 lo usa. Distinto por CONSTRUCCIÓN de aggroFocusFind #118 (nivel sobre el HÉROE) / strikeCommitFind #120 (sub-estado) / targetSpreadFind #121 (forma de la distribución AHORA): VAIVÉN = la VOLATILIDAD de la distribución en el TIEMPO. Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio de CONSIDERACIÓN (px) de la vecindad VIVA: V se computa sobre los mobs VIVOS ENGANCHADOS con historia y los jugadores VIVOS dentro del radio del héroe. Mirror #87/…/#121 radius. CEO balance knob.
  window: 6,                 // VENTANA W (ticks) del ring-buffer per-mob de aiTarget: el aiTarget ACTUAL se compara con el de hace W ticks. El buffer guarda W+1 muestras (t−W..t); mob con <W+1 muestras (cold-start) ⇒ e._swHas=false ⇒ NO cuenta. Más grande = ventana de churn más larga. CEO balance knob.
  minMobs: 3,                // nº MÍNIMO de mobs aggro-ENGANCHADOS CON historia (e._swHas) en radio (DENOMINADOR) para que el churn esté DEFINIDO: con <3 es degenerado ⇒ 0. ≥3 ⇒ una manada genuina en aggro con historia. Con el filtro de muertos (la víctima ya tiene e.dead=true en el TOP ⇒ NO cuenta) ⇒ ANTI-AUTO-CONTEO. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores VIVOS en radio (P) para que el switch esté DEFINIDO: con <2 (single-player) ⇒ sólo el héroe como objetivo ⇒ el aiTarget NUNCA puede cambiar ⇒ V=0 (colapso single-player LIMPIO). ≥2 ⇒ ≥2 opciones de objetivo ⇒ el switch es significativo. 🔑 el vaivén es INTRÍNSECAMENTE multijugador. CEO balance knob.
  midSwitch: 0.34,           // umbral de V para la banda SHIFTING/vaivén parcial (peso 1): V≥0.34 (≈1/3 de la manada re-apuntó en la ventana). Adimensional ∈[0,1]. CEO balance knob.
  hiSwitch: 0.67,            // umbral de V para la banda VOLATILE/vaivén amplio (peso 2): V≥0.67 (≈2/3+ de la manada re-apuntó en la ventana — targeting muy volátil). V<midSwitch (targeting estable) ⇒ 0. Umbrales sobre V adimensional ∈[0,1]. CEO balance knob.
  weights: { volatile: 2, shifting: 1 },   // PESO de un remate por la BANDA de CHURN de la manada ENGANCHADA VIVA en radio: VOLATILE/vaivén amplio (V ≥hiSwitch) pesa 2; SHIFTING/vaivén parcial (≥midSwitch) pesa 1; estable (<midSwitch) ⇒ 0. Reward = rematar en medio de una manada que re-apunta frenéticamente su aggro, no una que mantiene su objetivo fijo. CEO balance knobs.
  aggroSwitchBountyCap: 2,   // SUB-CAP DURO PROPIO de las fichas de vaivén por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.volatile)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE CHURN TEMPORAL (score∈{0,1,2}). Targeting más volátil = tier MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (targeting estable/single-player/cold-start) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate en SHIFTING/vaivén parcial, V≥midSwitch): +1 ficha de vaivén por kill. "remataste con la manada empezando a re-apuntar su aggro".
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate en pleno VOLATILE/vaivén amplio, V≥hiSwitch): +2 fichas por kill. "remataste con la manada re-apuntando frenéticamente su aggro". CEO balance knobs.
  ],
};

// CAS-2716: COBERTURA DE OBJETIVOS (AGGRO_CONTEST_SURGE, EVO#123 DARK) — AÑADE la dimensión de COBERTURA/AMPLITUD a la familia COMPOSICIÓN-DE-INTENCIÓN (#118 AGGRO-FOCUS = fracción sobre el HÉROE, un NIVEL; #120 EMBATE = sub-estado de combate; #121 REPARTO = ENTROPÍA de la distribución sobre el conjunto YA-targeteado, la UNIFORMIDAD; #122 VAIVÉN = CHURN TEMPORAL de la identidad del objetivo). COBERTURA mide CUÁNTOS jugadores DISTINTOS la manada ENGANCHADA está targeteando EN ABSOLUTO (la AMPLITUD del conjunto targeteado), sin importar cuán UNIFORME (spread #121) ni cuán volátil (switch #122) sea. MÉTRICA = contest C∈[0,1] = (#jugadores VIVOS en radio targeteados por ≥1 mob ENGANCHADO) / (#jugadores VIVOS en radio). Cada jugador con aggro de ≥1 mob cuenta UNA vez (breadth booleana, NO ponderada por cuántos mobs) ⇒ INDIFERENTE al reparto/pile. minMobs3 (enganchados, DENOMINADOR de significancia), minPlayers2 (la cobertura sólo es significativa con ≥2 jugadores). 🔑 INTRÍNSECAMENTE MULTIJUGADOR: single-player ⇒ P=1 ⇒ sólo el héroe puede estar cubierto ⇒ C∈{0,1} trivial ⇒ el guard minPlayers2 lo colapsa a 0 (LIMPIO); sin mobs enganchados ⇒ 0. Server-authoritative aggregate sobre la aggro-table COMPARTIDA (lee la tabla directo, como #121 — SNAPSHOT PURO, SIN buffer temporal, más simple/seguro que #122). FRACCIÓN AGREGADA INTENSIVA (invariante al conteo de MOBS), DETERMINISTA. Un ESCALAR de COMPOSICIÓN-DE-INTENCIÓN sobre la AMPLITUD de la cobertura de objetivos — ORTOGONAL a las 64 flags. Camino al 65º flag (serializa tras #122 AGGRO_SWITCH_SURGE LIVE&served 6697409ed3ae/815, base HEAD 5a87b06).
//  · PRE-FLIGHT ⊥#121 REPARTO (LA CRÍTICA — AMPLITUD-del-conjunto vs UNIFORMIDAD-dentro-del-conjunto ⇒ disocian): REPARTO S = qué tan UNIFORME es la distribución de aggro ENTRE los jugadores que SÍ están targeteados (entropía sobre el conjunto targeteado); COBERTURA C = CUÁNTOS jugadores están targeteados EN ABSOLUTO (amplitud del conjunto), sin importar la uniformidad. 4-CUAD (party de 4): (a) 2-de-4 cada uno por la mitad de la manada ⇒ contest 0.5 / spread MAX (uniforme ENTRE los 2 targeteados); (b) los 4 targeteados pero 97% de mobs sobre uno ⇒ contest 1.0 / spread BAJO (cobertura total, muy desigual); (c) TODA la manada sobre exactamente 1-de-4 ⇒ contest 0.25 / spread 0 (1 solo target ⇒ entropía 0); (d) los 4 targeteados uniforme ⇒ contest 1.0 / spread MAX. ⇒ contest mapea a spread MAX en (a) [contest 0.5] Y (d) [contest 1.0], y a spread BAJO/0 en (b) [contest 1.0] Y (c) [contest 0.25] ⇒ contest NO es función de spread: DISOCIAN. Los cuadrantes CLAVE: (a) "medio-cubierto-pero-uniforme" (contest bajo, spread MAX) y (b) "todo-cubierto-pero-pileado" (contest MAX, spread bajo).
//  · ⊥#122 AGGRO-SWITCH (AMPLITUD-estática-AHORA vs CHURN-TEMPORAL de la identidad): SWITCH = cuánto CAMBIA la identidad del objetivo en el tiempo; COBERTURA = cuántos jugadores están cubiertos ESTE tick. Una manada puede cubrir a los 4 y NUNCA cambiar (contest ALTO / switch 0), o cubrir a 1 solo mientras re-rollea frenético CUÁL mob lo targetea (contest BAJO / switch ALTO) ⇒ INDEPENDIENTES.
//  · ⊥#118 AGGRO-FOCUS (fracción-sobre-el-HÉROE vs amplitud-sobre-TODOS): FOCUS = fracción de mobs sobre el héroe específicamente; COBERTURA = amplitud sobre TODOS los jugadores, NO hero-específica. Una manada repartida sobre 4 jugadores NO-héroe ⇒ contest ALTO / focus 0.
//  · ⊥#120 EMBATE (sub-estado de combate vs amplitud-de-objetivo): COMPROMISO = sub-estado {windup|strike}; COBERTURA = amplitud del targeting. Independientes.
//  · ⊥ 2º-orden CINÉTICO #117 ACCEL/#119 JERK-SPLIT (churn de MOVIMIENTO, no de cobertura de objetivos). ⊥#67 FRENZY/#94 SWIFT (una RATIO de cobertura, NO kill-rate/buff-de-velocidad — contest MAX con 0 kills). ⊥ velocidad-1er-orden #110/#111/#112/#116. ⊥#87 PACKHARVEST (amplitud de JUGADORES targeteados, NO conteo de MOBS — C invariante al conteo de mobs). ⊥ ATRIBUTO #115/#88/#106. ⊥ GEOM/POSICIÓN #107/#108/#109/#113/#114 (a QUIÉN apuntan ≠ dónde están). ⊥ dynamic-state #83/#85/#86 (el frost-slow #85 escala velocidad NO a cuántos jugadores targetea). 🔑 requiere ≥2 jugadores para estar DEFINIDO ⇒ single-player degenera a 0 LIMPIO (P=1 ⇒ sólo el héroe ⇒ C trivial ⇒ guard minPlayers2 ⇒ 0); sin mobs enganchados degenera a 0 LIMPIO; métrica BIEN-DEFINIDA multijugador (verificada vía contestProbe LUT pura + driveContest party sintética con targets inyectados). Canal FRESCO `aggroContestFind` — NINGUNA de las 64 flags #59-#122 lo usa; recurso TRANSITORIO NUEVO h.aggroContestBounty, STATELESS (fuera del save allowlist + worldFingerprint), sub-cap propio aggroContestBountyCap, badge ⇉ "Cobertura".
export const AGGRO_CONTEST_SURGE = {
  enabled: true,             // LIVE (EVO#123, 65º flag CAS-2718). enabled:false ⇒ byte-neutral OFF (funciones de lectura PURAS gated + seam rama muerta + _aggroContestPre=0 const inerte + badge NO despachado + h.aggroContestBounty NUNCA creado + G._acParty test-scaffolding NUNCA creado). SNAPSHOT PURO (lee la aggro-table directo, SIN buffer temporal, como #121). Flip config-only false→true tras CEO Gate 2/2.
  channel: "aggroContestFind", // canal FRESCO (fichas de cobertura por rematar en medio de una manada que targetea a MUCHOS jugadores distintos — la AMPLITUD de la cobertura) — NINGUNA de las 64 flags #59-#122 lo usa. Distinto por CONSTRUCCIÓN de aggroFocusFind #118 (nivel sobre el HÉROE) / strikeCommitFind #120 (sub-estado) / targetSpreadFind #121 (UNIFORMIDAD dentro del conjunto targeteado) / aggroSwitchFind #122 (CHURN TEMPORAL): COBERTURA = la AMPLITUD del conjunto targeteado AHORA. Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio de CONSIDERACIÓN (px) de la vecindad VIVA: C se computa sobre los mobs VIVOS ENGANCHADOS y los jugadores VIVOS dentro del radio del héroe. Mirror #87/…/#122 radius. CEO balance knob.
  minMobs: 3,                // nº MÍNIMO de mobs aggro-ENGANCHADOS en radio (DENOMINADOR de significancia) para que la cobertura sea significativa: con <3 es degenerado ⇒ 0. ≥3 ⇒ una manada genuina en aggro. Con el filtro de muertos (la víctima ya tiene e.dead=true en el TOP ⇒ NO cuenta) ⇒ ANTI-AUTO-CONTEO. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores VIVOS en radio (P, DENOMINADOR) para que la cobertura esté DEFINIDA: con <2 (single-player) ⇒ sólo el héroe ⇒ C∈{0,1} trivial ⇒ colapsa a 0 (single-player LIMPIO). ≥2 ⇒ ≥2 jugadores que cubrir ⇒ la amplitud es significativa. 🔑 la cobertura es INTRÍNSECAMENTE multijugador. CEO balance knob.
  midContest: 0.34,          // umbral de C para la banda PARTIAL/cobertura parcial (peso 1): C≥0.34 (≈1/3+ de los jugadores en radio están cubiertos). Adimensional ∈[0,1]. CEO balance knob.
  hiContest: 0.67,           // umbral de C para la banda BROAD/cobertura amplia (peso 2): C≥0.67 (≈2/3+ de los jugadores en radio están cubiertos — cobertura amplia). C<midContest (cobertura estrecha) ⇒ 0. Umbrales sobre C adimensional ∈[0,1]. CEO balance knob.
  weights: { broad: 2, partial: 1 },   // PESO de un remate por la BANDA de COBERTURA de la manada ENGANCHADA VIVA en radio: BROAD/cobertura amplia (C ≥hiContest) pesa 2; PARTIAL/cobertura parcial (≥midContest) pesa 1; estrecha (<midContest) ⇒ 0. Reward = rematar en medio de una manada que acosa a MUCHOS jugadores a la vez, no una fijada en 1 solo. CEO balance knobs.
  aggroContestBountyCap: 2,  // SUB-CAP DURO PROPIO de las fichas de cobertura por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.broad)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE COBERTURA/AMPLITUD (score∈{0,1,2}). Más jugadores cubiertos = tier MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (cobertura estrecha/single-player) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate en PARTIAL/cobertura parcial, C≥midContest): +1 ficha de cobertura por kill. "remataste con la manada acosando a parte de la party".
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate en pleno BROAD/cobertura amplia, C≥hiContest): +2 fichas por kill. "remataste con la manada acosando a casi toda la party a la vez". CEO balance knobs.
  ],
};

// CAS-2720: APILAMIENTO DE OBJETIVOS (AGGRO_PILE_SURGE, EVO#124 DARK) — AÑADE la dimensión de DOMINANCIA/PICO a la familia COMPOSICIÓN-DE-INTENCIÓN (#118 AGGRO-FOCUS = fracción sobre el HÉROE, un NIVEL; #120 EMBATE = sub-estado de combate; #121 REPARTO = ENTROPÍA/UNIFORMIDAD de la distribución sobre el conjunto YA-targeteado; #122 VAIVÉN = CHURN TEMPORAL de la identidad; #123 COBERTURA = AMPLITUD/cuántos jugadores DISTINTOS se cubren). APILAMIENTO mide qué tan TOP-HEAVY es la distribución: qué FRACCIÓN de la manada ENGANCHADA está PILADA sobre el ÚNICO jugador más targeteado (el PICO/dominancia del bucket máximo), sin importar la FORMA de la cola (spread #121) ni CUÁNTOS jugadores se tocan (contest #123). MÉTRICA = pile P∈[0,1] = max_j(#mobs ENGANCHADOS que targetean a j) / N, con N=#mobs ENGANCHADOS en radio. P≈1 ⇒ (casi) toda la manada CONVERGE su aggro en UN solo jugador (foco pesado, sea el héroe o no — un "pile-on"); P≈0.25 ⇒ la manada reparte parejo (pico bajo). minMobs3 (enganchados, DENOMINADOR de significancia), minPlayers2 (el pico sólo es significativo con ≥2 objetivos posibles). 🔑 INTRÍNSECAMENTE MULTIJUGADOR: single-player ⇒ P_players=1 ⇒ todo el aggro pila trivialmente sobre el ÚNICO héroe ⇒ pile=1 constante ⇒ el guard minPlayers2 lo colapsa a 0 (LIMPIO); sin mobs enganchados ⇒ 0. Server-authoritative aggregate sobre la aggro-table COMPARTIDA (lee la tabla directo, SNAPSHOT PURO, SIN buffer temporal, como #121/#123). FRACCIÓN AGREGADA INTENSIVA (invariante al conteo de MOBS), DETERMINISTA. 🔑 CRUX ⊥#121 REPARTO (la CRÍTICA — PICO-del-bucket-máximo vs UNIFORMIDAD-de-toda-la-distribución; comparten el bucket MAX pero pile IGNORA la forma de la cola: party-de-4 `[6,6,0,0]`⇒pile0.5/spread0.5 vs `[6,3,3,0]`⇒pile0.5/spread0.75 [MISMO pile, DISTINTO spread]; `[3,3,3,3]`⇒pile0.25-BAJO/spread1.0-MAX; `[12,0,0,0]`⇒pile1.0-MAX/spread0 ⇒ DISOCIAN). ⊥#123 COBERTURA (PROFUNDIDAD-en-uno vs AMPLITUD-a-través: `[12,0,0,0]`⇒pile1.0/contest0.25; `[3,3,3,3]`⇒pile0.25/contest1.0 [anti-correlados en los extremos]; `[6,6,0,0]`⇒pile0.5/contest0.5 vs `[6,3,3,0]`⇒pile0.5/contest0.75 [MISMO pile, DISTINTO contest]). ⊥#118 AGGRO-FOCUS (pile es IDENTITY-BLIND al héroe — pico sobre CUALQUIER jugador; todo-en-1-NO-héroe ⇒ pile1.0/focus0). ⊥#122 VAIVÉN (pico-estático-AHORA ⊥ churn-temporal: pila-fija ⇒ pile-alto/switch0; pila-re-rolleando-cuál-mob ⇒ pile-alto/switch-alto). ⊥#120 EMBATE (distribución sobre jugadores ⊥ sub-estado de combate). ⊥ cinético #117 ACCEL/#119 JERK. ⊥#67 FRENZY/#94 SWIFT (ratio NO kill-rate). ⊥ velocidad #110/#111/#112/#116. ⊥ atributo #115/#88/#106. ⊥ posición #107/#108/#109/#113/#114. ⊥#87 PACKHARVEST (dominancia-FRACCIONAL invariante al conteo de MOBS). ⊥#86/#85/#83. Un ESCALAR de COMPOSICIÓN-DE-INTENCIÓN sobre la DOMINANCIA/PICO del apilamiento de objetivos — ORTOGONAL a las 65 flags. Camino al 66º flag (serializa tras #123 AGGRO_CONTEST_SURGE LIVE&served 44fee5ef2f31/815, base HEAD 314f73a).
//    ⊥ COBERTURA #123 aggroContestFind (AMPLITUD del conjunto targeteado — CUÁNTOS jugadores DISTINTOS) — APILAMIENTO = la PROFUNDIDAD/dominancia sobre el jugador PICO (max-share). Canal FRESCO `aggroPileFind` — NINGUNA de las 65 flags #59-#123 lo usa; recurso TRANSITORIO NUEVO h.aggroPileBounty, STATELESS (fuera del save allowlist + worldFingerprint), sub-cap propio aggroPileBountyCap, badge ⧉ "Apilamiento".
export const AGGRO_PILE_SURGE = {
  enabled: true,             // LIVE (EVO#124, 66º flag CAS-2724 flip). enabled:true ⇒ byte-neutral OFF (funciones de lectura PURAS gated + seam rama muerta + _aggroPilePre=0 const inerte + badge NO despachado + h.aggroPileBounty NUNCA creado + G._apParty test-scaffolding NUNCA creado). SNAPSHOT PURO (lee la aggro-table directo, SIN buffer temporal, como #121/#123). Flip config-only false→true tras CEO Gate 2/2.
  channel: "aggroPileFind",  // canal FRESCO (fichas de apilamiento por rematar en medio de una manada PILADA sobre UN solo jugador — la DOMINANCIA del pico) — NINGUNA de las 65 flags #59-#123 lo usa. Distinto por CONSTRUCCIÓN de aggroContestFind #123 (AMPLITUD/cuántos jugadores distintos) / targetSpreadFind #121 (UNIFORMIDAD de toda la distribución) / aggroSwitchFind #122 (CHURN TEMPORAL) / aggroFocusFind #118 (nivel sobre el HÉROE) / strikeCommitFind #120 (sub-estado): APILAMIENTO = la PROFUNDIDAD/max-share sobre el jugador PICO. Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio de CONSIDERACIÓN (px) de la vecindad VIVA: P se computa sobre los mobs VIVOS ENGANCHADOS y los jugadores VIVOS dentro del radio del héroe. Mirror #87/…/#123 radius. CEO balance knob.
  minMobs: 3,                // nº MÍNIMO de mobs aggro-ENGANCHADOS en radio (DENOMINADOR N de significancia) para que el pico sea significativo: con <3 es degenerado ⇒ 0. ≥3 ⇒ una manada genuina en aggro. Con el filtro de muertos (la víctima ya tiene e.dead=true en el TOP ⇒ NO cuenta) ⇒ ANTI-AUTO-CONTEO. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores VIVOS en radio para que el pico esté DEFINIDO: con <2 (single-player) ⇒ sólo el héroe ⇒ todo el aggro pila trivialmente sobre él ⇒ pile=1 constante ⇒ colapsa a 0 (single-player LIMPIO). ≥2 ⇒ ≥2 objetivos posibles ⇒ el pico es significativo. 🔑 el apilamiento es INTRÍNSECAMENTE multijugador. CEO balance knob.
  midPile: 0.34,             // umbral de P para la banda SOME/pila parcial (peso 1): P≥0.34 (≈1/3+ de la manada enganchada pilada sobre un jugador). Adimensional ∈[0,1]. CEO balance knob.
  hiPile: 0.67,              // umbral de P para la banda HEAVY/pila pesada (peso 2): P≥0.67 (≈2/3+ de la manada enganchada pilada sobre UN jugador — foco pesado). P<midPile (pico bajo/repartido) ⇒ 0. Umbrales sobre P adimensional ∈[0,1]. CEO balance knob.
  weights: { heavy: 2, some: 1 },   // PESO de un remate por la BANDA de APILAMIENTO de la manada ENGANCHADA VIVA en radio: HEAVY/pila pesada (P ≥hiPile) pesa 2; SOME/pila parcial (≥midPile) pesa 1; repartida (<midPile) ⇒ 0. Reward = rematar en medio de una manada PILADA sobre 1 solo jugador, no una repartida parejo. CEO balance knobs.
  aggroPileBountyCap: 2,     // SUB-CAP DURO PROPIO de las fichas de apilamiento por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.heavy)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE APILAMIENTO/DOMINANCIA (score∈{0,1,2}). Pila más pesada = tier MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (pico bajo/repartido/single-player) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate en SOME/pila parcial, P≥midPile): +1 ficha de apilamiento por kill. "remataste con la manada pilando parcialmente sobre un jugador".
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate en pleno HEAVY/pila pesada, P≥hiPile): +2 fichas por kill. "remataste con casi toda la manada pilada sobre UN solo jugador". CEO balance knobs.
  ],
};

// CAS-2726: VENTAJA DE APILAMIENTO (AGGRO_MARGIN_SURGE, EVO#125 DARK) — AÑADE la dimensión de DOMINANCIA-GAP/LEAD a la familia COMPOSICIÓN-DE-INTENCIÓN (#118 AGGRO-FOCUS = fracción sobre el HÉROE, un NIVEL; #120 EMBATE = sub-estado de combate; #121 REPARTO = ENTROPÍA/UNIFORMIDAD de toda la distribución; #122 VAIVÉN = CHURN TEMPORAL de la identidad; #123 COBERTURA = AMPLITUD/cuántos jugadores DISTINTOS se cubren; #124 APILAMIENTO = max-share/PICO absoluto del bucket máximo). VENTAJA mide la SEPARACIÓN/lead-de-runaway del jugador MÁS pilado sobre el SEGUNDO: cuánto DESPUNTA el pico sobre el resto. MÉTRICA = margin M∈[0,1] = (top − second) / N, con top=max #mobs ENGANCHADOS sobre un jugador, second=2º-mayor de esos conteos, N=#mobs ENGANCHADOS en radio. M≈1 ⇒ un único jugador acapara TODO el aggro con un margen enorme sobre el resto (runaway leader); M≈0 ⇒ el pico está EMPATADO con el segundo (co-dominancia, sin runaway) aunque el PICO absoluto sea alto. minMobs3 (enganchados, DENOMINADOR N) + minPlayers2 (el lead sólo está definido con ≥2 objetivos posibles). 🔑 INTRÍNSECAMENTE MULTIJUGADOR: single-player ⇒ P_players=1 ⇒ el lead trivialmente sobre el ÚNICO héroe ⇒ colapsa a 0 (LIMPIO); sin mobs enganchados ⇒ 0. Server-authoritative aggregate sobre la aggro-table COMPARTIDA (lee la tabla directo, SNAPSHOT PURO, SIN buffer temporal, como #121/#123/#124). FRACCIÓN AGREGADA INTENSIVA (invariante al conteo de MOBS), DETERMINISTA. 🔑 CRUX ⊥#124 APILAMIENTO (el HERMANO — LA CRÍTICA): PILE=max/N (el PICO absoluto); MARGIN=(max−second)/N (el LEAD sobre el segundo). MISMO PICO, distinto margen: `[6,6,0,0]`⇒pile0.5-ALTO/margin0 (empate, sin runaway) vs `[6,1,1,1]`⇒pile0.667/margin0.556-ALTO (runaway); y con N/max FIJOS `[6,1,1,1]`⇒pile0.667/margin0.556 vs `[6,3,0,0]`⇒pile0.667-MISMO/margin0.333-DISTINTO ⇒ margin NO es función de pile ⇒ DISOCIAN. ⊥#121 REPARTO (evenness/entropía de TODA la distribución) ⊥#123 COBERTURA (amplitud/cuántos jugadores) ⊥#118 AGGRO-FOCUS (IDENTITY-BLIND al héroe). ⊥ cinético #117/#119, ⊥ velocidad #110/#111/#112/#116, ⊥ atributo #115/#88/#106, ⊥ posición #107/#108/#109/#113/#114, ⊥#87 PACKHARVEST (lead-FRACCIONAL invariante al conteo de MOBS). Un ESCALAR de COMPOSICIÓN-DE-INTENCIÓN sobre la DOMINANCIA-GAP/LEAD del apilamiento — el 7º eje de la familia, ORTOGONAL a las 66 flags. Camino al 67º flag (serializa tras #124 AGGRO_PILE LIVE&served f332e84bf145/815, base master 750de9e).
export const AGGRO_MARGIN_SURGE = {
  enabled: true,             // LIVE (EVO#125, 67º flag) — CAS-2728 flip false→true tras CEO Gate 2/2 (byteverify 9421415 + DARK QA CAS-2727 PASS 20/20). enabled:false era byte-neutral OFF (funciones de lectura PURAS gated + seam rama muerta + _aggroMarginPre=0 const inerte + badge NO despachado + h.aggroMarginBounty NUNCA creado + G._amParty test-scaffolding NUNCA creado). SNAPSHOT PURO (lee la aggro-table directo, SIN buffer temporal, como #121/#123/#124). Flip config-only false→true tras CEO Gate 2/2.
  channel: "aggroMarginFind",  // canal FRESCO (fichas de ventaja por rematar en medio de una manada con un jugador ACAPARANDO el aggro con margen sobre el resto — el LEAD/runaway) — NINGUNA de las 66 flags #59-#124 lo usa. Distinto por CONSTRUCCIÓN de aggroPileFind #124 (max-share/PICO absoluto) / aggroContestFind #123 (AMPLITUD) / targetSpreadFind #121 (UNIFORMIDAD) / aggroSwitchFind #122 (CHURN) / aggroFocusFind #118 (nivel sobre el HÉROE): VENTAJA = la SEPARACIÓN top−second (el runaway del líder). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio de CONSIDERACIÓN (px) de la vecindad VIVA: M se computa sobre los mobs VIVOS ENGANCHADOS y los jugadores VIVOS dentro del radio del héroe. Mirror #87/…/#124 radius. CEO balance knob.
  minMobs: 3,                // nº MÍNIMO de mobs aggro-ENGANCHADOS en radio (DENOMINADOR N de significancia) para que el lead sea significativo: con <3 es degenerado ⇒ 0. ≥3 ⇒ una manada genuina en aggro. Con el filtro de muertos (la víctima ya tiene e.dead=true en el TOP ⇒ NO cuenta) ⇒ ANTI-AUTO-CONTEO. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores VIVOS en radio para que el lead esté DEFINIDO: con <2 (single-player) ⇒ sólo el héroe ⇒ no hay segundo ⇒ lead trivial ⇒ colapsa a 0 (single-player LIMPIO). ≥2 ⇒ ≥2 objetivos posibles ⇒ el runaway es significativo. 🔑 la ventaja es INTRÍNSECAMENTE multijugador. CEO balance knob.
  midMargin: 0.25,           // umbral de M para la banda SOME/lead parcial (peso 1): M≥0.25 (el líder despunta ≥1/4·N sobre el segundo). Adimensional ∈[0,1]. CEO balance knob.
  hiMargin: 0.50,            // umbral de M para la banda HEAVY/runaway (peso 2): M≥0.50 (el líder acapara ≥1/2·N MÁS que el segundo — runaway pronunciado). M<midMargin (empate/co-dominancia) ⇒ 0. Umbrales sobre M adimensional ∈[0,1]. CEO balance knob.
  weights: { heavy: 2, some: 1 },   // PESO de un remate por la BANDA de VENTAJA/LEAD de la manada ENGANCHADA VIVA en radio: HEAVY/runaway (M ≥hiMargin) pesa 2; SOME/lead parcial (≥midMargin) pesa 1; empate (<midMargin) ⇒ 0. Reward = rematar cuando UN jugador acapara el aggro con margen (runaway), no cuando el pico está empatado. CEO balance knobs.
  aggroMarginBountyCap: 2,   // SUB-CAP DURO PROPIO de las fichas de ventaja por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.heavy)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE VENTAJA/LEAD (score∈{0,1,2}). Lead más pronunciado = tier MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (empate/co-dominancia/single-player) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate en SOME/lead parcial, M≥midMargin): +1 ficha de ventaja por kill. "remataste con un líder despuntando parcialmente sobre el resto".
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate en pleno HEAVY/runaway, M≥hiMargin): +2 fichas por kill. "remataste con un jugador acaparando el aggro con margen enorme sobre el segundo". CEO balance knobs.
  ],
};

// CAS-2730: AVALANCHA (AGGRO_DENSITY_SURGE, EVO#126 DARK) — AÑADE la dimensión de OUTNUMBER/SWARM-PRESSURE (1er MOMENTO / MAGNITUD) a la familia COMPOSICIÓN-DE-INTENCIÓN (#118 AGGRO-FOCUS = fracción sobre el HÉROE, un NIVEL; #120 EMBATE = sub-estado; #121 REPARTO = ENTROPÍA/UNIFORMIDAD; #122 VAIVÉN = CHURN TEMPORAL; #123 COBERTURA = AMPLITUD/cuántos jugadores; #124 APILAMIENTO = max-share/PICO absoluto; #125 VENTAJA = LEAD/gap top−second). AVALANCHA mide la CARGA MEDIA de aggro por jugador — cuán SUPERADOS EN NÚMERO / SWARMED está la party. MÉTRICA = densidad D∈[0,1] = min(N/P, densityCap)/densityCap, con N=#mobs ALIVE aggro-ENGANCHADOS en radio, P=#jugadores ALIVE en radio (incl. héroe). N/P = load medio por jugador. D≈1 ⇒ la party está masivamente superada en número (zerg/add-pressure); D≈0 ⇒ pocos mobs por jugador. Bandas sobre el LOAD CRUDO N/P: midDensity avg-load ≥2 ⇒ peso 1; hiDensity avg-load ≥4 ⇒ peso 2. minMobs3 (N≥3) + minPlayers2 (P≥2). 🔑 INTRÍNSECAMENTE MULTIJUGADOR: single-player / P<2 ⇒ D=0 (colapso LIMPIO — "superado en número relativo a la party" es indefinido en solo). Server-authoritative aggregate sobre la aggro-table COMPARTIDA (SNAPSHOT PURO, SIN buffer temporal, como #121/#123/#124/#125). 🔑 CRUX (LA CRÍTICA) — el 1er MOMENTO/MAGNITUD vs TODA forma NORMALIZADA (scale-free): DENSITY es la MEDIA, cada eje previo es un estadístico de FORMA. `[1,1,1,1]` (N=4,P=4 ⇒ D-load=1) vs `[6,6,6,6]` (N=24,P=4 ⇒ D-load=6) — IDÉNTICO pile-share/margin/entropía/varianza normalizada (MISMA forma) — pero la densidad difiere 6× ⇒ el reward dispara sobre la MAGNITUD del swarm, no su forma. ⊥#123 COBERTURA: `[8,0,0,0]` (D-load=2, contest=1 jugador) vs `[2,2,2,2]` (D-load=2 IGUAL, contest=4) — MISMA densidad, cobertura opuesta. ⊥#122 VAIVÉN (snapshot puro ⊥ churn temporal). ⊥#118 AGGRO-FOCUS (agregado de party, hero-agnóstico). ⊥ cinético #117/#119, ⊥ velocidad #110/#111/#112/#116, ⊥ atributo #115/#88/#106, ⊥ posición #107/#108/#109/#113/#114, ⊥#87 PACKHARVEST (load-medio ⊥ conteo bruto de MOBS por kill). El 8º eje de COMPOSICIÓN-DE-INTENCIÓN (OUTNUMBERED/ZERG/add-pressure, el señal "getting swarmed" núcleo de MMORPG) — ORTOGONAL a las 67 flags. Camino al 68º flag (serializa tras #125 AGGRO_MARGIN LIVE&served 664e24117516/815, base master cc11557).
export const AGGRO_DENSITY_SURGE = {
  enabled: true,             // LIVE (EVO#126, 68º flag) — CAS-2734 config-only flip false→true tras CEO Gate 2/2 (byteverify PASS + DARK QA CAS-2733 PASS + Gate 2/2 no-drift PASS). SNAPSHOT PURO (lee la aggro-table directo, SIN buffer temporal, como #121/#123/#124/#125). sim/render/game byte-idénticos a 129916a (pure enable flip, 0 logic change).
  channel: "aggroDensityFind",  // canal FRESCO (fichas de avalancha por rematar en medio de un swarm que SUPERA EN NÚMERO a la party — la add-pressure) — NINGUNA de las 67 flags #59-#125 lo usa. Distinto por CONSTRUCCIÓN de aggroMarginFind #125 (LEAD/gap) / aggroPileFind #124 (max-share/PICO) / aggroContestFind #123 (AMPLITUD) / targetSpreadFind #121 (UNIFORMIDAD) / aggroSwitchFind #122 (CHURN) / aggroFocusFind #118 (nivel sobre el HÉROE): AVALANCHA = la MAGNITUD/load-medio N/P (cuán superados en número). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio de CONSIDERACIÓN (px) de la vecindad VIVA: D se computa sobre los mobs VIVOS ENGANCHADOS y los jugadores VIVOS dentro del radio del héroe. Mirror #87/…/#125 radius. CEO balance knob.
  minMobs: 3,                // nº MÍNIMO de mobs aggro-ENGANCHADOS en radio (NUMERADOR N de significancia) para que el swarm sea significativo: con <3 es degenerado ⇒ 0. ≥3 ⇒ una manada genuina en aggro. Con el filtro de muertos (la víctima ya tiene e.dead=true ⇒ NO cuenta) ⇒ ANTI-AUTO-CONTEO. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores VIVOS en radio (DENOMINADOR P) para que el load-medio esté DEFINIDO como PARTY-relativo: con <2 (single-player) ⇒ "superado en número relativo a la party" es indefinido ⇒ colapsa a 0 (single-player LIMPIO). ≥2 ⇒ una party genuina. 🔑 la avalancha es INTRÍNSECAMENTE multijugador. CEO balance knob.
  densityCap: 6,             // TECHO de normalización del load: D = min(N/P, densityCap)/densityCap ∈[0,1]. avg-load ≥6 mobs/jugador ⇒ D=1.0 (saturación). Sólo afecta el idx NORMALIZADO del badge; las BANDAS operan sobre el load CRUDO N/P (midDensity/hiDensity). CEO balance knob.
  midDensity: 2,             // umbral de LOAD CRUDO N/P para la banda SOME/superado parcial (peso 1): N/P≥2 (≥2 mobs por jugador de media). Sobre el load CRUDO (NO el D normalizado). CEO balance knob.
  hiDensity: 4,              // umbral de LOAD CRUDO N/P para la banda HEAVY/zerg (peso 2): N/P≥4 (≥4 mobs por jugador — swarm pronunciado). N/P<midDensity ⇒ 0. Sobre el load CRUDO N/P. CEO balance knob.
  weights: { heavy: 2, some: 1 },   // PESO de un remate por la BANDA de DENSIDAD/load del swarm ENGANCHADO VIVO en radio: HEAVY/zerg (N/P ≥hiDensity) pesa 2; SOME/superado parcial (≥midDensity) pesa 1; poca-densidad (<midDensity) ⇒ 0. Reward = rematar cuando la party está genuinamente superada en número (swarm), no cuando hay pocos mobs por jugador. CEO balance knobs.
  aggroDensityBountyCap: 2,  // SUB-CAP DURO PROPIO de las fichas de avalancha por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.heavy)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE DENSIDAD/load (score∈{0,1,2}). Swarm más denso = tier MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (poca densidad/single-player) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con SOME/superado parcial, N/P≥midDensity): +1 ficha de avalancha por kill. "remataste con la party superada en número parcialmente".
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate en pleno HEAVY/zerg, N/P≥hiDensity): +2 fichas por kill. "remataste con la party masivamente superada en número (swarm)". CEO balance knobs.
  ],
};

// CAS-2736: MEZCLA (AGGRO_VARIETY_SURGE, EVO#127 DARK) — AÑADE la dimensión THREAT-HETEROGENEITY / MIXED-PULL (la composición del ENJAMBRE EN SÍ, del lado de los MOBS) a la familia COMPOSICIÓN-DE-INTENCIÓN. Las 8 flags previas (#118 FOCUS/nivel-sobre-el-HÉROE, #121 SPREAD/entropía, #123 CONTEST/cobertura, #124 PILE/pico, #125 MARGIN/gap, #126 DENSITY/magnitud, #122 SWITCH/churn) miden cómo el aggro se reparte sobre los JUGADORES (targets). MEZCLA mide cuán HETEROGÉNEO es el pack de mobs ENGANCHADOS — un zerg monotipo vs un pull mixto (melee+ranged+caster / familias distintas / clases de talla). MÉTRICA = V∈[0,1] = riqueza normalizada de ARQUETIPOS DE MOB DISTINTOS entre los mobs VIVOS aggro-ENGANCHADOS en radio (enganchados con CUALQUIER jugador en radio — la amenaza COMPARTIDA de la party). V=(K−1)/(varietyCap−1), K=# tipos/familias de mob distintas (clave = e.type, índice de ETPL — server-authoritative y replicado, la MISMA fuente estática que #88 BULK/#79 SIZECLASS). Bandas sobre K CRUDO: midVariety K≥2 ⇒ peso 1; hiVariety K≥3 ⇒ peso 2. minMobs3 (N≥3) + minPlayers2 (P≥2). 🔑 INTRÍNSECAMENTE MULTIJUGADOR: single-player / P<2 ⇒ V=0 (colapso LIMPIO — el mixed-pull COMPARTIDO de la party es la señal). SNAPSHOT PURO (lee la aggro-table COMPARTIDA + el campo tipo/familia del mob, SIN buffer temporal, como #121/#123/#124/#125/#126). 🔑 CRUX (LA CRÍTICA) — VARIETY vive en el eje MOB-TYPE, TODOS los priores son distribución-sobre-JUGADORES: ⊥#126 DENSITY (magnitud): [melee×6] (N=6, K=1 ⇒ V=0) vs [melee×2,ranged×2,caster×2] (N=6 IGUAL, K=3 ⇒ V=hi) — MISMA densidad, variedad opuesta; y [melee×1,ranged×1] (baja densidad, K=2) vs [melee×6] (alta densidad, K=1) — density y variety anti-mueven. ⊥#121 SPREAD/#124 PILE/#125 MARGIN/#123 CONTEST (forma/cobertura del reparto sobre JUGADORES): variety se computa sobre TIPOS de mob, no share-de-jugador — [6 mobs de 3 tipos TODOS pilados en player1] (V=hi, pile=1/contest=1) vs [6 mobs de 1 tipo repartidos sobre 4 jugadores] (V=0, spread=hi/contest=4) — ortogonales. ⊥#122 SWITCH (V es snapshot puro ⊥ churn temporal). ⊥#118 FOCUS (V es agregado de party sobre el pack COMPARTIDO, hero-agnóstico). El 9º eje de COMPOSICIÓN-DE-INTENCIÓN (MIXED-PULL / add-variety — la señal MMORPG núcleo "pulleaste un pack mixto, adapta tu kit"). Camino al 69º flag (serializa tras #126 AGGRO_DENSITY LIVE&served b04ec1eb79d6/815, base master b12ca3aa).
export const AGGRO_VARIETY_SURGE = {
  enabled: true,             // LIVE (EVO#127, 69º flag) — CAS-2740 config-only flip false→true tras CEO Gate 2/2 (byteverify PASS + DARK QA CAS-2739 PASS 19/19 + Gate 2/2 no-drift PASS). SNAPSHOT PURO (lee la aggro-table + e.type directo, SIN buffer temporal, como #121/#123/#124/#125/#126). sim/render/game byte-idénticos a fd57515 (pure enable flip, 0 logic change).
  channel: "aggroVarietyFind",  // canal FRESCO (fichas de mezcla por rematar en medio de un pull HETEROGÉNEO — melee+ranged+caster / familias dispares) — NINGUNA de las 68 flags #59-#126 lo usa. Distinto por CONSTRUCCIÓN de aggroDensityFind #126 (MAGNITUD/load-medio N/P) / aggroMarginFind #125 (LEAD/gap) / aggroPileFind #124 (max-share/PICO) / aggroContestFind #123 (AMPLITUD) / targetSpreadFind #121 (UNIFORMIDAD) / aggroSwitchFind #122 (CHURN): MEZCLA = la HETEROGENEIDAD del pack (cuántos ARQUETIPOS distintos). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio de CONSIDERACIÓN (px) de la vecindad VIVA: V se computa sobre los mobs VIVOS ENGANCHADOS dentro del radio del héroe y los jugadores VIVOS en radio. Mirror #87/…/#126 radius. CEO balance knob.
  minMobs: 3,                // nº MÍNIMO de mobs aggro-ENGANCHADOS en radio (N de significancia) para que el pack sea significativo: con <3 es degenerado ⇒ 0. ≥3 ⇒ un pack genuino en aggro. Con el filtro de muertos (la víctima ya tiene e.dead=true ⇒ NO cuenta) ⇒ ANTI-AUTO-CONTEO. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores VIVOS en radio (P) para que el mixed-pull COMPARTIDO esté DEFINIDO como party-relativo: con <2 (single-player) ⇒ colapsa a 0 (single-player LIMPIO). ≥2 ⇒ una party genuina. 🔑 la mezcla del pull compartido es INTRÍNSECAMENTE multijugador. CEO balance knob.
  varietyCap: 4,             // TECHO de normalización de la riqueza: V = (K−1)/(varietyCap−1) ∈[0,1]. K≥4 arquetipos distintos ⇒ V=1.0 (saturación). K=1 ⇒ V=0 (monotipo). Sólo afecta el idx NORMALIZADO del badge; las BANDAS operan sobre K CRUDO (midVariety/hiVariety). CEO balance knob.
  midVariety: 2,             // umbral de K CRUDO para la banda SOME/mixto (peso 1): K≥2 (≥2 arquetipos distintos). Sobre K CRUDO (NO el V normalizado). CEO balance knob.
  hiVariety: 3,              // umbral de K CRUDO para la banda HEAVY/motley (peso 2): K≥3 (≥3 arquetipos — pull marcadamente heterogéneo). K<midVariety ⇒ 0. Sobre K CRUDO. CEO balance knob.
  weights: { heavy: 2, some: 1 },   // PESO de un remate por la BANDA de VARIEDAD/riqueza del pack ENGANCHADO VIVO en radio: HEAVY/motley (K≥hiVariety) pesa 2; SOME/mixto (≥midVariety) pesa 1; monotipo (<midVariety) ⇒ 0. Reward = rematar cuando el pull es genuinamente heterogéneo (adapta tu kit), no cuando es un zerg monotipo. CEO balance knobs.
  aggroVarietyBountyCap: 2,  // SUB-CAP DURO PROPIO de las fichas de mezcla por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.heavy)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE VARIEDAD (score∈{0,1,2}). Pull más heterogéneo = tier MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (monotipo/single-player) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con SOME/mixto, K≥midVariety): +1 ficha de mezcla por kill. "remataste en un pull mixto (2 arquetipos)".
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate en pleno HEAVY/motley, K≥hiVariety): +2 fichas por kill. "remataste en un pull marcadamente heterogéneo (≥3 arquetipos)". CEO balance knobs.
  ],
};


// CAS-2743: ESCALADA (AGGRO_MOMENTUM_SURGE, EVO#128 DARK) — AÑADE la dimensión ESCALATION/BUILD-UP (la TENDENCIA aggro, NO el snapshot) a la familia COMPOSICIÓN-DE-INTENCIÓN. Las 9 flags previas miden cómo el aggro se distribuye AHORA (#118 FOCUS/nivel-sobre-el-HÉROE, #121 SPREAD/entropía, #123 CONTEST/cobertura, #124 PILE/pico, #125 MARGIN/gap, #126 DENSITY/magnitud, #127 VARIETY/mob-type) o churn sin-signo (#122 SWITCH). ESCALADA es la TASA DE CAMBIO CON SIGNO de la presión del pack ENGANCHADO sobre una ventana corta Δ: ¿el pull está CRECIENDO (adds entrando, la pelea escalando) o disolviéndose? MÉTRICA = M∈[0,1] = build-up POSITIVO normalizado del tamaño del pack ENGANCHADO (N) sobre Δ: muestrea N enganchado en t−Δ y t; rawM = max(0, N_t − N_{t−Δ}); M = min(1, rawM/momentumCap). PREMIA la ESCALADA (amenaza creciente), NO el decay (pack menguante ⇒ 0). Bandas sobre rawM CRUDO (ΔN): midMomentum ΔN≥2 ⇒ peso 1; hiMomentum ΔN≥3 ⇒ peso 2. minMobs3 (N≥3 en t) + minPlayers2 (P≥2). 🔑 INTRÍNSECAMENTE MULTIJUGADOR: single-player / P<2 ⇒ M=0 (colapso LIMPIO). TEMPORAL/WINDOWED (como #122 SWITCH — lee la aggro-table COMPARTIDA server-auth sobre 2 snapshots vía un ring-buffer per-héroe; NO snapshot-puro). El ring-buffer es cliente-derivado del estado replicado (como #122), NUNCA serializado (fuera del save + worldFingerprint). 🔑 CRUX (LA CRÍTICA) — MOMENTUM es la DERIVADA CON SIGNO; todos los priores snapshot son el NIVEL: ⊥#126 DENSITY (magnitud/nivel): momentum es dN/dt, NO N. N=6 estable (density hi, M=0) vs N: 2→4→6 (MISMA densidad instantánea en t, M=hi) — un pack grande estable tiene 0 momentum; uno pequeño-pero-creciendo tiene hi momentum. ⊥#122 SWITCH (churn SIN signo): SWITCH = tasa de cambio de IDENTIDAD del objetivo (magnitud, ciega al signo); MOMENTUM = tendencia CON SIGNO del TAMAÑO del pack. Un pack puede churnear objetivos salvajemente con 0 cambio neto de tamaño (switch hi, M=0), o crecer estable con 0 cambios de objetivo (M hi, switch 0). ⊥#124 PILE/#125 MARGIN/#121 SPREAD/#123 CONTEST/#118 FOCUS/#127 VARIETY (todos snapshots de distribución/tipo): momentum es agnóstico a CÓMO se distribuye o QUÉ tipos — sólo si CRECE. El 10º eje de COMPOSICIÓN-DE-INTENCIÓN (ESCALATION / "la pelea se está armando, vienen adds, prepárate" — señal de pacing MMORPG núcleo). Camino al 70º flag (serializa tras #127 AGGRO_VARIETY LIVE&served 8dd689f8c58f/815, base master ec5de3d).
export const AGGRO_MOMENTUM_SURGE = {
  enabled: true,             // LIVE (EVO#128, 70º flag). enabled:false ⇒ byte-neutral OFF (momentumTick NUNCA corre + funciones de lectura PURAS gated + seam rama muerta + _aggroMomentumPre=0 const inerte + badge NO despachado + h.aggroMomentumBounty NUNCA creado + ring-buffer per-héroe G._momR NUNCA creado ⇒ NO perturba el fingerprint). CAPTURA TEMPORAL (ring-buffer per-héroe de N enganchado, como #122 SWITCH). Flip config-only false→true tras CEO Gate 2/2.
  channel: "aggroMomentumFind", // canal FRESCO (fichas de escalada por rematar mientras el pull ENGANCHADO CRECE — la pelea escalando, adds entrando) — NINGUNA de las 69 flags #59-#127 lo usa. Distinto por CONSTRUCCIÓN de aggroVarietyFind #127 (HETEROGENEIDAD/tipos) / aggroDensityFind #126 (MAGNITUD/load-medio N/P) / aggroMarginFind #125 (LEAD) / aggroPileFind #124 (max-share) / aggroContestFind #123 (AMPLITUD) / targetSpreadFind #121 (UNIFORMIDAD) / aggroSwitchFind #122 (CHURN sin-signo): ESCALADA = la TENDENCIA CON SIGNO del TAMAÑO del pack (dN/dt>0). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio de CONSIDERACIÓN (px) de la vecindad VIVA: N (mobs enganchados) y P (jugadores) se cuentan dentro del radio del héroe. Mirror #87/…/#127 radius. CEO balance knob.
  window: 4,                 // VENTANA Δ (ticks) del ring-buffer per-héroe de N enganchado: N_t (última muestra) se compara con N_{t−Δ} (primera). El buffer guarda Δ+1 muestras (t−Δ..t); buffer <Δ+1 (cold-start) ⇒ M=0 (sin historia). Más grande = ventana de escalada más larga. CEO balance knob.
  minMobs: 3,                // nº MÍNIMO de mobs aggro-ENGANCHADOS en radio en t (N_t) para que el pack sea significativo: con <3 es degenerado ⇒ 0. ≥3 ⇒ un pack genuino en aggro. Con el filtro de muertos ⇒ ANTI-AUTO-CONTEO. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores VIVOS en radio (P) para que la escalada del pull COMPARTIDO esté DEFINIDA como party-relativa: con <2 (single-player) ⇒ colapsa a 0 (single-player LIMPIO). ≥2 ⇒ una party genuina. 🔑 la escalada del pull compartido es INTRÍNSECAMENTE multijugador. CEO balance knob.
  momentumCap: 4,            // TECHO de normalización del build-up: M = min(1, rawM/momentumCap) ∈[0,1], rawM = max(0, N_t − N_{t−Δ}). rawM≥4 (el pack creció en ≥4 mobs) ⇒ M=1.0 (saturación). rawM≤0 (estable/menguante) ⇒ M=0. Sólo afecta el idx NORMALIZADO del badge; las BANDAS operan sobre rawM CRUDO (midMomentum/hiMomentum). CEO balance knob.
  midMomentum: 2,            // umbral de rawM CRUDO (ΔN) para la banda SOME/creciendo (peso 1): rawM≥2 (el pack creció en ≥2 mobs en la ventana). Sobre rawM CRUDO (NO el M normalizado). CEO balance knob.
  hiMomentum: 3,             // umbral de rawM CRUDO (ΔN) para la banda HEAVY/surging (peso 2): rawM≥3 (el pack creció en ≥3 mobs — escalada marcada). rawM<midMomentum ⇒ 0. Sobre rawM CRUDO. CEO balance knob.
  weights: { heavy: 2, some: 1 },   // PESO de un remate por la BANDA de ESCALADA/build-up del pack ENGANCHADO VIVO en radio: HEAVY/surging (rawM≥hiMomentum) pesa 2; SOME/creciendo (≥midMomentum) pesa 1; estable/menguante (<midMomentum) ⇒ 0. Reward = rematar cuando el pull está genuinamente CRECIENDO (la pelea escalando, adds entrando), no cuando es estable o se disuelve. CEO balance knobs.
  aggroMomentumBountyCap: 2, // SUB-CAP DURO PROPIO de las fichas de escalada por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.heavy)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE ESCALADA (score∈{0,1,2}). Pull creciendo más rápido = tier MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (estable/menguante/single-player/cold-start) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con SOME/creciendo, rawM≥midMomentum): +1 ficha de escalada por kill. "remataste con el pull empezando a crecer (adds entrando)".
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate en pleno HEAVY/surging, rawM≥hiMomentum): +2 fichas por kill. "remataste con el pull escalando fuerte (oleada de adds)". CEO balance knobs.
  ],
};

// CAS-2749: CERCO (AGGRO_SURROUND_SURGE, EVO#129 DARK) — AÑADE la dimensión SPATIAL-ENCIRCLEMENT (la GEOMETRÍA ANGULAR de la amenaza, no el conteo/tendencia/reparto) a la familia COMPOSICIÓN-DE-INTENCIÓN. Las 10 flags previas miden CUÁNTO aggro hay (#126 DENSITY/magnitud), CÓMO TIENDE (#128 MOMENTUM/dN/dt), o CÓMO se reparte sobre JUGADORES/objetivos/TIPOS (#118 FOCUS/#121 SPREAD/#122 SWITCH/#123 CONTEST/#124 PILE/#125 MARGIN/#127 VARIETY) — NINGUNA lee la GEOMETRÍA ANGULAR física del pack ENGANCHADO alrededor del héroe. CERCO = el encirclement: ¿los mobs enganchados están repartidos 360° alrededor del héroe (cercado, sin lado seguro — "boxed in, hold the center") o amontonados a un lado (flankable, una línea de retirada)? MÉTRICA = S∈[0,1] = DISPERSIÓN ANGULAR de los rumbos hero→mob de los mobs ENGANCHADOS VIVOS en radio. Por cada mob toma el VECTOR UNIDAD de su rumbo desde el héroe; R = |media de esos vectores unidad| (mean resultant length ∈[0,1]); S = 1 − R. Todos a un lado ⇒ R≈1 ⇒ S≈0 (no cercado); repartidos 360° ⇒ R≈0 ⇒ S≈1 (cercado). Bandas sobre S: midSurround S≥0.5 ⇒ peso 1; hiSurround S≥0.75 ⇒ peso 2. minMobs3 (N≥3 — hacen falta ≥3 para cercar) + minPlayers2 (P≥2). 🔑 SNAPSHOT/PURO (a diferencia de #122 SWITCH/#128 MOMENTUM — SIN ring-buffer; lee las posiciones replicadas de los mobs enganchados relativas al héroe en UN frame). 🔑 DETERMINISMO (sev-1): rumbos CUANTIZADOS a bins ENTEROS (angleBins 64) vía atan2→bin; resultante sumada de un LUT ENTERO (cos/sin ×1000); banda por comparación ENTERA de R² — 0-float en el score/decisión ⇒ 2-cliente 0-desync. 🔑 INTRÍNSECAMENTE MULTIJUGADOR: single-player / P<2 ⇒ S=0 (colapso LIMPIO). CRUX (LA CRÍTICA) — GEOMETRÍA ANGULAR ⊥ TODOS los priores (conteo/tendencia/reparto/tipo): ⊥#128 MOMENTUM (dN/dt): N=6 estable en anillo (M=0, S=hi) vs N:2→6 todos a un lado (M=hi, S=0). ⊥#126 DENSITY (magnitud N): N=6 amontonados a un lado (density hi, S≈0) vs N=3 a 120° (density menor, S≈1) — S invariante a N. ⊥#124 PILE/#118 FOCUS/#121 SPREAD/#123 CONTEST/#125 MARGIN/#127 VARIETY (leen la aggro-TABLE — quién tankea o qué tipos): CERCO es ciego a QUIÉN tankea o QUÉ son los mobs; lee sólo DÓNDE están físicamente alrededor del héroe. [6 mobs todos en p1, pilados NE] (pile hi, S≈0) vs [6 mobs split, anillo 360°] (S≈1). ⊥ posición #107-#114 (pos/zona/heading absolutos): CERCO es la distribución angular RELATIVA de los mobs enganchados alrededor DEL HÉROE, no ninguna posición o facing absoluto. El 11º eje de COMPOSICIÓN-DE-INTENCIÓN (SPATIAL ENCIRCLEMENT / "estás cercado, sin lado seguro, aguanta el centro" — señal de posicionamiento MMORPG núcleo). Las posiciones de mob ya son server-authoritative + replicadas; el campo es un snapshot puro sobre ellas (sin estado persistido). Camino al 71º flag (serializa tras #128 AGGRO_MOMENTUM LIVE&served 00bb0660d3e1/815, base master 0e25cad).
export const AGGRO_SURROUND_SURGE = {
  enabled: true,             // LIVE (EVO#129, 71º flag). CAS-2751 flip false→true tras CEO Gate 2/2 no-drift PASS + DARK QA CAS-2750 PASS 20/20. SNAPSHOT PURO (SIN ring-buffer, a diferencia de #122/#128). Byte-neutral: el snapshot es LECTURA PURA; el fingerprint sigue world-build-only (sim.js:19184) ⇒ 2-cliente 0-desync fp 15920977.
  channel: "aggroSurroundFind", // canal FRESCO (fichas de cerco por rematar CERCADO 360° sin lado seguro) — NINGUNA de las 70 flags #59-#128 lo usa. Distinto por CONSTRUCCIÓN de aggroMomentumFind #128 (ESCALADA/dN/dt) / aggroVarietyFind #127 (HETEROGENEIDAD/tipos) / aggroDensityFind #126 (MAGNITUD N/P) / aggroMarginFind #125 (LEAD) / aggroPileFind #124 (max-share) / aggroContestFind #123 (AMPLITUD) / targetSpreadFind #121 (UNIFORMIDAD) / aggroSwitchFind #122 (CHURN): CERCO = la GEOMETRÍA ANGULAR del pack (encirclement 360°). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio de CONSIDERACIÓN (px) de la vecindad VIVA: S se computa sobre los mobs VIVOS ENGANCHADOS dentro del radio del héroe y los jugadores VIVOS en radio. Mirror #87/…/#128 radius. CEO balance knob.
  minMobs: 3,                // nº MÍNIMO de mobs aggro-ENGANCHADOS en radio (N) para que el cerco sea significativo: con <3 no se puede cercar de forma significativa ⇒ degenerado ⇒ 0. ≥3 ⇒ un anillo genuino. Con el filtro de muertos ⇒ ANTI-AUTO-CONTEO. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores VIVOS en radio (P) para que el cerco de la party esté DEFINIDO como party-relativo: con <2 (single-player) ⇒ colapsa a 0 (single-player LIMPIO). ≥2 ⇒ una party genuina. 🔑 el cerco compartido es INTRÍNSECAMENTE multijugador. CEO balance knob.
  angleBins: 64,             // resolución de CUANTIZACIÓN del rumbo (LUT ENTERO de 64 vectores unidad ×1000): cada rumbo hero→mob se cuantiza a 1 de 64 bins vía atan2→bin ⇒ resultante ENTERA ⇒ 0-float en la decisión ⇒ 2-cliente byte-idéntico. Más bins = ángulo más fino. CEO balance knob.
  midSurround: 0.5,          // umbral de S para la banda SOME/cerco-parcial (peso 1): S≥0.5 (medio-cercado). Banda por comparación ENTERA de R²: midLim=(1−0.5)*N*1000 ⇒ mag²≤midLim² ⟺ R≤0.5 ⟺ S≥0.5. CEO balance knob.
  hiSurround: 0.75,          // umbral de S para la banda HEAVY/cercado (peso 2): S≥0.75 (rodeado, sin lado seguro). hiLim=(1−0.75)*N*1000 ⇒ mag²≤hiLim² ⟺ R≤0.25 ⟺ S≥0.75. S<midSurround (un-solo-lado) ⇒ 0. CEO balance knob.
  weights: { heavy: 2, some: 1 },   // PESO de un remate por la BANDA de CERCO/encirclement del pack ENGANCHADO VIVO en radio: HEAVY/cercado (S≥hiSurround) pesa 2; SOME/cerco-parcial (≥midSurround) pesa 1; un-solo-lado (<midSurround) ⇒ 0. Reward = rematar cuando estás genuinamente CERCADO (sin lado seguro), no cuando el pack está a un lado (flankable). CEO balance knobs.
  aggroSurroundBountyCap: 2, // SUB-CAP DURO PROPIO de las fichas de cerco por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.heavy)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE CERCO (score∈{0,1,2}). Pack más rodeando (más uniforme angularmente) = tier MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (un-solo-lado/single-player/<minMobs) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con SOME/cerco-parcial, S≥midSurround): +1 ficha de cerco por kill. "remataste medio-cercado (mobs abarcando >medio-círculo)".
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate en pleno HEAVY/cercado, S≥hiSurround): +2 fichas por kill. "remataste rodeado 360° (sin lado seguro)". CEO balance knobs.
  ],
};

// CAS-2753: ACOSO (AGGRO_PRESSURE_SURGE, EVO#130 DARK) — AÑADE la dimensión RADIAL-PROXIMITY (el OTRO componente GEOMÉTRICO del pack: cuán CERCA se aprieta, no DÓNDE está angularmente #129, no el conteo/tendencia/reparto/tipo) a la familia COMPOSICIÓN-DE-INTENCIÓN. #129 CERCO abrió la GEOMETRÍA PURA con el componente ANGULAR (¿los enganchados repartidos 360° o a un lado?). La geometría de un pack alrededor de un punto se descompone en EXACTAMENTE DOS componentes ORTOGONALES: ANGULAR (SURROUND, shipped) y RADIAL (PRESSURE, este eje). ACOSO = el apretón radial: ¿el pack enganchado está pegado a QUEMARROPA en tu cara (presión alta, sin aire) o abrazando el radio exterior de engage (presión baja, sitio para kitear)? MÉTRICA = P∈[0,1] = cercanía radial MEDIA de los mobs ENGANCHADOS VIVOS en radio. Por cada mob a distancia d≤radius: closeness=(radius−d)/radius ∈[0,1] (1 en el héroe, 0 en el borde); P = mean(closeness) sobre los N enganchados. Enjambre a quemarropa ⇒ P≈1 (apretado); mobs en el borde de engage ⇒ P≈0 (sitio). Bandas sobre P: midPress P≥0.5 ⇒ peso 1; hiPress P≥0.75 ⇒ peso 2. minMobs3 (N≥3) + minPlayers2 (P≥2). 🔑 SNAPSHOT/PURO (como #129 — SIN ring-buffer; lee las distancias replicadas de los mobs enganchados al héroe en UN frame). 🔑 DETERMINISMO (sev-1): cercanía por mob CUANTIZADA a niveles ENTEROS (closeBins) vía comparación ENTERA d²·Q² ≤ R²(Q−L)² (NO float sqrt en el score/decisión; banda por comparación de sumas ENTERAS) ⇒ 2-cliente 0-desync. 🔑 INTRÍNSECAMENTE MULTIJUGADOR: single-player / P<2 ⇒ P=0 (colapso LIMPIO). CRUX (LA CRÍTICA) — RADIAL POSICIÓN ⊥ TODOS los priores: ⊥#129 SURROUND (ANGULAR): SURROUND=dispersión angular (DÓNDE alrededor, invariante a distancia); PRESSURE=cercanía radial media (CUÁN CERCA, invariante a la disposición angular) — los DOS componentes ortogonales de la geometría del pack: N=3 en anillo 360° al borde (S=hi, P≈0) vs N=3 a un lado a quemarropa (S=0, P≈1). ⊥#126 DENSITY (magnitud N): PRESSURE es cercanía media normalizada, invariante a N: N=6 todos al borde (density hi, P lo) vs N=3 a quemarropa (density menor, P hi). ⊥#112 convergeFind (VELOCIDAD radial/closing speed): PRESSURE es POSICIÓN radial ESTÁTICA (cuán cerca AHORA), CERO dependencia del movimiento: pack a quemarropa e INMÓVIL (converge=0, P=hi) vs pack lejos pero entrando rápido (converge=hi, P=lo). ⊥#128 MOMENTUM (dN/dt), ⊥#124 PILE/#118 FOCUS/#121 SPREAD/#123 CONTEST/#125 MARGIN/#127 VARIETY (aggro-TABLE — quién tankea/qué tipo): PRESSURE es ciego a tendencia, a quién sostiene aggro y a qué son los mobs; lee sólo cuán FÍSICAMENTE cerca está el pack enganchado. ⊥ posición #107-#114 (pos/zona/heading absolutos): PRESSURE es la distribución radial RELATIVA de los enganchados alrededor DEL HÉROE, no ninguna posición absoluta. El 12º eje de COMPOSICIÓN-DE-INTENCIÓN (RADIAL PROXIMITY / "los tienes encima, sin aire, crea espacio" — señal de spacing MMORPG núcleo). Posiciones de mob = server-authoritative + replicadas; el campo es snapshot puro (sin estado persistido). Camino al 72º flag (base master fa17d8c tras #129 AGGRO_SURROUND LIVE, served 61dd36e1005a/815).
export const AGGRO_PRESSURE_SURGE = {
  enabled: true,             // LIVE (EVO#130, 72º flag). CAS-2755 flip false→true tras CEO Gate 1/2+2/2 + DARK QA CAS-2754 PASS 20/20. Byte-neutral seams: el snapshot es LECTURA PURA; fingerprint = world-build-only ⇒ fp 15920977 estable.
  channel: "aggroPressureFind", // canal FRESCO (fichas de acoso por rematar APRETADO a quemarropa) — NINGUNA de las 71 flags #59-#129 lo usa. Distinto por CONSTRUCCIÓN de aggroSurroundFind #129 (ANGULAR/dispersión) / aggroMomentumFind #128 (dN/dt) / aggroVarietyFind #127 (tipos) / aggroDensityFind #126 (MAGNITUD N/P): ACOSO = la CERCANÍA RADIAL del pack (radial squeeze). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio de CONSIDERACIÓN (px): P se computa sobre los mobs VIVOS ENGANCHADOS dentro del radio del héroe y los jugadores VIVOS en radio. Mirror #87/…/#129 radius. CEO balance knob.
  minMobs: 3,                // nº MÍNIMO de mobs aggro-ENGANCHADOS en radio (N) para que el acoso sea significativo: con <3 degenerado ⇒ 0. Con el filtro de muertos ⇒ ANTI-AUTO-CONTEO. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores VIVOS en radio (P): con <2 (single-player) ⇒ colapsa a 0 (single-player LIMPIO). El acoso compartido es INTRÍNSECAMENTE multijugador. CEO balance knob.
  closeBins: 64,             // resolución de CUANTIZACIÓN de la cercanía radial (Q): la cercanía de cada mob se cuantiza a 1 de Q niveles ENTEROS ∈[0,Q] vía comparación ENTERA Q²·d² ≤ R²(Q−L)² ⇒ 0-float sqrt en la decisión ⇒ 2-cliente byte-idéntico. Q múltiplo de 4 ⇒ límites de banda (hi/mid=k/4)·N·Q ENTEROS. Más bins = cercanía más fina. CEO balance knob.
  midPress: 0.5,             // umbral de P para la banda SOME/acoso-parcial (peso 1): P≥0.5 (medio-apretado). Banda por comparación ENTERA: sum≥mid·N·Q (con Q múlt. de 4 ⇒ 0.5·N·Q=N·Q/2 ENTERO). CEO balance knob.
  hiPress: 0.75,             // umbral de P para la banda HEAVY/apretado (peso 2): P≥0.75 (a quemarropa, sin aire). sum≥hi·N·Q (0.75·N·Q=3·N·Q/4 ENTERO). P<midPress (con sitio) ⇒ 0. CEO balance knob.
  weights: { heavy: 2, some: 1 },   // PESO de un remate por la BANDA de PRESIÓN/cercanía del pack ENGANCHADO VIVO en radio: HEAVY/apretado (P≥hiPress) pesa 2; SOME/acoso-parcial (≥midPress) pesa 1; con-sitio (<midPress) ⇒ 0. Reward = rematar cuando estás genuinamente ACOSADO (sin aire), no cuando el pack está en el borde (kiteable). CEO balance knobs.
  aggroPressureBountyCap: 2, // SUB-CAP DURO PROPIO de las fichas de acoso por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.heavy)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE PRESIÓN (score∈{0,1,2}). Pack más apretado (mayor cercanía media) = tier MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (con-sitio/single-player/<minMobs) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con SOME/acoso-parcial, P≥midPress): +1 ficha de acoso por kill. "remataste medio-apretado".
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate en pleno HEAVY/apretado, P≥hiPress): +2 fichas por kill. "remataste a quemarropa (sin aire)". CEO balance knobs.
  ],
};

// CAS-2757: BRÍO (AGGRO_VIGOR_SURGE, EVO#131 DARK) — AÑADE la dimensión MOB-HP-STATE (cuánta pelea LE QUEDA al pack ENGANCHADO — no DÓNDE está #129 ANGULAR/#130 RADIAL, no CUÁNTOS #126, no de qué TIPO #127, no a QUIÉN targetean #118/#121/#123/#124/#125) a la familia COMPOSICIÓN-DE-INTENCIÓN. Los dos últimos ejes cerraron la GEOMETRÍA PURA: #129 SURROUND (ANGULAR) + #130 PRESSURE (RADIAL) = los DOS componentes ortogonales de la POSICIÓN del pack. BRÍO abre una sub-familia genuinamente nueva: el ESTADO-DE-HP del mob. Lee lo ÚNICO que ningún flag previo (#59-#130) lee — la SALUD RESTANTE del pack enganchado. ¿El pack está FRESCO y a full-HP (pelea larga por delante, alta peligrosidad/compromiso) o MALTRECHO y casi limpiado (remátalo, bajo riesgo)? MÉTRICA = V∈[0,1] = fracción-de-HP MEDIA de los mobs ENGANCHADOS VIVOS en radio. Por cada mob: hpFrac=hp/hpMax ∈[0,1]; V=mean(hpFrac) sobre los N enganchados. Pack fresco ⇒ V≈1 (pelea entera); pack casi-limpiado ⇒ V≈0 (rematando). Bandas sobre V: midVigor V≥0.5 ⇒ peso 1; hiVigor V≥0.75 ⇒ peso 2 (premia enganchar un pack genuinamente fresco/peligroso). 🔑 SNAPSHOT/PURO (como #129/#130 — SIN ring-buffer; lee el HP replicado de los mobs enganchados en UN frame). 🔑 DETERMINISMO (sev-1): hp y hpMax son ENTEROS ⇒ el nivel-de-HP se CUANTIZA a niveles ENTEROS (vigBins Q) vía comparación ENTERA hp·Q ≥ L·hpMax; banda por comparación de sumas ENTERAS ⇒ 0-float division en el score/decisión ⇒ 2-cliente 0-desync. 🔑 INTRÍNSECAMENTE MULTIJUGADOR: single-player / P<2 ⇒ V=0 (colapso LIMPIO). CRUX (LA CRÍTICA) — HP-STATE ⊥ TODOS los priores (posición/conteo/velocidad/tendencia/reparto/tipo): ⊥#130 PRESSURE (RADIAL) & ⊥#129 SURROUND (ANGULAR): VIGOR es CIEGO a la posición. N=3 a quemarropa todos a full-HP (P=hi, V=hi) vs N=3 a quemarropa todos casi-muertos (P=hi, V=lo) — MISMA geometría, brío OPUESTO. Y un pack fresco al borde (P=lo, V=hi) vs un pack maltrecho a quemarropa (P=hi, V=lo). ⊥#126 DENSITY (magnitud N): VIGOR es fracción-de-HP media normalizada, invariante a N: N=6 todos casi-muertos (density hi, V lo) vs N=3 todos frescos (density menor, V hi). ⊥#112 convergeFind (VELOCIDAD radial): VIGOR es una lectura HP ESTÁTICA, 0 dependencia del movimiento. ⊥#128 MOMENTUM (dN/dt) / #122 SWITCH (churn): VIGOR es un snapshot del HP ACTUAL, no una tendencia. ⊥#127 VARIETY (tipo de mob): VIGOR es CIEGO a QUÉ son los mobs; lee sólo su HP. 3 mobs idénticos al 20% HP (variety lo, V lo) vs 3 tipos distintos a full (variety hi, V hi). ⊥ aggro-TABLE (#118 FOCUS/#121 SPREAD/#123 CONTEST/#124 PILE/#125 MARGIN): VIGOR es CIEGO a quién sostiene el aggro; lee el HP del mob. El 13º eje de COMPOSICIÓN-DE-INTENCIÓN (MOB-HP-STATE / "cuánta pelea queda — comprométete o remata" — señal núcleo de engage/disengage MMORPG). El HP del mob = server-authoritative + replicado; el campo es un snapshot puro (sin estado persistido). Camino al 73º flag (base master 8c82c7b tras #130 AGGRO_PRESSURE LIVE, served 297d259bb5f9/815).
export const AGGRO_VIGOR_SURGE = {
  enabled: true,             // LIVE (EVO#131, 73º flag) — CAS-2759 flip false→true tras CEO Gate 1/2 (byteverify) + Gate 2/2 (no-drift) + DARK QA CAS-2758 PASS 21/21 / 0-desync / fpMatch 15920977. byte-neutral OFF: el snapshot es LECTURA PURA (0 side-effects); fingerprint = world-build-only ⇒ fp 15920977 estable.
  channel: "aggroVigorFind", // canal FRESCO (fichas de brío por enganchar/rematar un pack FRESCO a full-HP) — NINGUNA de las 72 flags #59-#130 lo usa. Distinto por CONSTRUCCIÓN de aggroPressureFind #130 (RADIAL/cercanía) / aggroSurroundFind #129 (ANGULAR/dispersión) / aggroDensityFind #126 (MAGNITUD N) / aggroVarietyFind #127 (tipos): BRÍO = el ESTADO-DE-HP del pack (cuánta pelea le queda). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio de CONSIDERACIÓN (px): V se computa sobre los mobs VIVOS ENGANCHADOS dentro del radio del héroe y los jugadores VIVOS en radio. Mirror #87/…/#130 radius. CEO balance knob.
  minMobs: 3,                // nº MÍNIMO de mobs aggro-ENGANCHADOS en radio (N) para que el brío sea significativo: con <3 degenerado ⇒ 0. Con el filtro de muertos ⇒ ANTI-AUTO-CONTEO. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores VIVOS en radio (P): con <2 (single-player) ⇒ colapsa a 0 (single-player LIMPIO). El brío compartido es INTRÍNSECAMENTE multijugador. CEO balance knob.
  vigBins: 64,               // resolución de CUANTIZACIÓN de la fracción-de-HP (Q): la fracción de cada mob se cuantiza a 1 de Q niveles ENTEROS ∈[0,Q] vía comparación ENTERA hp·Q ≥ L·hpMax ⇒ 0-float division en la decisión ⇒ 2-cliente byte-idéntico. Q múltiplo de 4 ⇒ límites de banda (hi/mid=k/4)·N·Q ENTEROS. Más bins = fracción más fina. CEO balance knob.
  midVigor: 0.5,             // umbral de V para la banda SOME/medio-fresco (peso 1): V≥0.5. Banda por comparación ENTERA: sum≥mid·N·Q (con Q múlt. de 4 ⇒ 0.5·N·Q=N·Q/2 ENTERO). CEO balance knob.
  hiVigor: 0.75,             // umbral de V para la banda FRESH/full-HP (peso 2): V≥0.75 (pack fresco, pelea entera). sum≥hi·N·Q (0.75·N·Q=3·N·Q/4 ENTERO). V<midVigor (maltrecho) ⇒ 0. CEO balance knob.
  weights: { fresh: 2, some: 1 },   // PESO de un remate por la BANDA de BRÍO/HP del pack ENGANCHADO VIVO en radio: FRESH/full-HP (V≥hiVigor) pesa 2; SOME/medio-fresco (≥midVigor) pesa 1; maltrecho (<midVigor) ⇒ 0. Reward = enganchar/rematar cuando el pack está genuinamente FRESCO (pelea larga, peligro alto), no cuando ya está casi limpiado. CEO balance knobs.
  aggroVigorBountyCap: 2,    // SUB-CAP DURO PROPIO de las fichas de brío por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.fresh)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE BRÍO (score∈{0,1,2}). Pack más fresco (mayor HP media) = tier MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (maltrecho/single-player/<minMobs) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con SOME/medio-fresco, V≥midVigor): +1 ficha de brío por kill. "remataste un pack medio-fresco".
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (enganche/remate en pleno FRESH/full-HP, V≥hiVigor): +2 fichas por kill. "enganchaste un pack fresco (pelea entera)". CEO balance knobs.
  ],
};

// CAS-2763: TEMPLE (PARTY_VITAL_SURGE, EVO#132 DARK) — CIERRA la sub-familia HP-STATE con el LADO ALIADO. #131 VIGOR abrió HP-STATE leyendo la SALUD RESTANTE del pack ENEMIGO (fresco vs maltrecho); TEMPLE lee lo ÚNICO que ningún flag #59-#131 lee — la SALUD RESTANTE de la PARTY de JUGADORES enganchada. ¿La party está FRESCA y a full-HP (puede comprometerse / apretar la pelea) o MALTRECHA y al borde (retirarse / clutch)? Es el compañero espejo-pero-ortogonal de #131, exactamente como #129 SURROUND (ANGULAR) + #130 PRESSURE (RADIAL) fueron los dos componentes de la POSICIÓN del pack — aquí #131 (HP del mob) + #132 (HP del aliado) son los DOS SUJETOS del HP-STATE. MÉTRICA = W∈[0,1] = fracción-de-HP MEDIA de los JUGADORES VIVOS de la party enganchada en radio. Por cada jugador: hpFrac=hp/hpMax ∈[0,1]; W=mean(hpFrac) sobre los P jugadores. Party fresca ⇒ W≈1; party maltrecha ⇒ W≈0. Bandas sobre W: midVital W≥0.5 ⇒ peso 1; hiVital W≥0.75 ⇒ peso 2 (premia comprometer/rematar cuando la party está genuinamente sana — sustain con destreza, no un trade desesperado). 🔑 SNAPSHOT/PURO (como #129/#130/#131 — SIN ring-buffer; lee el HP replicado de los jugadores en UN frame). minMobs3 (un pack enganchado DEBE estar presente, N≥3) + minPlayers2 (P≥2). 🔑 MULTIJUGADOR-NATIVO: single-player / P<2 ⇒ W=0 (colapso LIMPIO — la vitalidad de la party es intrínsecamente multijugador). 🔑 DETERMINISMO (sev-1): hp y hpMax son ENTEROS ⇒ el nivel-de-HP se CUANTIZA a niveles ENTEROS (vitBins Q) vía comparación ENTERA hp·Q ≥ L·hpMax; banda por comparación de sumas ENTERAS ⇒ 0-float division en el score/decisión ⇒ 2-cliente 0-desync. CRUX (LA CRÍTICA) — ALLY-HP-STATE ⊥ TODOS los priores: ⊥#131 VIGOR (MOB-HP-STATE): VIGOR lee el HP del ENEMIGO; TEMPLE lee el HP del JUGADOR. party-maltrecha vs pack-fresco (W lo, V hi = peligro) vs party-sana vs pack-maltrecho (W hi, V lo = limpieza) — MISMA pelea, sujeto OPUESTO (la disociación crux). ⊥#130 PRESSURE (RADIAL) & ⊥#129 SURROUND (ANGULAR): TEMPLE es CIEGO a la posición (misma geometría, distinto HP de party). ⊥#126 DENSITY (magnitud): fracción-de-HP media normalizada, invariante a P y N. ⊥#127 VARIETY (tipo de mob) / aggro-TABLE #118/#121/#123/#124/#125: ciego al tipo/reparto/quién-tankea; lee el HP del jugador. ⊥#128 MOMENTUM / #122 SWITCH: snapshot del HP ACTUAL de la party, no una tendencia. El 14º eje de COMPOSICIÓN-DE-INTENCIÓN (ALLY-HP-STATE / "¿puede sostener la party — aprieta o retírate?" — señal núcleo de commit/retreat MMORPG). El HP del jugador = server-authoritative + replicado; el campo es un snapshot puro. Camino al 74º flag (base master 472dd92 tras #131 AGGRO_VIGOR LIVE, served a05466f6d03e/815).
export const PARTY_VITAL_SURGE = {
  enabled: true,             // LIVE — EVO#132 (CAS-2765, 74º flag). Flip false→true tras CEO Gate 1/2 (byteverify) + Gate 2/2 (no-drift) PASS + DARK QA CAS-2764 PASS 21/21 ×2 @3f91e84 (fp 15920977, 0-desync 2-cli). Base master 3f91e84. Mirror #131 flip CAS-2759.
  channel: "partyVitalFind", // canal FRESCO (fichas de temple por comprometer/rematar con la PARTY sana a full-HP) — NINGUNA de las 73 flags #59-#131 lo usa. Distinto por CONSTRUCCIÓN de aggroVigorFind #131 (HP del ENEMIGO) / aggroPressureFind #130 (RADIAL) / aggroSurroundFind #129 (ANGULAR) / aggroDensityFind #126 (MAGNITUD N) / aggroVarietyFind #127 (tipos): TEMPLE = el ESTADO-DE-HP de la PARTY ALIADA. Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio de CONSIDERACIÓN (px): W se computa sobre los jugadores VIVOS en radio del héroe; requiere un pack VIVO ENGANCHADO (N≥minMobs) en radio. Mirror #87/…/#131 radius. CEO balance knob.
  minMobs: 3,                // nº MÍNIMO de mobs aggro-ENGANCHADOS en radio (N) para que el temple sea significativo: un pack enganchado DEBE estar presente; con <3 degenerado ⇒ 0. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores VIVOS en radio (P): con <2 (single-player) ⇒ colapsa a 0 (single-player LIMPIO). El temple de la party es INTRÍNSECAMENTE multijugador. CEO balance knob.
  vitBins: 64,               // resolución de CUANTIZACIÓN de la fracción-de-HP (Q): la fracción de cada jugador se cuantiza a 1 de Q niveles ENTEROS ∈[0,Q] vía comparación ENTERA hp·Q ≥ L·hpMax ⇒ 0-float division en la decisión ⇒ 2-cliente byte-idéntico. Q múltiplo de 4 ⇒ límites de banda (hi/mid=k/4)·P·Q ENTEROS. Más bins = fracción más fina. CEO balance knob.
  midVital: 0.5,             // umbral de W para la banda SOME/media-sana (peso 1): W≥0.5. Banda por comparación ENTERA: sum≥mid·P·Q (con Q múlt. de 4 ⇒ 0.5·P·Q=P·Q/2 ENTERO). CEO balance knob.
  hiVital: 0.75,             // umbral de W para la banda FRESH/full-HP (peso 2): W≥0.75 (party sana, puede comprometerse). sum≥hi·P·Q (0.75·P·Q=3·P·Q/4 ENTERO). W<midVital (maltrecha) ⇒ 0. CEO balance knob.
  weights: { fresh: 2, some: 1 },   // PESO de un remate por la BANDA de VITALIDAD/HP de la PARTY VIVA en radio: FRESH/full-HP (W≥hiVital) pesa 2; SOME/media-sana (≥midVital) pesa 1; maltrecha (<midVital) ⇒ 0. Reward = comprometer/rematar cuando la party está genuinamente SANA (puede sostener, sustain con destreza), no en un trade desesperado. CEO balance knobs.
  partyVitalBountyCap: 2,    // SUB-CAP DURO PROPIO de las fichas de temple por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.fresh)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE VITALIDAD (score∈{0,1,2}). Party más sana (mayor HP media) = tier MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (maltrecha/single-player/<minMobs) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con la party SOME/media-sana, W≥midVital): +1 ficha de temple por kill. "rematas con la party medio-sana".
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (compromiso/remate con la party en pleno FRESH/full-HP, W≥hiVital): +2 fichas por kill. "aprietas con la party sana (puedes sostener)". CEO balance knobs.
  ],
};

// CAS-2767: RETO (MOB_TIER_SURGE, EVO#133 DARK) — abre la sub-familia MOB-POWER-RATING. Las flags de HP-STATE (#131 VIGOR = HP del mob, #132 TEMPLE = HP del aliado) leían cuánta pelea LE QUEDA; RETO lee una propiedad INTRÍNSECA distinta que ningún flag #59-#132 toca: cuán PELIGROSO POR DISEÑO es el pack ENGANCHADO — su TIER/RANGO-DE-RETO (challenge rating), independiente del HP ACTUAL. Un pack de élites de alto tier a full-HP y un pack de basura de bajo tier a full-HP tienen IDÉNTICO HP-state (#131 V-hi) pero poder OPUESTO. Premia comprometerse contra un pack GENUINAMENTE de alto tier (reto con destreza), no farmear basura. MÉTRICA = T∈[0,1] = tier-de-reto MEDIO normalizado de los mobs VIVOS ENGANCHADOS en radio. Por cada mob ENGANCHADO: tier = su RANGO-DE-PODER intrínseco (mobTierRank, la ESCALERA server-auth boss/capstone=4 > champion/champElite=3 > elite=2 > afijo=1 > normal=0 — el ÚNICO tier-de-poder intrínseco que carga un mob, ya que los mobs planos no llevan `.level` numérico; #73 APEX lee ESE contenedor SÓLO como booleano de proximidad de UN apex, NUNCA como MEDIA del pack). T=mean(tier)/tierCap clamped. Pack basura ⇒ T≈0; pack élite ⇒ T≈1. Bandas sobre T: midTier T≥0.5 ⇒ peso 1; hiTier T≥0.75 ⇒ peso 2. 🔑 SNAPSHOT/PURO (como #129/#130/#131/#132 — SIN ring-buffer; lee el rango replicado del mob en UN frame). minMobs3 (N≥3) + minPlayers2 (P≥2). 🔑 MULTIJUGADOR-NATIVO: single-player / P<2 ⇒ T=0 (colapso LIMPIO). 🔑 DETERMINISMO (sev-1): el rango de cada mob es un ENTERO pequeño ⇒ la decisión se mantiene ENTERA (Σ rangos ENTEROS vs umbrales ENTEROS k·tierCap·N/4; 0-float division en el score/decisión; float SÓLO cosmético para el badge idx) ⇒ 2-cliente 0-desync. CRUX (LA CRÍTICA) — MOB-POWER-RATING ⊥ TODOS los priores: ⊥#131 VIGOR (MOB-HP-STATE): VIGOR lee la FRACCIÓN-DE-HP restante; RETO lee el TIER intrínseco. `pack élite al 20% HP` (T hi, V lo) vs `pack basura a full-HP` (T lo, V hi) — poder opuesto, HP opuesto (la disociación crux). ⊥#132 TEMPLE (HP del aliado): lee el poder del mob, no el HP del jugador. ⊥#127 VARIETY (tipo del mob): ciego a la mezcla de tipos — 3 élites idénticos de alto tier (T hi, variety lo) vs 3 de tipo mixto de bajo tier (T lo, variety hi). ⊥#126 DENSITY (conteo): media normalizada, invariante a N y P. ⊥#130 PRESSURE/#129 SURROUND (posición): el tier-de-poder es CIEGO a la posición. ⊥#128 MOMENTUM/#122 SWITCH: snapshot del tier ACTUAL del pack, no una tendencia. ⊥#91 ZONE-TIER (dificultad del ÁREA): RETO lee el rango POR-MOB del pack enganchado, no el tier de la zona. El 15º eje de COMPOSICIÓN-DE-INTENCIÓN (MOB-POWER-RATING / "¿es este pack un reto real o basura?" — señal núcleo de riesgo/recompensa-de-compromiso MMORPG). El rango/tier del mob = server-authoritative + replicado; el campo es un snapshot puro. Camino al 75º flag (base master c69e63c tras #132 PARTY-VITALITY LIVE, served a3468f49fc9f/815).
export const MOB_TIER_SURGE = {
  enabled: true,             // LIVE (EVO#133, 75º flag) CAS-2772 flip false→true — byte-neutral OFF: el snapshot es LECTURA PURA (0 side-effects); fingerprint = world-build-only ⇒ fp 15920977 estable. CEO Gate 1/2 (byteverify) + Gate 2/2 (no-drift) + DARK QA CAS-2771 PASS 22/22 ×2 0-desync.
  channel: "mobTierFind",    // canal FRESCO (fichas de reto por comprometer/rematar contra un pack de ALTO TIER) — NINGUNA de las 74 flags #59-#132 lo usa. Distinto por CONSTRUCCIÓN de aggroVigorFind #131 (HP del ENEMIGO) / partyVitalFind #132 (HP del ALIADO) / aggroVarietyFind #127 (tipos) / aggroDensityFind #126 (MAGNITUD N): RETO = el TIER-DE-PODER intrínseco del pack. Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio de CONSIDERACIÓN (px): T se computa sobre los mobs ENGANCHADOS VIVOS en radio del héroe; requiere N≥minMobs y P≥minPlayers en radio. Mirror #87/…/#132 radius. CEO balance knob.
  minMobs: 3,                // nº MÍNIMO de mobs aggro-ENGANCHADOS en radio (N) para que el tier-de-reto sea significativo: un pack DEBE estar presente; con <3 degenerado ⇒ 0. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores VIVOS en radio (P): con <2 (single-player) ⇒ colapsa a 0 (single-player LIMPIO). El reto de pack es INTRÍNSECAMENTE multijugador. CEO balance knob.
  tierCap: 4,                // TECHO del rango-de-poder por mob (Q): mobTierRank ∈[0,tierCap] (boss/capstone=4, champion/champElite=3, elite=2, afijo=1, normal=0). T=Σrango/(N·tierCap). Múltiplo de 4 ⇒ límites de banda (hi/mid=k/4)·tierCap ENTEROS. CEO balance knob.
  midTier: 0.5,              // umbral de T para la banda MID/reto-medio (peso 1): T≥0.5. Banda por comparación ENTERA: sum≥mid·N·tierCap (con tierCap=4 ⇒ 0.5·N·4=2N ENTERO). CEO balance knob.
  hiTier: 0.75,              // umbral de T para la banda HI/élite (peso 2): T≥0.75 (pack de alto tier, reto real). sum≥hi·N·tierCap (0.75·N·4=3N ENTERO). T<midTier (basura) ⇒ 0. CEO balance knob.
  weights: { elite: 2, some: 1 },   // PESO de un remate por la BANDA de TIER-DE-PODER del pack ENGANCHADO en radio: HI/élite (T≥hiTier) pesa 2; MID/reto-medio (≥midTier) pesa 1; basura (<midTier) ⇒ 0. Reward = comprometerse/rematar contra un pack GENUINAMENTE de alto tier (reto con destreza), no farmear basura. CEO balance knobs.
  mobTierBountyCap: 2,       // SUB-CAP DURO PROPIO de las fichas de reto por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.elite)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE TIER-DE-PODER (score∈{0,1,2}). Pack de mayor tier medio = tier de recompensa MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (basura/single-player/<minMobs) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate contra el pack MID/reto-medio, T≥midTier): +1 ficha de reto por kill. "rematas contra un pack de tier medio".
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (compromiso/remate contra el pack en pleno HI/élite, T≥hiTier): +2 fichas por kill. "te comprometes contra un pack élite (reto real)". CEO balance knobs.
  ],
};

// CAS-2775: CASTA (ELITE_SHARE_SURGE, EVO#134 DARK) — 16º eje de COMPOSICIÓN-DE-INTENCIÓN, 2º de la sub-familia MOB-POWER-RATING (abierta por #133 RETO). Premia comprometerse contra un pack que es MAYORÍA-ÉLITE POR COMPOSICIÓN — qué FRACCIÓN del pack enganchado es genuinamente peligrosa — señal DISTINTA de #133 RETO que lee la MAGNITUD MEDIA de poder. Usa la MISMA ESCALERA server-auth `mobTierRank` que #133 (boss/capstone=4 > champion/champElite=3 > elite=2 > afijo=1 > normal=0) pero como PROPORCIÓN-CON-UMBRAL, NO como media. MÉTRICA = S∈[0,1] = count(mobs VIVOS ENGANCHADOS con mobTierRank ≥ eliteRank [élite+]) / N_enganchados. Bandas sobre S: S≥midShare (0.5) ⇒ peso 1; S≥hiShare (0.75) ⇒ peso 2. minMobs3 (N≥3) + minPlayers2 (P≥2); single-player / P<2 ⇒ S=0 (colapso LIMPIO — multijugador-nativo). 🔑 SNAPSHOT PURO/STATELESS (como #129/#130/#131/#132/#133 — SIN ring-buffer; lee el rango replicado en UN frame); h.eliteShareBounty transitorio. 🔑 DETERMINISMO (sev-1): la decisión se mantiene ENTERA — count(rank≥eliteRank) comparado a umbrales ENTEROS ⌈k·N⌉ (share·N con k∈{0.5,0.75} es producto EXACTO IEEE-754 + Math.ceil devuelve ENTERO exacto ⇒ ambos operandos enteros); 0-float DIVISION en score/decisión (float SÓLO cosmético para el badge idx) ⇒ 2-cliente 0-desync. CRUX — CASTA ⊥ TODOS los priores: ⊥#133 RETO (media de tier): RETO lee mean(rank)/cap; CASTA lee la fracción por encima del umbral élite — `{boss(4),normal(0),normal(0)}` ⇒ RETO mean=1.33 (T≈0.33, peso 0) pero elite-share=1/3; `{elite(2),elite(2),normal(0)}` ⇒ MISMA media 1.33 (RETO 0) pero elite-share=2/3 (peso 1) — un ápice vs dos élites = misma media, share OPUESTO. ⊥#131 VIGOR / ⊥#132 TEMPLE (lee rango intrínseco, no HP). ⊥#127 VARIETY (ciego a la mezcla de tipos). ⊥#126 DENSITY (fracción normalizada, invariante a N). ⊥#129/#130 (ciego a la posición). ⊥#128 MOMENTUM (snapshot, no tendencia). ⊥#91 ZONE-TIER (rango POR-MOB del pack, no el tier de la ZONA). ⊥#73 APEX (APEX = booleano de proximidad de UN apex; CASTA = FRACCIÓN del pack por encima del umbral). Camino al 76º flag (base master 0062597 tras #133 MOB_TIER LIVE, served edf6508226ba/815).
export const ELITE_SHARE_SURGE = {
  enabled: true,             // LIVE (EVO#134, 76º flag) — CAS-2777 LIVE flip false→true (config-only 1-line, arc #59-#134). Byte-neutral OFF verified: el snapshot es LECTURA PURA (0 side-effects; _eliteSharePre=0 const inerte + seam rama muerta + h.eliteShareBounty NUNCA creado + G._eliteShareParty NUNCA creada) ⇒ world-build fingerprint = 15920977 estable, version.json UNTOUCHED edf6508226ba/815.
  channel: "eliteShareFind", // canal FRESCO (fichas de casta por comprometer/rematar contra un pack MAYORÍA-ÉLITE por composición) — NINGUNA de las 75 flags #59-#133 lo usa. Distinto por CONSTRUCCIÓN de mobTierFind #133 (MEDIA del rango): CASTA = la FRACCIÓN por encima del umbral élite. Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio de CONSIDERACIÓN (px): S se computa sobre los mobs ENGANCHADOS VIVOS en radio del héroe; requiere N≥minMobs y P≥minPlayers en radio. Mirror #131/#132/#133 radius. CEO balance knob.
  minMobs: 3,                // nº MÍNIMO de mobs aggro-ENGANCHADOS en radio (N) para que la composición sea significativa: un pack DEBE estar presente; con <3 degenerado ⇒ 0. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores VIVOS en radio (P): con <2 (single-player) ⇒ colapsa a 0 (single-player LIMPIO). La composición de pack es INTRÍNSECAMENTE multijugador. CEO balance knob.
  eliteRank: 2,              // UMBRAL de rango para contar un mob como "peligroso"/élite+: se cuenta un mob si mobTierRank(e) ≥ eliteRank. =2 (elite) ⇒ elite/champion/champElite/boss/capstone cuentan; afijo(1)/normal(0) NO. CEO balance knob.
  tierCap: 4,                // TECHO del rango-de-poder por mob (mobTierRank ∈[0,tierCap]); usado por el probe/clamp. Mirror #133. CEO balance knob.
  midShare: 0.5,             // umbral de S (fracción élite) para la banda MID/casta-parcial (peso 1): S≥0.5. Decisión ENTERA: count ≥ ⌈0.5·N⌉. CEO balance knob.
  hiShare: 0.75,             // umbral de S para la banda HI/casta-mayoritaria (peso 2): S≥0.75 (pack mayoría-élite). count ≥ ⌈0.75·N⌉. S<midShare ⇒ 0. CEO balance knob.
  weights: { elite: 2, some: 1 },   // PESO de un remate por la BANDA de fracción-élite del pack ENGANCHADO: HI/mayoría-élite (S≥hiShare) pesa 2; MID/casta-parcial (≥midShare) pesa 1; poca-élite (<midShare) ⇒ 0. Reward = comprometerse contra un pack MAYORÍA-ÉLITE, no farmear un pack de relleno con un solo ápice. CEO balance knobs.
  eliteShareBountyCap: 2,    // SUB-CAP DURO PROPIO de las fichas de casta por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.elite)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE FRACCIÓN-ÉLITE (score∈{0,1,2}). Mayor fracción élite = tier de recompensa MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (poca-élite/single-player/<minMobs) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate contra un pack casta-parcial, S≥midShare): +1 ficha de casta por kill.
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (compromiso/remate contra un pack MAYORÍA-ÉLITE, S≥hiShare): +2 fichas por kill. CEO balance knobs.
  ],
};

// CAS-2783: BRECHA (RANK_SPREAD_SURGE, EVO#135 DARK) — 17º eje de COMPOSICIÓN-DE-INTENCIÓN, 3º y CIERRE de la sub-familia MOB-POWER-RATING (tras #133 RETO y #134 CASTA). La sub-familia lee la DISTRIBUCIÓN de poder del pack vía tres momentos ortogonales: #133 RETO = MEDIA (mean(rank)/cap), #134 CASTA = FRACCIÓN-CON-UMBRAL (count(rank≥eliteRank)/N), #135 BRECHA = RANGO/DISPERSIÓN (cuán SEPARADOS están los tiers de poder del pack). MÉTRICA = R = maxRank − minRank sobre los mobs VIVOS ENGANCHADOS, usando la MISMA ESCALERA server-auth `mobTierRank` que #133/#134 (boss/capstone=4 > champion/champElite=3 > elite=2 > afijo=1 > normal=0). R∈[0,4] ENTERO por construcción. Bandas sobre R: R≥midSpread (2) ⇒ peso 1; R≥hiSpread (3) ⇒ peso 2. Premia comprometerse contra un pack de AMENAZA-MIXTA (un ápice escoltado por relleno — composición liderada por élite), NO un pack uniforme. minMobs3 (N≥3) + minPlayers2 (P≥2); single-player / P<2 ⇒ R=0 (colapso LIMPIO — multijugador-nativo, mirror #133/#134). 🔑 SNAPSHOT PURO/STATELESS (como #129–#134 — SIN ring-buffer; lee el rango replicado en UN frame); h.rankSpreadBounty transitorio. 🔑 DETERMINISMO (sev-1) — el MÁS LIMPIO del arco: R = maxRank − minRank es RESTA ENTERA de dos rangos ENTEROS; los umbrales de banda {2,3} son ENTEROS; CERO float, CERO division en score/decisión (float SÓLO cosmético para el badge idx) ⇒ 2-cliente 0-desync garantizado. CRUX (LA CRÍTICA) — BRECHA ⊥ TODOS los priores, DISOCIA de #133 RETO Y #134 CASTA SIMULTÁNEAMENTE (flagship): `{4,2,0}` vs `{3,3,0}` ⇒ IDÉNTICA media RETO (2) E IDÉNTICA fracción-élite CASTA (2/3), pero SPREAD R=4 vs R=3 — misma magnitud, misma fracción, DISPERSIÓN distinta. Además `{4,0,0}` (R=4, w2) vs `{2,2,2}` (R=0, w0): RETO mean 1.33 vs 2, CASTA 1/3 vs 3/3, SPREAD máx vs cero — los TRES ejes discrepan. ⊥#127 VARIETY (mezcla de tipos, ciego a la escalera de poder). ⊥#126 DENSITY (conteo, invariante a la composición: `{4,0}` vs `{4,0,0,0}` MISMO R=4). ⊥#131 VIGOR / #132 TEMPLE (estado de HP, no rango). ⊥#129/#130 (posición). ⊥#128 MOMENTUM (snapshot, no tendencia). ⊥#91 ZONE-TIER (rango POR-MOB del pack, no el tier de la ZONA). ⊥#73 APEX (booleano de proximidad de apex, no la brecha intra-pack). Camino al 77º flag (base master 7dca0983 tras #134 CASTA ELITE_SHARE LIVE, served 1b6b45728268/815).
export const RANK_SPREAD_SURGE = {
  enabled: true,             // LIVE (EVO#135, 77º flag) — CAS-2788 flip false→true. Snapshot LECTURA PURA (0 side-effects); DARK verified CEO Gate 1/2+2/2 + DARK QA CAS-2787 (23/23 ×3, fp 15920977, 0-desync).
  channel: "rankSpreadFind", // canal FRESCO (fichas de brecha por comprometer/rematar contra un pack de AMENAZA-MIXTA — tiers de poder muy SEPARADOS por RANGO) — NINGUNA de las 76 flags #59-#134 lo usa. Distinto por CONSTRUCCIÓN de mobTierFind #133 (MEDIA del rango) y eliteShareFind #134 (FRACCIÓN por encima del umbral): BRECHA = el RANGO maxRank−minRank. Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio de CONSIDERACIÓN (px): R se computa sobre los mobs ENGANCHADOS VIVOS en radio del héroe; requiere N≥minMobs y P≥minPlayers en radio. Mirror #131/#132/#133/#134 radius. CEO balance knob.
  minMobs: 3,                // nº MÍNIMO de mobs aggro-ENGANCHADOS en radio (N) para que la dispersión sea significativa: un pack DEBE estar presente; con <3 degenerado ⇒ 0. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores VIVOS en radio (P): con <2 (single-player) ⇒ colapsa a 0 (single-player LIMPIO). La composición de pack es INTRÍNSECAMENTE multijugador. CEO balance knob.
  tierCap: 4,                // TECHO del rango-de-poder por mob (mobTierRank ∈[0,tierCap]); usado por el probe/clamp + el badge idx cosmético (R/tierCap). Mirror #133/#134. CEO balance knob.
  midSpread: 2,              // umbral ENTERO de R (brecha de rango) para la banda MID/amenaza-mixta (peso 1): R≥2. Decisión ENTERA: maxRank−minRank ≥ 2. CEO balance knob.
  hiSpread: 3,               // umbral ENTERO de R para la banda HI/brecha-amplia (peso 2): R≥3 (ápice + relleno, spread apex-a-trash). R<midSpread ⇒ 0. CEO balance knob.
  weights: { wide: 2, some: 1 },   // PESO de un remate por la BANDA de brecha del pack ENGANCHADO: HI/brecha-amplia (R≥hiSpread) pesa 2; MID/amenaza-mixta (≥midSpread) pesa 1; pack-uniforme (<midSpread) ⇒ 0. Reward = comprometerse contra un pack de AMENAZA-MIXTA (ápice escoltado por relleno), no farmear un pack uniforme. CEO balance knobs.
  rankSpreadBountyCap: 2,    // SUB-CAP DURO PROPIO de las fichas de brecha por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.wide)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE BRECHA (score∈{0,1,2}). Mayor brecha de rango = tier de recompensa MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (pack-uniforme/single-player/<minMobs) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate contra un pack de amenaza-mixta, R≥midSpread): +1 ficha de brecha por kill.
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (compromiso/remate contra un pack de brecha-amplia ápice+relleno, R≥hiSpread): +2 fichas por kill. CEO balance knobs.
  ],
};

// CAS-2792: CÓNCLAVE (CO_STRIKE_SURGE, EVO#136 DARK) — 18º eje de COMPOSICIÓN-DE-INTENCIÓN, ABRE la NUEVA sub-familia PLAYER-COORDINATION (la señal es cómo se ENGANCHA la PARTY, no las stats del mob — la sub-familia MOB-POWER-RATING #133/#134/#135 quedó CERRADA por sus 3 momentos de distribución). La sub-familia PLAYER-COORDINATION lee la ESTRUCTURA de la aggro-table server-auth desde el lado JUGADOR. MÉTRICA (ENTERO PURO) = F = nº de JUGADORES DISTINTOS VIVOS en radio ENGANCHADOS por ≥1 mob del pack (targeteados por ≥1 mob aggro-ENGANCHADO) = la MUSTER / tamaño de la HUESTE-DE-GUERRA que converge en la MISMA pelea. F∈[0,P] ENTERO por construcción (cuenta de jugadores, no fracción). Bandas sobre F: F≥midMuster (2) ⇒ peso 1; F≥hiMuster (3) ⇒ peso 2. Premia comprometerse cuando MUCHOS aliados están METIDOS en la MISMA refriega (asalto coordinado), NO farmear en solitario. minMobs2 (N≥2 — hacen falta ≥2 mobs enganchados para que ≥2 jugadores estén targeteados, pues cada mob targetea a UN jugador) + minPlayers2 (P≥2); single-player ⇒ SÓLO el héroe enganchado ⇒ F=1<midMuster ⇒ 0 (colapso LIMPIO — IMPOSIBLE en solitario por construcción: se necesitan ≥2 jugadores PRESENTES Y enganchados). 🔑 SNAPSHOT PURO/STATELESS (como #129–#135 — SIN ring-buffer; lee la aggro-table replicada en UN frame, la MISMA que #121/#123/#124/#125); h.coStrikeBounty transitorio. 🔑 DETERMINISMO (sev-1): F = cuenta ENTERA de jugadores distintos targeteados vs umbrales ENTEROS {midMuster,hiMuster}; CERO float, CERO division en score/decisión (float SÓLO cosmético para el badge idx = F/P) ⇒ 2-cliente 0-desync garantizado. 🔑 MULTIJUGADOR-NATIVO (el eje MÁS multijugador-nativo del arco): literalmente NO puede dispararse en solitario. CRUX (LA CRÍTICA) — CÓNCLAVE = MAGNITUD CRUDA en el eje JUGADOR (cuántos aliados en la pelea), ⊥ TODOS los priores. ⊥#123 CONTEST (COBERTURA/fracción cov/P): CÓNCLAVE = cov CRUDO (conteo), CONTEST = cov/P (fracción). `{2 de 2}` F2/C1.0 vs `{2 de 4}` F2/C0.5 (MISMO F, distinta C); `{3 de 3}` F3/C1.0 vs `{2 de 2}` F2/C1.0 (MISMA C, distinto F) — CONTEO ⊥ FRACCIÓN, PRECEDENTE EXACTO de #126 DENSITY (conteo crudo N de mobs) ⊥ los flags de fracción: CÓNCLAVE abre la MAGNITUD-CRUDA en el eje JUGADOR igual que DENSITY la abrió en el eje MOB. ⊥#124 PILE (max mobs sobre 1 jugador): PILE mide CONCENTRACIÓN de mobs-por-jugador; CÓNCLAVE cuenta JUGADORES enganchados, INVARIANTE a cuántos mobs por jugador — `{6 mobs todos sobre 1 jugador}` PILE-hi/F1 (gated) vs `{1 mob c/u sobre 3 jugadores}` PILE-lo/F3, OPUESTO. ⊥#121 SPREAD (entropía de Shannon de la distribución mob→jugador): forma de la distribución vs headcount. ⊥#125 MARGIN (lead del top). ⊥#118 aggroFocus (fracción del pack sobre el héroe). ⊥#126 DENSITY (conteo de MOBS N, no de JUGADORES: `{2 jug,10 mobs}` vs `{4 jug,3 mobs}` — N y F anti-mueven). ⊥#127 VARIETY (tipos). ⊥#131 VIGOR/#132 TEMPLE (HP). ⊥#133/#134/#135 (distribución de PODER del mob). ⊥#128 MOMENTUM (snapshot, no tendencia). ⊥#129/#130 (posición/geometría — CÓNCLAVE lee la aggro-TABLE/targeting, NO coords). Canal FRESCO coStrikeFind — NINGUNO de los 77 flags #59-#135 lo usa. Camino al 78º flag (base master 91bf3e7 tras #135 BRECHA RANK_SPREAD LIVE, served 876e25989358/815).
export const CO_STRIKE_SURGE = {
  enabled: true,             // LIVE (EVO#136, 78º flag) — CAS-2795 flip false→true. Snapshot LECTURA PURA (0 side-effects; _coStrikePre=0 const inerte + seam rama muerta + h.coStrikeBounty transitorio + G._coStrikeParty transitoria) ⇒ world-build fingerprint = 15920977 estable. DARK verified CEO Gate 1/2+2/2 + DARK QA CAS-2794 (21/21 ×2, fp 15920977, 0-desync).
  channel: "coStrikeFind",   // canal FRESCO (fichas de cónclave por comprometer/rematar mientras ≥2 aliados están ENGANCHADOS en la MISMA refriega — la HUESTE que converge) — NINGUNO de los 77 flags #59-#135 lo usa. Distinto por CONSTRUCCIÓN de aggroContestFind #123 (FRACCIÓN cov/P), aggroPileFind #124 (max-share), targetSpreadFind #121 (entropía): CÓNCLAVE = el CONTEO CRUDO de jugadores enganchados (muster). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio de CONSIDERACIÓN (px): F se computa sobre los jugadores VIVOS + mobs ENGANCHADOS VIVOS en radio del héroe; requiere N≥minMobs y P≥minPlayers en radio. Mirror #121/#123/#124 radius. CEO balance knob.
  minMobs: 2,                // nº MÍNIMO de mobs aggro-ENGANCHADOS en radio (N): hacen falta ≥2 mobs para que ≥2 jugadores DISTINTOS puedan estar targeteados (cada mob targetea a UN jugador); con <2 degenerado ⇒ 0. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores VIVOS en radio (P): con <2 (single-player) ⇒ colapsa a 0 (single-player LIMPIO). La coordinación de party es INTRÍNSECAMENTE multijugador — IMPOSIBLE en solitario. CEO balance knob.
  midMuster: 2,              // umbral ENTERO de F (cuenta de jugadores enganchados) para la banda MID/cónclave-parcial (peso 1): F≥2 (≥2 aliados metidos en la refriega). Decisión ENTERA: #jugadores-distintos-targeteados ≥ 2. CEO balance knob.
  hiMuster: 3,               // umbral ENTERO de F para la banda HI/cónclave-pleno (peso 2): F≥3 (≥3 aliados convergen en la misma pelea). F<midMuster ⇒ 0. CEO balance knob.
  playerCap: 4,              // TECHO de jugadores usado por el probe/clamp + el badge idx cosmético (F/playerCap). CEO balance knob.
  weights: { full: 2, some: 1 },   // PESO de un remate por la BANDA de cónclave (cuántos aliados enganchados): HI/cónclave-pleno (F≥hiMuster) pesa 2; MID/cónclave-parcial (≥midMuster) pesa 1; solitario/dúo-mínimo (<midMuster) ⇒ 0. Reward = pelear JUNTO a la hueste, no en solitario. CEO balance knobs.
  coStrikeBountyCap: 2,      // SUB-CAP DURO PROPIO de las fichas de cónclave por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.full)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE CÓNCLAVE (score∈{0,1,2}). Más aliados enganchados = tier de recompensa MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (solitario/<minPlayers/<minMobs) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con la hueste parcial reunida, F≥midMuster): +1 ficha de cónclave por kill.
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (compromiso/remate con la hueste plena convergida, F≥hiMuster): +2 fichas por kill. CEO balance knobs.
  ],
};

// CAS-2797: COHORTE (CO_PRESENCE_SURGE, EVO#137 DARK) — 19º eje de COMPOSICIÓN-DE-INTENCIÓN, la SEGUNDA faceta de la sub-familia PLAYER-COORDINATION que ABRIÓ #136 CÓNCLAVE. Donde CÓNCLAVE mide la MUSTER de COMBATE (F=nº de jugadores ENGANCHADOS por ≥1 mob en la MISMA refriega), COHORTE mide la CO-PRESENCIA/RALLY: R = nº de JUGADORES DISTINTOS VIVOS DENTRO DEL RADIO del héroe, PRESENTES, INDEPENDIENTE de si algún mob los está enganchando. R∈[1,P] ENTERO por construcción (cuenta de jugadores presentes, no fracción). Bandas sobre R: R≥midRally (2) ⇒ peso 1; R≥hiRally (3) ⇒ peso 2. Premia AGRUPARSE físicamente en el mundo compartido (reunirse junto a la party), no cazar solo. Gate minPlayers2; single-player ⇒ SÓLO el héroe presente ⇒ R=1<midRally ⇒ 0 (colapso LIMPIO — IMPOSIBLE en solitario por construcción). 🔑 SNAPSHOT PURO/STATELESS (como #121/#123/#124/#136 — SIN ring-buffer; lee el roster de jugadores replicado en UN frame — las posiciones/roster YA están replicadas); h.coPresenceBounty transitorio. 🔑 DETERMINISMO (sev-1): R = cuenta ENTERA de jugadores presentes vs umbrales ENTEROS {midRally,hiRally}; CERO float, CERO division en score/decisión (float SÓLO cosmético para el badge idx = R/playerCap) ⇒ 2-cliente 0-desync garantizado. 🔑 MULTIJUGADOR-NATIVO: literalmente NO puede dispararse en solitario (R=1 ⇒ 0). CRUX (LA CRÍTICA) — COHORTE = CO-PRESENCIA (roster presente), ⊥ CÓNCLAVE #136 (muster ENGANCHADA en combate): {3 presentes, 1 enganchado} = R3/COHORTE-2 pero CÓNCLAVE-F1/0 (una party rallevada que aún NO pelea puntúa R pero no F); {2 presentes, 2 enganchados} = R2/COHORTE-1 y CÓNCLAVE-F2/1. PRESENTE ⊥ ENGANCHADO — COHORTE es el eje de CO-PRESENCIA, CÓNCLAVE el de CO-ATAQUE. ⊥#123 CONTEST (cobertura fracción cov/P): COHORTE = conteo CRUDO de presentes. ⊥#124 PILE (mobs-por-jugador). ⊥#121 SPREAD (entropía). ⊥#126 DENSITY (conteo de MOBS, no de JUGADORES). ⊥#133/#134/#135 (distribución de PODER del mob). ⊥#128 MOMENTUM (snapshot, no tendencia). ⊥#129/#130 (COHORTE cuenta JUGADORES en radio — NO coords/geometría del mob). ⊥ CONGREGATION (headcount ZONAL-COMPLETO que alimenta un pasivo restedMult: COHORTE es RADIO-LOCAL alrededor del héroe (300px) y alimenta un canal FRESCO de fichas-por-kill — {8 jugadores dispersos por la zona} ⇒ CONGREGATION T3 pero COHORTE R1 (nadie cerca); {3 jugadores pegados al héroe en zona vacía} ⇒ COHORTE R3 pero CONGREGATION T1 — LOCAL ⊥ ZONAL, canal find ⊥ canal restedMult). ⊥ FELLOWSHIP/KINSHIP (bonos de banda rotativa por reloj, no conteo de presencia LIVE). Canal FRESCO coPresenceFind — NINGUNO de los 78 flags #59-#136 lo usa. Camino al 79º flag (base master d9e60e8 tras #136 CÓNCLAVE CO_STRIKE LIVE, served 75fda9c79c56/815).
export const CO_PRESENCE_SURGE = {
  enabled: true,             // LIVE (EVO#137, 79º flag — CAS-2800 CTO flip false→true) — snapshot LECTURA PURA (0 side-effects; _coPresencePre=0 const inerte + seam rama muerta + h.coPresenceBounty transitorio + G._coPresenceParty transitoria) ⇒ world-build fingerprint = 15920977 estable. flip false→true = paso separado del CTO (config-only 1-línea).
  channel: "coPresenceFind", // canal FRESCO (fichas de cohorte por rematar mientras ≥2 aliados están PRESENTES/rallevados en el radio del héroe — la co-presencia física) — NINGUNO de los 78 flags #59-#136 lo usa. Distinto por CONSTRUCCIÓN de coStrikeFind #136 (CÓNCLAVE = ENGANCHADOS en combate), aggroContestFind #123 (FRACCIÓN cov/P), restedMult (CONGREGATION headcount zonal). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio de RALLY (px): R se computa sobre los jugadores VIVOS dentro del radio del héroe. Mirror #121/#123/#124/#136 radius. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores VIVOS presentes en radio (R) para cualquier banda: con <2 (single-player) ⇒ colapsa a 0 (single-player LIMPIO). La co-presencia de party es INTRÍNSECAMENTE multijugador — IMPOSIBLE en solitario. CEO balance knob.
  midRally: 2,               // umbral ENTERO de R (cuenta de jugadores presentes) para la banda MID/rally-parcial (peso 1): R≥2 (≥2 aliados agrupados). Decisión ENTERA: #jugadores-presentes ≥ 2. CEO balance knob.
  hiRally: 3,                // umbral ENTERO de R para la banda HI/rally-pleno (peso 2): R≥3 (≥3 aliados agrupados en el mismo radio). R<midRally ⇒ 0. CEO balance knob.
  playerCap: 4,              // TECHO de jugadores usado por el clamp + el badge idx cosmético (R/playerCap). CEO balance knob.
  weights: { full: 2, some: 1 },   // PESO de un remate por la BANDA de rally (cuántos aliados presentes): HI/rally-pleno (R≥hiRally) pesa 2; MID/rally-parcial (≥midRally) pesa 1; solitario/<minPlayers (<midRally) ⇒ 0. Reward = agruparse con la party, no cazar solo. CEO balance knobs.
  coPresenceBountyCap: 2,    // SUB-CAP DURO PROPIO de las fichas de cohorte por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.full)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE RALLY (score∈{0,1,2}). Más aliados presentes = tier de recompensa MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (solitario/<minPlayers) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con la party parcialmente rallevada, R≥midRally): +1 ficha de cohorte por kill.
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate con la party plenamente rallevada, R≥hiRally): +2 fichas por kill. CEO balance knobs.
  ],
};

// CAS-2802: CUADRILLA (CO_KILL_SURGE, EVO#138 DARK) — 20º eje de COMPOSICIÓN-DE-INTENCIÓN, la TERCERA faceta de la sub-familia PLAYER-COORDINATION (#136 CÓNCLAVE=ENGANCHADOS por mobs, #137 COHORTE=PRESENTES en radio). Donde CÓNCLAVE mide quién ATRAE aggro y COHORTE mide quién está PRESENTE, CUADRILLA mide el OUTPUT/CO-KILL: K = nº de JUGADORES DISTINTOS VIVOS ACREDITADOS con un golpe-de-gracia/remate (kill-credit) DENTRO DEL RADIO COMPARTIDO del cluster de kill. K∈[1,P] ENTERO por construcción (conteo de killers acreditados, no fracción). Bandas sobre K: K≥midAssist (2) ⇒ peso 1; K≥hiAssist (3) ⇒ peso 2. Premia CONTRIBUIR remates JUNTOS (assist-cluster), no estar presente ni atraer aggro ni apuntar al mismo objetivo. Gate minPlayers2; single-player ⇒ SÓLO el héroe acreditado ⇒ K=1<midAssist ⇒ 0 (colapso LIMPIO — IMPOSIBLE en solitario por construcción). 🔑 SNAPSHOT PURO/STATELESS (como #121/#123/#124/#136/#137 — SIN ring-buffer; lee el kill-credit ya replicado (quién landeó un golpe en el cluster) en UN frame — el crédito de daño/kill YA es server-auth por diseño MMO); h.coKillBounty transitorio, NINGÚN estado per-tick nuevo mantenido por esta flag. 🔑 DETERMINISMO (sev-1): K = cuenta ENTERA de killers acreditados vs umbrales ENTEROS {midAssist,hiAssist}; CERO float, CERO division en score/decisión (float SÓLO cosmético para el badge idx = K/playerCap) ⇒ 2-cliente 0-desync garantizado. 🔑 MULTIJUGADOR-NATIVO: literalmente NO puede dispararse en solitario (K=1 ⇒ 0). CRUX (LA CRÍTICA) — CUADRILLA = OUTPUT/CO-KILL (quién REMATA), ⊥ las OTRAS DOS facetas Y ⊥ FOCUS_FIRE #62: (a) ⊥ COHORTE #137 (PRESENTES): {4 presentes, 1 rematando} = COHORTE R4/w2 pero CUADRILLA K1/0 (party llena presente, pero sólo uno contribuye kills); {2 dispersos cada uno rematando} = CUADRILLA K2/w1 aunque no co-presentes en un radio — presente ⊥ contribuir. (b) ⊥ CÓNCLAVE #136 (ENGANCHADOS por mob): un tank ENGANCHADO (mob aggro'd) que landea 0 kills ⇒ CÓNCLAVE lo cuenta, CUADRILLA no; un DPS ranged NO enganchado que landea el golpe ⇒ CUADRILLA lo cuenta, CÓNCLAVE no — enganchado ⊥ contribuir. (c) ⊥ FOCUS_FIRE #62 (N jugadores APUNTANDO al MISMO objetivo VIVO, canal goldFind passive, in-progress): un boss focus-fireado por 3 durante 30s sin morir ⇒ FOCUS_FIRE 3 pero CUADRILLA 0 (ningún kill landeado); 3 jugadores cada uno soleando mobs débiles distintos ⇒ CUADRILLA K3 (3 killers distintos) pero FOCUS_FIRE bajo (ningún objetivo compartido) — aim-en-progreso-sobre-objetivo-ÚNICO ⊥ output-de-kills-COMPLETADOS de killers DISTINTOS; además FOCUS_FIRE alimenta un multiplicador PASSIVE de oro (goldFind) mientras CUADRILLA es un canal FIND de fichas-por-kill en el seam de REMATE. ⊥#123 CONTEST (cov/P FRACCIÓN — CUADRILLA = conteo CRUDO de killers). ⊥#124 PILE (mobs-por-jugador). ⊥#121 SPREAD (entropía). ⊥#126 DENSITY (conteo de MOBS, no de JUGADORES). ⊥#133/#134/#135 (distribución de PODER del mob). ⊥#128 MOMENTUM (snapshot, no tendencia). ⊥#129/#130 (posición/geometría del MOB). ⊥ CONGREGATION (headcount ZONAL → restedMult passive; CUADRILLA es RADIO-LOCAL → canal find). ⊥ FELLOWSHIP/KINSHIP (banda por reloj, no output LIVE). Canal FRESCO coKillFind — NINGUNO de los 79 flags #59-#137 lo usa. Camino al 80º flag (base master be102e9 tras #137 COHORTE CO_PRESENCE LIVE, served d2fe18611bd2/815).
export const CO_KILL_SURGE = {
  enabled: true,             // CAS-2804 LIVE (EVO#138, 80º flag) — flip false→true config-only 1-línea. snapshot LECTURA PURA (0 side-effects; _coKillPre=0 const inerte + seam rama muerta + h.coKillBounty transitorio + G._coKillParty transitoria) ⇒ world-build fingerprint = 15920977 estable. flip false→true = paso separado del CTO (config-only 1-línea).
  channel: "coKillFind",     // canal FRESCO (fichas de cuadrilla por rematar mientras ≥2 aliados están ACREDITADOS con un golpe-de-gracia en el radio compartido — el co-kill/assist-cluster) — NINGUNO de los 79 flags #59-#137 lo usa. Distinto por CONSTRUCCIÓN de coPresenceFind #137 (COHORTE = PRESENTES), coStrikeFind #136 (CÓNCLAVE = ENGANCHADOS), goldFind (FOCUS_FIRE #62 = APUNTAR al mismo objetivo, passive). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio del CLUSTER de kill compartido (px): K se computa sobre los jugadores VIVOS ACREDITADOS dentro del radio del héroe (sitio del remate). Mirror #121/#123/#124/#136/#137 radius. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de killers ACREDITADOS VIVOS en radio (K) para cualquier banda: con <2 (single-player) ⇒ colapsa a 0 (single-player LIMPIO). El co-kill de party es INTRÍNSECAMENTE multijugador — IMPOSIBLE en solitario. CEO balance knob.
  midAssist: 2,              // umbral ENTERO de K (cuenta de killers acreditados) para la banda MID/assist-parcial (peso 1): K≥2 (≥2 jugadores contribuyeron el remate). Decisión ENTERA: #killers-acreditados ≥ 2. CEO balance knob.
  hiAssist: 3,               // umbral ENTERO de K para la banda HI/assist-pleno (peso 2): K≥3 (≥3 jugadores acreditados en el mismo cluster). K<midAssist ⇒ 0. CEO balance knob.
  playerCap: 4,              // TECHO de jugadores usado por el clamp + el badge idx cosmético (K/playerCap). CEO balance knob.
  weights: { full: 2, some: 1 },   // PESO de un remate por la BANDA de assist (cuántos killers acreditados): HI/assist-pleno (K≥hiAssist) pesa 2; MID/assist-parcial (≥midAssist) pesa 1; solitario/<minPlayers (<midAssist) ⇒ 0. Reward = rematar EN CUADRILLA, no solo. CEO balance knobs.
  coKillBountyCap: 2,        // SUB-CAP DURO PROPIO de las fichas de cuadrilla por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.full)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE ASSIST (score∈{0,1,2}). Más killers acreditados = tier de recompensa MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (solitario/<minPlayers) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con la cuadrilla parcialmente acreditada, K≥midAssist): +1 ficha de cuadrilla por kill.
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate con la cuadrilla plenamente acreditada, K≥hiAssist): +2 fichas por kill. CEO balance knobs.
  ],
};

// CAS-2807: SOCORRO (CO_SUPPORT_SURGE, EVO#139 DARK) — 21º eje de COMPOSICIÓN-DE-INTENCIÓN, la CUARTA faceta de la sub-familia PLAYER-COORDINATION (#136 CÓNCLAVE=ENGANCHADOS por mobs, #137 COHORTE=PRESENTES en radio, #138 CUADRILLA=quién REMATA/kill-output). Donde CUADRILLA mide el OUTPUT (quién landea el golpe-de-gracia), SOCORRO mide el ENABLER/SUCCOR: S = nº de JUGADORES DISTINTOS VIVOS en el radio compartido que APLICARON un socorro (heal/shield/buff/revive) a un aliado en este frame (support-credit). S∈[0,P] ENTERO por construcción (conteo de socorredores acreditados, no fracción). Bandas sobre S: S≥midSuccor (2) ⇒ peso 1; S≥hiSuccor (3) ⇒ peso 2. Premia SOSTENER a la party JUNTOS (banco de sanadores/apoyo), no rematar ni estar presente ni atraer aggro. Gate minPlayers2; single-player ⇒ el héroe NO aporta socorro a nadie ⇒ S=0 ⇒ 0 (colapso LIMPIO — IMPOSIBLE en solitario por construcción: hace falta ≥2 aliados socorriéndose). 🔑 SNAPSHOT PURO/STATELESS (como #121/#123/#124/#136/#137/#138 — SIN ring-buffer; lee el support-credit ya replicado (quién aplicó un socorro en el cluster) en UN frame — el crédito de heal/buff/revive YA es server-auth por diseño MMO, MISMA clase de evento replicado que el kill-credit de #138); h.coSupportBounty transitorio, NINGÚN estado per-tick nuevo mantenido por esta flag. 🔑 DETERMINISMO (sev-1): S = cuenta ENTERA de socorredores acreditados vs umbrales ENTEROS {midSuccor,hiSuccor}; CERO float, CERO division en score/decisión (float SÓLO cosmético para el badge idx = S/playerCap) ⇒ 2-cliente 0-desync garantizado. 🔑 MULTIJUGADOR-NATIVO: literalmente NO puede dispararse en solitario (S=0 sin aliados). CRUX (LA CRÍTICA) — SOCORRO = ENABLER/SUCCOR (quién SOSTIENE a la party), ⊥ las OTRAS TRES facetas Y ⊥ PARTY_VITAL #132: (a) ⊥ CUADRILLA #138 (kill-OUTPUT): un sanador que mantiene viva a la party landea 0 kills ⇒ CUADRILLA 0 pero SOCORRO lo cuenta (support=true/kill=false); un DPS soleando ⇒ CUADRILLA K1 pero SOCORRO 0 (kill=true/support=false) — contribuir-remates ⊥ habilitar-sostén. (b) ⊥ COHORTE #137 (PRESENTES): una party LLENA presente pero NADIE socorriendo ⇒ COHORTE R alta / SOCORRO S0 — presente ⊥ apoyando. (c) ⊥ CÓNCLAVE #136 (ENGANCHADOS por mob): estar aggro'd por un mob ⊥ estar socorriendo activamente a un aliado. (d) ⊥ PARTY_VITAL #132 TEMPLE (ally-HP-STATE): TEMPLE = FRACCIÓN-de-HP MEDIA (float ∈[0,1]) de la party = cómo de SANA ESTÁ la party (ESTADO pasivo); SOCORRO = CONTEO ENTERO de jugadores que ACTÚAN para sostenerla (quién HABILITA). Party a full-HP con nadie socorriendo ⇒ TEMPLE W≈1 pero SOCORRO S0; party maltrecha con 3 sanadores bombeando ⇒ TEMPLE W bajo pero SOCORRO S3 — ESTADO-de-HP ⊥ ACCIÓN-de-socorro. ⊥#123 CONTEST (cov/P FRACCIÓN — SOCORRO = conteo CRUDO de socorredores). ⊥#124 PILE (mobs-por-jugador). ⊥#121 SPREAD (entropía). ⊥#126 DENSITY (conteo de MOBS, no de JUGADORES). ⊥#133/#134/#135 (distribución de PODER del mob). ⊥#128 MOMENTUM (snapshot, no tendencia). ⊥#129/#130 (posición/geometría del MOB). ⊥ CONGREGATION (headcount ZONAL → restedMult passive; SOCORRO es RADIO-LOCAL → canal find). ⊥ FELLOWSHIP/KINSHIP/RALLY (banda por reloj/rally de combate, no output de socorro LIVE). Canal FRESCO coSupportFind — NINGUNO de los 80 flags #59-#138 lo usa. Camino al 81º flag (base master d58d6736 tras #138 CUADRILLA CO_KILL LIVE, served 9527556851d4/815).
export const CO_SUPPORT_SURGE = {
  enabled: true,             // LIVE (EVO#139, 81º flag) — CAS-2809 CTO flip false→true — snapshot LECTURA PURA (0 side-effects; _coSupportPre=0 const inerte + seam rama muerta + h.coSupportBounty transitorio + G._coSupportParty transitoria) ⇒ world-build fingerprint estable. flip false→true = paso separado del CTO (config-only 1-línea).
  channel: "coSupportFind",  // canal FRESCO (fichas de socorro por rematar mientras ≥2 aliados están APLICANDO un socorro/heal/shield/buff/revive en el radio compartido — el co-support/succor) — NINGUNO de los 80 flags #59-#138 lo usa. Distinto por CONSTRUCCIÓN de coKillFind #138 (CUADRILLA = REMATAN), coPresenceFind #137 (COHORTE = PRESENTES), coStrikeFind #136 (CÓNCLAVE = ENGANCHADOS). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio del CLUSTER de socorro compartido (px): S se computa sobre los jugadores VIVOS que APLICARON socorro dentro del radio del héroe. Mirror #121/#123/#124/#136/#137/#138 radius. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de socorredores APLICANDO socorro VIVOS en radio (S) para cualquier banda: con <2 (single-player) ⇒ colapsa a 0 (single-player LIMPIO). El co-support de party es INTRÍNSECAMENTE multijugador — IMPOSIBLE en solitario. CEO balance knob.
  midSuccor: 2,              // umbral ENTERO de S (cuenta de socorredores acreditados) para la banda MID/socorro-parcial (peso 1): S≥2 (≥2 jugadores aplicaron socorro). Decisión ENTERA: #socorredores-acreditados ≥ 2. CEO balance knob.
  hiSuccor: 3,               // umbral ENTERO de S para la banda HI/socorro-pleno (peso 2): S≥3 (≥3 jugadores socorriendo en el mismo cluster). S<midSuccor ⇒ 0. CEO balance knob.
  playerCap: 4,              // TECHO de jugadores usado por el clamp + el badge idx cosmético (S/playerCap). CEO balance knob.
  weights: { full: 2, some: 1 },   // PESO de un remate por la BANDA de socorro (cuántos socorredores acreditados): HI/socorro-pleno (S≥hiSuccor) pesa 2; MID/socorro-parcial (≥midSuccor) pesa 1; solitario/<minPlayers (<midSuccor) ⇒ 0. Reward = sostener EN SOCORRO, no solo. CEO balance knobs.
  coSupportBountyCap: 2,     // SUB-CAP DURO PROPIO de las fichas de socorro por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.full)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE SOCORRO (score∈{0,1,2}). Más socorredores acreditados = tier de recompensa MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (solitario/<minPlayers) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con el socorro parcialmente acreditado, S≥midSuccor): +1 ficha de socorro por kill.
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate con el socorro plenamente acreditado, S≥hiSuccor): +2 fichas por kill. CEO balance knobs.
  ],
};

// CAS-2812: MURALLA (CO_TANK_SURGE, EVO#140 DARK) — 22º eje de COMPOSICIÓN-DE-INTENCIÓN, la QUINTA faceta de la sub-familia PLAYER-COORDINATION (#136 CÓNCLAVE=ENGANCHADOS por mobs, #137 COHORTE=PRESENTES en radio, #138 CUADRILLA=quién REMATA/kill-output, #139 SOCORRO=quién SOSTIENE/support-enabler). Donde SOCORRO mide el ENABLER (quién aplica el socorro), MURALLA mide el BURDEN-SPREAD/CO-TANK: T = nº de JUGADORES DISTINTOS VIVOS en el radio compartido que ABSORBIERON/TOMARON daño de un mob en este frame (damage-intake credit — quién REPARTE la carga de golpes para que nadie se funda focalizado). T∈[0,P] ENTERO por construcción (conteo de absorbedores acreditados, no fracción). Bandas sobre T: T≥midBrunt (2) ⇒ peso 1; T≥hiBrunt (3) ⇒ peso 2. Premia REPARTIR la carga de golpes JUNTOS (línea de frente distribuida), no rematar (#138) ni socorrer (#139) ni estar presente (#137) ni atraer aggro (#136). Gate minPlayers2; single-player ⇒ el héroe solo NO puede REPARTIR la carga entre nadie ⇒ T colapsa <midBrunt ⇒ 0 (colapso LIMPIO — hace falta ≥2 aliados absorbiendo golpes para que exista burden-spread). 🔑 SNAPSHOT PURO/STATELESS (como #121/#123/#124/#136/#137/#138/#139 — SIN ring-buffer; lee el damage-intake ya replicado (quién comió un tick de daño de mob en el cluster) en UN frame — el crédito de daño YA es server-auth por diseño MMO, MISMA clase de evento replicado que el kill-credit de #138 y el support-credit de #139); h.coTankBounty transitorio, NINGÚN estado per-tick nuevo mantenido por esta flag. 🔑 DETERMINISMO (sev-1): T = cuenta ENTERA de absorbedores acreditados vs umbrales ENTEROS {midBrunt,hiBrunt}; CERO float, CERO division en score/decisión (float SÓLO cosmético para el badge idx = T/playerCap) ⇒ 2-cliente 0-desync garantizado. 🔑 MULTIJUGADOR-NATIVO: single-player ⇒ T colapsa ⇒ 0 (IMPOSIBLE en solitario por construcción). CRUX (LA CRÍTICA) — MURALLA = BURDEN-SPREAD/CO-TANK (quién ABSORBE los golpes), ⊥ las OTRAS CUATRO facetas Y ⊥ PARTY_VITAL #132: (a) ⊥ CÓNCLAVE #136 (ENGANCHADOS/aggro'd): estar TARGETEADO por un mob ⊥ TOMAR un tick de daño — un jugador puede estar aggro'd pero ESQUIVAR cada golpe (engaged, T=0), y comer un AoE/splash SIN ser el objetivo del aggro (T cuenta, NO engaged). aggro-target ⊥ daño-absorbido. (b) ⊥ SOCORRO #139 (support-applied): TOMAR golpes ⊥ APLICAR heals — un tank de la línea de frente absorbiendo daño landea 0 heals (T cuenta / S0); un sanador de retaguardia toma 0 golpes (S cuenta / T0). (c) ⊥ CUADRILLA #138 (kill-OUTPUT): absorber ⊥ rematar — un tank que come todos los golpes landea 0 kills. (d) ⊥ COHORTE #137 (PRESENTES): absorber ⊥ meramente estar ahí. (e) ⊥ PARTY_VITAL #132 TEMPLE (ally-HP-STATE): TEMPLE = FRACCIÓN-de-HP MEDIA (float ∈[0,1]) = cómo de SANA ESTÁ la party (ESTADO pasivo); MURALLA = CONTEO ENTERO de quién comió un golpe ESTE frame (ACCIÓN/evento). Party a full-HP con 3 comiendo chip-damage este frame ⇒ TEMPLE W≈1 pero MURALLA T3; party maltrecha sin golpes este frame ⇒ TEMPLE W bajo pero MURALLA T0 — ESTADO-de-HP ⊥ ACCIÓN-de-absorber. ⊥#123 CONTEST (cov/P FRACCIÓN — MURALLA = conteo CRUDO de absorbedores). ⊥#124 PILE (mobs-por-jugador). ⊥#121 SPREAD (entropía). ⊥#126 DENSITY (conteo de MOBS, no de JUGADORES). ⊥#133/#134/#135 (distribución de PODER del mob). ⊥#128 MOMENTUM (snapshot, no tendencia). ⊥#129/#130 (posición/geometría del MOB). ⊥ CONGREGATION (headcount ZONAL → restedMult passive; MURALLA es RADIO-LOCAL → canal find). ⊥ FELLOWSHIP/KINSHIP/RALLY (banda por reloj/rally de combate, no damage-intake LIVE). Canal FRESCO coTankFind — NINGUNO de los 81 flags #59-#139 lo usa. Camino al 82º flag (base master 3b77140/8bac315 tras #139 SOCORRO CO_SUPPORT LIVE, served 5f4a11037d5f/815).
export const CO_TANK_SURGE = {
  enabled: true,             // LIVE (EVO#140, 82º flag) — flipped false→true by CTO CAS-2813 (config-only 1-línea). Snapshot LECTURA PURA (0 side-effects; _coTankPre=0 const inerte + seam rama muerta + h.coTankBounty transitorio + G._coTankParty transitoria) ⇒ world-build fingerprint = 15920977 estable.
  channel: "coTankFind",     // canal FRESCO (fichas de muralla por rematar mientras ≥2 aliados están ABSORBIENDO/TOMANDO daño de mob en el radio compartido — el co-tank/burden-spread) — NINGUNO de los 81 flags #59-#139 lo usa. Distinto por CONSTRUCCIÓN de coSupportFind #139 (SOCORRO = SOSTIENEN/heal), coKillFind #138 (CUADRILLA = REMATAN), coPresenceFind #137 (COHORTE = PRESENTES), coStrikeFind #136 (CÓNCLAVE = ENGANCHADOS). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio del CLUSTER de co-tank compartido (px): T se computa sobre los jugadores VIVOS que TOMARON daño dentro del radio del héroe. Mirror #121/#123/#124/#136/#137/#138/#139 radius. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de absorbedores TOMANDO daño VIVOS en radio (T) para cualquier banda: con <2 (single-player) ⇒ colapsa a 0 (single-player LIMPIO). El co-tank de party es INTRÍNSECAMENTE multijugador — IMPOSIBLE repartir la carga en solitario. CEO balance knob.
  midBrunt: 2,               // umbral ENTERO de T (cuenta de absorbedores acreditados) para la banda MID/carga-parcial (peso 1): T≥2 (≥2 jugadores comieron un golpe). Decisión ENTERA: #absorbedores-acreditados ≥ 2. CEO balance knob.
  hiBrunt: 3,                // umbral ENTERO de T para la banda HI/carga-plena (peso 2): T≥3 (≥3 jugadores absorbiendo en el mismo cluster). T<midBrunt ⇒ 0. CEO balance knob.
  playerCap: 4,              // TECHO de jugadores usado por el clamp + el badge idx cosmético (T/playerCap). CEO balance knob.
  weights: { full: 2, some: 1 },   // PESO de un remate por la BANDA de co-tank (cuántos absorbedores acreditados): HI/carga-plena (T≥hiBrunt) pesa 2; MID/carga-parcial (≥midBrunt) pesa 1; solitario/<minPlayers (<midBrunt) ⇒ 0. Reward = REPARTIR la carga de golpes, no comerla solo. CEO balance knobs.
  coTankBountyCap: 2,        // SUB-CAP DURO PROPIO de las fichas de muralla por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.full)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE CO-TANK (score∈{0,1,2}). Más absorbedores acreditados = tier de recompensa MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (solitario/<minPlayers) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con la carga parcialmente repartida, T≥midBrunt): +1 ficha de muralla por kill.
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate con la carga plenamente repartida, T≥hiBrunt): +2 fichas por kill. CEO balance knobs.
  ],
};

// CAS-2815: REPLIEGUE (CO_FLEE_SURGE, EVO#141 DARK) — 23º eje de COMPOSICIÓN-DE-INTENCIÓN, la SEXTA faceta de la sub-familia PLAYER-COORDINATION (#136 CÓNCLAVE=ENGANCHADOS por mobs, #137 COHORTE=PRESENTES en radio, #138 CUADRILLA=quién REMATA/kill-output, #139 SOCORRO=quién SOSTIENE/support-enabler, #140 MURALLA=quién ABSORBE/damage-intake). Donde MURALLA mide el BURDEN-SPREAD (quién TOMA el golpe), REPLIEGUE mide el KITE/DISENGAGE-REPOSITION: D = nº de JUGADORES DISTINTOS VIVOS en el radio compartido que ROMPIERON el contacto / se REPOSICIONARON ALEJÁNDOSE de su amenaza más cercana en este frame (kite coordinado / retirada táctica para que la party no sea arrollada). D∈[0,P] ENTERO por construcción (conteo de jugadores que se repliegan, no fracción). Bandas sobre D: D≥midFlee (2) ⇒ peso 1; D≥hiFlee (3) ⇒ peso 2. Premia REPLEGARSE JUNTOS bajo amenaza (kite/reposición coordinada), no absorber (#140) ni socorrer (#139) ni rematar (#138) ni estar presente (#137) ni atraer aggro (#136). Gate minPlayers2; single-player ⇒ el héroe solo NO puede co-kitear con nadie ⇒ D colapsa <midFlee ⇒ 0 (colapso LIMPIO — hace falta ≥2 aliados replegándose para que exista kite coordinado). 🔑 SNAPSHOT PURO/STATELESS (como #121/#123/#124/#136/#137/#138/#139/#140 — SIN ring-buffer; lee la pos/vel ya replicada (quién se movió con un componente de velocidad ALEJÁNDOSE de su mob-amenaza más cercano superando un umbral entero) en UN frame — la pos/vel del jugador YA es server-auth por diseño MMO, MISMA clase de estado replicado que lee #129/#130; el disengage se computa server-side una vez y se replica como un evento/flag booleano por frame); h.coFleeBounty transitorio, NINGÚN estado per-tick nuevo mantenido por esta flag. 🔑 DETERMINISMO (sev-1): D = cuenta ENTERA de jugadores que se repliegan vs umbrales ENTEROS {midFlee,hiFlee}; CERO float, CERO division en score/decisión (float SÓLO cosmético para el badge idx = D/playerCap) ⇒ 2-cliente 0-desync garantizado. 🔑 MULTIJUGADOR-NATIVO: single-player ⇒ D colapsa ⇒ 0 (IMPOSIBLE en solitario por construcción). CRUX (LA CRÍTICA) — REPLIEGUE = KITE/DISENGAGE-REPOSITION (quién SE ALEJA de la amenaza), ⊥ las OTRAS CINCO facetas Y ⊥ PARTY_VITAL #132: (a) ⊥ MURALLA #140 (damage-intake, el vecino MÁS cercano): ABSORBER un golpe (plantarse, comer el tick) ⊥ REPLEGARSE (moverse para evitar el SIGUIENTE) — un tank absorbe (T cuenta / D0); un kiter se reposiciona ileso (D cuenta / T0). TOMAR-el-golpe ⊥ EVITAR-el-golpe. (b) ⊥ CÓNCLAVE #136 (ENGANCHADOS/aggro'd): estar aggro'd ⊥ disengaging — un jugador puede estar aggro'd y plantarse (engaged, D0) o aggro'd y kitear alejándose (D cuenta). (c) ⊥ SOCORRO #139 (support-applied): REPOSICIONARSE ⊥ APLICAR heals — un kiter que se retira landea 0 heals (D cuenta / S0); un sanador plantado toma 0 pasos de retirada (S cuenta / D0). (d) ⊥ CUADRILLA #138 (kill-OUTPUT): replegarse ⊥ rematar. (e) ⊥ COHORTE #137 (PRESENTES): replegarse ⊥ meramente estar ahí. (f) ⊥ PARTY_VITAL #132 TEMPLE (ally-HP-STATE): TEMPLE = FRACCIÓN-de-HP MEDIA (float ∈[0,1]) = cómo de SANA ESTÁ la party (ESTADO pasivo); REPLIEGUE = CONTEO ENTERO de quién se repliegó ESTE frame (ACCIÓN/evento). Party a full-HP y 3 kiteando este frame ⇒ TEMPLE W≈1 pero REPLIEGUE D3; party maltrecha plantada sin moverse ⇒ TEMPLE W bajo pero REPLIEGUE D0 — ESTADO-de-HP ⊥ ACCIÓN-de-replegarse. ⊥#128 MOMENTUM/#122 SWITCH (snapshot de UN frame, no derivada/tendencia temporal). ⊥#123 CONTEST (cov/P FRACCIÓN — REPLIEGUE = conteo CRUDO de kiters). ⊥#124 PILE (mobs-por-jugador). ⊥#121 SPREAD (entropía). ⊥#126 DENSITY (conteo de MOBS, no de JUGADORES). ⊥#129/#130 (esos leen la GEOMETRÍA/distribución del MOB; REPLIEGUE lee el CONTEO de la ACCIÓN-de-disengage del JUGADOR). ⊥#133/#134/#135 (distribución de PODER del mob). ⊥ CONGREGATION (headcount ZONAL → restedMult passive; REPLIEGUE es RADIO-LOCAL → canal find). ⊥ FELLOWSHIP/KINSHIP/RALLY (banda por reloj/rally de combate, no disengage-action LIVE). Canal FRESCO coFleeFind — NINGUNO de los 82 flags #59-#140 lo usa. Camino al 83º flag (base master 0ebe414 tras #140 MURALLA CO_TANK LIVE, served e1267a40e18c/815).
export const CO_FLEE_SURGE = {
  enabled: true,             // LIVE (EVO#141, 83º flag) — flipped false→true by CTO (CAS-2818, config-only 1-línea). Snapshot LECTURA PURA (0 side-effects; _coFleePre=0 const inerte + seam rama muerta + h.coFleeBounty transitorio + G._coFleeParty transitoria) ⇒ world-build fingerprint = 15920977 estable.
  channel: "coFleeFind",     // canal FRESCO (fichas de repliegue por rematar mientras ≥2 aliados están ROMPIENDO contacto / repositionándose ALEJÁNDOSE de una amenaza en el radio compartido — el co-flee/disengage-reposition) — NINGUNO de los 82 flags #59-#140 lo usa. Distinto por CONSTRUCCIÓN de coTankFind #140 (MURALLA = ABSORBEN/damage-intake), coSupportFind #139 (SOCORRO = SOSTIENEN/heal), coKillFind #138 (CUADRILLA = REMATAN), coPresenceFind #137 (COHORTE = PRESENTES), coStrikeFind #136 (CÓNCLAVE = ENGANCHADOS). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio del CLUSTER de co-flee compartido (px): D se computa sobre los jugadores VIVOS que se REPLEGARON dentro del radio del héroe. Mirror #121/#123/#124/#136/#137/#138/#139/#140 radius. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores REPLEGÁNDOSE VIVOS en radio (D) para cualquier banda: con <2 (single-player) ⇒ colapsa a 0 (single-player LIMPIO). El co-flee de party es INTRÍNSECAMENTE multijugador — IMPOSIBLE co-kitear en solitario. CEO balance knob.
  midFlee: 2,                // umbral ENTERO de D (cuenta de jugadores replegándose) para la banda MID/repliegue-parcial (peso 1): D≥2 (≥2 jugadores rompieron contacto). Decisión ENTERA: #kiters ≥ 2. CEO balance knob.
  hiFlee: 3,                 // umbral ENTERO de D para la banda HI/repliegue-pleno (peso 2): D≥3 (≥3 jugadores replegándose en el mismo cluster). D<midFlee ⇒ 0. CEO balance knob.
  playerCap: 4,              // TECHO de jugadores usado por el clamp + el badge idx cosmético (D/playerCap). CEO balance knob.
  weights: { full: 2, some: 1 },   // PESO de un remate por la BANDA de co-flee (cuántos jugadores se repliegan): HI/repliegue-pleno (D≥hiFlee) pesa 2; MID/repliegue-parcial (≥midFlee) pesa 1; solitario/<minPlayers (<midFlee) ⇒ 0. Reward = REPLEGARSE JUNTOS bajo amenaza, no plantarse solo. CEO balance knobs.
  coFleeBountyCap: 2,        // SUB-CAP DURO PROPIO de las fichas de repliegue por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.full)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE CO-FLEE (score∈{0,1,2}). Más jugadores replegándose = tier de recompensa MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (solitario/<minPlayers) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con el repliegue parcialmente coordinado, D≥midFlee): +1 ficha de repliegue por kill.
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate con el repliegue plenamente coordinado, D≥hiFlee): +2 fichas por kill. CEO balance knobs.
  ],
};

// CAS-2820: PIÑA (CO_COHESION_SURGE, EVO#142 DARK) — 24º eje de COMPOSICIÓN-DE-INTENCIÓN, la SÉPTIMA faceta de la sub-familia PLAYER-COORDINATION (#136 CÓNCLAVE=ENGANCHADOS por mobs, #137 COHORTE=PRESENTES en radio, #138 CUADRILLA=quién REMATA/kill-output, #139 SOCORRO=quién SOSTIENE/support-enabler, #140 MURALLA=quién ABSORBE/damage-intake, #141 REPLIEGUE=quién SE ALEJA/disengage). PIÑA ABRE una sub-dimensión NUEVA dentro de PLAYER-COORDINATION: las SEIS priores son CONTEOS-DE-QUIÉN-HIZO-UNA-ACCIÓN (aggro'd / presente / remató / sanó / absorbió / se replegó); PIÑA mide la GEOMETRÍA — no QUIÉN hizo X, sino cuán TIGHTLY-CLUSTERED (apiñado) está el roster VIVO en este frame (una falange apretada vs una línea dispersa). MÉTRICA (ENTERO PURO) = C = nº de JUGADORES DISTINTOS VIVOS DENTRO de un radio de cohesión APRETADO alrededor del héroe (el ancla de la formación) en este frame = el nº de aliados HOMBRO-CON-HOMBRO / apiñados. C∈[0,P] ENTERO por construcción (conteo de apiñados, NO fracción, NO distancia-media float). Lee posiciones YA-replicadas server-auth (MISMA clase de estado que #129/#130/#141) — SNAPSHOT de un solo frame, SIN replicación pesada nueva. Bandas sobre C: C≥midCohesion (2) ⇒ peso 1; C≥hiCohesion (3) ⇒ peso 2. Premia AGRUPARSE APRETADO bajo un mismo estandarte (formar la piña/falange), no dispersarse. Gate minPlayers2; single-player ⇒ SÓLO el héroe apiñado consigo mismo ⇒ C=1<midCohesion ⇒ 0 (colapso LIMPIO — IMPOSIBLE formar piña en solitario). 🔑 SNAPSHOT PURO/STATELESS (como #121/#123/#124/#136/#137/#138/#139/#140/#141 — SIN ring-buffer; lee las posiciones replicadas en UN frame — la pos del jugador YA es server-auth por diseño MMO, MISMA clase de estado que lee #129/#130/#141); h.coCohesionBounty transitorio, NINGÚN estado per-tick nuevo mantenido por esta flag. 🔑 DETERMINISMO (sev-1): C = cuenta ENTERA de apiñados en radio (comparación de distancia-cuadrada ENTERA dx²+dy²≤R², la MISMA aritmética de posición que coFleeRoster/coPresenceRoster — CERO division nueva, CERO centroide float) vs umbrales ENTEROS {midCohesion,hiCohesion}; CERO float, CERO division en score/decisión (float SÓLO cosmético para el badge idx = C/playerCap) ⇒ 2-cliente 0-desync garantizado. 🔑 MULTIJUGADOR-NATIVO: single-player ⇒ C colapsa ⇒ 0 (IMPOSIBLE en solitario por construcción). CRUX (LA CRÍTICA) — PIÑA = GEOMETRÍA/COHESIÓN-ESPACIAL (cuán APRETADO está el roster), ⊥ las OTRAS SEIS facetas Y ⊥ PARTY_VITAL #132: (a) ⊥ #137 COHORTE (present-in-radius, EL VECINO MÁS CERCANO — MISMA sub-familia, dimensión OPUESTA): COHORTE cuenta a CUALQUIERA dentro del radio GRANDE del héroe (300px) = quién ESTÁ AHÍ / quién apareció; PIÑA cuenta a quién está dentro del radio APRETADO (132px) = quién está AGRUPADO/hombro-con-hombro. Una party DISPERSA pero toda dentro del radio grande ⇒ COHORTE R alta pero PIÑA C baja (presentes pero no apiñados); una party apretada ⇒ ambas altas. ESTAR-PRESENTE ⊥ ESTAR-APIÑADO — {3 presentes a ~200px del héroe} ⇒ COHORTE R3/w2 pero PIÑA C0/w0 (fuera del radio apretado); {3 apiñados a ~80px} ⇒ COHORTE R3 y PIÑA C3. (b) ⊥ #141 REPLIEGUE (disengage-count): replegarse/repositionarse (ACCIÓN de velocidad-alejándose) ⊥ estar apiñado (ESTADO de posición-tightness). Una party puede replegarse APRETADA (D y C ambos altos) o replegarse DISPERSA (D alto/C bajo) o plantarse apiñada (C alto/D0). ACCIÓN-de-huir ⊥ ESTADO-de-tightness. (c) ⊥ #136 CÓNCLAVE (aggro'd) / #138 CUADRILLA (kill) / #139 SOCORRO (heal) / #140 MURALLA (absorb): apiñarse ⊥ estar-aggro'd / rematar / sanar / absorber — la geometría del roster es INDEPENDIENTE de qué ACCIÓN de combate ejecuta cada quién. (d) ⊥ #132 PARTY_VITAL (fracción-de-HP MEDIA float = ESTADO-de-salud): la tightness ESPACIAL ⊥ el estado de HP — una party a full-HP puede estar dispersa (C0) o apiñada (C3); una party maltrecha, igual. (e) ⊥ #121 SPREAD / #126 DENSITY / #129 SURROUND / #130 PRESSURE: esos leen la distribución/geometría de los MOBS (posición/ángulo/radio del enjambre); PIÑA lee el clustering del ROSTER DE JUGADORES. (f) ⊥ #128 MOMENTUM / #122 SWITCH (snapshot de UN frame, NO derivada temporal). ⊥#123 CONTEST (cov/P FRACCIÓN — PIÑA = conteo CRUDO de apiñados). Canal FRESCO coCohesionFind — NINGUNO de los 83 flags #59-#141 lo usa. Camino al 84º flag (base master b8c741a tras #141 REPLIEGUE CO_FLEE LIVE, served 83032b54f4e8/815).
export const CO_COHESION_SURGE = {
  enabled: true,             // CAS-2822 LIVE (EVO#142, 84º flag). Flip false→true config-only tras Gates+DARK QA (CAS-2821). Antes: DARK enabled:false ⇒ byte-neutral OFF (_coCohesionPre=0 const inerte + seam rama muerta + h.coCohesionBounty transitorio + G._coCohesionParty transitoria) ⇒ world-build fingerprint = 15920977 estable. El flip false→true es un paso CTO separado (config-only 1-línea) tras las Gates.
  channel: "coCohesionFind", // canal FRESCO (fichas de piña por rematar mientras ≥2 aliados están APIÑADOS hombro-con-hombro dentro del radio de cohesión APRETADO — la formation-cohesion/rally-tightness) — NINGUNO de los 83 flags #59-#141 lo usa. Distinto por CONSTRUCCIÓN de coFleeFind #141 (REPLIEGUE = SE ALEJAN/disengage), coTankFind #140 (MURALLA = ABSORBEN), coSupportFind #139 (SOCORRO = SOSTIENEN), coKillFind #138 (CUADRILLA = REMATAN), coPresenceFind #137 (COHORTE = PRESENTES en radio GRANDE), coStrikeFind #136 (CÓNCLAVE = ENGANCHADOS). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 132,               // 🔑 radio de cohesión APRETADO (px): C se computa sobre los jugadores VIVOS DENTRO de este radio APRETADO del héroe = quién está hombro-con-hombro/apiñado. INTENCIONALMENTE < el radio de presencia de #137 COHORTE (300px) — ESTE es el crux ⊥ COHORTE: 300px=presencia/quién-está-ahí, 132px=cohesión/quién-está-agrupado. ~4 tiles. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores APIÑADOS VIVOS en radio (C) para cualquier banda: con <2 (single-player) ⇒ colapsa a 0 (single-player LIMPIO). La piña de party es INTRÍNSECAMENTE multijugador — IMPOSIBLE apiñarse en solitario. CEO balance knob.
  midCohesion: 2,            // umbral ENTERO de C (cuenta de apiñados) para la banda MID/piña-parcial (peso 1): C≥2 (≥2 jugadores hombro-con-hombro). Decisión ENTERA: #apiñados ≥ 2. CEO balance knob.
  hiCohesion: 3,             // umbral ENTERO de C para la banda HI/piña-apretada (peso 2): C≥3 (≥3 jugadores apiñados en el mismo radio apretado). C<midCohesion ⇒ 0. CEO balance knob.
  playerCap: 4,              // TECHO de jugadores usado por el clamp + el badge idx cosmético (C/playerCap). CEO balance knob.
  weights: { full: 2, some: 1 },   // PESO de un remate por la BANDA de cohesión (cuán apiñada la party): HI/piña-apretada (C≥hiCohesion) pesa 2; MID/piña-parcial (≥midCohesion) pesa 1; solitario/<minPlayers (<midCohesion) ⇒ 0. Reward = APIÑARSE JUNTOS, no dispersarse. CEO balance knobs.
  coCohesionBountyCap: 2,    // SUB-CAP DURO PROPIO de las fichas de piña por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.full)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE COHESIÓN (score∈{0,1,2}). Party MÁS apiñada = tier de recompensa MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (solitario/<minPlayers) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con la party parcialmente apiñada, C≥midCohesion): +1 ficha de piña por kill.
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate con la party plenamente apiñada, C≥hiCohesion): +2 fichas por kill. CEO balance knobs.
  ],
};

// CAS-2824: ENVITE (CO_ADVANCE_SURGE, EVO#143 DARK) — 25º eje de COMPOSICIÓN-DE-INTENCIÓN, la OCTAVA faceta de la sub-familia PLAYER-COORDINATION (#136 CÓNCLAVE=ENGANCHADOS por mobs, #137 COHORTE=PRESENTES en radio, #138 CUADRILLA=quién REMATA/kill-output, #139 SOCORRO=quién SOSTIENE/support-enabler, #140 MURALLA=quién ABSORBE/damage-intake, #141 REPLIEGUE=quién SE ALEJA/disengage, #142 PIÑA=cuán APIÑADO/spatial-cohesion STATE). ENVITE es el INVERSO MILITANTE de #141 REPLIEGUE: donde REPLIEGUE cuenta velocidad-ALEJÁNDOSE (retirada/kite), ENVITE cuenta velocidad-HACIA (empuje/commit). MÉTRICA (ENTERO PURO) = A = nº de JUGADORES DISTINTOS VIVOS dentro del radio compartido cuyo VECTOR DE MOVIMIENTO de este frame apunta HACIA la amenaza compartida = el nº de aliados que EMPUJAN/avanzan-a-comprometer. A∈[0,P] ENTERO por construcción. La dirección se decide por un test de SIGNO ENTERO del producto punto `(amenaza−pos)·vel > 0` (aritmética ENTERA sobre offsets relativos ya replicados — el MISMO patrón pos/vel que lee #141 REPLIEGUE / #129 / #130) — CERO float en el score/decisión (float SÓLO cosmético para el badge idx = A/playerCap). Lee pos/vel YA-replicada server-auth (MISMA clase de estado que #141) — SNAPSHOT de un solo frame, SIN replicación pesada nueva. Bandas sobre A: A≥midAdvance (2) ⇒ peso 1; A≥hiAdvance (3) ⇒ peso 2. Premia EMPUJAR JUNTOS hacia una amenaza/objetivo compartido (avance coordinado a comprometer), no replegarse. Gate minPlayers2; single-player ⇒ SIN party sintética Y SIN amenaza compartida ⇒ A colapsa ⇒ 0 (colapso LIMPIO — IMPOSIBLE avanzar-en-conjunto en solitario). 🔑 SNAPSHOT PURO/STATELESS (como #121/#123/#124/#136/#137/#138/#139/#140/#141/#142 — SIN ring-buffer; lee pos/vel replicada en UN frame; el toward-test se computa server-side una vez); h.coAdvanceBounty transitorio, NINGÚN estado per-tick nuevo mantenido por esta flag. 🔑 DETERMINISMO (sev-1): A = cuenta ENTERA de avanzando-hacia (dot `(tx−dx)*vx+(ty−dy)*vy>0` ENTERO sobre offsets/velocidades ENTERAS — CERO division, CERO normalización float, CERO ángulo) vs umbrales ENTEROS {midAdvance,hiAdvance}; CERO float en el score/decisión ⇒ 2-cliente 0-desync garantizado. 🔑 MULTIJUGADOR-NATIVO: single-player ⇒ A colapsa ⇒ 0. CRUX (LA CRÍTICA) — ENVITE = EMPUJE/AVANCE-HACIA (velocidad-toward), el INVERSO DIRECTO de #141 REPLIEGUE (velocidad-away): (a) ⊥ #141 REPLIEGUE (PRIMARIO, inverso exacto): REPLIEGUE cuenta quién SE ALEJA (dot<0/kite); ENVITE cuenta quién EMPUJA-HACIA (dot>0/commit). Divergen en AMBOS sentidos: party puede avanzar apiñada (A alto/D0), replegarse (D alto/A0) o partirse (unos empujan, otros kitean) — HACIA ⊥ ALEJÁNDOSE es el crux más afilado posible. (b) ⊥ #142 PIÑA (STATE espacial vs ACCIÓN de velocidad): la cohesión es un ESTADO de posición-tightness; avanzar es una ACCIÓN de dirección-de-velocidad. Una party puede avanzar DISPERSA (A alto/C bajo) o plantarse APIÑADA sin moverse (C alto/A0). (c) ⊥ #136 CÓNCLAVE (aggro'd) / #138 CUADRILLA (kill) / #139 SOCORRO (heal) / #140 MURALLA (absorb): avanzar ⊥ estar-aggro'd / rematar / sanar / absorber. (d) ⊥ #137 COHORTE (moving-toward ⊥ merely-present): estar PRESENTE en el radio ⊥ MOVERSE-HACIA la amenaza. (e) ⊥ #132 PARTY_VITAL (movimiento ⊥ HP-STATE float). (f) ⊥ #128 MOMENTUM (snapshot de UN frame, no derivada temporal). (g) ⊥ #112 CONVERGE (eso lee la velocidad RADIAL de los MOBS; ENVITE lee el MOVIMIENTO del JUGADOR). Canal FRESCO coAdvanceFind — NINGUNO de los 84 flags #59-#142 lo usa. Camino al 85º flag (base master 3b3001b tras #142 PIÑA CO_COHESION LIVE, served 3a014051fb4f/815).
export const CO_ADVANCE_SURGE = {
  enabled: true,             // LIVE (EVO#143, 85º flag). CAS-2827 CTO flip false→true (Envite ➤). Antes DARK: enabled:false ⇒ byte-neutral OFF (_coAdvancePre=0 const inerte + seam rama muerta + h.coAdvanceBounty transitorio + G._coAdvanceParty/G._coAdvanceThreat transitorias) ⇒ world-build fingerprint = 15920977 estable. El flip false→true es un paso CTO separado (config-only 1-línea) tras las Gates.
  channel: "coAdvanceFind",  // canal FRESCO (fichas de envite por rematar mientras ≥2 aliados EMPUJAN/avanzan con velocidad-HACIA la amenaza compartida — el co-advance/push-count) — NINGUNO de los 84 flags #59-#142 lo usa. Distinto por CONSTRUCCIÓN de coCohesionFind #142 (PIÑA = APIÑADOS/spatial-state), coFleeFind #141 (REPLIEGUE = SE ALEJAN/disengage), coTankFind #140 (MURALLA = ABSORBEN), coSupportFind #139 (SOCORRO = SOSTIENEN), coKillFind #138 (CUADRILLA = REMATAN), coPresenceFind #137 (COHORTE = PRESENTES), coStrikeFind #136 (CÓNCLAVE = ENGANCHADOS). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio del CLUSTER de avance compartido (px): A se computa sobre los jugadores VIVOS dentro del radio del héroe cuya velocidad apunta HACIA la amenaza compartida. Mirror #121/#123/#124/#136/#137/#138/#139/#140/#141 radius (radio GRANDE compartido, ⊥ el 132 APRETADO de PIÑA #142 — ENVITE mide DIRECCIÓN, no tightness). CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores AVANZANDO-HACIA VIVOS en radio (A) para cualquier banda: con <2 (single-player) ⇒ colapsa a 0 (single-player LIMPIO). El co-advance de party es INTRÍNSECAMENTE multijugador — IMPOSIBLE co-empujar en solitario. CEO balance knob.
  midAdvance: 2,             // umbral ENTERO de A (cuenta de jugadores avanzando-hacia) para la banda MID/avance-parcial (peso 1): A≥2 (≥2 jugadores empujando hacia la amenaza). Decisión ENTERA: #pushers ≥ 2. CEO balance knob.
  hiAdvance: 3,              // umbral ENTERO de A para la banda HI/avance-pleno (peso 2): A≥3 (≥3 jugadores empujando en el mismo cluster). A<midAdvance ⇒ 0. CEO balance knob.
  playerCap: 4,              // TECHO de jugadores usado por el clamp + el badge idx cosmético (A/playerCap). CEO balance knob.
  weights: { full: 2, some: 1 },   // PESO de un remate por la BANDA de co-advance (cuántos jugadores empujan): HI/avance-pleno (A≥hiAdvance) pesa 2; MID/avance-parcial (≥midAdvance) pesa 1; solitario/<minPlayers (<midAdvance) ⇒ 0. Reward = EMPUJAR JUNTOS hacia la amenaza, no replegarse solo. CEO balance knobs.
  coAdvanceBountyCap: 2,     // SUB-CAP DURO PROPIO de las fichas de envite por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.full)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE CO-ADVANCE (score∈{0,1,2}). Más jugadores empujando = tier de recompensa MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (solitario/<minPlayers) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con el avance parcialmente coordinado, A≥midAdvance): +1 ficha de envite por kill.
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate con el avance plenamente coordinado, A≥hiAdvance): +2 fichas por kill. CEO balance knobs.
  ],
};

// CAS-2829: DIANA (CO_FOCUS_SURGE, EVO#144 DARK) — 26º eje de COMPOSICIÓN-DE-INTENCIÓN, la NOVENA faceta de la sub-familia PLAYER-COORDINATION (#136 CÓNCLAVE=ENGANCHADOS por mobs, #137 COHORTE=PRESENTES en radio, #138 CUADRILLA=quién REMATA/kill-output, #139 SOCORRO=quién SOSTIENE/support-enabler, #140 MURALLA=quién ABSORBE/damage-intake, #141 REPLIEGUE=quién SE ALEJA/disengage, #142 PIÑA=cuán APIÑADO/spatial-cohesion STATE, #143 ENVITE=quién EMPUJA/velocity-toward). DIANA ABRE una sub-dimensión NUEVA dentro de PLAYER-COORDINATION: las OCHO priores leen el AUTO-ESTADO de cada jugador (aggro'd / presente / remató / sanó / absorbió / se replegó / apiñado / avanzando); DIANA lee la RELACIÓN jugador→OBJETIVO — cuántos aliados están DIRIGIENDO su ataque/aggro al MISMO ÚNICO mob este frame (el momento de FUEGO CONVERGENTE / focus-fire coordinado). MÉTRICA (ENTERO PURO) = G = MÁX sobre los mobs de (nº de JUGADORES DISTINTOS VIVOS dentro del radio compartido cuyo `targetId` ACTUAL == ese mob) = el tamaño del mayor grupo convergido sobre un solo objetivo. G∈[0,P] ENTERO por construcción (conteo por igualdad-de-id ENTERA — bucketea a los jugadores por `targetId`, toma el bucket MÁXIMO). Reusa el `targetId`/objetivo-de-ataque YA replicado + el id del mob — SNAPSHOT de un solo frame, SIN replicación pesada nueva. Bandas sobre G: G≥midFocus (2) ⇒ peso 1; G≥hiFocus (3) ⇒ peso 2. Premia CONCENTRAR el fuego en UN objetivo compartido (converger), no dispersar el aim. Gate minPlayers2; single-player ⇒ ≤1 jugador sobre cualquier objetivo ⇒ G colapsa ⇒ 0 (colapso LIMPIO — IMPOSIBLE converger fuego en solitario). 🔑 SNAPSHOT PURO/STATELESS (como #121/#123/#124/#136/#137/#138/#139/#140/#141/#142/#143 — SIN ring-buffer; lee el `targetId` replicado en UN frame — la asignación-de-objetivo del jugador YA es server-auth por diseño MMO); h.coFocusBounty transitorio, NINGÚN estado per-tick nuevo mantenido por esta flag. 🔑 DETERMINISMO (sev-1): G = MÁX de un tally por igualdad-de-id ENTERA (bucket por `targetId|0`, comparación de id ENTERA, el MÁX es CONMUTATIVO ⇒ INDEPENDIENTE del orden de iteración) vs umbrales ENTEROS {midFocus,hiFocus}; CERO float, CERO division en score/decisión (float SÓLO cosmético para el badge idx = G/playerCap) ⇒ 2-cliente 0-desync garantizado. 🔑 MULTIJUGADOR-NATIVO: single-player ⇒ G colapsa ⇒ 0 (IMPOSIBLE en solitario por construcción). CRUX (LA CRÍTICA) — DIANA = CONVERGENCIA-DE-OBJETIVO (relación player→target: quién APUNTA-AL-MISMO), ⊥ las OCHO facetas priores Y ⊥ FOCUS_FIRE #62: (a) ⊥ #143 ENVITE (PRIMARIO — velocidad ⊥ objetivo): ENVITE lee la DIRECCIÓN de la velocidad del jugador (moviéndose-hacia); DIANA lee la IDENTIDAD del objetivo de ataque (apuntando-al-mismo). Una party puede converger fuego QUIETA (G alto / A0 — todos disparan al mismo mob sin moverse) o avanzar SIN objetivo compartido (A alto / G bajo — empujan pero cada quien a un mob distinto). velocidad-toward ⊥ target-identity, divergen en AMBOS sentidos. (b) ⊥ #62 FOCUS_FIRE (⚠️ VERIFICADO — MISMO tema, mecánica DISTINTA): FOCUS_FIRE #62 = un multiplicador PASSIVE de oro (canal goldFind, cede a KINSHIP) por-jugador SOSTENIDO/con-decay (half-life buffer) mientras se apunta a un objetivo compartido en ZONAS de fuego; DIANA #144 = un CONTEO ENTERO de un solo frame de jugadores DISTINTOS convergidos que alimenta un canal FIND FRESCO `coFocusFind` de fichas-por-remate + badge de surge. CANAL distinto (coFocusFind ⊥ goldFind), FORMA de recompensa distinta (find por-kill ⊥ passive de oro sostenido), TEMPORALIDAD distinta (single-frame snapshot ⊥ sostenido con decay), y DIANA es un CONTEO (cuántos convergen) mientras FOCUS_FIRE es un bono passive por-jugador. #138 CUADRILLA ya estableció co-kill ⊥ FOCUS_FIRE; DIANA extiende: converge-de-aim ⊥ passive-de-oro. (c) ⊥ #142 PIÑA (target-identity ⊥ position-tightness): apuntar-al-mismo-mob ⊥ estar-apiñado; party dispersa puede converger fuego (G alto/C bajo) o apiñada disparar a mobs distintos (C alto/G bajo). (d) ⊥ #136 CÓNCLAVE (aggro'd) / #138 CUADRILLA (kill) / #139 SOCORRO (heal) / #140 MURALLA (absorb) / #141 REPLIEGUE (flee): a-quién-apuntas ⊥ qué te pasa / qué ACCIÓN ejecutas. (e) ⊥ #137 COHORTE (converging-aim ⊥ merely-present): estar en el radio ⊥ apuntar todos al mismo. (f) ⊥ #132 PARTY_VITAL (target ⊥ HP-STATE float). (g) ⊥ #112 CONVERGE (eso lee la velocidad RADIAL de los MOBS; DIANA lee la IDENTIDAD del objetivo del JUGADOR). (h) ⊥ #128 MOMENTUM (snapshot de UN frame, no derivada temporal). ⊥ #123 CONTEST (cov/P FRACCIÓN — DIANA = conteo CRUDO del mayor grupo convergido). Canal FRESCO coFocusFind — NINGUNO de los 85 flags #59-#143 lo usa. Camino al 86º flag (base master c9fecae tras #143 ENVITE CO_ADVANCE LIVE, served 10f8be94fd1f/815).
export const CO_FOCUS_SURGE = {
  enabled: true,             // LIVE (EVO#144, 86º flag). CAS-2832 CTO flip false→true (Diana ⌾). Antes DARK: enabled:false ⇒ byte-neutral OFF (_coFocusPre=0 const inerte + seam rama muerta + h.coFocusBounty transitorio + G._coFocusParty transitoria) ⇒ world-build fingerprint = 15920977 estable. El flip false→true es un paso CTO separado (config-only 1-línea) tras las Gates.
  channel: "coFocusFind",    // canal FRESCO (fichas de diana por rematar mientras ≥2 aliados DISTINTOS convergen su ataque/aggro sobre el MISMO ÚNICO mob — el shared-target/focus-fire) — NINGUNO de los 85 flags #59-#143 lo usa. Distinto por CONSTRUCCIÓN de coAdvanceFind #143 (ENVITE = velocidad-TOWARD), coCohesionFind #142 (PIÑA = APIÑADOS/spatial-state), coFleeFind #141 (REPLIEGUE = SE ALEJAN), coTankFind #140 (MURALLA = ABSORBEN), coSupportFind #139 (SOCORRO = SOSTIENEN), coKillFind #138 (CUADRILLA = REMATAN), coPresenceFind #137 (COHORTE = PRESENTES), coStrikeFind #136 (CÓNCLAVE = ENGANCHADOS), goldFind (FOCUS_FIRE #62 = APUNTAR al mismo objetivo, PASSIVE sostenido). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio del CLUSTER de convergencia compartido (px): G se computa sobre los jugadores VIVOS dentro del radio del héroe, bucketeados por `targetId`, tomando el bucket MÁXIMO. Mirror #121/#123/#124/#136/#137/#138/#139/#140/#141/#143 radius (radio GRANDE compartido, ⊥ el 132 APRETADO de PIÑA #142 — DIANA mide IDENTIDAD-de-objetivo, no tightness). CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de jugadores convergidos sobre un mismo objetivo (G) para cualquier banda: con <2 (single-player) ⇒ colapsa a 0 (single-player LIMPIO). El focus-fire de party es INTRÍNSECAMENTE multijugador — IMPOSIBLE converger fuego en solitario. CEO balance knob.
  midFocus: 2,               // umbral ENTERO de G (tamaño del mayor grupo convergido) para la banda MID/convergencia-parcial (peso 1): G≥2 (≥2 jugadores sobre el mismo mob). Decisión ENTERA: #converged ≥ 2. CEO balance knob.
  hiFocus: 3,                // umbral ENTERO de G para la banda HI/convergencia-plena (peso 2): G≥3 (≥3 jugadores sobre el mismo mob). G<midFocus ⇒ 0. CEO balance knob.
  playerCap: 4,              // TECHO de jugadores usado por el clamp + el badge idx cosmético (G/playerCap). CEO balance knob.
  weights: { full: 2, some: 1 },   // PESO de un remate por la BANDA de convergencia (cuántos jugadores convergen sobre un objetivo): HI/convergencia-plena (G≥hiFocus) pesa 2; MID/convergencia-parcial (≥midFocus) pesa 1; solitario/<minPlayers (<midFocus) ⇒ 0. Reward = CONVERGER el fuego en UN objetivo, no dispersar el aim. CEO balance knobs.
  coFocusBountyCap: 2,       // SUB-CAP DURO PROPIO de las fichas de diana por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.full)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE CONVERGENCIA (score∈{0,1,2}). Más jugadores convergidos = tier de recompensa MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (solitario/<minPlayers) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con el fuego parcialmente convergido, G≥midFocus): +1 ficha de diana por kill.
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate con el fuego plenamente convergido, G≥hiFocus): +2 fichas por kill. CEO balance knobs.
  ],
};

// CAS-2835: TENAZA (CO_FLANK_SURGE, EVO#145 DARK) — 27º eje de COMPOSICIÓN-DE-INTENCIÓN, la DÉCIMA faceta de la sub-familia PLAYER-COORDINATION (#136 CÓNCLAVE=ENGANCHADOS, #137 COHORTE=PRESENTES, #138 CUADRILLA=REMATAN, #139 SOCORRO=SOSTIENEN, #140 MURALLA=ABSORBEN, #141 REPLIEGUE=SE ALEJAN, #142 PIÑA=APIÑADOS/spatial-cohesion STATE, #143 ENVITE=EMPUJAN/velocity-toward, #144 DIANA=CONVERGEN-EL-AIM/target-count). TENAZA es la sub-faceta GEOMETRÍA de la CONVERGENCIA que ABRIÓ #144 DIANA: donde DIANA cuenta CUÁNTOS jugadores comparten `targetId` sobre un mob (un CONTEO crudo), TENAZA lee el ARREGLO ANGULAR de esos atacantes ALREDEDOR del mob — ¿lo están RODEANDO/pinzando (sectores OPUESTOS) o amontonados en UN solo flanco? Dos jugadores pueden converger su fuego desde el MISMO lado (DIANA alta, flank bajo) o desde lados OPUESTOS (flank alto). CONTEO ⊥ GEOMETRÍA — divergen en AMBOS sentidos. MÉTRICA (ENTERO PURO) = F = MÁX sobre los mobs de (nº de SECTORES ANGULARES DISTINTOS ocupados por los jugadores VIVOS que apuntan a ese mob, dentro del radio de engagement del mob) = el mayor grado de envolvimiento/pinza sobre un solo objetivo. Cada jugador se bucketea a un sector por SIGNO ENTERO puro de su offset mob-céntrico: `sx=sign(dx), sy=sign(dy)`, sector=`(sx+1)*3+(sy+1)` ∈ {0..8} (celda de una rejilla 3×3 de signos, 8 sectores no-centrales + centro). F = popcount de la máscara de bits OR de los sectores ocupados (UNIÓN de conjuntos ⇒ CONMUTATIVA/orden-independiente ⇒ determinista). F∈[0,9] ENTERO. Reusa el `targetId`+posiciones YA replicados (MISMO snapshot que DIANA #144) — SNAPSHOT de un solo frame, SIN replicación pesada nueva, SIN ring-buffer. Bandas sobre F: F≥midFlank (2) ⇒ peso 1 (pinza parcial, atacan desde ≥2 sectores); F≥hiFlank (3) ⇒ peso 2 (envolvimiento pleno, ≥3 sectores ⇒ rodeo real). Gate minPlayers2; single-player ⇒ ≤1 atacante sobre cualquier mob ⇒ ≤1 sector ⇒ F<midFlank ⇒ 0 (colapso LIMPIO — IMPOSIBLE pinzar en solitario). 🔑 SNAPSHOT PURO/STATELESS (como #121/#123/#124/#136-#144 — SIN ring-buffer; lee `targetId`+pos replicados en UN frame); h.coFlankBounty transitorio, NINGÚN estado per-tick nuevo. 🔑 DETERMINISMO (sev-1): F = popcount de una máscara de sectores por SIGNO ENTERO (CERO `atan2`/ángulo/normalización float; el OR de máscaras es CONMUTATIVO ⇒ orden-independiente) vs umbrales ENTEROS {midFlank,hiFlank}; CERO float en el score/decisión (float SÓLO cosmético para el badge idx = F/playerCap) ⇒ 2-cliente 0-desync garantizado. 🔑 MULTIJUGADOR-NATIVO: single-player ⇒ F colapsa ⇒ 0. CRUX (LA CRÍTICA) — TENAZA = GEOMETRÍA-DE-ENVOLVIMIENTO (arreglo angular player↔target), ⊥ TODOS los priores: (a) ⊥ #144 DIANA (PRIMARIO — CONTEO ⊥ GEOMETRÍA): DIANA cuenta CUÁNTOS convergen su `targetId` (G=tamaño del bucket); TENAZA cuenta DESDE-CUÁNTOS-SECTORES lo hacen (F=# sectores distintos). Divergen en AMBOS sentidos: 2 jugadores MISMO lado ⇒ G2/F1 (convergen pero NO rodean), 2 jugadores OPUESTOS ⇒ G2/F2 (rodean); 3 amontonados en un flanco ⇒ G3/F1, 2 en pinza ⇒ G2/F2 (G bajo/F alto). CONTEO ⊥ ARREGLO. (b) ⊥ #108 FLANK_SURGE (⚠️ VERIFICADO — MISMO nombre-raíz, mecánica OPUESTA): FLANK_SURGE #108 lee la GEOMETRÍA DE LOS MOBS (concentración angular de los RUMBOS hero→mob del ENJAMBRE VIVO, LONGITUD RESULTANTE float R∈[0,1], canal `flankFind`); TENAZA #145 lee la GEOMETRÍA DE LOS JUGADORES (# sectores distintos de los ALIADOS alrededor de UN mob objetivo, CONTEO ENTERO, canal FRESCO `coFlankFind`). LADO distinto (enemigo vs party), MÉTRICA distinta (float resultant-length vs conteo-entero de sectores), CANAL distinto. (c) ⊥ #142 PIÑA (player↔player tightness ⊥ player↔target arrangement): PIÑA mide cuán apretados están los jugadores ENTRE SÍ (radio 132 hero-anchored); TENAZA mide cómo se distribuyen ALREDEDOR del mob objetivo. Party apiñada puede atacar desde UN flanco (C alto/F1) o dispersa rodear (C bajo/F alto). (d) ⊥ #112 CONVERGE (velocidad RADIAL de los MOBS ⊥ arreglo POSICIONAL de los JUGADORES). (e) ⊥ #137 COHORTE (present ⊥ surrounding). (f) ⊥ #143 ENVITE (velocity-toward) / #141 REPLIEGUE / #140 MURALLA / #139 SOCORRO / #138 CUADRILLA / #136 CÓNCLAVE (qué ACCIÓN ejecuta cada quién ⊥ desde qué sector). (g) ⊥ #132 PARTY_VITAL (HP-STATE float). (h) ⊥ #123 CONTEST (cov/P FRACCIÓN — TENAZA = conteo ENTERO de sectores). (i) ⊥ #128 MOMENTUM (snapshot de UN frame). Canal FRESCO coFlankFind — NINGUNO de los 86 flags #59-#144 lo usa. Camino al 87º flag (base master 8f7961c tras #144 DIANA CO_FOCUS LIVE, served a210d646fa96/815, census 72).
export const CO_FLANK_SURGE = {
  enabled: true,             // LIVE (EVO#145, 87º flag — CAS-2837 CTO flip). enabled:true ⇒ TENAZA activa: fichas de tenaza por rematar mientras ≥2 aliados DISTINTOS pinzan el MISMO mob desde ≥2 SECTORES angulares (F=popcount máscara de sectores por SIGNO ENTERO). DARK QA PASS 21/21 ×2 (CAS-2836 @322b7ab), CEO Gate 1/2 PASS. Flip config-only 1-línea false→true, mirror #144 DIANA (CAS-2832).
  channel: "coFlankFind",    // canal FRESCO (fichas de tenaza por rematar mientras ≥2 aliados DISTINTOS pinzan/rodean el MISMO mob desde ≥2 SECTORES angulares — el pincer/surround) — NINGUNO de los 86 flags #59-#144 lo usa. Distinto por CONSTRUCCIÓN de coFocusFind #144 (DIANA = CONTEO de convergentes), flankFind #108 (FLANK_SURGE = concentración angular float de MOBS), coCohesionFind #142 (PIÑA = APIÑADOS player↔player), coAdvanceFind #143 (ENVITE = velocidad-TOWARD). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  radius: 300,               // radio de engagement mob-céntrico (px): un jugador cuenta como flanqueador de un mob sólo si su offset mob-céntrico dx²+dy²≤radius² (está cerca del mob que ataca). Mirror #144 radius (radio GRANDE compartido). CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de sectores/jugadores distintos pinzando un mismo mob (F) para cualquier banda: con <2 (single-player, ≤1 sector) ⇒ colapsa a 0 (single-player LIMPIO). El pincer de party es INTRÍNSECAMENTE multijugador — IMPOSIBLE rodear en solitario. CEO balance knob.
  midFlank: 2,               // umbral ENTERO de F (# sectores distintos) para la banda MID/pinza-parcial (peso 1): F≥2 (atacan desde ≥2 sectores angulares). Decisión ENTERA: #sectores ≥ 2. CEO balance knob.
  hiFlank: 3,                // umbral ENTERO de F para la banda HI/envolvimiento-pleno (peso 2): F≥3 (≥3 sectores ⇒ rodeo real, no sólo dos flancos). F<midFlank ⇒ 0. CEO balance knob.
  playerCap: 4,              // TECHO usado por el clamp + el badge idx cosmético (F/playerCap). CEO balance knob.
  weights: { full: 2, some: 1 },   // PESO de un remate por la BANDA de envolvimiento (desde cuántos sectores pinzan): HI/envolvimiento-pleno (F≥hiFlank) pesa 2; MID/pinza-parcial (≥midFlank) pesa 1; solitario/un-flanco (<midFlank) ⇒ 0. Reward = RODEAR/pinzar un objetivo desde lados opuestos, no amontonarse en un flanco. CEO balance knobs.
  coFlankBountyCap: 2,       // SUB-CAP DURO PROPIO de las fichas de tenaza por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.charge)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE ENVOLVIMIENTO (score∈{0,1,2}). Más sectores angulares ocupados = tier de recompensa MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (un-flanco/solitario) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con pinza parcial, F≥midFlank ⇒ atacan desde ≥2 sectores): +1 ficha de tenaza por kill.
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate con envolvimiento pleno, F≥hiFlank ⇒ ≥3 sectores/rodeo): +2 fichas por kill. CEO balance knobs.
  ],
};

// CAS-2840: CONJURO (CO_CAST_SURGE, EVO#146 DARK) — 28º eje de COMPOSICIÓN-DE-INTENCIÓN, la ONCEAVA faceta de la sub-familia PLAYER-COORDINATION (#136 CÓNCLAVE=ENGANCHADOS, #137 COHORTE=PRESENTES, #138 CUADRILLA=REMATAN, #139 SOCORRO=SOSTIENEN/heal-application, #140 MURALLA=ABSORBEN, #141 REPLIEGUE=SE ALEJAN, #142 PIÑA=APIÑADOS, #143 ENVITE=EMPUJAN, #144 DIANA=CONVERGEN-el-aim/CONTEO, #145 TENAZA=GEOMETRÍA-de-la-convergencia). CONJURO es la sub-faceta ACTION-EVENT (evento-de-activación): ¿cuántos jugadores DISTINTOS están ACTIVANDO una habilidad EL MISMO frame (cualquier cast/canalización/uso de skill), independiente de QUÉ apuntan (#144), DÓNDE están (#142/#145) o QUÉ TIPO de soporte aplican (#139)? Dos jugadores pueden estar presentes y convergiendo (DIANA/TENAZA altas) pero sólo uno mid-cast, o TODOS lanzando habilidades a la vez. MÉTRICA (ENTERO PURO) = U = nº de jugadores VIVOS DISTINTOS con un flag de activación-de-habilidad ACTIVO este frame (un tally BOOLEANO sobre el roster de estado cast/skill YA replicado — SNAPSHOT de un solo frame, STATELESS, SIN ring-buffer). Bandas sobre U: U≥midCast (2) ⇒ peso 1 (sync parcial); U≥hiCast (3) ⇒ peso 2 (sync pleno). Gate minPlayers2; single-player ⇒ ≤1 caster ⇒ U<midCast ⇒ 0 (colapso LIMPIO — IMPOSIBLE co-castear en solitario). 🔑 SNAPSHOT PURO/STATELESS (lee el flag `cast` YA replicado en UN frame); h.coCastBounty transitorio, NINGÚN estado per-tick nuevo. 🔑 DETERMINISMO (sev-1): U = tally ENTERO de banderas booleanas (CERO float/atan2 en el score/decisión; el conteo es CONMUTATIVO ⇒ orden-independiente) vs umbrales ENTEROS {midCast,hiCast}; float SÓLO cosmético para el badge idx = U/playerCap ⇒ 2-cliente 0-desync garantizado. 🔑 MULTIJUGADOR-NATIVO: single-player ⇒ U colapsa ⇒ 0. CRUX (LA CRÍTICA) — CONJURO = SIMULTANEIDAD-DE-ACTIVACIÓN (evento cast/skill) ⊥ TODOS los priores: (a) ⊥ #139 SOCORRO (PRIMARIO — ANY-ABILITY ⊥ SUPPORT-ONLY): SOCORRO cuenta específicamente la APLICACIÓN de soporte/heal/escudo/buff/revive (una CLASE de habilidad); CONJURO cuenta CUALQUIER activación de habilidad (ofensiva+defensiva+utilidad). Divergen: 3 jugadores lanzando DPS ⇒ CONJURO U3/w2 pero SOCORRO S0/w0 (ningún heal); 1 sanando + 2 quietos ⇒ SOCORRO S1 pero CONJURO U1/w0. (b) ⊥ #138 CUADRILLA (casting ⊥ killing): una canalización que NO remata cuenta para CONJURO, NO para CUADRILLA (que cuenta remates/kill-output). (c) ⊥ #144 DIANA (activar-habilidad ⊥ compartir-objetivo: 2 lanzando a mobs DISTINTOS ⇒ CONJURO U2 pero DIANA G1). (d) ⊥ #145 TENAZA (evento-de-acción ⊥ arreglo-geométrico). (e) ⊥ #140 MURALLA (activar ⊥ absorber daño). (f) ⊥ #132 PARTY_VITAL (HP-STATE float). (g) ⊥ ninguna flag de cast/ability/skill/cooldown previa (grep verificado: castAbility/castSpell/ABILITY_RANKS son el sistema de cast PROPIO del héroe, INTERRUPT_SURGE es mob-side; NINGUNA cuenta jugadores-DISTINTOS-casteando-este-frame). Canal FRESCO coCastFind — NINGUNO de los 87 flags #59-#145 lo usa. Camino al 88º flag (base master 4758b411 tras #145 TENAZA CO_FLANK LIVE, served 251653b6d194/815, census 73).
export const CO_CAST_SURGE = {
  enabled: true,             // LIVE (EVO#146, 88º — CAS-2842 flip). enabled:true tras CEO Gate 1/2 BOTH PASS + DARK QA CAS-2841 PASS 21/21 ×2. Flip 1-línea false→true, mirror #145 TENAZA (CAS-2837).
  channel: "coCastFind",     // canal FRESCO (fichas de conjuro por rematar mientras ≥2 aliados DISTINTOS activan una habilidad EL MISMO frame — el co-cast/ability-sync) — NINGUNO de los 87 flags #59-#145 lo usa. Distinto por CONSTRUCCIÓN de coFlankFind #145 (TENAZA = GEOMETRÍA de sectores), coFocusFind #144 (DIANA = CONTEO de convergentes), coSupportFind #139 (SOCORRO = APLICACIÓN de heal/buff, una CLASE), coKillFind #138 (CUADRILLA = remates). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de casters DISTINTOS (U) para cualquier banda: con <2 (single-player, ≤1 caster) ⇒ colapsa a 0 (single-player LIMPIO). El co-cast es INTRÍNSECAMENTE multijugador — IMPOSIBLE sincronizar en solitario. CEO balance knob.
  midCast: 2,                // umbral ENTERO de U (# casters distintos) para la banda MID/sync-parcial (peso 1): U≥2 (≥2 jugadores activan una habilidad este frame). Decisión ENTERA: #casters ≥ 2. CEO balance knob.
  hiCast: 3,                 // umbral ENTERO de U para la banda HI/sync-pleno (peso 2): U≥3 (≥3 casters simultáneos ⇒ salva coordinada). U<midCast ⇒ 0. CEO balance knob.
  playerCap: 4,              // TECHO usado por el clamp + el badge idx cosmético (U/playerCap). CEO balance knob.
  weights: { full: 2, some: 1 },   // PESO de un remate por la BANDA de sincronía (cuántos casters simultáneos): HI/sync-pleno (U≥hiCast) pesa 2; MID/sync-parcial (≥midCast) pesa 1; solitario (<midCast) ⇒ 0. Reward = activar habilidades EN CONCIERTO, no en solitario. CEO balance knobs.
  coCastBountyCap: 2,        // SUB-CAP DURO PROPIO de las fichas de conjuro por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.charge)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE SINCRONÍA (score∈{0,1,2}). Más casters simultáneos = tier de recompensa MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (solitario) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con sync parcial, U≥midCast ⇒ ≥2 casters simultáneos): +1 ficha de conjuro por kill.
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate con sync pleno, U≥hiCast ⇒ ≥3 casters/salva): +2 fichas por kill. CEO balance knobs.
  ],
};

// CAS-2845: QUIEBRO (CO_DODGE_SURGE, EVO#147 DARK) — 29º eje de COMPOSICIÓN-DE-INTENCIÓN, la DOCEAVA faceta de la sub-familia PLAYER-COORDINATION (#136 CÓNCLAVE=ENGANCHADOS, #137 COHORTE=PRESENTES, #138 CUADRILLA=REMATAN, #139 SOCORRO=SOSTIENEN/heal, #140 MURALLA=ABSORBEN, #141 REPLIEGUE=SE ALEJAN/velocidad, #142 PIÑA=APIÑADOS, #143 ENVITE=EMPUJAN, #144 DIANA=CONVERGEN-el-aim/CONTEO, #145 TENAZA=GEOMETRÍA, #146 CONJURO=ACTIVAN-habilidad/cast). QUIEBRO es la sub-faceta DEFENSIVE REACTION-EVENT (evento-de-reacción-defensiva): ¿cuántos jugadores DISTINTOS están EN un estado de esquiva/rodada/i-frame/evade ACTIVO este frame (una esquiva-sincronizada), independiente de si ACTIVAN cualquier habilidad (#146 CONJURO cuenta el `cast`), QUÉ apuntan (#144), DÓNDE están (#142/#145), CÓMO se alejan por velocidad (#141), o QUÉ TIPO de soporte aplican (#139)? Todos pueden estar casteando (#146 alto) sin esquivar, o todos rodando fuera de un AoE a la vez. MÉTRICA (ENTERO PURO) = D = nº de jugadores VIVOS DISTINTOS con un flag de esquiva/i-frame (`dodge`) ACTIVO este frame (un tally BOOLEANO sobre el roster de estado dodge/roll YA replicado — SNAPSHOT de un solo frame, STATELESS, SIN ring-buffer). Bandas sobre D: D≥midDodge (2) ⇒ peso 1 (sync parcial); D≥hiDodge (3) ⇒ peso 2 (sync pleno). Gate minPlayers2; single-player ⇒ ≤1 dodger ⇒ D<midDodge ⇒ 0 (colapso LIMPIO — IMPOSIBLE co-esquivar en solitario). 🔑 SNAPSHOT PURO/STATELESS (lee el flag `dodge` YA replicado en UN frame); h.coDodgeBounty transitorio, NINGÚN estado per-tick nuevo. 🔑 DETERMINISMO (sev-1): D = tally ENTERO de banderas booleanas (CERO float/atan2 en el score/decisión; el conteo es CONMUTATIVO ⇒ orden-independiente) vs umbrales ENTEROS {midDodge,hiDodge}; float SÓLO cosmético para el badge idx = D/playerCap ⇒ 2-cliente 0-desync garantizado. 🔑 MULTIJUGADOR-NATIVO: single-player ⇒ D colapsa ⇒ 0. CRUX (LA CRÍTICA) — QUIEBRO = SIMULTANEIDAD-DE-ESQUIVA (evento dodge/roll/i-frame) ⊥ TODOS los priores: (a) ⊥ #146 CONJURO (PRIMARIO — DODGE-STATE ⊥ ANY-CAST): CONJURO cuenta el flag `cast` (activar CUALQUIER habilidad); QUIEBRO cuenta el flag `dodge` (estado DEFENSIVO de esquiva/rodada/i-frame — un STATE DISTINTO del `cast`, verificado por grep: h.rolling/h.iframe/CFG.rollIFrame/DODGE.iframeMs son el sistema de rodada REACTIVA, SIN relación con castAbility/castSpell). Divergen: 3 jugadores mid-cast ⇒ CONJURO U3/w2 pero QUIEBRO D0/w0 (ninguno rodando); 3 rodando un AoE ⇒ QUIEBRO D3/w2 pero CONJURO U0/w0 (ninguno casteando). (b) ⊥ #141 REPLIEGUE (evento-discreto-dodge ⊥ velocidad-away): REPLIEGUE lee la MAGNITUD de velocidad-alejándose; QUIEBRO lee el flag DISCRETO de esquiva/i-frame. Un jugador puede esquivar EN EL SITIO (D alto, flee-velocity 0) o retroceder SIN i-frame (flee alto, D0). (c) ⊥ #140 MURALLA (esquivar ⊥ absorber): esquivar EVITA el daño (i-frame), tankear lo ABSORBE — OPUESTO. (d) ⊥ #138 CUADRILLA (esquivar ⊥ rematar). (e) ⊥ #144 DIANA (esquivar ⊥ compartir-objetivo). (f) ⊥ #145 TENAZA (evento-defensivo ⊥ arreglo-geométrico). (g) ⊥ #132 PARTY_VITAL (HP-STATE float). (h) ⊥ ninguna flag de dodge/roll/iframe previa (grep verificado: h.rolling/h.iframe/DODGE/DODGE_COUNTER/perfectDodge son mecánicas de rodada PROPIAS del héroe single-player; NINGUNA cuenta jugadores-DISTINTOS-esquivando-este-frame). Canal FRESCO coDodgeFind — NINGUNO de los 88 flags #59-#146 lo usa. Camino al 89º flag (base master 80a5208 tras #146 CONJURO CO_CAST LIVE, served b8c147402d1f/815, census 74).
export const CO_DODGE_SURGE = {
  enabled: true,             // LIVE (EVO#147, 89º flag). CAS-2847 flip false→true tras CEO Gate 1/2 + DARK QA CAS-2846 PASS 22/22 ×2 (mirror #146 CO_CAST CAS-2842).
  channel: "coDodgeFind",    // canal FRESCO (fichas de quiebro por rematar mientras ≥2 aliados DISTINTOS están en estado de esquiva/rodada/i-frame EL MISMO frame — el co-dodge/evade-sync) — NINGUNO de los 88 flags #59-#146 lo usa. Distinto por CONSTRUCCIÓN de coCastFind #146 (CONJURO = ACTIVAR-habilidad/cast), coFlankFind #145 (TENAZA = GEOMETRÍA), coFocusFind #144 (DIANA = CONTEO de convergentes), coSupportFind #139 (SOCORRO = heal/buff). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de dodgers DISTINTOS (D) para cualquier banda: con <2 (single-player, ≤1 dodger) ⇒ colapsa a 0 (single-player LIMPIO). El co-dodge es INTRÍNSECAMENTE multijugador — IMPOSIBLE sincronizar una esquiva en solitario. CEO balance knob.
  midDodge: 2,               // umbral ENTERO de D (# dodgers distintos) para la banda MID/sync-parcial (peso 1): D≥2 (≥2 jugadores en esquiva/i-frame este frame). Decisión ENTERA: #dodgers ≥ 2. CEO balance knob.
  hiDodge: 3,                // umbral ENTERO de D para la banda HI/sync-pleno (peso 2): D≥3 (≥3 esquivas simultáneas ⇒ evade coordinado). D<midDodge ⇒ 0. CEO balance knob.
  playerCap: 4,              // TECHO usado por el clamp + el badge idx cosmético (D/playerCap). CEO balance knob.
  weights: { full: 2, some: 1 },   // PESO de un remate por la BANDA de sincronía (cuántas esquivas simultáneas): HI/sync-pleno (D≥hiDodge) pesa 2; MID/sync-parcial (≥midDodge) pesa 1; solitario (<midDodge) ⇒ 0. Reward = esquivar EN CONCIERTO, no en solitario. CEO balance knobs.
  coDodgeBountyCap: 2,       // SUB-CAP DURO PROPIO de las fichas de quiebro por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.charge)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE SINCRONÍA (score∈{0,1,2}). Más esquivas simultáneas = tier de recompensa MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (solitario) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con sync parcial, D≥midDodge ⇒ ≥2 esquivas simultáneas): +1 ficha de quiebro por kill.
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate con sync pleno, D≥hiDodge ⇒ ≥3 esquivas/evade): +2 fichas por kill. CEO balance knobs.
  ],
};

// CAS-2849: YUGO (CO_INTERRUPT_SURGE, EVO#148 DARK) — 30º eje de COMPOSICIÓN-DE-INTENCIÓN, la TRECEAVA faceta de la sub-familia PLAYER-COORDINATION (#136 CÓNCLAVE=ENGANCHADOS, #137 COHORTE=PRESENTES, #138 CUADRILLA=REMATAN, #139 SOCORRO=SOSTIENEN/heal, #140 MURALLA=ABSORBEN, #141 REPLIEGUE=SE ALEJAN/velocidad, #142 PIÑA=APIÑADOS, #143 ENVITE=EMPUJAN, #144 DIANA=CONVERGEN-el-aim/CONTEO, #145 TENAZA=GEOMETRÍA, #146 CONJURO=ACTIVAN-habilidad/cast, #147 QUIEBRO=ESQUIVAN/dodge-evento-defensivo). YUGO es la sub-faceta OFFENSIVE CONTROL-EFFECT REACTION-EVENT (evento-de-reacción-ofensiva de control): el ESPEJO OFENSIVO de #147 QUIEBRO (defensivo). ¿cuántos jugadores DISTINTOS están LANDEANDO un interrupt/stagger/poise-break/stun sobre un ENEMIGO EL MISMO frame (un lockdown-sincronizado), independiente de si ACTIVAN cualquier habilidad (#146 CONJURO cuenta el `cast`), ESQUIVAN (#147 QUIEBRO cuenta el `dodge`), QUÉ apuntan (#144), DÓNDE están (#142/#145), CÓMO se alejan (#141), o QUÉ TIPO de soporte aplican (#139)? Todos pueden estar casteando (#146 alto) o rodando (#147 alto) SIN landear ningún stagger; o los tres poise-breakean al MISMO jefe a la vez. MÉTRICA (ENTERO PURO) = I = nº de jugadores VIVOS DISTINTOS con un flag de interrupt/stagger/poise-break (`interrupt`) LANDEADO sobre un enemigo este frame (un tally BOOLEANO sobre el roster del EVENTO de control YA replicado — SNAPSHOT de un solo frame, STATELESS, SIN ring-buffer). Bandas sobre I: I≥midInterrupt (2) ⇒ peso 1 (lockdown parcial); I≥hiInterrupt (3) ⇒ peso 2 (lockdown pleno). Gate minPlayers2; single-player ⇒ ≤1 interrupter ⇒ I<midInterrupt ⇒ 0 (colapso LIMPIO — IMPOSIBLE co-interrumpir en solitario). 🔑 SNAPSHOT PURO/STATELESS (lee el flag `interrupt` YA replicado en UN frame — el EVENTO de que un poise-break/stagger LANDEÓ, no un estado sostenido); h.coInterruptBounty transitorio, NINGÚN estado per-tick nuevo. 🔑 DETERMINISMO (sev-1): I = tally ENTERO de banderas booleanas (CERO float/atan2 en el score/decisión; el conteo es CONMUTATIVO ⇒ orden-independiente) vs umbrales ENTEROS {midInterrupt,hiInterrupt}; float SÓLO cosmético para el badge idx = I/playerCap ⇒ 2-cliente 0-desync garantizado. 🔑 MULTIJUGADOR-NATIVO: single-player ⇒ I colapsa ⇒ 0. CRUX (LA CRÍTICA) — YUGO = SIMULTANEIDAD-DE-CONTROL-LANDEADO (evento interrupt/stagger/poise-break sobre ENEMIGO) ⊥ TODOS los priores: (a) ⊥ #146 CONJURO (PRIMARIO — LANDED-CONTROL-EFFECT ⊥ ANY-CAST): CONJURO cuenta el flag `cast` (ACTIVAR cualquier habilidad); YUGO cuenta el flag `interrupt` (el EFECTO DE CONTROL LANDEADO sobre un enemigo — un stagger/poise-break/stun APLICADO, DISTINTO del `cast`, verificado por grep: POISE.poise/e.staggerT/breakStun/enemyPoise son el sistema de POSTURA/stagger REAL del enemigo, NO el sistema castAbility del jugador). Divergen: 3 jugadores mid-cast ⇒ CONJURO U3/w2 pero YUGO I0/w0 (ninguno LANDEA un stagger — castear ≠ romper postura); 3 poise-breakeando un jefe ⇒ YUGO I3/w2 pero CONJURO U0/w0 (el poise-break puede venir de un swing MELEE pesado, sin cast). (b) ⊥ #147 QUIEBRO (interrupt-EVENTO-OFENSIVO ⊥ dodge-STATE-DEFENSIVO): QUIEBRO cuenta el flag `dodge` (estado DEFENSIVO que EVITA daño/i-frame); YUGO cuenta el flag `interrupt` (evento OFENSIVO que LOCKEA a un enemigo). OPUESTOS: un jugador puede esquivar SIN interrumpir (D alto/I0) o poise-breakear sin esquivar (I alto/D0). (c) ⊥ #138 CUADRILLA (interrumpir/staggerear ⊥ landear el REMATE/killing blow — se puede staggerear sin matar). (d) ⊥ #143 ENVITE (evento-de-CONTROL ⊥ desplazamiento/push por velocidad — si el stagger se implementa como knockback, YUGO lee el flag de CONTROL LANDEADO, NO la velocidad-toward que lee ENVITE). (e) ⊥ INTERRUPT_SURGE #89 (mob-side: rematar a UN mob mid-acción, interruptFind/estado-del-MOB-al-kill ⊥ player-side coInterruptFind/CONTEO-de-jugadores-DISTINTOS-que-landean-control — mismo par que FLANK_SURGE #108 vs CO_FLANK #145). (f) ⊥ #144 DIANA (interrumpir ⊥ compartir-objetivo). (g) ⊥ #145 TENAZA (evento-de-control ⊥ arreglo-geométrico). (h) ⊥ #132 PARTY_VITAL (HP-STATE float). Canal FRESCO coInterruptFind — NINGUNO de los 89 flags #59-#147 lo usa. Camino al 90º flag (base master df6dd56 tras #147 QUIEBRO CO_DODGE LIVE, served 90e5b2600ac2/815, census 75).
export const CO_INTERRUPT_SURGE = {
  enabled: true,             // LIVE (EVO#148, 90º flag). CAS-2851 flip false→true tras CEO Gate 1/2 + DARK QA CAS-2850 PASS 22/22 ×2 (mirror #147 CO_DODGE CAS-2847).
  channel: "coInterruptFind",// canal FRESCO (fichas de yugo por rematar mientras ≥2 aliados DISTINTOS están LANDEANDO un interrupt/stagger/poise-break sobre un enemigo EL MISMO frame — el co-interrupt/lockdown-sync) — NINGUNO de los 89 flags #59-#147 lo usa. Distinto por CONSTRUCCIÓN de coDodgeFind #147 (QUIEBRO = ESQUIVAR/dodge-defensivo), coCastFind #146 (CONJURO = ACTIVAR-habilidad/cast), interruptFind #89 (INTERRUPT_SURGE = mob-side/estado-del-mob-al-kill). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de interrupters DISTINTOS (I) para cualquier banda: con <2 (single-player, ≤1 interrupter) ⇒ colapsa a 0 (single-player LIMPIO). El co-interrupt es INTRÍNSECAMENTE multijugador — IMPOSIBLE sincronizar un lockdown en solitario. CEO balance knob.
  midInterrupt: 2,           // umbral ENTERO de I (# interrupters distintos) para la banda MID/lockdown-parcial (peso 1): I≥2 (≥2 jugadores landeando un stagger/poise-break este frame). Decisión ENTERA: #interrupters ≥ 2. CEO balance knob.
  hiInterrupt: 3,            // umbral ENTERO de I para la banda HI/lockdown-pleno (peso 2): I≥3 (≥3 interrupts simultáneos ⇒ lockdown coordinado). I<midInterrupt ⇒ 0. CEO balance knob.
  playerCap: 4,              // TECHO usado por el clamp + el badge idx cosmético (I/playerCap). CEO balance knob.
  weights: { full: 2, some: 1 },   // PESO de un remate por la BANDA de lockdown (cuántos interrupts simultáneos): HI/lockdown-pleno (I≥hiInterrupt) pesa 2; MID/lockdown-parcial (≥midInterrupt) pesa 1; solitario (<midInterrupt) ⇒ 0. Reward = interrumpir EN CONCIERTO, no en solitario. CEO balance knobs.
  coInterruptBountyCap: 2,   // SUB-CAP DURO PROPIO de las fichas de yugo por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.charge)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE LOCKDOWN (score∈{0,1,2}). Más interrupts simultáneos = tier de recompensa MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (solitario) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con lockdown parcial, I≥midInterrupt ⇒ ≥2 interrupts simultáneos): +1 ficha de yugo por kill.
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate con lockdown pleno, I≥hiInterrupt ⇒ ≥3 interrupts/lockdown): +2 fichas por kill. CEO balance knobs.
  ],
};

// CAS-2853: ÉGIDA (CO_BUFF_UPTIME_SURGE, EVO#149 DARK) — 31º eje de COMPOSICIÓN-DE-INTENCIÓN, la CATORCEAVA faceta de la sub-familia PLAYER-COORDINATION (#136 CÓNCLAVE=ENGANCHADOS, #137 COHORTE=PRESENTES, #138 CUADRILLA=REMATAN, #139 SOCORRO=SOSTIENEN/APLICAN-heal, #140 MURALLA=ABSORBEN, #141 REPLIEGUE=SE ALEJAN/velocidad, #142 PIÑA=APIÑADOS, #143 ENVITE=EMPUJAN, #144 DIANA=CONVERGEN-el-aim/CONTEO, #145 TENAZA=GEOMETRÍA, #146 CONJURO=ACTIVAN-habilidad/cast-EVENTO, #147 QUIEBRO=ESQUIVAN/dodge-EVENTO-defensivo, #148 YUGO=INTERRUMPEN/interrupt-EVENTO-ofensivo). ÉGIDA es la sub-faceta SUSTAINED-STATE / SHARED-EMPOWERMENT (estado-sostenido de potenciación compartida): el CONTRAPUNTO de las TRES sub-facetas EVENTO (#146 cast / #147 dodge / #148 interrupt). ¿cuántos jugadores VIVOS DISTINTOS PORTAN un buff/aura ACTIVO (shield/haste/might/regen/resina) EL MISMO frame (una potenciación-compartida SOSTENIDA), independiente de QUIÉN lo aplicó, de si ALGUIEN aplicó uno este frame (#139 SOCORRO cuenta el crédito de APLICACIÓN), de si ACTIVAN una habilidad (#146 CONJURO cuenta el `cast`), ESQUIVAN (#147), INTERRUMPEN (#148), QUÉ apuntan (#144), o DÓNDE están (#142/#145)? Todos pueden estar casteando un buff (#146 alto) SIN estar aún buffeados (B0); o los tres PORTAR un aura de hace rato SIN aplicar ninguno este frame (S0 pero B3). MÉTRICA (ENTERO PURO) = B = nº de jugadores VIVOS DISTINTOS con un flag de buff/aura ACTIVO (`buff`) este frame (un tally BOOLEANO sobre el roster del ESTADO de potenciación YA replicado — SNAPSHOT de un solo frame, STATELESS, SIN ring-buffer; lee la UPTIME, no la aplicación). Bandas sobre B: B≥midBuff (2) ⇒ peso 1 (égida parcial); B≥hiBuff (3) ⇒ peso 2 (égida plena). Gate minPlayers2; single-player ⇒ ≤1 buffeado ⇒ B<midBuff ⇒ 0 (colapso LIMPIO — IMPOSIBLE co-buffear/compartir-aegis en solitario). 🔑 SNAPSHOT PURO/STATELESS (lee el flag `buff` YA replicado en UN frame — el ESTADO SOSTENIDO de portar un aura activo, NO el evento de aplicarlo); h.coBuffBounty transitorio, NINGÚN estado per-tick nuevo. 🔑 DETERMINISMO (sev-1): B = tally ENTERO de banderas booleanas (CERO float/atan2 en el score/decisión; el conteo es CONMUTATIVO ⇒ orden-independiente) vs umbrales ENTEROS {midBuff,hiBuff}; float SÓLO cosmético para el badge idx = B/playerCap ⇒ 2-cliente 0-desync garantizado. 🔑 MULTIJUGADOR-NATIVO: single-player ⇒ B colapsa ⇒ 0. CRUX (LA CRÍTICA) — ÉGIDA = SIMULTANEIDAD-DE-ESTADO-SOSTENIDO-DE-BUFF (uptime de aura/shield/might/regen ACTIVO) ⊥ TODOS los priores: (a) ⊥ #139 SOCORRO (PRIMARIO — SUSTAINED-STATE ⊥ APPLICATION-EVENT): SOCORRO cuenta el crédito de APLICAR un heal/shield/buff/revive a aliados ESTE frame (el flag `support`, un EVENTO de aplicación); ÉGIDA cuenta el flag `buff` (el ESTADO SOSTENIDO de PORTAR un aura activo — la uptime — SIN importar quién lo aplicó ni si alguien aplicó uno este frame; verificado por grep: WEAPON_BUFFS/h._wbuff/h.wbuffT = tipo+segundos-restantes de un buff ACTIVO del héroe, el sistema de UPTIME REAL, DISTINTO del support-credit de #139). Divergen: 3 jugadores mid-support-cast ⇒ SOCORRO S3/w2 pero ÉGIDA B0/w0 (aplicando pero nadie buffeado AÚN — aplicar ≠ portar); 3 jugadores portando un aura de hace rato ⇒ ÉGIDA B3/w2 pero SOCORRO S0/w0 (0 aplicaciones este frame, sólo uptime). (b) ⊥ #146 CONJURO (STATE-DE-BUFF ⊥ ANY-CAST-EVENTO): CONJURO cuenta el flag `cast` (ACTIVAR cualquier habilidad, un evento discreto); castear un buff ≠ tenerlo ya activo. 3 mid-cast ⇒ CONJURO U3 pero ÉGIDA B0 (el cast aún no resolvió el buff); 3 portando un aura pasivo ⇒ ÉGIDA B3 pero CONJURO U0 (nadie activando nada este frame). (c) ⊥ #132 PARTY_VITAL (buff-uptime BOOLEANA-ENTERA ⊥ HP-fracción-media-STATE float): ÉGIDA cuenta CUÁNTOS portan un buff (conteo entero), NO la media de HP (PARTY_VITAL es float). (d) ⊥ #140 MURALLA (portar-un-buff ⊥ ABSORBER-un-golpe: estar buffeado ≠ tomar daño de mob). (e) ⊥ #138 CUADRILLA (portar-un-buff ⊥ landear-el-REMATE). (f) ⊥ #147 QUIEBRO / #148 YUGO (estado-de-buff SOSTENIDO ⊥ dodge-EVENTO / interrupt-EVENTO — se puede portar un aura sin esquivar ni interrumpir). (g) ⊥ #144 DIANA (buff ⊥ compartir-objetivo). (h) ⊥ #145 TENAZA (estado ⊥ geometría). Canal FRESCO coBuffFind — NINGUNO de los 90 flags #59-#148 lo usa. Camino al 91º flag (base master 9c136a6 tras #148 YUGO CO_INTERRUPT LIVE, served 4a0403b7e832/815, census 76).
export const CO_BUFF_UPTIME_SURGE = {
  enabled: true,             // LIVE — EVO#149 (CAS-2855, 91º flag). Flip false→true tras CEO Gate 1/2 byte-verify + DARK QA CAS-2854 PASS 22/22 ×2 (fp 15920977 fpMatch, byte-neutral OFF, ⊥SOCORRO both dirs, census 77) + CEO re-verify @a5353f2 (mirror #148 CO_INTERRUPT CAS-2849→2851).
  channel: "coBuffFind",     // canal FRESCO (fichas de égida por rematar mientras ≥2 aliados DISTINTOS PORTAN un buff/aura activo EL MISMO frame — el co-buff/shared-aegis/uptime) — NINGUNO de los 90 flags #59-#148 lo usa. Distinto por CONSTRUCCIÓN de coInterruptFind #148 (YUGO = INTERRUMPIR/interrupt-EVENTO), coDodgeFind #147 (QUIEBRO = ESQUIVAR/dodge-EVENTO), coCastFind #146 (CONJURO = ACTIVAR/cast-EVENTO), coSupportFind #139 (SOCORRO = APLICAR-socorro/support-EVENTO). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de buffeados DISTINTOS (B) para cualquier banda: con <2 (single-player, ≤1 buffeado) ⇒ colapsa a 0 (single-player LIMPIO). El co-buff es INTRÍNSECAMENTE multijugador — IMPOSIBLE compartir una égida en solitario. CEO balance knob.
  midBuff: 2,                // umbral ENTERO de B (# buffeados distintos) para la banda MID/égida-parcial (peso 1): B≥2 (≥2 jugadores portando un buff/aura activo este frame). Decisión ENTERA: #buffeados ≥ 2. CEO balance knob.
  hiBuff: 3,                 // umbral ENTERO de B para la banda HI/égida-plena (peso 2): B≥3 (≥3 auras activos simultáneos ⇒ potenciación coordinada). B<midBuff ⇒ 0. CEO balance knob.
  playerCap: 4,              // TECHO usado por el clamp + el badge idx cosmético (B/playerCap). CEO balance knob.
  weights: { full: 2, some: 1 },   // PESO de un remate por la BANDA de égida (cuántos auras activos): HI/égida-plena (B≥hiBuff) pesa 2; MID/égida-parcial (≥midBuff) pesa 1; solitario (<midBuff) ⇒ 0. Reward = potenciar EN CONJUNTO, no en solitario. CEO balance knobs.
  coBuffBountyCap: 2,        // SUB-CAP DURO PROPIO de las fichas de égida por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.charge)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE ÉGIDA (score∈{0,1,2}). Más auras activos simultáneos = tier de recompensa MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (solitario) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con égida parcial, B≥midBuff ⇒ ≥2 auras activos simultáneos): +1 ficha de égida por kill.
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate con égida plena, B≥hiBuff ⇒ ≥3 auras/potenciación): +2 fichas por kill. CEO balance knobs.
  ],
};

// CAS-2857: BROQUEL (CO_GUARD_SURGE, EVO#150 DARK) — 32º eje de COMPOSICIÓN-DE-INTENCIÓN, la QUINCEAVA faceta de la sub-familia PLAYER-COORDINATION (#136 CÓNCLAVE=ENGANCHADOS, #137 COHORTE=PRESENTES, #138 CUADRILLA=REMATAN, #139 SOCORRO=SOSTIENEN/APLICAN-heal, #140 MURALLA=ABSORBEN, #141 REPLIEGUE=SE ALEJAN, #142 PIÑA=APIÑADOS, #143 ENVITE=EMPUJAN, #144 DIANA=CONVERGEN-CONTEO, #145 TENAZA=GEOMETRÍA, #146 CONJURO=ACTIVAN-habilidad/cast-EVENTO, #147 QUIEBRO=ESQUIVAN/dodge-EVENTO-DEFENSIVO, #148 YUGO=INTERRUMPEN/interrupt-EVENTO-OFENSIVO, #149 ÉGIDA=PORTAN-buff/sustained-STATE). CONTEXTO: co-revive COLAPSA (grep: NO existe estado downed/bleedout/incap — heroDie()→pantalla-muerte→respawn es INSTANTÁNEO; y #139 SOCORRO ya incluye `revive` en el support-credit `p.support`). FALLBACK autorizado por el umbrella ⇒ CO-BLOQUEO/CO-PARADA. BROQUEL es la SEGUNDA sub-faceta DEFENSIVE REACTION-EVENT (par con #147 QUIEBRO): pero QUIEBRO=ESQUIVAR (EVADIR — rodada/i-frame, `h.rolling`/`h.iframe`) mientras BROQUEL=GUARDIA-ACTIVA (INTERCEPTAR — block/parry, `h.blocking`/`h.parryT`; sistemas REALES SHIELD_BLOCK/PARRY/GUARD_COUNTER/DEFLECT/RIPOSTE). Son las DOS opciones defensivas CANÓNICAS y OPUESTAS de un ARPG. ¿cuántos jugadores VIVOS DISTINTOS están en GUARDIA ACTIVA (bloqueando con escudo o en ventana de parada — el flag `guard`) EL MISMO frame? MÉTRICA (ENTERO PURO) = G = nº de jugadores VIVOS DISTINTOS con el flag `guard` (block/parry/guard-up) ACTIVO este frame (tally BOOLEANO sobre el estado de guardia YA replicado — SNAPSHOT de un solo frame, STATELESS, SIN ring-buffer). Bandas sobre G: G≥midGuard (2) ⇒ peso 1 (broquel parcial); G≥hiGuard (3) ⇒ peso 2 (broquel pleno). Gate minPlayers2; single-player ⇒ ≤1 en guardia ⇒ G<midGuard ⇒ 0 (colapso LIMPIO — IMPOSIBLE co-bloquear en solitario). 🔑 SNAPSHOT PURO/STATELESS (lee el flag `guard` YA replicado en UN frame — el ESTADO de guardia-activa, un evento-de-reacción defensivo distinto de la rodada #147). 🔑 DETERMINISMO (sev-1): G = tally ENTERO de banderas booleanas (CERO float/atan2 en el score/decisión; conteo CONMUTATIVO ⇒ orden-independiente) vs umbrales ENTEROS {midGuard,hiGuard}; float SÓLO cosmético para el badge idx = G/playerCap ⇒ 2-cliente 0-desync garantizado. 🔑 MULTIJUGADOR-NATIVO: single-player ⇒ G colapsa ⇒ 0. CRUX (LA CRÍTICA) — BROQUEL = SIMULTANEIDAD-DE-GUARDIA-ACTIVA (block/parry-up) ⊥ TODOS los priores: (a) ⊥ #147 QUIEBRO (PRIMARIO — INTERCEPTAR ⊥ EVADIR, las dos reacciones defensivas OPUESTAS): QUIEBRO cuenta el flag `dodge` (rodada/i-frame que SE APARTA del golpe); BROQUEL cuenta el flag `guard` (bloqueo/parada que SE PLANTA y absorbe/desvía el golpe — verificado por grep: h.blocking/h.parryT/GUARD_COUNTER/RIPOSTE/DEFLECT = guardia-activa REAL del héroe, DISTINTA de h.rolling/h.iframe #147). OPUESTOS: 3 rodando ⇒ QUIEBRO D3/w2 pero BROQUEL G0/w0 (esquivando, nadie plantado); 3 bloqueando ⇒ BROQUEL G3/w2 pero QUIEBRO D0/w0 (plantados en guardia, nadie rodando). (b) ⊥ #140 MURALLA (GUARDIA-ACCIÓN ⊥ ABSORBER-DAÑO-INTAKE): MURALLA cuenta quién TOMA daño de mob (damage-intake); BROQUEL cuenta la GUARDIA-ACTIVA sostenida ESTE frame SIN importar si un golpe conectó. 3 en guardia sin recibir golpe ⇒ BROQUEL G3 pero MURALLA T0 (guardia sin intake); 3 tanqueando a pecho descubierto (sin block) ⇒ MURALLA T3 pero BROQUEL G0. (c) ⊥ #148 YUGO (guardia-DEFENSIVA ⊥ interrupt-OFENSIVO sobre enemigo). (d) ⊥ #146 CONJURO (guardia ⊥ ACTIVAR-cualquier-habilidad/cast: bloquear no es castear). (e) ⊥ #149 ÉGIDA (guardia-ACCIÓN-EVENTO ⊥ portar-un-buff/uptime-STATE: bloquear ≠ tener un aura activo). (f) ⊥ #139 SOCORRO (bloquear ⊥ aplicar-socorro a aliado). (g) ⊥ #138 CUADRILLA (bloquear ⊥ rematar). (h) ⊥ #132 PARTY_VITAL (guardia BOOLEANA-ENTERA ⊥ HP-fracción-media float). (i) ⊥ #144 DIANA / #145 TENAZA (estado ⊥ objetivo/geometría). Canal FRESCO coGuardFind — NINGUNO de los 91 flags #59-#149 lo usa. Camino al 92º flag (base master 94b4895 tras #149 ÉGIDA CO_BUFF_UPTIME LIVE, served 7d6126f3e504/815, census 77).
export const CO_GUARD_SURGE = {
  enabled: false,            // DARK — EVO#150 (CAS-2857, 92º flag candidato). Flip false→true SÓLO tras CEO Gate 1/2 byte-verify + DARK QA PASS (fp fpMatch, byte-neutral OFF, ⊥QUIEBRO both dirs, census 78) (mirror #149 ÉGIDA CAS-2853→2855).
  channel: "coGuardFind",    // canal FRESCO (fichas de broquel por rematar mientras ≥2 aliados DISTINTOS están en GUARDIA ACTIVA/block/parry EL MISMO frame — el co-block/co-parry) — NINGUNO de los 91 flags #59-#149 lo usa. Distinto por CONSTRUCCIÓN de coBuffFind #149 (ÉGIDA = PORTAR-buff/uptime-STATE), coInterruptFind #148 (YUGO = INTERRUMPIR/ofensivo), coDodgeFind #147 (QUIEBRO = ESQUIVAR/EVADIR), coCastFind #146 (CONJURO = ACTIVAR/cast). Recurso TRANSITORIO, fuera del save + fingerprint. CEO balance knob.
  minPlayers: 2,             // nº MÍNIMO de en-guardia DISTINTOS (G) para cualquier banda: con <2 (single-player, ≤1 en guardia) ⇒ colapsa a 0 (single-player LIMPIO). El co-bloqueo es INTRÍNSECAMENTE multijugador — IMPOSIBLE co-bloquear en solitario. CEO balance knob.
  midGuard: 2,               // umbral ENTERO de G (# en guardia distintos) para la banda MID/broquel-parcial (peso 1): G≥2 (≥2 jugadores bloqueando/parando este frame). Decisión ENTERA: #en-guardia ≥ 2. CEO balance knob.
  hiGuard: 3,                // umbral ENTERO de G para la banda HI/broquel-pleno (peso 2): G≥3 (≥3 guardias activas simultáneas ⇒ muro de escudos coordinado). G<midGuard ⇒ 0. CEO balance knob.
  playerCap: 4,              // TECHO usado por el clamp + el badge idx cosmético (G/playerCap). CEO balance knob.
  weights: { full: 2, some: 1 },   // PESO de un remate por la BANDA de broquel (cuántas guardias activas): HI/broquel-pleno (G≥hiGuard) pesa 2; MID/broquel-parcial (≥midGuard) pesa 1; solitario (<midGuard) ⇒ 0. Reward = defender EN CONJUNTO, no en solitario. CEO balance knobs.
  coGuardBountyCap: 2,       // SUB-CAP DURO PROPIO de las fichas de broquel por kill — acota el bono aunque la tabla se re-tunee. = max(tiers.charge)=2 ⇒ neutral con la tabla actual. CEO balance knob.
  // TABLA de tiers por PESO DE BROQUEL (score∈{0,1,2}). Más guardias activas simultáneas = tier de recompensa MÁS ALTO. El tier vigente = el más intenso (mayor `min`) cuyo score se satisface. Score < 1 (solitario) ⇒ Tier 0 ⇒ +0. Determinista, LUT pura.
  tiers: [
    { min: 1, charge: 1 },   // Tier 1 — score ≥1 (remate con broquel parcial, G≥midGuard ⇒ ≥2 guardias activas): +1 ficha de broquel por kill.
    { min: 2, charge: 2 },   // Tier 2 — score ≥2 (remate con broquel pleno, G≥hiGuard ⇒ ≥3 guardias): +2 fichas por kill. CEO balance knobs.
  ],
};
