...
export function createRenderer(ctx){
  ...
  function render(alpha){
    ...
    if(JERK_DIR_SURGE.enabled) renderJerkDirBadge(); // CAS-2698: Quiebre/Regate — badge de FRACCIÓN TANGENCIAL del churn / 2º-orden PROYECTADO del movimiento de la formación de mobs VIVOS alrededor del héroe (J=Σtang/Σ|Δm|: weave/regate-coordinado⇒2/juke/regate-parcial⇒1/radial-línea-recta⇒0) (canal jerkDirFind, sub-cap jerkDirBountyCap; ⊥ accelFind #117-MAGNITUD-|Δm| — JERK es la FRACCIÓN tangencial tang/|Δm| ⊂ ACCEL); resalta si el héroe está en medio de una manada que REORIENTA/regatea/zigzaguea (no una que acelera/frena en línea recta), donde rematar rinde fichas de regate. Cosmético puro. DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(AGGRO_VARIETY_SURGE.enabled) renderAggroVarietyBadge(); // CAS-2736: Mezcla — badge de THREAT-HETEROGENEITY/MIXED-PULL / cuán heterogéneo es el pack ENGANCHADO (V=(K−1)/(cap−1): motley K≥3⇒2/mixto K≥2⇒1/monotipo⇒0) (canal aggroVarietyFind, sub-cap aggroVarietyBountyCap; ⊥ aggroDensityFind #126-MAGNITUD/aggroMarginFind #125-LEAD/aggroPileFind #124-max-share/aggroContestFind #123-AMPLITUD/targetSpreadFind #121-UNIFORMIDAD — MEZCLA vive en el eje MOB-TYPE, no en el reparto sobre jugadores); resalta si el héroe está en medio de un pull mixto (melee+ranged+caster / familias dispares), donde rematar rinde fichas de mezcla. Cosmético puro. INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ V=0 ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(AGGRO_SURROUND_SURGE.enabled) renderAggroSurroundBadge(); // CAS-2749: Cerco — badge de SPATIAL-ENCIRCLEMENT (la GEOMETRÍA ANGULAR de la amenaza, no el conteo/tendencia/reparto/tipo) / cuán rodeado 360° está el héroe (S=1−R, dispersión angular de rumbos: cercado S≥0.75⇒2/cerco-parcial S≥0.5⇒1/un-solo-lado⇒0) (canal aggroSurroundFind, sub-cap aggroSurroundBountyCap; ⊥ aggroMomentumFind #128-dN/dt/aggroVarietyFind #127-tipos/aggroDensityFind #126-MAGNITUD-N/aggroPileFind #124-max-share — CERCO vive en el eje GEOMÉTRICO-ANGULAR, no en el conteo/tendencia/reparto); resalta si el héroe está rodeado sin lado seguro, donde rematar rinde fichas de cerco. Cosmético puro. INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ S=0 ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(AGGRO_PRESSURE_SURGE.enabled) renderAggroPressureBadge(); // CAS-2753: Acoso — badge de RADIAL-PROXIMITY (el OTRO componente GEOMÉTRICO: cuán CERCA se aprieta el pack, no DÓNDE está angularmente #129) / cuán a quemarropa está el pack ENGANCHADO (P=cercanía radial media: apretado P≥0.75⇒2/acoso-parcial P≥0.5⇒1/con-sitio⇒0) (canal aggroPressureFind, sub-cap aggroPressureBountyCap; ⊥ aggroSurroundFind #129-ANGULAR/aggroDensityFind #126-MAGNITUD-N/convergeFind #112-VELOCIDAD-radial — ACOSO vive en el eje RADIAL-POSICIÓN, no en el ángulo/conteo/velocidad); resalta si el héroe tiene el pack encima sin aire, donde rematar rinde fichas de acoso. Cosmético puro. INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ P=0 ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(AGGRO_VIGOR_SURGE.enabled) renderAggroVigorBadge(); // CAS-2757: Brío — badge de MOB-HP-STATE (cuánta pelea LE QUEDA al pack, no DÓNDE está #129/#130, no CUÁNTOS #126, no de qué TIPO #127) / cuán fresco/full-HP está el pack ENGANCHADO (V=fracción-de-HP media: fresco V≥0.75⇒2/medio-fresco V≥0.5⇒1/maltrecho⇒0) (canal aggroVigorFind, sub-cap aggroVigorBountyCap; ⊥ aggroPressureFind #130-RADIAL/aggroSurroundFind #129-ANGULAR/aggroDensityFind #126-MAGNITUD-N/aggroVarietyFind #127-tipos — BRÍO vive en el eje HP-STATE, no en la posición/conteo/tipo); resalta si el héroe tiene enganchado un pack fresco (pelea larga por delante), donde enganchar/rematar rinde fichas de brío. Cosmético puro. INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ V=0 ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(MOB_TIER_SURGE.enabled) renderMobTierBadge(); // CAS-2767: Reto — badge de MOB-POWER-RATING (cuán PELIGROSO POR DISEÑO es el pack, su TIER/RANGO-DE-RETO intrínseco, no el HP del mob #131, no el HP del aliado #132, no DÓNDE está #129/#130, no CUÁNTOS #126, no de qué TIPO #127, no el tier de la ZONA #91) / cuán de alto tier es el pack ENGANCHADO (T=rango-de-poder medio: élite T≥0.75⇒2/reto-medio T≥0.5⇒1/basura⇒0) (canal mobTierFind, sub-cap mobTierBountyCap; ⊥ aggroVigorFind #131-HP-del-mob/partyVitalFind #132-HP-del-aliado/aggroVarietyFind #127-tipos — RETO lee el RANGO-DE-PODER del mob, no HP/tipo/posición; ⊥#73 APEX lee ese contenedor sólo como proximidad de UN apex, RETO lee la MEDIA del pack); resalta si el héroe tiene enganchado un pack de alto tier (reto real), donde rematar rinde fichas de reto. Cosmético puro. MULTIJUGADOR-NATIVO (single-player ⇒ T=0 ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(CO_SUPPORT_SURGE.enabled) renderCoSupportBadge(); // CAS-2807: Socorro — badge de PLAYER-COORDINATION #4 (cuántos JUGADORES DISTINTOS están APLICANDO un socorro/heal/shield/buff/revive en el radio compartido = S: socorro-pleno S≥3⇒2/socorro-parcial S≥2⇒1/solitario⇒0) (canal coSupportFind, sub-cap coSupportBountyCap; ⊥ coKillFind #138 CUADRILLA (REMATAN) — SOCORRO = quién SOSTIENE: sanador 0-kills contado por SOCORRO no CUADRILLA; ⊥ coPresenceFind #137 COHORTE (PRESENTES) — party presente sin socorrer R-alta/S0; ⊥ coStrikeFind #136 CÓNCLAVE (ENGANCHADOS); ⊥ PARTY_VITAL #132 (fracción-de-HP MEDIA = ESTADO vs conteo de quién ACTÚA = ACCIÓN)); resalta si el héroe remata mientras la party está EN SOCORRO (≥2 socorredores acreditados en el cluster), donde rematar rinde fichas de socorro. Cosmético puro. MULTIJUGADOR-NATIVO (single-player ⇒ S=0 ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(CO_GUARD_SURGE.enabled) renderCoGuardBadge(); // CAS-2857: Broquel — badge de PLAYER-COORDINATION #15 / SIMULTANEIDAD-DE-GUARDIA-ACTIVA (cuántos JUGADORES DISTINTOS están en block/parry/guard-up EL MISMO frame = G: broquel-pleno G≥3⇒2/broquel-parcial G≥2⇒1/solitario⇒0) (canal coGuardFind, sub-cap coGuardBountyCap; la SEGUNDA sub-faceta DEFENSIVE REACTION-EVENT, OPUESTA a #147 QUIEBRO: QUIEBRO=EVADIR/dodge, BROQUEL=INTERCEPTAR/block-parry; ⊥ coDodgeFind #147 (INTERCEPTAR ⊥ EVADIR — 3 rodando D3 pero G0; 3 bloqueando G3 pero D0); ⊥ coTankFind #140 MURALLA (guardia-ACCIÓN ⊥ absorber-daño-intake); ⊥ coInterruptFind #148 YUGO (defensivo ⊥ ofensivo); ⊥ coBuffFind #149 ÉGIDA (guardia-ACCIÓN ⊥ portar-buff-STATE); ⊥ PARTY_VITAL #132 (guardia ENTERA ⊥ HP-media float)); co-revive COLAPSÓ (grep: sin estado downed/bleedout — muerte instantánea; #139 SOCORRO ya cuenta revive) ⇒ fallback co-block/co-parry. Cosmético puro. MULTIJUGADOR-NATIVO (single-player ⇒ G colapsa ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(CO_BUFF_UPTIME_SURGE.enabled) renderCoBuffBadge(); // CAS-2853: Égida — badge de PLAYER-COORDINATION #14 / SIMULTANEIDAD-DE-ESTADO-SOSTENIDO-DE-BUFF (cuántos JUGADORES DISTINTOS PORTAN un buff/aura activo EL MISMO frame = B: égida-plena B≥3⇒2/égida-parcial B≥2⇒1/solitario⇒0) (canal coBuffFind, sub-cap coBuffBountyCap; el CONTRAPUNTO SOSTENIDO de las tres sub-facetas EVENTO #146 cast / #147 dodge / #148 interrupt; ⊥ coSupportFind #139 SOCORRO (SUSTAINED-STATE ⊥ APPLICATION-EVENT — 3 mid-support-cast S3 pero B0; 3 portando un aura B3 pero S0); ⊥ coCastFind #146 CONJURO (STATE-DE-BUFF ⊥ ANY-CAST — castear un buff ≠ tenerlo activo); ⊥ PARTY_VITAL #132 (buff-uptime BOOLEANA-ENTERA ⊥ HP-fracción-media float); ⊥ coTankFind #140 MURALLA (portar-un-buff ⊥ absorber); ⊥ coKillFind #138 CUADRILLA (portar-un-buff ⊥ rematar)); resalta si el héroe remata mientras la party PORTA una égida EN CONJUNTO (≥2 auras activos), donde rematar rinde fichas de égida. Cosmético puro. MULTIJUGADOR-NATIVO (single-player ⇒ B colapsa ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(CO_INTERRUPT_SURGE.enabled) renderCoInterruptBadge(); // CAS-2849: Yugo — badge de PLAYER-COORDINATION #13 / SIMULTANEIDAD-DE-CONTROL-LANDEADO (cuántos JUGADORES DISTINTOS están LANDEANDO un interrupt/stagger/poise-break sobre un enemigo EL MISMO frame = I: lockdown-pleno I≥3⇒2/lockdown-parcial I≥2⇒1/solitario⇒0) (canal coInterruptFind, sub-cap coInterruptBountyCap; el ESPEJO OFENSIVO de #147 QUIEBRO; ⊥ coCastFind #146 CONJURO (LANDED-CONTROL-EFFECT ⊥ ANY-CAST — 3 mid-cast U3 pero I0; 3 poise-breakeando I3 pero U0); ⊥ coDodgeFind #147 QUIEBRO (interrupt-EVENTO-OFENSIVO ⊥ dodge-STATE-DEFENSIVO — esquivar sin interrumpir D-alto/I0 vs poise-breakear sin esquivar I-alto/D0); ⊥ coKillFind #138 CUADRILLA (staggerear ⊥ rematar); ⊥ coAdvanceFind #143 ENVITE (control ⊥ push/velocidad); ⊥ INTERRUPT_SURGE #89 (mob-side/estado-del-mob-al-kill ⊥ player-side/CONTEO); ⊥ PARTY_VITAL #132 (HP-STATE)); resalta si el héroe remata mientras la party interrumpe EN CONCIERTO (≥2 interrupts simultáneos), donde rematar rinde fichas de yugo. Cosmético puro. MULTIJUGADOR-NATIVO (single-player ⇒ I colapsa ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(CO_DODGE_SURGE.enabled) renderCoDodgeBadge(); // CAS-2845: Quiebro — badge de PLAYER-COORDINATION #12 / SIMULTANEIDAD-DE-ESQUIVA (cuántos JUGADORES DISTINTOS están en estado de esquiva/rodada/i-frame EL MISMO frame = D: sync-pleno D≥3⇒2/sync-parcial D≥2⇒1/solitario⇒0) (canal coDodgeFind, sub-cap coDodgeBountyCap; ⊥ coCastFind #146 CONJURO (DODGE-STATE ⊥ ANY-CAST — 3 mid-cast U3 pero D0; 3 rodando D3 pero U0); ⊥ CO_FLEE #141 REPLIEGUE (evento-dodge ⊥ velocidad-away); ⊥ CO_TANK #140 MURALLA (esquivar/i-frame EVITA ⊥ tank ABSORBE); ⊥ coKillFind #138 CUADRILLA (esquivar ⊥ rematar); ⊥ PARTY_VITAL #132 (HP-STATE); ⊥ toda flag dodge/roll/iframe previa (rodada PROPIA del héroe single-player)); resalta si el héroe remata mientras la party esquiva EN CONCIERTO (≥2 esquivas simultáneas), donde rematar rinde fichas de quiebro. Cosmético puro. MULTIJUGADOR-NATIVO (single-player ⇒ D colapsa ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(CO_CAST_SURGE.enabled) renderCoCastBadge(); // CAS-2840: Conjuro — badge de PLAYER-COORDINATION #11 / SIMULTANEIDAD-DE-ACTIVACIÓN (cuántos JUGADORES DISTINTOS activan una habilidad EL MISMO frame = U: sync-pleno U≥3⇒2/sync-parcial U≥2⇒1/solitario⇒0) (canal coCastFind, sub-cap coCastBountyCap; ⊥ coSupportFind #139 SOCORRO (ANY-ABILITY ⊥ SUPPORT-ONLY — 3 lanzando DPS U3 pero S0; 1 sanando S1 pero U1); ⊥ coKillFind #138 CUADRILLA (casting ⊥ killing); ⊥ coFocusFind #144 DIANA (activar ⊥ compartir-objetivo); ⊥ coFlankFind #145 TENAZA (evento ⊥ geometría); ⊥ PARTY_VITAL #132 (HP-STATE); ⊥ toda flag cast/ability/skill previa (sistema de cast PROPIO del héroe / INTERRUPT_SURGE mob-side)); resalta si el héroe remata mientras la party lanza habilidades EN CONCIERTO (≥2 casters simultáneos), donde rematar rinde fichas de conjuro. Cosmético puro. MULTIJUGADOR-NATIVO (single-player ⇒ U colapsa ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(CO_FLANK_SURGE.enabled) renderCoFlankBadge(); // CAS-2835: Tenaza — badge de PLAYER-COORDINATION #10 / GEOMETRÍA-DE-ENVOLVIMIENTO (desde CUÁNTOS SECTORES angulares distintos ≥2 jugadores pinzan/rodean el MISMO mob = F: envolvimiento-pleno F≥3⇒2/pinza-parcial F≥2⇒1/un-flanco-solitario⇒0) (canal coFlankFind, sub-cap coFlankBountyCap; ⊥ coFocusFind #144 DIANA (CONTEO de convergentes ⊥ GEOMETRÍA de sectores — 2 MISMO lado G2/F1 vs 2 OPUESTOS G2/F2); ⊥ FLANK_SURGE #108 (geometría de MOBS float/flankFind ⊥ geometría de JUGADORES entero/coFlankFind); ⊥ coCohesionFind #142 PIÑA (player↔player tightness ⊥ player↔target arrangement); ⊥ CONVERGE #112 (velocidad RADIAL de MOBS); ⊥ coAdvanceFind #143 ENVITE/coFleeFind #141/coTankFind #140/coSupportFind #139/coKillFind #138/coPresenceFind #137/coStrikeFind #136 (qué ACCIÓN ⊥ desde qué sector); ⊥ PARTY_VITAL #132 (HP-STATE)); resalta si el héroe remata mientras la party RODEA/pinza un objetivo desde ≥2 sectores, donde rematar rinde fichas de tenaza. Cosmético puro. MULTIJUGADOR-NATIVO (single-player ⇒ F colapsa ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(CO_FOCUS_SURGE.enabled) renderCoFocusBadge(); // CAS-2829: Diana — badge de PLAYER-COORDINATION #9 / CONVERGENCIA-DE-OBJETIVO (cuántos JUGADORES DISTINTOS convergen su ataque/aggro sobre el MISMO ÚNICO mob = G: convergencia-plena G≥3⇒2/convergencia-parcial G≥2⇒1/solitario-disperso⇒0) (canal coFocusFind, sub-cap coFocusBountyCap; ⊥ coAdvanceFind #143 ENVITE (velocidad-toward ⊥ target-identity — converger fuego QUIETA G/A0 o avanzar sin objetivo compartido A/G bajo); ⊥ FOCUS_FIRE #62 (canal coFocusFind ⊥ goldFind, find por-kill ⊥ passive de oro sostenido con decay, single-frame ⊥ sostenido, CONTEO ⊥ bono passive por-jugador); ⊥ coCohesionFind #142 PIÑA (target-identity ⊥ position-tightness); ⊥ coTankFind #140 MURALLA (ABSORBEN); ⊥ coSupportFind #139 SOCORRO (heal); ⊥ coKillFind #138 CUADRILLA (REMATAN); ⊥ coFleeFind #141 REPLIEGUE (SE ALEJAN); ⊥ coPresenceFind #137 COHORTE (converging-aim ⊥ merely-present); ⊥ coStrikeFind #136 CÓNCLAVE (aggro'd); ⊥ PARTY_VITAL #132 (HP-STATE); ⊥ CONVERGE #112 (velocidad RADIAL de MOBS)); resalta si el héroe remata mientras la party CONVERGE el fuego sobre un objetivo (≥2 convergidos), donde rematar rinde fichas de diana. Cosmético puro. MULTIJUGADOR-NATIVO (single-player ⇒ G colapsa ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(CO_ADVANCE_SURGE.enabled) renderCoAdvanceBadge(); // CAS-2824: Envite — badge de PLAYER-COORDINATION #8 (cuántos JUGADORES DISTINTOS EMPUJAN/avanzan con velocidad-HACIA la amenaza compartida = A: avance-pleno A≥3⇒2/avance-parcial A≥2⇒1/solitario-replegándose⇒0) (canal coAdvanceFind, sub-cap coAdvanceBountyCap; INVERSO MILITANTE de coFleeFind #141 REPLIEGUE (velocidad-HACIA ⊥ velocidad-away) — party puede avanzar (A/D0), replegarse (D/A0) o partirse; ⊥ coCohesionFind #142 PIÑA (STATE espacial vs ACCIÓN de velocidad: avanzar DISPERSO A/C bajo o plantarse APIÑADO C/A0); ⊥ coTankFind #140 MURALLA (ABSORBEN); ⊥ coSupportFind #139 SOCORRO (heal); ⊥ coKillFind #138 CUADRILLA (REMATAN); ⊥ coPresenceFind #137 COHORTE (moving-toward ⊥ merely-present); ⊥ coStrikeFind #136 CÓNCLAVE (aggro'd); ⊥ PARTY_VITAL #132 (HP-STATE); ⊥ CONVERGE #112 (velocidad RADIAL de MOBS)); resalta si el héroe remata mientras la party EMPUJA hacia la amenaza (≥2 pushers), donde rematar rinde fichas de envite. Cosmético puro. MULTIJUGADOR-NATIVO (single-player ⇒ A colapsa ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(CO_COHESION_SURGE.enabled) renderCoCohesionBadge(); // CAS-2820: Piña — badge de PLAYER-COORDINATION #7 / GEOMETRÍA (cuántos JUGADORES DISTINTOS están APIÑADOS hombro-con-hombro dentro del radio de cohesión APRETADO = C: piña-apretada C≥3⇒2/piña-parcial C≥2⇒1/solitario-dispersa⇒0) (canal coCohesionFind, sub-cap coCohesionBountyCap; ⊥ coPresenceFind #137 COHORTE (PRESENTES en radio GRANDE 300px) — PIÑA = quién está AGRUPADO en radio APRETADO 132px: {3 presentes a ~200px} COHORTE R3/PIÑA C0, PRESENTE ⊥ APIÑADO; ⊥ coFleeFind #141 REPLIEGUE (disengage ACCIÓN vs tightness ESTADO); ⊥ coTankFind #140 MURALLA (ABSORBEN); ⊥ coSupportFind #139 SOCORRO (heal); ⊥ coKillFind #138 CUADRILLA (REMATAN); ⊥ coStrikeFind #136 CÓNCLAVE (aggro'd); ⊥ PARTY_VITAL #132 (fracción-de-HP MEDIA = ESTADO-de-salud vs cohesión ESPACIAL)); resalta si el héroe remata mientras la party está APIÑADA (≥2 apiñados hombro-con-hombro), donde rematar rinde fichas de piña. Cosmético puro. MULTIJUGADOR-NATIVO (single-player ⇒ C colapsa ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(CO_FLEE_SURGE.enabled) renderCoFleeBadge(); // CAS-2815: Repliegue — badge de PLAYER-COORDINATION #6 (cuántos JUGADORES DISTINTOS están ROMPIENDO contacto / repositionándose ALEJÁNDOSE de una amenaza en el radio compartido = D: repliegue-pleno D≥3⇒2/repliegue-parcial D≥2⇒1/solitario⇒0) (canal coFleeFind, sub-cap coFleeBountyCap; ⊥ coTankFind #140 MURALLA (ABSORBEN) — REPLIEGUE = quién SE ALEJA: ABSORBER (comer el tick) ⊥ REPLEGARSE (evitar el siguiente) — tank T/D0, kiter D/T0; ⊥ coStrikeFind #136 CÓNCLAVE (aggro'd) — aggro'd y plantado F/D0 vs aggro'd y kiteando D cuenta; ⊥ coSupportFind #139 SOCORRO (heal); ⊥ coKillFind #138 CUADRILLA (REMATAN); ⊥ coPresenceFind #137 COHORTE (PRESENTES); ⊥ PARTY_VITAL #132 (fracción-de-HP MEDIA = ESTADO vs conteo de quién se repliegó este frame = ACCIÓN)); resalta si el héroe remata mientras la party está REPLEGÁNDOSE de la amenaza (≥2 kiters en el cluster), donde rematar rinde fichas de repliegue. Cosmético puro. MULTIJUGADOR-NATIVO (single-player ⇒ D colapsa ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(CO_TANK_SURGE.enabled) renderCoTankBadge(); // CAS-2812: Muralla — badge de PLAYER-COORDINATION #5 (cuántos JUGADORES DISTINTOS están ABSORBIENDO/TOMANDO daño de mob en el radio compartido = T: carga-plena T≥3⇒2/carga-parcial T≥2⇒1/solitario⇒0) (canal coTankFind, sub-cap coTankBountyCap; ⊥ coSupportFind #139 SOCORRO (SOSTIENEN) — MURALLA = quién ABSORBE: tank 0-heals contado por MURALLA no SOCORRO; ⊥ coStrikeFind #136 CÓNCLAVE (ENGANCHADOS/aggro'd) — aggro'd pero esquivando ⇒ CÓNCLAVE F/MURALLA T0, AoE sin ser aggro-target ⇒ MURALLA T/CÓNCLAVE 0; ⊥ coKillFind #138 CUADRILLA (REMATAN); ⊥ coPresenceFind #137 COHORTE (PRESENTES); ⊥ PARTY_VITAL #132 (fracción-de-HP MEDIA = ESTADO vs conteo de quién comió un golpe este frame = ACCIÓN)); resalta si el héroe remata mientras la party está REPARTIENDO la carga (≥2 absorbedores acreditados en el cluster), donde rematar rinde fichas de muralla. Cosmético puro. MULTIJUGADOR-NATIVO (single-player ⇒ T colapsa ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(CO_KILL_SURGE.enabled) renderCoKillBadge(); // CAS-2802: Cuadrilla — badge de PLAYER-COORDINATION #3 (cuántos JUGADORES DISTINTOS están ACREDITADOS con un golpe-de-gracia/remate en el radio compartido = K: assist-pleno K≥3⇒2/assist-parcial K≥2⇒1/solitario⇒0) (canal coKillFind, sub-cap coKillBountyCap; ⊥ coPresenceFind #137 COHORTE (PRESENTES) — CUADRILLA = quién REMATA: {4 presentes,1 rematando} K1/R4; ⊥ coStrikeFind #136 CÓNCLAVE (ENGANCHADOS) — tank enganchado 0-kills contado por CÓNCLAVE no CUADRILLA; ⊥ FOCUS_FIRE #62 (APUNTAR al mismo objetivo, passive goldFind) — boss focus-fireado sin morir ⇒ FOCUS_FIRE 3/CUADRILLA 0); resalta si el héroe remata EN CUADRILLA (≥2 killers acreditados en el cluster), donde rematar rinde fichas de cuadrilla. Cosmético puro. MULTIJUGADOR-NATIVO (single-player ⇒ K=1 ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(CO_PRESENCE_SURGE.enabled) renderCoPresenceBadge(); // CAS-2797: Cohorte — badge de PLAYER-COORDINATION #2 (cuántos JUGADORES DISTINTOS están PRESENTES/rallevados en el radio del héroe = R: rally-pleno R≥3⇒2/rally-parcial R≥2⇒1/solitario⇒0) (canal coPresenceFind, sub-cap coPresenceBountyCap; ⊥ coStrikeFind #136 CÓNCLAVE (ENGANCHADOS en combate) — COHORTE = PRESENTES: {3 presentes,1 enganchado} R3/F1; ⊥ aggroContestFind #123 (FRACCIÓN cov/P)/aggroDensityFind #126 (conteo de MOBS)/CONGREGATION (headcount ZONAL → restedMult vs RADIO-LOCAL → find)); resalta si el héroe está agrupado con una party PRESENTE (≥2 aliados en radio), donde rematar rinde fichas de cohorte. Cosmético puro. MULTIJUGADOR-NATIVO (single-player ⇒ R=1 ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(CO_STRIKE_SURGE.enabled) renderCoStrikeBadge(); // CAS-2792: Cónclave — badge de PLAYER-COORDINATION (cuántos JUGADORES DISTINTOS están ENGANCHADOS en la MISMA refriega = F=muster: cónclave-pleno F≥3⇒2/cónclave-parcial F≥2⇒1/solitario⇒0) (canal coStrikeFind, sub-cap coStrikeBountyCap; ⊥ aggroContestFind #123 CONTEST (FRACCIÓN cov/P) — CÓNCLAVE = cov CRUDO/conteo: {2 de 2} F2/C1 vs {2 de 4} F2/C0.5; ⊥ aggroPileFind #124 (max-share)/targetSpreadFind #121 (entropía)/aggroDensityFind #126 (conteo de MOBS)); resalta si el héroe pelea junto a una HUESTE reunida (≥2 aliados metidos en la refriega), donde rematar rinde fichas de cónclave. Cosmético puro. MULTIJUGADOR-NATIVO (single-player ⇒ F=0 ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(RANK_SPREAD_SURGE.enabled) renderRankSpreadBadge(); // CAS-2783: Brecha — badge de COMPOSICIÓN-DISPERSIÓN (cuán SEPARADOS están los tiers de poder del pack ENGANCHADO = R=maxRank−minRank: brecha-amplia R≥3⇒2/amenaza-mixta R≥2⇒1/uniforme⇒0) (canal rankSpreadFind, sub-cap rankSpreadBountyCap; ⊥ mobTierFind #133 RETO (MEDIA) Y eliteShareFind #134 CASTA (FRACCIÓN) SIMULTÁNEAMENTE — {4,2,0} vs {3,3,0}: misma media (2) y misma fracción-élite (2/3) pero R=4 vs R=3); resalta si el héroe tiene enganchado un pack de AMENAZA-MIXTA (ápice escoltado por relleno), donde rematar rinde fichas de brecha. Cosmético puro. MULTIJUGADOR-NATIVO (single-player ⇒ R=0 ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(ELITE_SHARE_SURGE.enabled) renderEliteShareBadge(); // CAS-2775: Casta — badge de COMPOSICIÓN-PROPORCIÓN (qué FRACCIÓN del pack ENGANCHADO es genuinamente peligrosa/élite+ = S=count(mobTierRank≥eliteRank)/N: mayoría-élite S≥0.75⇒2/casta-parcial S≥0.5⇒1/poca-élite⇒0) (canal eliteShareFind, sub-cap eliteShareBountyCap; ⊥ mobTierFind #133 RETO — RETO lee la MEDIA del rango, CASTA lee la FRACCIÓN por encima del umbral élite: {boss,normal,normal} misma media que {elite,elite,normal} pero share OPUESTO); resalta si el héroe tiene enganchado un pack MAYORÍA-ÉLITE, donde rematar rinde fichas de casta. Cosmético puro. MULTIJUGADOR-NATIVO (single-player ⇒ S=0 ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(PARTY_VITAL_SURGE.enabled) renderPartyVitalBadge(); // CAS-2763: Temple — badge de ALLY-HP-STATE (cuánta pelea LE QUEDA a la PARTY, la SALUD RESTANTE de los JUGADORES, no el HP del mob #131, no DÓNDE está #129/#130, no CUÁNTOS #126, no de qué TIPO #127) / cuán sana/full-HP está la party ALIADA (W=fracción-de-HP media de los jugadores: sana W≥0.75⇒2/media-sana W≥0.5⇒1/maltrecha⇒0) (canal partyVitalFind, sub-cap partyVitalBountyCap; ⊥ aggroVigorFind #131-HP-del-mob — TEMPLE lee el HP del JUGADOR, no del enemigo); resalta si la party está sana con un pack enganchado (puede comprometerse/apretar), donde rematar rinde fichas de temple. Cosmético puro. MULTIJUGADOR-NATIVO (single-player ⇒ W=0 ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(AGGRO_MOMENTUM_SURGE.enabled) renderAggroMomentumBadge(); // CAS-2743: Escalada — badge de ESCALATION/BUILD-UP (la TENDENCIA aggro CON SIGNO, no el snapshot) / cuánto CRECE el pack ENGANCHADO sobre Δ (M=min(1,rawM/cap), rawM=max(0,N_t−N_{t−Δ}): surging⇒2/creciendo⇒1/estable-menguante⇒0) (canal aggroMomentumFind, sub-cap aggroMomentumBountyCap; ⊥ aggroVarietyFind #127-tipos/aggroDensityFind #126-MAGNITUD-N/aggroSwitchFind #122-churn-de-identidad); resalta si el héroe está en medio de un pull que ESCALA (adds entrando), donde rematar rinde fichas de escalada. Cosmético puro. INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ M=0 ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(AGGRO_DENSITY_SURGE.enabled) renderAggroDensityBadge(); // CAS-2730: Avalancha — badge de OUTNUMBER/SWARM-PRESSURE (1er MOMENTO/MAGNITUD) / cuán superada en número está la party (D=min(N/P,cap)/cap: heavy/zerg⇒2/some/superado-parcial⇒1/poca-densidad⇒0) (canal aggroDensityFind, sub-cap aggroDensityBountyCap; ⊥ aggroMarginFind #125-LEAD/aggroPileFind #124-max-share/aggroContestFind #123-AMPLITUD/targetSpreadFind #121-UNIFORMIDAD — AVALANCHA es la MAGNITUD/load-medio N/P, no la forma del reparto); resalta si el héroe está en medio de un swarm que SUPERA EN NÚMERO a la party, donde rematar rinde fichas de avalancha. Cosmético puro. INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ D=0 ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(AGGRO_MARGIN_SURGE.enabled) renderAggroMarginBadge(); // CAS-2726: Ventaja de Apilamiento — badge de DOMINANCIA-GAP/LEAD del targeting / cuánto DESPUNTA el jugador más pilado sobre el segundo (M=(top−second)/N: heavy/runaway⇒2/some/lead-parcial⇒1/empate⇒0) (canal aggroMarginFind, sub-cap aggroMarginBountyCap; ⊥ aggroPileFind #124-max-share/PICO-absoluto/aggroContestFind #123-AMPLITUD/targetSpreadFind #121-UNIFORMIDAD/aggroSwitchFind #122-churn-TEMPORAL/aggroFocusFind #118-nivel-sobre-el-HÉROE — VENTAJA es la SEPARACIÓN top−second/el runaway del líder); resalta si el héroe está en medio de una manada con UN jugador acaparando el aggro con margen (no un pico empatado), donde rematar rinde fichas de ventaja. Cosmético puro. INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ M=0 ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(AGGRO_PILE_SURGE.enabled) renderAggroPileBadge(); // CAS-2720: Apilamiento de Objetivos — badge de DOMINANCIA/PICO del targeting / qué FRACCIÓN de la manada ENGANCHADA VIVA está PILADA sobre UN solo jugador (P=maxBucket/N: heavy/pila-pesada⇒2/some/pila-parcial⇒1/repartida⇒0) (canal aggroPileFind, sub-cap aggroPileBountyCap; ⊥ aggroContestFind #123-AMPLITUD/cuántos-jugadores-distintos/targetSpreadFind #121-UNIFORMIDAD-de-toda-la-distribución/aggroSwitchFind #122-churn-TEMPORAL/aggroFocusFind #118-nivel-sobre-el-HÉROE/strikeCommitFind #120-sub-estado — APILAMIENTO es la PROFUNDIDAD/max-share sobre el jugador PICO); resalta si el héroe está en medio de una manada CONVERGIENDO sobre 1 solo jugador (no una repartida parejo), donde rematar rinde fichas de apilamiento. Cosmético puro. INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ P=0 ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(AGGRO_CONTEST_SURGE.enabled) renderAggroContestBadge(); // CAS-2716: Cobertura de Objetivos — badge de COBERTURA/AMPLITUD del targeting / CUÁNTOS jugadores DISTINTOS cubre la manada ENGANCHADA VIVA (C=cubiertos/jugadores-en-radio: broad/cobertura-amplia⇒2/partial/cobertura-parcial⇒1/estrecha⇒0) (canal aggroContestFind, sub-cap aggroContestBountyCap; ⊥ targetSpreadFind #121-UNIFORMIDAD-dentro-del-conjunto/aggroSwitchFind #122-churn-TEMPORAL/aggroFocusFind #118-nivel-sobre-el-HÉROE/strikeCommitFind #120-sub-estado — COBERTURA es la AMPLITUD del conjunto targeteado AHORA); resalta si el héroe está en medio de una manada que acosa a MUCHOS jugadores a la vez (no una fijada en 1 solo), donde rematar rinde fichas de cobertura. Cosmético puro. INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ C=0 ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(AGGRO_SWITCH_SURGE.enabled) renderAggroSwitchBadge(); // CAS-2712: Vaivén de Objetivos — badge de CHURN TEMPORAL del targeting / VOLATILIDAD de la distribución de objetivos de la manada ENGANCHADA VIVA (V=cambiados/enganchados-con-historia: volatile/vaivén-amplio⇒2/shifting/vaivén-parcial⇒1/estable⇒0) (canal aggroSwitchFind, sub-cap aggroSwitchBountyCap; ⊥ targetSpreadFind #121-forma-de-distribución-AHORA/aggroFocusFind #118-nivel-sobre-el-HÉROE/strikeCommitFind #120-sub-estado — VAIVÉN es la VOLATILIDAD del targeting en el TIEMPO); resalta si el héroe está en medio de una manada que RE-APUNTA frenéticamente su aggro (no una con targeting fijo), donde rematar rinde fichas de vaivén. Cosmético puro. INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ V=0 ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(TARGET_SPREAD_SURGE.enabled) renderTargetSpreadBadge(); // CAS-2706: Dispersión de Objetivos — badge de ENTROPÍA de la DISTRIBUCIÓN de objetivos / DISTRIBUCIÓN-DE-OBJETIVOS de la manada ENGANCHADA VIVA (S=H(targets)/ln(nPlayers): scattered/reparto-amplio⇒2/divided/reparto-parcial⇒1/concentrado⇒0) (canal targetSpreadFind, sub-cap targetSpreadBountyCap; ⊥ aggroFocusFind #118-fracción-sobre-el-HÉROE/strikeCommitFind #120-sub-estado — DISPERSIÓN es la UNIFORMIDAD del reparto de aggro sobre la party); resalta si el héroe está en medio de una manada que REPARTE su aggro sobre toda la party (no una que fija todo en 1 blanco), donde rematar rinde fichas de dispersión. Cosmético puro. INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ S=0 ⇒ nunca resalta). DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(STRIKE_COMMIT_SURGE.enabled) renderStrikeCommitBadge(); // CAS-2702: Embate — badge de FRACCIÓN de COMPROMISO / SUB-PARTICIÓN de la manada ENGANCHADA por sub-estado de combate (SC=comprometidos{windup|strike}/enganchados: onslaught/embate-coordinado⇒2/pressing/embate-parcial⇒1/sin-comprometer⇒0) (canal strikeCommitFind, sub-cap strikeCommitBountyCap; ⊥ aggroFocusFind #118-fracción-sobre-VIVOS — EMBATE es la sub-fracción de compromiso-de-golpe sobre el DENOMINADOR ENGANCHADO); resalta si el héroe está bajo una lluvia de golpes comprometidos (no una manada que persigue sin atacar), donde rematar rinde fichas de embate. Cosmético puro. DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
    if(AGGRO_FOCUS_SURGE.enabled) renderAggroFocusBadge(); // CAS-2693: Remate de Acoso — badge de FRACCIÓN de AGGRO-FOCUS / COMPOSICIÓN-DE-INTENCIÓN de la formación de mobs VIVOS alrededor del héroe (F=enganchados/vivos: locked/acoso-coordinado⇒2/harrying/acoso-parcial⇒1/disperso⇒0) (canal aggroFocusFind, sub-cap aggroFocusBountyCap; ⊥ accelFind #117-2º-orden-cinético/orbitFind #116-tangencial/convergeFind #112-radial/headingFind #90-rumbo-de-1-víctima/skirmishFind #84-fracción-ranged-ATRIBUTO — AGGRO-FOCUS es la fracción de aggro-ESTADO, INTENCIÓN no movimiento); resalta si el héroe está en medio de una manada COORDINADA en aggro sobre él (locked-on), donde rematar rinde fichas de acoso. Cosmético puro. DARK (enabled:false) ⇒ NO se dibuja hasta el flip.
  ...
  // CAS-2693: badge de REMATE DE ACOSO (AGGRO_FOCUS_SURGE, EVO#118 — familia COMPOSICIÓN-DE-INTENCIÓN / FRACCIÓN de aggro-focus de la manada VIVA). Refleja el VM PURO (sim.aggroFocusVM, autoridad en sim) ⇒ MISMO idx/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderAggroFocusBadge(){
    const k=sim.aggroFocusVM&&sim.aggroFocusVM(); if(!k) return;          // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+1660, sw=14, sh=14;                            // bajo el Remate-de-Revuelo (@+1638); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0;
    const pulse=here?(0.72+0.24*Math.sin(G.t*(2.2+tier*0.7))):0.55;       // pulso de acoso (fijación coordinada = latido sostenido)
    const cx=bx+sw/2, cy=by+sh/2, rr=sh*0.34;
    // retícula de ACOSO: círculo-diana cuyo arco RELLENO crece con la fracción F (más manada fijada = más cerco de la diana); apagado gris = manada dispersa (diana abierta)
    const col=here?"#7ad0ff":"rgba(150,160,176,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.lineWidth=1.6; ctx.lineCap="round";
    ctx.beginPath(); ctx.arc(cx,cy,rr,0,Math.PI*2); ctx.stroke();          // anillo exterior de la diana
    // arco de fijación = fracción F barrida desde arriba (F=1 ⇒ círculo completo)
    const frac=here?Math.max(0,Math.min(1,idx)):0;
    if(here){ ctx.lineWidth=2.2; ctx.beginPath(); ctx.arc(cx,cy,rr*0.62,-Math.PI/2,-Math.PI/2+Math.PI*2*frac); ctx.stroke(); }
    ctx.beginPath(); ctx.arc(cx,cy,1.1,0,Math.PI*2); ctx.fillStyle=col; ctx.fill();   // punto central (el héroe fijado)
    // micro-label
    ctx.globalAlpha=pulse;
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Fijación: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Fijación" (fracción de aggro-focus, INTENCIÓN/lock-on) — distinta de "Acoso" #94 SWIFT (velocidad-base)/"Revuelo" #117 (churn)/"Órbita" #116 (tangencial)/"Cierre" #112 (radial)
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#d6f0ff":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de fijación por kill + F; si disperso, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2b=here?("+"+charge+" Fijación F"+idx.toFixed(2)):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2b,bx+150,ty);
    ctx.fillStyle=here?"#7ad0ff":"#8a9bb0"; ctx.fillText(st2b,bx+150,ty);
    ctx.restore();
  }

  // CAS-2698: badge de QUIEBRE / REGATE (JERK_DIR_SURGE, EVO#119 — 2º-orden PROYECTADO / FRACCIÓN TANGENCIAL del churn de la manada VIVA). Refleja el VM PURO (sim.jerkDirVM, autoridad en sim) ⇒ MISMO idx/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderJerkDirBadge(){
    const k=sim.jerkDirVM&&sim.jerkDirVM(); if(!k) return;                // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+1682, sw=14, sh=14;                            // bajo el Remate-de-Acoso (@+1660); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0;
    const pulse=here?(0.72+0.24*Math.sin(G.t*(2.4+tier*0.8))):0.55;       // pulso de regate (zigzag = latido nervioso)
    const cx=bx+sw/2, cy=by+sh/2, amp=sh*0.30;
    // retícula de REGATE: una onda ∿ cuya AMPLITUD crece con la fracción tangencial J (más reorientación = más zigzag); apagado gris = churn radial/línea recta (onda plana)
    const col=here?"#b0f0c0":"rgba(150,160,176,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.lineWidth=1.8; ctx.lineCap="round"; ctx.lineJoin="round";
    const aa=here?amp*Math.max(0.18,Math.min(1,idx)):amp*0.10;            // amplitud de la onda = J (línea recta ⇒ casi plana)
    ctx.beginPath();
    const N=16, x0=cx-sw*0.44, x1=cx+sw*0.44;
    for(let i=0;i<=N;i++){ const tt=i/N, xx=x0+(x1-x0)*tt, yy=cy+aa*Math.sin(tt*Math.PI*3+G.t*(here?4:0)); if(i===0) ctx.moveTo(xx,yy); else ctx.lineTo(xx,yy); }
    ctx.stroke();
    // micro-label
    ctx.globalAlpha=pulse;
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Regate: "+(here?("T"+tier):"—");     // etiqueta ÚNICA "Regate" (fracción TANGENCIAL del churn, reorientación) — distinta de "Revuelo" #117 (MAGNITUD del churn)/"Fijación" #118 (intención)/"Órbita" #116 (tangencial-INSTANTÁNEA)/"Cierre" #112 (radial)
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#d6ffe0":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de regate por kill + J; si radial, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2b=here?("+"+charge+" Regate J"+idx.toFixed(2)):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2b,bx+150,ty);
    ctx.fillStyle=here?"#b0f0c0":"#8a9bb0"; ctx.fillText(st2b,bx+150,ty);
    ctx.restore();
  }

  // CAS-2702: badge de EMBATE (STRIKE_COMMIT_SURGE, EVO#120 — COMPOSICIÓN-DE-INTENCIÓN / FRACCIÓN de COMPROMISO de la manada ENGANCHADA VIVA). Refleja el VM PURO (sim.strikeCommitVM, autoridad en sim) ⇒ MISMO idx/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderStrikeCommitBadge(){
    const k=sim.strikeCommitVM&&sim.strikeCommitVM(); if(!k) return;      // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+1704, sw=14, sh=14;                            // bajo el Quiebre/Regate (@+1682); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0;
    const pulse=here?(0.72+0.24*Math.sin(G.t*(2.6+tier*0.9))):0.55;       // pulso de embate (golpe comprometido = latido agudo)
    const cx=bx+sw/2, cy=by+sh/2;
    // retícula de EMBATE: dos chevrones ↯ DESCENDENTES (golpe cayendo) cuya SEPARACIÓN/nitidez crece con la fracción SC (más manada mid-golpe = más impacto); apagado gris = enganchada sin comprometer (chevrones tenues)
    const col=here?"#ffd27a":"rgba(150,160,176,0.36)";
    const frac=here?Math.max(0.18,Math.min(1,idx)):0.10, wSpan=sw*0.40, hSpan=sh*0.30*(0.5+frac);
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.lineWidth=1.9; ctx.lineCap="round"; ctx.lineJoin="round";
    for(let r=0;r<2;r++){ const oy=cy-hSpan*0.5+r*hSpan*0.7*(here?1:0.6);   // dos chevrones apilados, más separados con SC alto
      ctx.beginPath(); ctx.moveTo(cx-wSpan,oy-hSpan*0.5); ctx.lineTo(cx,oy+hSpan*0.5); ctx.lineTo(cx+wSpan,oy-hSpan*0.5); ctx.stroke(); }
    // micro-label
    ctx.globalAlpha=pulse;
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Embate: "+(here?("T"+tier):"—");     // etiqueta ÚNICA "Embate" (fracción de COMPROMISO de golpe, sub-estado {windup|strike} sobre la enganchada) — distinta de "Fijación" #118 (intención/lock-on sobre VIVOS)/"Regate" #119 (churn tangencial)/"Revuelo" #117 (churn magnitud)/"Acometida"/"Embestida" (otras)
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#ffe8c0":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de embate por kill + SC; si sin comprometer, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2b=here?("+"+charge+" Embate SC"+idx.toFixed(2)):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2b,bx+150,ty);
    ctx.fillStyle=here?"#ffd27a":"#8a9bb0"; ctx.fillText(st2b,bx+150,ty);
    ctx.restore();
  }

  // CAS-2706: badge de DISPERSIÓN DE OBJETIVOS (TARGET_SPREAD_SURGE, EVO#121 — COMPOSICIÓN-DE-INTENCIÓN / ENTROPÍA de la DISTRIBUCIÓN de objetivos de la manada ENGANCHADA VIVA). Refleja el VM PURO (sim.targetSpreadVM, autoridad en sim) ⇒ MISMO idx/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ S=0 ⇒ tier0 ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderTargetSpreadBadge(){
    const k=sim.targetSpreadVM&&sim.targetSpreadVM(); if(!k) return;      // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+1726, sw=14, sh=14;                            // bajo el Embate (@+1704); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.2+tier*0.7))):0.55;       // pulso de dispersión (reparto = latido amplio)
    const cx=bx+sw/2, cy=by+sh/2;
    // retícula de DISPERSIÓN: un nodo central del que IRRADIAN flechas hacia fuera (aggro repartiéndose) cuyo alcance crece con la entropía S (más repartido = radios más largos); apagado gris = aggro concentrado (radios cortos)
    const col=here?"#8fd0ff":"rgba(150,160,176,0.36)";
    const frac=here?Math.max(0.20,Math.min(1,idx)):0.12, reach=sw*0.44*(0.5+frac);
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.lineWidth=1.7; ctx.lineCap="round"; ctx.lineJoin="round";
    for(let r=0;r<4;r++){ const ang=(-Math.PI/2)+r*(Math.PI/2)+Math.PI/4;   // 4 radios diagonales que irradian del centro (divergencia)
      const ex=cx+Math.cos(ang)*reach, ey=cy+Math.sin(ang)*reach;
      ctx.beginPath(); ctx.moveTo(cx,cy); ctx.lineTo(ex,ey); ctx.stroke(); }
    ctx.beginPath(); ctx.arc(cx,cy,1.6,0,Math.PI*2); ctx.fillStyle=col; ctx.fill();   // nodo central
    // micro-label
    ctx.globalAlpha=pulse;
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Reparto: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Reparto" (entropía de la DISTRIBUCIÓN de OBJETIVOS de la manada enganchada sobre la party) — distinta de "Dispersión" #107 DISPERSE (spread radial de POSICIONES)/"Embate" #120 (sub-estado)/"Fijación" #118 (intención/lock-on)/"Regate" #119 (churn tangencial)/otras
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#cfeaff":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de reparto por kill + S; si concentrado/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2b=here?("+"+charge+" Reparto S"+idx.toFixed(2)):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2b,bx+150,ty);
    ctx.fillStyle=here?"#8fd0ff":"#8a9bb0"; ctx.fillText(st2b,bx+150,ty);
    ctx.restore();
  }

  // CAS-2712: badge de VAIVÉN DE OBJETIVOS (AGGRO_SWITCH_SURGE, EVO#122 — COMPOSICIÓN-DE-INTENCIÓN / CHURN TEMPORAL del targeting de la manada ENGANCHADA VIVA). Refleja el VM PURO (sim.aggroSwitchVM, autoridad en sim) ⇒ MISMO idx/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ V=0 ⇒ tier0 ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderAggroSwitchBadge(){
    const k=sim.aggroSwitchVM&&sim.aggroSwitchVM(); if(!k) return;         // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+1748, sw=14, sh=14;                            // bajo el Reparto (@+1726); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0;
    const pulse=here?(0.70+0.26*Math.sin(G.t*(2.6+tier*0.9))):0.55;       // pulso de vaivén (churn = latido rápido/nervioso)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de VAIVÉN: dos flechas contrapuestas (⇄) que oscilan lateralmente con el churn V (más volátil = mayor amplitud/vaivén); apagado gris = targeting estable (flechas quietas y cortas)
    const col=here?"#c9a0ff":"rgba(150,160,176,0.36)";
    const frac=here?Math.max(0.20,Math.min(1,idx)):0.12, amp=sw*0.30*(0.4+frac), sway=here?amp*0.5*Math.sin(G.t*(3.2+tier*0.8)):0, reach=sw*0.34;
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.lineWidth=1.7; ctx.lineCap="round"; ctx.lineJoin="round";
    // flecha superior → (apuntando a la derecha, desplazada por sway)
    const y0=cy-sh*0.18, y1=cy+sh*0.18;
    ctx.beginPath(); ctx.moveTo(cx-reach+sway,y0); ctx.lineTo(cx+reach+sway,y0); ctx.moveTo(cx+reach*0.4+sway,y0-2.4); ctx.lineTo(cx+reach+sway,y0); ctx.lineTo(cx+reach*0.4+sway,y0+2.4); ctx.stroke();
    // flecha inferior ← (apuntando a la izquierda, sway opuesto)
    ctx.beginPath(); ctx.moveTo(cx+reach-sway,y1); ctx.lineTo(cx-reach-sway,y1); ctx.moveTo(cx-reach*0.4-sway,y1-2.4); ctx.lineTo(cx-reach-sway,y1); ctx.lineTo(cx-reach*0.4-sway,y1+2.4); ctx.stroke();
    // micro-label
    ctx.globalAlpha=pulse;
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Vaivén: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Vaivén" (churn TEMPORAL del targeting de la manada enganchada) — distinta de "Reparto" #121 (forma de la distribución AHORA)/"Fijación" #118 (nivel/lock-on)/"Embate" #120 (sub-estado)/"Regate" #119 (churn tangencial de MOVIMIENTO)/otras
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#e6d5ff":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de vaivén por kill + V; si estable/single-player/cold-start, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2b=here?("+"+charge+" Vaivén V"+idx.toFixed(2)):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2b,bx+150,ty);
    ctx.fillStyle=here?"#c9a0ff":"#8a9bb0"; ctx.fillText(st2b,bx+150,ty);
    ctx.restore();
  }

  // CAS-2716: badge de COBERTURA DE OBJETIVOS (AGGRO_CONTEST_SURGE, EVO#123 — COMPOSICIÓN-DE-INTENCIÓN / AMPLITUD de la cobertura de objetivos de la manada ENGANCHADA VIVA). Refleja el VM PURO (sim.aggroContestVM, autoridad en sim) ⇒ MISMO idx/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ C=0 ⇒ tier0 ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderAggroContestBadge(){
    const k=sim.aggroContestVM&&sim.aggroContestVM(); if(!k) return;        // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+1770, sw=14, sh=14;                            // bajo el Vaivén (@+1748); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.2+tier*0.7))):0.55;       // pulso de cobertura (amplitud = latido amplio)
    const cx=bx+sw*0.30, cy=by+sh/2;
    // glifo de COBERTURA: un origen (la manada) con N flechas divergentes (⇉) que se ABREN en abanico según C (más cobertura = más flechas/mayor apertura); apagado gris = cobertura estrecha (1 sola flecha corta)
    const col=here?"#a0e0c0":"rgba(150,160,176,0.36)";
    const frac=here?Math.max(0.20,Math.min(1,idx)):0.12, reach=sw*0.62, spread=sh*0.36*(0.4+frac);
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.lineWidth=1.6; ctx.lineCap="round"; ctx.lineJoin="round";
    // 3 flechas divergentes desde el origen (cx,cy) hacia la derecha con apertura vertical creciente (cobertura amplia = abanico ancho)
    const offs=here?[-1,0,1]:[0];
    for(const o of offs){ const ey=cy+o*spread, ex=cx+reach;
      ctx.beginPath(); ctx.moveTo(cx,cy); ctx.lineTo(ex,ey); ctx.moveTo(ex-3.0,ey-2.2); ctx.lineTo(ex,ey); ctx.lineTo(ex-3.0,ey+2.2); ctx.stroke(); }
    // micro-label
    ctx.globalAlpha=pulse;
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Cobertura: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Cobertura" (AMPLITUD del conjunto de jugadores targeteado) — distinta de "Vaivén" #122 (churn TEMPORAL)/"Reparto" #121 (UNIFORMIDAD dentro del conjunto)/"Fijación" #118 (nivel sobre el héroe)/"Embate" #120 (sub-estado)/otras
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#d5f5e6":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de cobertura por kill + C; si estrecha/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2b=here?("+"+charge+" Cobertura C"+idx.toFixed(2)):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2b,bx+150,ty);
    ctx.fillStyle=here?"#a0e0c0":"#8a9bb0"; ctx.fillText(st2b,bx+150,ty);
    ctx.restore();
  }

  // CAS-2720: badge de APILAMIENTO DE OBJETIVOS (AGGRO_PILE_SURGE, EVO#124 — COMPOSICIÓN-DE-INTENCIÓN / DOMINANCIA/PICO del apilamiento de objetivos de la manada ENGANCHADA VIVA). Refleja el VM PURO (sim.aggroPileVM, autoridad en sim) ⇒ MISMO idx/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ P=0 ⇒ tier0 ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderAggroPileBadge(){
    const k=sim.aggroPileVM&&sim.aggroPileVM(); if(!k) return;             // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+1792, sw=14, sh=14;                            // bajo la Cobertura (@+1770); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.2+tier*0.7))):0.55;       // pulso de apilamiento (pila = latido convergente)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de APILAMIENTO (⧉): N flechas convergentes que CIERRAN hacia un punto único (la pila) según P (más pile = más flechas apuntando al mismo destino); apagado gris = repartido (1 sola flecha corta)
    const col=here?"#e0c0a0":"rgba(150,160,176,0.36)";
    const frac=here?Math.max(0.20,Math.min(1,idx)):0.12, reach=sw*0.42, spread=sh*0.38*(1.1-0.7*frac);   // más pile ⇒ orígenes MÁS juntos verticalmente (convergencia apretada)
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.lineWidth=1.6; ctx.lineCap="round"; ctx.lineJoin="round";
    // 3 flechas convergentes desde la izquierda que CIERRAN sobre el punto (cx+reach,cy) — apilamiento pesado = abanico que colapsa a un foco
    const offs=here?[-1,0,1]:[0];
    const fx=cx+reach, fy=cy;
    for(const o of offs){ const sy2=cy+o*spread, sx2=cx-reach*0.5;
      ctx.beginPath(); ctx.moveTo(sx2,sy2); ctx.lineTo(fx,fy); ctx.stroke(); }
    ctx.beginPath(); ctx.arc(fx,fy,here?2.0:1.4,0,Math.PI*2); ctx.fillStyle=col; ctx.fill();   // el foco/pila
    // micro-label
    ctx.globalAlpha=pulse;
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Apilamiento: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Apilamiento" (DOMINANCIA/max-share sobre el jugador PICO) — distinta de "Cobertura" #123 (AMPLITUD del conjunto)/"Vaivén" #122 (churn TEMPORAL)/"Reparto" #121 (UNIFORMIDAD de la distribución)/"Fijación" #118 (nivel sobre el héroe)/"Embate" #120 (sub-estado)/otras
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#f5e6d5":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de apilamiento por kill + P; si repartida/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2b=here?("+"+charge+" Apilamiento P"+idx.toFixed(2)):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2b,bx+150,ty);
    ctx.fillStyle=here?"#e0c0a0":"#8a9bb0"; ctx.fillText(st2b,bx+150,ty);
    ctx.restore();
  }

  // CAS-2726: badge de VENTAJA DE APILAMIENTO (AGGRO_MARGIN_SURGE, EVO#125 — COMPOSICIÓN-DE-INTENCIÓN / DOMINANCIA-GAP/LEAD del apilamiento de objetivos de la manada ENGANCHADA VIVA). Refleja el VM PURO (sim.aggroMarginVM, autoridad en sim) ⇒ MISMO idx/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ M=0 ⇒ tier0 ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderAggroMarginBadge(){
    const k=sim.aggroMarginVM&&sim.aggroMarginVM(); if(!k) return;         // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+1814, sw=14, sh=14;                            // bajo el Apilamiento (@+1792); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.2+tier*0.7))):0.55;       // pulso de ventaja (el líder que despunta)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de VENTAJA (⪢): dos barras — una LARGA (el líder) muy por encima de una CORTA (el segundo) según M (más margen = mayor brecha entre barras); apagado gris = empate (barras iguales)
    const col=here?"#a0d0e0":"rgba(150,160,176,0.36)";
    const frac=here?Math.max(0.15,Math.min(1,idx)):0.0, base=sh*0.30, lead=sh*0.62*frac;   // más margin ⇒ la barra del líder DESPUNTA más sobre la del segundo
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.lineWidth=1.6; ctx.lineCap="round";
    const gx=cx-sw*0.18, sx2=cx+sw*0.18, bot=cy+sh*0.34;
    // barra LÍDER (izquierda, alta) y barra SEGUNDO (derecha, baja): la brecha vertical = el margen M
    ctx.beginPath(); ctx.moveTo(gx,bot); ctx.lineTo(gx,bot-(base+lead)); ctx.stroke();     // líder = base + lead(M)
    ctx.beginPath(); ctx.moveTo(sx2,bot); ctx.lineTo(sx2,bot-base); ctx.stroke();          // segundo = base
    ctx.beginPath(); ctx.arc(gx,bot-(base+lead),here?1.8:1.3,0,Math.PI*2); ctx.fillStyle=col; ctx.fill();  // cabeza del líder
    // micro-label
    ctx.globalAlpha=pulse;
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Ventaja: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Ventaja" (DOMINANCIA-GAP/lead top−second) — distinta de "Apilamiento" #124 (max-share/PICO absoluto)/"Cobertura" #123 (AMPLITUD)/"Vaivén" #122 (churn TEMPORAL)/"Reparto" #121 (UNIFORMIDAD)/"Fijación" #118 (nivel sobre el héroe)/"Embate" #120 (sub-estado)/otras
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#d5eef5":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de ventaja por kill + M; si empate/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2b=here?("+"+charge+" Ventaja M"+idx.toFixed(2)):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2b,bx+150,ty);
    ctx.fillStyle=here?"#a0d0e0":"#8a9bb0"; ctx.fillText(st2b,bx+150,ty);
    ctx.restore();
  }

  // CAS-2730: badge de AVALANCHA (AGGRO_DENSITY_SURGE, EVO#126 — COMPOSICIÓN-DE-INTENCIÓN / OUTNUMBER/SWARM-PRESSURE / cuán superada en número está la party = load medio N/P del swarm ENGANCHADO VIVO). Refleja el VM PURO (sim.aggroDensityVM, autoridad en sim) ⇒ MISMO idx/load/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ D=0 ⇒ tier0 ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderAggroDensityBadge(){
    const k=sim.aggroDensityVM&&sim.aggroDensityVM(); if(!k) return;        // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+1836, sw=14, sh=14;                            // bajo la Ventaja (@+1814); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, load=+k.load||0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.2+tier*0.7))):0.55;       // pulso de avalancha (el swarm cerrándose)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de AVALANCHA (▧): una rejilla/enjambre de puntos cuya DENSIDAD crece con D (más puntos encendidos = party más superada en número); apagado gris = poca densidad
    const col=here?"#c0e0b0":"rgba(150,160,176,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.fillStyle=col;
    // rejilla 3×3 de puntos: cuántos se encienden ∝ D (idx normalizado); apagados = puntos base tenues
    const gx0=cx-sw*0.30, gy0=cy-sh*0.30, step=sw*0.30, lit=Math.round(Math.max(here?1:0,Math.min(9,idx*9)));
    let di=0; for(let r=0;r<3;r++){ for(let c=0;c<3;c++){ const on=di<lit; ctx.globalAlpha=on?pulse:pulse*0.28; ctx.beginPath(); ctx.arc(gx0+c*step,gy0+r*step,on?1.5:1.0,0,Math.PI*2); ctx.fill(); di++; } }
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Avalancha: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Avalancha" (OUTNUMBER/load-medio N/P) — distinta de "Ventaja" #125 (LEAD top−second)/"Apilamiento" #124 (max-share/PICO)/"Cobertura" #123 (AMPLITUD)/"Reparto" #121 (UNIFORMIDAD)/otras
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#d5f0c8":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de avalancha por kill + load N/P; si poca densidad/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2b=here?("+"+charge+" Aval N/P"+load.toFixed(1)):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2b,bx+150,ty);
    ctx.fillStyle=here?"#c0e0b0":"#8a9bb0"; ctx.fillText(st2b,bx+150,ty);
    ctx.restore();
  }

  // CAS-2736: badge de MEZCLA (AGGRO_VARIETY_SURGE, EVO#127 — COMPOSICIÓN-DE-INTENCIÓN / THREAT-HETEROGENEITY / MIXED-PULL / cuán heterogéneo es el pack ENGANCHADO VIVO = riqueza de arquetipos K). Refleja el VM PURO (sim.aggroVarietyVM, autoridad en sim) ⇒ MISMO idx/types/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ V=0 ⇒ tier0 ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderAggroVarietyBadge(){
    const k=sim.aggroVarietyVM&&sim.aggroVarietyVM(); if(!k) return;        // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+1858, sw=14, sh=14;                            // bajo la Avalancha (@+1836); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, types=k.types|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.2+tier*0.7))):0.55;       // pulso de mezcla (el pull heterogéneo cerrándose)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de MEZCLA (⁘): un cúmulo de puntos de "colores"/tamaños distintos cuya DIVERSIDAD crece con V (más puntos variados = pull más heterogéneo); apagado gris = monotipo
    const col=here?"#d0c0e8":"rgba(150,160,176,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.fillStyle=col;
    // rejilla 3×3 de puntos: cuántos se encienden ∝ V (idx normalizado); los encendidos alternan radio (heterogeneidad visual); apagados = puntos base tenues
    const gx0=cx-sw*0.30, gy0=cy-sh*0.30, step=sw*0.30, lit=Math.round(Math.max(here?1:0,Math.min(9,idx*9)));
    let di=0; for(let r=0;r<3;r++){ for(let c=0;c<3;c++){ const on=di<lit; ctx.globalAlpha=on?pulse:pulse*0.28; ctx.beginPath(); ctx.arc(gx0+c*step,gy0+r*step,on?(1.2+0.5*(di%2)):1.0,0,Math.PI*2); ctx.fill(); di++; } }
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Mezcla: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Mezcla" (HETEROGENEIDAD/tipos K) — distinta de "Avalancha" #126 (MAGNITUD/load-medio)/"Ventaja" #125 (LEAD)/"Apilamiento" #124 (max-share)/"Cobertura" #123 (AMPLITUD)/"Reparto" #121 (UNIFORMIDAD)/otras
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#e0d0f4":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de mezcla por kill + K arquetipos; si monotipo/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2b=here?("+"+charge+" Mez K"+types):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2b,bx+150,ty);
    ctx.fillStyle=here?"#d0c0e8":"#8a9bb0"; ctx.fillText(st2b,bx+150,ty);
    ctx.restore();
  }

  // CAS-2743: badge de ESCALADA (AGGRO_MOMENTUM_SURGE, EVO#128 — COMPOSICIÓN-DE-INTENCIÓN / ESCALATION / BUILD-UP / cuánto CRECE el pack ENGANCHADO sobre Δ = rawM=N_t−N_{t−Δ}). Refleja el VM PURO (sim.aggroMomentumVM, autoridad en sim) ⇒ MISMO idx/raw/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ M=0 ⇒ tier0 ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderAggroMomentumBadge(){
    const k=sim.aggroMomentumVM&&sim.aggroMomentumVM(); if(!k) return;        // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+1880, sw=14, sh=14;                            // bajo la Mezcla (@+1858); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, raw=k.raw|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.2+tier*0.7))):0.55;       // pulso de escalada (el pull creciendo/adds entrando)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de ESCALADA (⇗): una escalera/flecha ascendente cuya PENDIENTE crece con M (más empinada = build-up más fuerte); apagado gris = estable/menguante
    const col=here?"#a0e0c8":"rgba(150,160,176,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.fillStyle=col; ctx.lineWidth=here?1.8:1.2; ctx.lineCap="round"; ctx.lineJoin="round";
    // escalera de 3 peldaños ascendentes (izq-abajo → der-arriba); la altura del último peldaño ∝ M (idx)
    const x0=cx-sw*0.34, y0=cy+sh*0.30, rise=sh*0.52*(here?(0.45+0.55*Math.min(1,idx)):0.35), run=sw*0.30;
    ctx.beginPath(); ctx.moveTo(x0,y0); ctx.lineTo(x0+run,y0); ctx.lineTo(x0+run,y0-rise*0.5); ctx.lineTo(x0+2*run,y0-rise*0.5); ctx.lineTo(x0+2*run,y0-rise); ctx.stroke();
    // punta de flecha ascendente en el vértice superior-derecho
    const ax=x0+2*run, ay=y0-rise; ctx.beginPath(); ctx.moveTo(ax,ay); ctx.lineTo(ax-3.0,ay+1.2); ctx.moveTo(ax,ay); ctx.lineTo(ax-1.2,ay+3.0); ctx.stroke();
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Escalada: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Escalada" (BUILD-UP/tendencia dN/dt) — distinta de "Mezcla" #127 (tipos K)/"Avalancha" #126 (MAGNITUD/load-medio N)/"Vaivén" #122 (churn de identidad)/otras
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#c0f0dc":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de escalada por kill + ΔN crudo; si estable/menguante/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Esc ΔN"+raw):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#a0e0c8":"#8a9bb0"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2749: badge de CERCO (AGGRO_SURROUND_SURGE, EVO#129 — COMPOSICIÓN-DE-INTENCIÓN / SPATIAL-ENCIRCLEMENT / cuán rodeado 360° está el héroe = S=1−R, dispersión angular de rumbos hero→mob). Refleja el VM PURO (sim.aggroSurroundVM, autoridad en sim) ⇒ MISMO idx/engaged/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ S=0 ⇒ tier0 ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderAggroSurroundBadge(){
    const k=sim.aggroSurroundVM&&sim.aggroSurroundVM(); if(!k) return;        // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+1902, sw=14, sh=14;                            // bajo la Escalada (@+1880); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, engaged=k.engaged|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.2+tier*0.7))):0.55;       // pulso de cerco (rodeado, sin lado seguro)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de CERCO (⊙): un anillo con puntos cardinales ENCENDIDOS ∝ S (más rodeado = más puntos alrededor); apagado gris = un-solo-lado/flankable
    const col=here?"#b0d8e8":"rgba(150,160,176,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.fillStyle=col; ctx.lineWidth=here?1.6:1.1; ctx.lineCap="round"; ctx.lineJoin="round";
    // anillo central
    const rad=sh*0.34; ctx.beginPath(); ctx.arc(cx,cy,rad,0,2*Math.PI); ctx.stroke();
    // punto central (el héroe cercado)
    ctx.beginPath(); ctx.arc(cx,cy,1.1,0,2*Math.PI); ctx.fill();
    // puntos alrededor del anillo, encendidos ∝ S (idx): 8 posiciones, el nº encendido crece con el cerco
    const lit=here?Math.max(2,Math.round(2+6*Math.min(1,idx))):1;
    for(let i=0;i<8;i++){ const on=i<lit; const ang=2*Math.PI*i/8, px=cx+Math.cos(ang)*(rad+2.4), py=cy+Math.sin(ang)*(rad+2.4);
      ctx.globalAlpha=on?pulse:pulse*0.28; ctx.beginPath(); ctx.arc(px,py,on?1.35:0.8,0,2*Math.PI); ctx.fill(); }
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Rodeo: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Rodeo" (SPATIAL-ENCIRCLEMENT/dispersión angular de RUMBOS de los ENGANCHADOS + party-gate) — distinta de "Cerco" #113 ENCIRCLE (cobertura angular/mayor-hueco de mobs VIVOS)/"Escalada" #128 (dN/dt)/"Mezcla" #127 (tipos K)/"Avalancha" #126 (MAGNITUD N)/otras
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#d0ecf6":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de rodeo por kill + nº enganchados; si un-solo-lado/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Rod N"+engaged):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#b0d8e8":"#8a9bb0"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2753: badge de ACOSO (AGGRO_PRESSURE_SURGE, EVO#130 — COMPOSICIÓN-DE-INTENCIÓN / RADIAL-PROXIMITY / cuán a quemarropa está el pack ENGANCHADO = P=cercanía radial media). Refleja el VM PURO (sim.aggroPressureVM, autoridad en sim) ⇒ MISMO idx/engaged/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ P=0 ⇒ tier0 ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderAggroPressureBadge(){
    const k=sim.aggroPressureVM&&sim.aggroPressureVM(); if(!k) return;        // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+1924, sw=14, sh=14;                            // bajo el Rodeo (@+1902); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, engaged=k.engaged|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.4+tier*0.7))):0.55;       // pulso de acoso (apretado, sin aire)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de ACOSO (⊗): un anillo con puntas apuntando HACIA DENTRO ∝ P (más apretado = puntas más cerca del centro); apagado gris = con-sitio/kiteable
    const col=here?"#e8b0c8":"rgba(150,160,176,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.fillStyle=col; ctx.lineWidth=here?1.6:1.1; ctx.lineCap="round"; ctx.lineJoin="round";
    // anillo exterior (el radio de engage)
    const rad=sh*0.40; ctx.beginPath(); ctx.arc(cx,cy,rad,0,2*Math.PI); ctx.stroke();
    // punto central (el héroe acosado)
    ctx.beginPath(); ctx.arc(cx,cy,1.1,0,2*Math.PI); ctx.fill();
    // 6 puntas apuntando hacia dentro; su punta INTERIOR se acerca al centro ∝ P (idx): a quemarropa = casi tocando al héroe
    const press=here?Math.min(1,idx):0;
    for(let i=0;i<6;i++){ const ang=2*Math.PI*i/6, ox=Math.cos(ang), oy=Math.sin(ang);
      const outer=rad+1.0, inner=rad-1.2-(rad-2.2)*press;               // más presión ⇒ punta interior más adentro
      ctx.globalAlpha=pulse; ctx.beginPath(); ctx.moveTo(cx+ox*outer,cy+oy*outer); ctx.lineTo(cx+ox*inner,cy+oy*inner); ctx.stroke();
      ctx.beginPath(); ctx.arc(cx+ox*inner,cy+oy*inner,here?1.15:0.7,0,2*Math.PI); ctx.fill(); }
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Apremio: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Apremio" (RADIAL-PROXIMITY/cercanía radial media de los ENGANCHADOS + party-gate) — 🔑 NO "Acoso" (COLISIONA #94 SWIFT LIVE @5019 + #118 FOCUS 'Remate de Acoso') — distinta de "Rodeo" #129 (ANGULAR/dispersión de rumbos)/"Fijación" #118 (fracción aggro)/"Escalada" #128 (dN/dt)/"Avalancha" #126 (MAGNITUD N)/otras
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#f0d0e0":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de acoso por kill + nº enganchados; si con-sitio/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Apr N"+engaged):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#e8b0c8":"#8a9bb0"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2757: badge de BRÍO (AGGRO_VIGOR_SURGE, EVO#131 — COMPOSICIÓN-DE-INTENCIÓN / MOB-HP-STATE / cuán fresco/full-HP está el pack ENGANCHADO = V=fracción-de-HP media). Refleja el VM PURO (sim.aggroVigorVM, autoridad en sim) ⇒ MISMO idx/engaged/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). INTRÍNSECAMENTE MULTIJUGADOR (single-player ⇒ V=0 ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderAggroVigorBadge(){
    const k=sim.aggroVigorVM&&sim.aggroVigorVM(); if(!k) return;        // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+1946, sw=14, sh=14;                            // bajo el Apremio (@+1924); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, engaged=k.engaged|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.4+tier*0.7))):0.55;       // pulso de brío (pack fresco, pelea entera)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de BRÍO (◉): un anillo con un disco interior que se LLENA ∝ V (pack fresco = disco casi lleno; maltrecho = casi vacío); apagado gris = maltrecho/kiteable
    const col=here?"#8ee0b4":"rgba(150,160,176,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.fillStyle=col; ctx.lineWidth=here?1.6:1.1; ctx.lineCap="round"; ctx.lineJoin="round";
    // anillo exterior (la "vida completa" del pack)
    const rad=sh*0.40; ctx.beginPath(); ctx.arc(cx,cy,rad,0,2*Math.PI); ctx.stroke();
    // disco interior que crece ∝ V (idx): fresco = casi lleno; maltrecho = punto
    const vig=here?Math.min(1,idx):0;
    const inner=Math.max(0.6,rad*0.86*vig);
    ctx.globalAlpha=pulse*0.9; ctx.beginPath(); ctx.arc(cx,cy,inner,0,2*Math.PI); ctx.fill();
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Brío: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Brío" (MOB-HP-STATE/fracción-de-HP media de los ENGANCHADOS + party-gate) — 🔑 sin colisión con las 72 labels #59-#130 (verificado); distinta de "Apremio" #130 (RADIAL)/"Rodeo" #129 (ANGULAR)/"Avalancha" #126 (MAGNITUD N)/"Mezcla" #127 (tipos)/"Aguante" (LAST_STAND, HP del HÉROE no del mob)
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#c8f0da":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de brío por kill + nº enganchados; si maltrecho/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Brí N"+engaged):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#8ee0b4":"#8a9bb0"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2763: badge de TEMPLE (PARTY_VITAL_SURGE, EVO#132 — COMPOSICIÓN-DE-INTENCIÓN / ALLY-HP-STATE / cuán sana/full-HP está la PARTY ALIADA = W=fracción-de-HP media de los jugadores). Refleja el VM PURO (sim.partyVitalVM, autoridad en sim) ⇒ MISMO idx/players/engaged/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). MULTIJUGADOR-NATIVO (single-player ⇒ W=0 ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderPartyVitalBadge(){
    const k=sim.partyVitalVM&&sim.partyVitalVM(); if(!k) return;        // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+1968, sw=14, sh=14;                            // bajo el Brío (@+1946); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, players=k.players|0, engaged=k.engaged|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.4+tier*0.7))):0.55;       // pulso de temple (party sana, puede comprometerse)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de TEMPLE (✚): una cruz de vida cuyo brazo interior CRECE ∝ W (party sana = cruz llena; maltrecha = punto); apagado gris = maltrecha/single-player
    const col=here?"#8ee0d8":"rgba(150,160,176,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.fillStyle=col; ctx.lineWidth=here?1.6:1.1; ctx.lineCap="round"; ctx.lineJoin="round";
    // cruz exterior (la "vida completa" de la party)
    const rad=sh*0.40; ctx.beginPath(); ctx.moveTo(cx,cy-rad); ctx.lineTo(cx,cy+rad); ctx.moveTo(cx-rad,cy); ctx.lineTo(cx+rad,cy); ctx.stroke();
    // núcleo interior que crece ∝ W (idx): sana = casi lleno; maltrecha = punto
    const vit=here?Math.min(1,idx):0;
    const inner=Math.max(0.6,rad*0.80*vit);
    ctx.globalAlpha=pulse*0.9; ctx.beginPath(); ctx.arc(cx,cy,inner,0,2*Math.PI); ctx.fill();
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Temple: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Temple" (ALLY-HP-STATE/fracción-de-HP media de la PARTY + gate de pack) — 🔑 sin colisión con las 73 labels #59-#131 (verificado); distinta de "Brío" #131 (HP del MOB, no del jugador)/"Aguante" (LAST_STAND, HP del HÉROE solo)
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#c8f0e6":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de temple por kill + nº jugadores; si maltrecha/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Tmp P"+players):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#8ee0d8":"#8a9bb0"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2767: badge de RETO (MOB_TIER_SURGE, EVO#133 — COMPOSICIÓN-DE-INTENCIÓN / MOB-POWER-RATING / cuán de alto tier es el pack ENGANCHADO = T=rango-de-poder medio de los mobs). Refleja el VM PURO (sim.mobTierVM, autoridad en sim) ⇒ MISMO idx/players/engaged/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). MULTIJUGADOR-NATIVO (single-player ⇒ T=0 ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderMobTierBadge(){
    const k=sim.mobTierVM&&sim.mobTierVM(); if(!k) return;                // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+1990, sw=14, sh=14;                            // bajo el Temple (@+1968); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, players=k.players|0, engaged=k.engaged|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.4+tier*0.7))):0.55;       // pulso de reto (pack de alto tier, reto real)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de RETO (⧫): un rombo/gema de poder cuyo núcleo CRECE ∝ T (pack élite = rombo lleno; basura = punto); apagado gris = basura/single-player
    const col=here?"#c89af0":"rgba(150,160,176,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.fillStyle=col; ctx.lineWidth=here?1.6:1.1; ctx.lineCap="round"; ctx.lineJoin="round";
    // rombo exterior (el "tope de reto" del pack)
    const rad=sh*0.42; ctx.beginPath(); ctx.moveTo(cx,cy-rad); ctx.lineTo(cx+rad,cy); ctx.lineTo(cx,cy+rad); ctx.lineTo(cx-rad,cy); ctx.closePath(); ctx.stroke();
    // núcleo interior (rombo) que crece ∝ T (idx): élite = casi lleno; basura = punto
    const pw=here?Math.min(1,idx):0;
    const inr=Math.max(0.6,rad*0.82*pw);
    ctx.globalAlpha=pulse*0.9; ctx.beginPath(); ctx.moveTo(cx,cy-inr); ctx.lineTo(cx+inr,cy); ctx.lineTo(cx,cy+inr); ctx.lineTo(cx-inr,cy); ctx.closePath(); ctx.fill();
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Reto: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Reto" (MOB-POWER-RATING/rango-de-poder medio del pack) — 🔑 sin colisión con las 74 labels #59-#132 (verificado); distinta de "Nivel"/"Rango" (ya usadas) y de "Brío" #131/"Temple" #132 (HP, no tier)
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#e0c8f4":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de reto por kill + nº jugadores; si basura/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Reto P"+players):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#c89af0":"#8a9bb0"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2775: badge de CASTA (ELITE_SHARE_SURGE, EVO#134 — COMPOSICIÓN-DE-INTENCIÓN / MOB-POWER-RATING / qué FRACCIÓN del pack ENGANCHADO es genuinamente peligrosa/élite+ = S=count(mobTierRank≥eliteRank)/N). Refleja el VM PURO (sim.eliteShareVM, autoridad en sim) ⇒ MISMO idx/players/engaged/elite/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). MULTIJUGADOR-NATIVO (single-player ⇒ S=0 ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderEliteShareBadge(){
    const k=sim.eliteShareVM&&sim.eliteShareVM(); if(!k) return;          // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+2012, sw=14, sh=14;                            // bajo el Reto (@+1990); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, players=k.players|0, engaged=k.engaged|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.4+tier*0.7))):0.55;       // pulso de casta (pack mayoría-élite)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de CASTA (✹): un estallido/aster de N radios cuyo brillo/núcleo CRECE ∝ S (mayoría-élite = estrella llena; poca-élite = punto); apagado gris = poca-élite/single-player
    const col=here?"#f0c860":"rgba(150,160,176,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.fillStyle=col; ctx.lineWidth=here?1.6:1.1; ctx.lineCap="round"; ctx.lineJoin="round";
    // estallido de 6 radios (el "cuerpo de casta" del pack)
    const rad=sh*0.44, spokes=6;
    for(let i=0;i<spokes;i++){ const ang=(Math.PI*2*i/spokes)-Math.PI/2; ctx.beginPath(); ctx.moveTo(cx,cy); ctx.lineTo(cx+rad*Math.cos(ang),cy+rad*Math.sin(ang)); ctx.stroke(); }
    // núcleo interior (disco) que crece ∝ S (idx): mayoría-élite = casi lleno; poca-élite = punto
    const pw=here?Math.min(1,idx):0;
    const inr=Math.max(0.6,rad*0.72*pw);
    ctx.globalAlpha=pulse*0.9; ctx.beginPath(); ctx.arc(cx,cy,inr,0,Math.PI*2); ctx.closePath(); ctx.fill();
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Casta: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Casta" (COMPOSICIÓN-PROPORCIÓN/fracción-élite del pack) — 🔑 sin colisión con las 75 labels #59-#133 (verificado); distinta de "Reto" #133 (MEDIA del rango, no la FRACCIÓN)/"Brío" #131/"Temple" #132 (HP, no rango)
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#f5da90":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de casta por kill + nº jugadores; si poca-élite/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Casta P"+players):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#f0c860":"#8a9bb0"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2783: badge de BRECHA (RANK_SPREAD_SURGE, EVO#135 — COMPOSICIÓN-DE-INTENCIÓN / MOB-POWER-RATING / cuán SEPARADOS están los tiers de poder del pack ENGANCHADO = R=maxRank−minRank ∈[0,4] ENTERO). Refleja el VM PURO (sim.rankSpreadVM, autoridad en sim) ⇒ MISMO idx/players/engaged/spread/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). MULTIJUGADOR-NATIVO (single-player ⇒ R=0 ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderRankSpreadBadge(){
    const k=sim.rankSpreadVM&&sim.rankSpreadVM(); if(!k) return;          // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+2034, sw=14, sh=14;                            // bajo la Casta (@+2012); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, players=k.players|0, engaged=k.engaged|0, spread=k.spread|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.4+tier*0.7))):0.55;       // pulso de brecha (pack de amenaza-mixta)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de BRECHA (⟡): dos rombos (min-tier y max-tier del pack) SEPARADOS por un hueco que CRECE ∝ R (idx=R/cap): brecha-amplia = rombos lejos; uniforme = rombos juntos/punto; apagado gris = uniforme/single-player
    const col=here?"#66c8c0":"rgba(150,160,176,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.fillStyle=col; ctx.lineWidth=here?1.6:1.1; ctx.lineCap="round"; ctx.lineJoin="round";
    // half-gap horizontal crece ∝ R (idx): los dos rombos se apartan del centro
    const maxHalf=sh*0.42, gap=here?Math.max(0.8,maxHalf*Math.min(1,idx)):1.0;
    // barra-de-rango tenue que une los dos extremos
    ctx.globalAlpha=pulse*0.55; ctx.beginPath(); ctx.moveTo(cx-gap,cy); ctx.lineTo(cx+gap,cy); ctx.stroke();
    ctx.globalAlpha=pulse;
    // dos rombos ⟡ (min-tier a la izq, max-tier a la der) — tamaño fijo, separación ∝ brecha
    const dr=Math.max(1.4,sh*0.20);
    const diamond=(dx)=>{ const ox=cx+dx; ctx.beginPath(); ctx.moveTo(ox,cy-dr); ctx.lineTo(ox+dr,cy); ctx.lineTo(ox,cy+dr); ctx.lineTo(ox-dr,cy); ctx.closePath(); };
    diamond(-gap); ctx.fill();
    diamond(gap); here?ctx.fill():ctx.stroke();
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Brecha: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Brecha" (COMPOSICIÓN-DISPERSIÓN/rango del pack) — 🔑 sin colisión con las 76 labels #59-#134 (verificado); distinta de "Reto" #133 (MEDIA)/"Casta" #134 (FRACCIÓN)/"Brío" #131/"Temple" #132 (HP)
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#a6ecE4":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de brecha por kill + nº jugadores; si uniforme/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Brecha P"+players):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#66c8c0":"#8a9bb0"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2792: badge de CÓNCLAVE (CO_STRIKE_SURGE, EVO#136 — PLAYER-COORDINATION / cuántos JUGADORES DISTINTOS están ENGANCHADOS en la MISMA refriega = F=muster ∈[0,P] ENTERO). Refleja el VM PURO (sim.coStrikeVM, autoridad en sim) ⇒ MISMO idx/players/engaged/muster/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). MULTIJUGADOR-NATIVO (single-player ⇒ F=0 ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderCoStrikeBadge(){
    const k=sim.coStrikeVM&&sim.coStrikeVM(); if(!k) return;              // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+2056, sw=14, sh=14;                            // bajo la Brecha (@+2034); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, players=k.players|0, engaged=k.engaged|0, muster=k.muster|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.4+tier*0.7))):0.55;       // pulso de cónclave (hueste reunida)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de CÓNCLAVE (⋈): dos triángulos que CONVERGEN hacia el centro conforme crece la muster (idx=F/playerCap): hueste plena = puntas casi tocándose; solitario = separadas; apagado gris = solitario/single-player
    const col=here?"#8a94e8":"rgba(150,160,176,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.fillStyle=col; ctx.lineWidth=here?1.6:1.1; ctx.lineCap="round"; ctx.lineJoin="round";
    // half-gap horizontal DECRECE ∝ muster (idx): las puntas convergen al centro cuanta más hueste
    const maxHalf=sh*0.40, gap=here?Math.max(1.0,maxHalf*(1-0.72*Math.min(1,idx))):maxHalf;
    const th=Math.max(2.2,sh*0.30);   // media-altura del triángulo
    // triángulo izquierdo (punta a la derecha, hacia el centro) + triángulo derecho (punta a la izquierda) = ⋈
    ctx.beginPath(); ctx.moveTo(cx-gap-th,cy-th); ctx.lineTo(cx-gap-th,cy+th); ctx.lineTo(cx-gap,cy); ctx.closePath(); here?ctx.fill():ctx.stroke();
    ctx.beginPath(); ctx.moveTo(cx+gap+th,cy-th); ctx.lineTo(cx+gap+th,cy+th); ctx.lineTo(cx+gap,cy); ctx.closePath(); here?ctx.fill():ctx.stroke();
    // chispa de convergencia en el centro cuando la hueste está plena
    if(here && idx>0){ ctx.globalAlpha=pulse*0.8; ctx.beginPath(); ctx.arc(cx,cy,Math.max(0.8,1.4*Math.min(1,idx)),0,Math.PI*2); ctx.fill(); }
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Cónclave: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Cónclave" (PLAYER-COORDINATION/muster) — 🔑 sin colisión con las 77 labels #59-#135 (verificado); distinta de "Brecha" #135/"Casta" #134/"Reto" #133
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#c0c6f4":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de cónclave por kill + nº jugadores enganchados (muster); si solitario/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Cónclave F"+muster):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#8a94e8":"#8a9bb0"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2797: badge de COHORTE (CO_PRESENCE_SURGE, EVO#137 — PLAYER-COORDINATION #2 / cuántos JUGADORES DISTINTOS están PRESENTES/rallevados en el radio del héroe = R ∈[1,P] ENTERO). Refleja el VM PURO (sim.coPresenceVM, autoridad en sim) ⇒ MISMO idx/players/rally/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). MULTIJUGADOR-NATIVO (single-player ⇒ R=1 ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderCoPresenceBadge(){
    const k=sim.coPresenceVM&&sim.coPresenceVM(); if(!k) return;             // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+2078, sw=14, sh=14;                              // bajo la Cónclave (@+2056); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, players=k.players|0, rally=k.rally|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.2+tier*0.6))):0.55;         // pulso de cohorte (party reunida)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de COHORTE (⬡): un HEXÁGONO/anillo de rally que se ILUMINA/CIERRA conforme crece R (idx=R/playerCap): party plena = hexágono lleno/brillante; solitario = contorno tenue gris
    const col=here?"#7cc8a8":"rgba(150,160,176,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.fillStyle=col; ctx.lineWidth=here?1.6:1.1; ctx.lineJoin="round";
    const rad=sh*0.40;
    ctx.beginPath();
    for(let i=0;i<6;i++){ const ang=(-90+i*60)*Math.PI/180, vx=cx+rad*Math.cos(ang), vy=cy+rad*Math.sin(ang); if(i===0) ctx.moveTo(vx,vy); else ctx.lineTo(vx,vy); }
    ctx.closePath(); ctx.stroke();
    // núcleo que crece ∝ R (idx): party plena = núcleo casi tocando los vértices; solitario/single-player = vacío
    if(here && idx>0){ ctx.globalAlpha=pulse*0.85; ctx.beginPath(); ctx.arc(cx,cy,Math.max(0.8,rad*0.7*Math.min(1,idx)),0,Math.PI*2); ctx.fill(); }
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Cohorte: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Cohorte" (PLAYER-COORDINATION #2 / co-presencia) — 🔑 sin colisión con las 78 labels #59-#136 (verificado); distinta de "Cónclave" #136/"Brecha" #135/"Casta" #134
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#a8e8cc":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de cohorte por kill + nº jugadores presentes (rally); si solitario/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Cohorte R"+rally):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#7cc8a8":"#8a9bb0"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2802: badge de CUADRILLA (CO_KILL_SURGE, EVO#138 — PLAYER-COORDINATION #3 / cuántos JUGADORES DISTINTOS están ACREDITADOS con un golpe-de-gracia/remate en el radio compartido = K ∈[1,P] ENTERO). Refleja el VM PURO (sim.coKillVM, autoridad en sim) ⇒ MISMO idx/players/assist/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). MULTIJUGADOR-NATIVO (single-player ⇒ K=1 ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderCoKillBadge(){
    const k=sim.coKillVM&&sim.coKillVM(); if(!k) return;                    // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+2100, sw=14, sh=14;                              // bajo la Cohorte (@+2078); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, players=k.players|0, assist=k.assist|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.2+tier*0.6))):0.55;         // pulso de cuadrilla (remates coordinados)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de CUADRILLA (⨂): un CÍRCULO con una X/aspa de remate que se ILUMINA conforme crece K (idx=K/playerCap): cuadrilla plena = aspa brillante/cerrada; solitario = contorno tenue gris
    const col=here?"#d8b878":"rgba(150,160,176,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.fillStyle=col; ctx.lineWidth=here?1.6:1.1; ctx.lineJoin="round"; ctx.lineCap="round";
    const rad=sh*0.40;
    ctx.beginPath(); ctx.arc(cx,cy,rad,0,Math.PI*2); ctx.stroke();          // anillo del cluster
    // aspa/X de remate que crece ∝ K (idx): cuadrilla plena = aspa casi tocando el anillo; solitario/single-player = mínima
    const xr=Math.max(0.8,rad*0.72*Math.min(1,Math.max(idx,here?0.5:0.28)));
    ctx.globalAlpha=pulse*(here?0.95:0.7);
    ctx.beginPath(); ctx.moveTo(cx-xr,cy-xr); ctx.lineTo(cx+xr,cy+xr); ctx.moveTo(cx+xr,cy-xr); ctx.lineTo(cx-xr,cy+xr); ctx.stroke();
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Cuadrilla: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Cuadrilla" (PLAYER-COORDINATION #3 / co-kill) — 🔑 sin colisión con las 79 labels #59-#137 (verificado); distinta de "Cohorte" #137/"Cónclave" #136/"Falange" #108
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#f0d8a0":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de cuadrilla por kill + nº killers acreditados (assist); si solitario/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Cuadrilla K"+assist):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#d8b878":"#8a9bb0"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2807: badge de SOCORRO (CO_SUPPORT_SURGE, EVO#139 — PLAYER-COORDINATION #4 / cuántos JUGADORES DISTINTOS están APLICANDO un socorro/heal/shield/buff/revive en el radio compartido = S ∈[0,P] ENTERO). Refleja el VM PURO (sim.coSupportVM, autoridad en sim) ⇒ MISMO idx/players/aid/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). MULTIJUGADOR-NATIVO (single-player ⇒ S=0 ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderCoSupportBadge(){
    const k=sim.coSupportVM&&sim.coSupportVM(); if(!k) return;               // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+2122, sw=14, sh=14;                              // bajo la Cuadrilla (@+2100); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, players=k.players|0, aid=k.aid|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.2+tier*0.6))):0.55;         // pulso de socorro (sostén coordinado)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de SOCORRO (⛑): una CRUZ de socorro dentro de un anillo que se ILUMINA conforme crece S (idx=S/playerCap): socorro pleno = cruz brillante/cerrada; solitario = contorno tenue gris
    const col=here?"#88d8b0":"rgba(150,160,176,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.fillStyle=col; ctx.lineWidth=here?1.6:1.1; ctx.lineJoin="round"; ctx.lineCap="round";
    const rad=sh*0.40;
    ctx.beginPath(); ctx.arc(cx,cy,rad,0,Math.PI*2); ctx.stroke();          // anillo del cluster de socorro
    // cruz de socorro (+) que crece ∝ S (idx): socorro pleno = cruz casi tocando el anillo; solitario/single-player = mínima
    const cr=Math.max(0.8,rad*0.72*Math.min(1,Math.max(idx,here?0.5:0.28)));
    ctx.globalAlpha=pulse*(here?0.95:0.7);
    ctx.beginPath(); ctx.moveTo(cx,cy-cr); ctx.lineTo(cx,cy+cr); ctx.moveTo(cx-cr,cy); ctx.lineTo(cx+cr,cy); ctx.stroke();
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Socorro: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Socorro" (PLAYER-COORDINATION #4 / co-support) — 🔑 sin colisión con las 80 labels #59-#138 (verificado); distinta de "Cuadrilla" #138/"Cohorte" #137/"Cónclave" #136
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#a0f0c8":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de socorro por kill + nº socorredores acreditados (aid); si solitario/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Socorro S"+aid):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#88d8b0":"#8a9bb0"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2812: badge de MURALLA (CO_TANK_SURGE, EVO#140 — PLAYER-COORDINATION #5 / cuántos JUGADORES DISTINTOS están ABSORBIENDO/TOMANDO daño de mob en el radio compartido = T ∈[0,P] ENTERO). Refleja el VM PURO (sim.coTankVM, autoridad en sim) ⇒ MISMO idx/players/tank/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). MULTIJUGADOR-NATIVO (single-player ⇒ T colapsa ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderCoTankBadge(){
    const k=sim.coTankVM&&sim.coTankVM(); if(!k) return;               // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+2144, sw=14, sh=14;                          // bajo el Socorro (@+2122); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, players=k.players|0, tank=k.tank|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.2+tier*0.6))):0.55;     // pulso de muralla (carga repartida)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de MURALLA (🛡): un ESCUDO/muro que se ILUMINA conforme crece T (idx=T/playerCap): carga plena = escudo brillante/cerrado; solitario = contorno tenue gris
    const col=here?"#9ab0d8":"rgba(150,160,176,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.fillStyle=col; ctx.lineWidth=here?1.6:1.1; ctx.lineJoin="round"; ctx.lineCap="round";
    const rad=sh*0.42;
    // contorno de ESCUDO (pentágono con punta abajo) — la muralla que reparte los golpes
    ctx.beginPath();
    ctx.moveTo(cx,cy-rad); ctx.lineTo(cx+rad*0.85,cy-rad*0.5); ctx.lineTo(cx+rad*0.62,cy+rad*0.55); ctx.lineTo(cx,cy+rad); ctx.lineTo(cx-rad*0.62,cy+rad*0.55); ctx.lineTo(cx-rad*0.85,cy-rad*0.5); ctx.closePath(); ctx.stroke();
    // barra interior horizontal que crece ∝ T (idx): carga plena = barra casi tocando los bordes; solitario/single-player = mínima
    const cr=Math.max(0.8,rad*0.7*Math.min(1,Math.max(idx,here?0.5:0.28)));
    ctx.globalAlpha=pulse*(here?0.95:0.7);
    ctx.beginPath(); ctx.moveTo(cx-cr,cy-rad*0.05); ctx.lineTo(cx+cr,cy-rad*0.05); ctx.stroke();
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Muralla: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Muralla" (PLAYER-COORDINATION #5 / co-tank) — 🔑 sin colisión con las 81 labels #59-#139 (verificado); distinta de "Socorro" #139/"Cuadrilla" #138/"Cohorte" #137/"Cónclave" #136
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#bcd0f0":"#8a9bb0"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de muralla por kill + nº absorbedores acreditados (tank); si solitario/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Muralla T"+tank):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#9ab0d8":"#8a9bb0"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2815: badge de REPLIEGUE (CO_FLEE_SURGE, EVO#141 — PLAYER-COORDINATION #6 / cuántos JUGADORES DISTINTOS están ROMPIENDO contacto / repositionándose ALEJÁNDOSE de una amenaza en el radio compartido = D ∈[0,P] ENTERO). Refleja el VM PURO (sim.coFleeVM, autoridad en sim) ⇒ MISMO idx/players/flee/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). MULTIJUGADOR-NATIVO (single-player ⇒ D colapsa ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderCoFleeBadge(){
    const k=sim.coFleeVM&&sim.coFleeVM(); if(!k) return;               // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+2166, sw=14, sh=14;                          // bajo la Muralla (@+2144); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, players=k.players|0, flee=k.flee|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.2+tier*0.6))):0.55;     // pulso de repliegue (kite coordinado)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de REPLIEGUE (💨): una FLECHA/estela de retirada que se ILUMINA conforme crece D (idx=D/playerCap): repliegue pleno = estela brillante alejándose; solitario = contorno tenue gris
    const col=here?"#9ad8c8":"rgba(150,170,164,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.fillStyle=col; ctx.lineWidth=here?1.6:1.1; ctx.lineJoin="round"; ctx.lineCap="round";
    const rad=sh*0.42;
    // FLECHA de retirada (punta a la izquierda = alejándose de la amenaza) — el repliegue coordinado
    ctx.beginPath();
    ctx.moveTo(cx-rad,cy); ctx.lineTo(cx+rad*0.5,cy-rad*0.7); ctx.moveTo(cx-rad,cy); ctx.lineTo(cx+rad*0.5,cy+rad*0.7); ctx.moveTo(cx-rad,cy); ctx.lineTo(cx+rad,cy); ctx.stroke();
    // estela horizontal detrás de la punta que crece ∝ D (idx): repliegue pleno = estela larga; solitario/single-player = mínima
    const cr=Math.max(0.8,rad*0.7*Math.min(1,Math.max(idx,here?0.5:0.28)));
    ctx.globalAlpha=pulse*(here?0.9:0.6);
    ctx.beginPath(); ctx.moveTo(cx+rad*0.2,cy-rad*0.05); ctx.lineTo(cx+rad*0.2+cr,cy-rad*0.05); ctx.stroke();
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Repliegue: "+(here?("T"+tier):"—");   // etiqueta ÚNICA "Repliegue" (PLAYER-COORDINATION #6 / co-flee) — 🔑 sin colisión con las 82 labels #59-#140 (verificado); distinta de "Muralla" #140/"Socorro" #139/"Cuadrilla" #138/"Cohorte" #137/"Cónclave" #136
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#bcf0e0":"#8ab0a4"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de repliegue por kill + nº kiters acreditados (flee); si solitario/single-player, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Repliegue D"+flee):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#9ad8c8":"#8ab0a4"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2820: badge de PIÑA (CO_COHESION_SURGE, EVO#142 — PLAYER-COORDINATION #7 / GEOMETRÍA / cuántos JUGADORES DISTINTOS están APIÑADOS hombro-con-hombro dentro del radio de cohesión APRETADO = C ∈[0,P] ENTERO). Refleja el VM PURO (sim.coCohesionVM, autoridad en sim) ⇒ MISMO idx/players/cohesion/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). MULTIJUGADOR-NATIVO (single-player ⇒ C colapsa ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderCoCohesionBadge(){
    const k=sim.coCohesionVM&&sim.coCohesionVM(); if(!k) return;             // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+2188, sw=14, sh=14;                              // bajo el Repliegue (@+2166); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, players=k.players|0, cohesion=k.cohesion|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.2+tier*0.6))):0.55;         // pulso de piña (formación apretada)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de PIÑA (⦿): un NÚCLEO de puntos que se APRIETAN conforme crece C (idx=C/playerCap): piña apretada = anillo denso alrededor de un centro brillante; solitario/dispersa = contorno tenue gris
    const col=here?"#bfe09a":"rgba(160,170,150,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.fillStyle=col; ctx.lineWidth=here?1.6:1.1; ctx.lineJoin="round"; ctx.lineCap="round";
    const rad=sh*0.42;
    // anillo exterior de la formación
    ctx.beginPath(); ctx.arc(cx,cy,rad,0,Math.PI*2); ctx.stroke();
    // núcleo central que se APRIETA/brilla ∝ C (idx): piña plena = núcleo grande; solitario/dispersa = punto mínimo
    const core=Math.max(0.9,rad*0.55*Math.min(1,Math.max(idx,here?0.5:0.28)));
    ctx.globalAlpha=pulse*(here?0.95:0.6);
    ctx.beginPath(); ctx.arc(cx,cy,core,0,Math.PI*2); ctx.fill();
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Piña: "+(here?("T"+tier):"—");            // etiqueta ÚNICA "Piña" (PLAYER-COORDINATION #7 / co-cohesion / GEOMETRÍA) — 🔑 sin colisión con las 83 labels #59-#141 (verificado); distinta de "Repliegue" #141/"Muralla" #140/"Socorro" #139/"Cuadrilla" #138/"Cohorte" #137/"Cónclave" #136/"Falange" #108
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#d4f0b4":"#9ab088"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de piña por kill + nº apiñados acreditados (cohesion); si solitario/dispersa, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Piña C"+cohesion):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#bfe09a":"#9ab088"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2824: badge de ENVITE (CO_ADVANCE_SURGE, EVO#143 — PLAYER-COORDINATION #8 / cuántos JUGADORES DISTINTOS EMPUJAN/avanzan con velocidad-HACIA la amenaza compartida = A ∈[0,P] ENTERO). Refleja el VM PURO (sim.coAdvanceVM, autoridad en sim) ⇒ MISMO idx/players/advance/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). MULTIJUGADOR-NATIVO (single-player ⇒ A colapsa ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderCoAdvanceBadge(){
    const k=sim.coAdvanceVM&&sim.coAdvanceVM(); if(!k) return;               // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+2210, sw=14, sh=14;                              // bajo la Piña (@+2188); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, players=k.players|0, advance=k.advance|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.2+tier*0.6))):0.55;         // pulso de envite (empuje coordinado)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de ENVITE (➤): una PUNTA DE FLECHA que apunta hacia delante (velocidad-HACIA), más brillante/afilada conforme crece A (idx=A/playerCap); solitario/replegándose = contorno tenue gris
    const col=here?"#a8c8f0":"rgba(150,160,175,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.fillStyle=col; ctx.lineWidth=here?1.6:1.1; ctx.lineJoin="round"; ctx.lineCap="round";
    const rad=sh*0.42;
    // punta de flecha (chevron ►) apuntando a la derecha = la dirección del empuje
    const push=Math.max(0.9,rad*Math.min(1,Math.max(idx,here?0.5:0.28)));   // longitud del empuje ∝ A (idx)
    ctx.beginPath(); ctx.moveTo(cx-rad*0.7,cy-rad); ctx.lineTo(cx+push,cy); ctx.lineTo(cx-rad*0.7,cy+rad); ctx.stroke();
    // línea de avance detrás de la punta (la estela del empuje) ∝ A
    ctx.globalAlpha=pulse*(here?0.9:0.55);
    ctx.beginPath(); ctx.moveTo(cx-rad,cy); ctx.lineTo(cx-rad*0.7,cy); ctx.stroke();
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Envite: "+(here?("T"+tier):"—");          // etiqueta ÚNICA "Envite" (PLAYER-COORDINATION #8 / co-advance / push-count) — 🔑 sin colisión con las 84 labels #59-#142 (verificado); distinta de "Piña" #142/"Repliegue" #141/"Muralla" #140/"Socorro" #139/"Cuadrilla" #138/"Cohorte" #137/"Cónclave" #136/"Embate" (otro flag)/"Acometida" (otro flag)
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#c4dcf6":"#8898ac"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de envite por kill + nº pushers acreditados (advance); si solitario/replegándose, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Envite A"+advance):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#a8c8f0":"#8898ac"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2829: badge de DIANA (CO_FOCUS_SURGE, EVO#144 — PLAYER-COORDINATION #9 / CONVERGENCIA-DE-OBJETIVO / cuántos JUGADORES DISTINTOS convergen su ataque/aggro sobre el MISMO ÚNICO mob = G ∈[0,P] ENTERO). Refleja el VM PURO (sim.coFocusVM, autoridad en sim) ⇒ MISMO idx/players/focus/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). MULTIJUGADOR-NATIVO (single-player ⇒ G colapsa ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderCoFocusBadge(){
    const k=sim.coFocusVM&&sim.coFocusVM(); if(!k) return;                  // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+2232, sw=14, sh=14;                              // bajo el Envite (@+2210); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, players=k.players|0, focus=k.focus|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.2+tier*0.6))):0.55;         // pulso de diana (fuego convergente)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de DIANA (⌾): un OJO DE BUEY (anillo + punto central) con ticks convergentes que se APRIETAN conforme crece G (idx=G/playerCap): más convergido = ticks más cerca del centro brillante; solitario/disperso = contorno tenue gris
    const col=here?"#f0c8a8":"rgba(150,160,175,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.fillStyle=col; ctx.lineWidth=here?1.6:1.1; ctx.lineJoin="round"; ctx.lineCap="round";
    const rad=sh*0.42;
    // anillo exterior (la diana)
    ctx.beginPath(); ctx.arc(cx,cy,rad,0,Math.PI*2); ctx.stroke();
    // punto central (el objetivo compartido) — brillo ∝ G
    ctx.globalAlpha=pulse*(here?1:0.6);
    ctx.beginPath(); ctx.arc(cx,cy,Math.max(1.0,rad*0.28),0,Math.PI*2); ctx.fill();
    // 4 ticks convergentes que APUNTAN hacia el centro desde los cardinales; su longitud interior crece con idx (aim convergiendo)
    ctx.globalAlpha=pulse;
    const conv=Math.min(1,Math.max(idx,here?0.5:0.28)), inR=rad*(1.35), outR=rad*(1.35+0.55*conv);   // más convergido ⇒ tick más largo apuntando adentro
    for(let i=0;i<4;i++){ const ang=i*Math.PI/2; const ca=Math.cos(ang), sa=Math.sin(ang);
      ctx.beginPath(); ctx.moveTo(cx+ca*outR,cy+sa*outR); ctx.lineTo(cx+ca*inR,cy+sa*inR); ctx.stroke(); }
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Diana: "+(here?("T"+tier):"—");           // etiqueta ÚNICA "Diana" (PLAYER-COORDINATION #9 / co-focus / shared-target) — 🔑 sin colisión con las 85 labels #59-#143 (verificado); distinta de "Envite" #143/"Piña" #142/"Repliegue" #141/"Muralla" #140/"Socorro" #139/"Cuadrilla" #138/"Cohorte" #137/"Cónclave" #136/"Fuego Conc." (FOCUS_FIRE #62)/"Embate"/"Acometida" (otros flags)
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#f6dcc4":"#8898ac"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de diana por kill + nº convergidos sobre el objetivo (focus); si solitario/disperso, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Diana G"+focus):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#f0c8a8":"#8898ac"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2835: badge de TENAZA (CO_FLANK_SURGE, EVO#145 — PLAYER-COORDINATION #10 / GEOMETRÍA-DE-ENVOLVIMIENTO / desde CUÁNTOS SECTORES angulares distintos ≥2 jugadores pinzan/rodean el MISMO mob = F ∈[0,9] ENTERO). Refleja el VM PURO (sim.coFlankVM, autoridad en sim) ⇒ MISMO idx/players/flank/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). MULTIJUGADOR-NATIVO (single-player ⇒ F colapsa ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderCoFlankBadge(){
    const k=sim.coFlankVM&&sim.coFlankVM(); if(!k) return;                  // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+2254, sw=14, sh=14;                              // bajo la Diana (@+2232); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, players=k.players|0, flank=k.flank|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.2+tier*0.6))):0.55;         // pulso de pinza (envolvimiento)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de TENAZA (⇹): DOS flechas OPUESTAS que convergen sobre un punto central (la pinza que se cierra desde lados contrarios); su apertura se ESTRECHA conforme crece F (idx=F/playerCap): más sectores = pinza más cerrada; un-flanco/solitario = contorno tenue gris
    const col=here?"#a8e0d0":"rgba(150,160,175,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.fillStyle=col; ctx.lineWidth=here?1.6:1.1; ctx.lineJoin="round"; ctx.lineCap="round";
    const rad=sh*0.42;
    // punto central (el objetivo rodeado)
    ctx.globalAlpha=pulse*(here?1:0.6);
    ctx.beginPath(); ctx.arc(cx,cy,Math.max(1.0,rad*0.30),0,Math.PI*2); ctx.fill();
    // DOS puntas de tenaza que aprietan desde IZQUIERDA y DERECHA (lados OPUESTOS) hacia el centro; su distancia interior se acerca con idx (pinza cerrándose)
    ctx.globalAlpha=pulse;
    const conv=Math.min(1,Math.max(idx,here?0.5:0.28)), inR=rad*(0.7+0.6*(1-conv)), outR=rad*(2.0);   // más envuelto ⇒ punta más cerca del centro
    for(let s=-1;s<=1;s+=2){ // izquierda (-1) y derecha (+1) — sectores OPUESTOS
      const ox=cx+s*outR, ix=cx+s*inR;
      ctx.beginPath(); ctx.moveTo(ox,cy-rad*0.7); ctx.lineTo(ix,cy); ctx.lineTo(ox,cy+rad*0.7); ctx.stroke(); }  // punta en "<"/">" apuntando al centro
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Tenaza: "+(here?("T"+tier):"—");          // etiqueta ÚNICA "Tenaza" (PLAYER-COORDINATION #10 / co-flank / pincer-surround) — 🔑 sin colisión con las 86 labels #59-#144 (verificado); distinta de "Diana" #144/"Falange" (FLANK_SURGE #108)/"Envite" #143/"Piña" #142/"Repliegue" #141/"Muralla" #140/"Socorro" #139/"Cuadrilla" #138/"Cohorte" #137/"Cónclave" #136
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#c4f0e4":"#8898ac"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de tenaza por kill + nº de sectores de envolvimiento (flank); si un-flanco/solitario, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Tenaza F"+flank):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#a8e0d0":"#8898ac"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2840: badge de CONJURO (CO_CAST_SURGE, EVO#146 — PLAYER-COORDINATION #11 / SIMULTANEIDAD-DE-ACTIVACIÓN / cuántos JUGADORES DISTINTOS activan una habilidad EL MISMO frame = U ∈[0,P] ENTERO). Refleja el VM PURO (sim.coCastVM, autoridad en sim) ⇒ MISMO idx/players/cast/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). MULTIJUGADOR-NATIVO (single-player ⇒ U colapsa ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderCoCastBadge(){
    const k=sim.coCastVM&&sim.coCastVM(); if(!k) return;                    // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+2276, sw=14, sh=14;                              // bajo la Tenaza (@+2254); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, players=k.players|0, cast=k.cast|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.4+tier*0.6))):0.55;         // pulso de conjuro (destello de activación)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de CONJURO (✷): un DESTELLO/estrella de rayos que emanan de un núcleo (la habilidad activándose); su nº de rayos y brillo crecen con U (idx=U/playerCap): más casters simultáneos = destello más pleno; solitario = contorno tenue gris
    const col=here?"#c8b8ec":"rgba(150,160,175,0.36)";
    ctx.save(); ctx.globalAlpha=pulse;
    ctx.strokeStyle=col; ctx.fillStyle=col; ctx.lineWidth=here?1.6:1.1; ctx.lineJoin="round"; ctx.lineCap="round";
    const rad=sh*0.42;
    // núcleo (el foco del cast)
    ctx.globalAlpha=pulse*(here?1:0.6);
    ctx.beginPath(); ctx.arc(cx,cy,Math.max(1.0,rad*0.34),0,Math.PI*2); ctx.fill();
    // rayos del destello que emanan del núcleo; su LONGITUD crece con idx (destello más pleno con más casters); 8 direcciones (estrella ✷)
    ctx.globalAlpha=pulse;
    const reach=Math.min(1,Math.max(idx,here?0.5:0.28)), outR=rad*(1.15+0.95*reach);   // más casters ⇒ rayos más largos
    for(let s=0;s<8;s++){ const ang=s*(Math.PI/4);
      ctx.beginPath(); ctx.moveTo(cx+Math.cos(ang)*rad*0.42, cy+Math.sin(ang)*rad*0.42);
      ctx.lineTo(cx+Math.cos(ang)*outR, cy+Math.sin(ang)*outR); ctx.stroke(); }
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Conjuro: "+(here?("T"+tier):"—");         // etiqueta ÚNICA "Conjuro" (PLAYER-COORDINATION #11 / co-cast / ability-sync) — 🔑 sin colisión con las 87 labels #59-#145 (verificado); distinta de "Tenaza" #145/"Diana" #144/"Socorro" #139/"Cuadrilla" #138
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#dcd0f4":"#8898ac"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de conjuro por kill + nº de casters simultáneos (cast); si solitario, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Conjuro U"+cast):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#c8b8ec":"#8898ac"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2845: badge de QUIEBRO (CO_DODGE_SURGE, EVO#147 — PLAYER-COORDINATION #12 / SIMULTANEIDAD-DE-ESQUIVA / cuántos JUGADORES DISTINTOS están en estado de esquiva/rodada/i-frame EL MISMO frame = D ∈[0,P] ENTERO). Refleja el VM PURO (sim.coDodgeVM, autoridad en sim) ⇒ MISMO idx/players/dodge/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). MULTIJUGADOR-NATIVO (single-player ⇒ D colapsa ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderCoDodgeBadge(){
    const k=sim.coDodgeVM&&sim.coDodgeVM(); if(!k) return;                    // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+2298, sw=14, sh=14;                              // bajo el Conjuro (@+2276); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, players=k.players|0, dodge=k.dodge|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.6+tier*0.6))):0.55;         // pulso de quiebro (destello de la esquiva)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de QUIEBRO (⟿): estelas/afterimages curvas de motion-blur que barren de izq a der (la esquiva rodante); su nº y longitud crecen con D (idx=D/playerCap): más esquivas simultáneas = estela más plena; solitario = trazo tenue gris
    const col=here?"#8fe6d6":"rgba(150,160,175,0.36)";
    ctx.save();
    ctx.strokeStyle=col; ctx.lineWidth=here?1.7:1.1; ctx.lineJoin="round"; ctx.lineCap="round";
    const rad=sh*0.42;
    // 3 estelas curvas de rodada (motion streaks): su LONGITUD crece con idx (barrido más pleno con más esquivas)
    const reach=Math.min(1,Math.max(idx,here?0.5:0.30)), span=rad*(1.15+0.95*reach);   // más esquivas ⇒ estelas más largas
    for(let s=0;s<3;s++){ const oy=(s-1)*rad*0.52, al=pulse*(1-Math.abs(s-1)*0.28);
      ctx.globalAlpha=al;
      ctx.beginPath();
      ctx.moveTo(cx-span, cy+oy);
      ctx.quadraticCurveTo(cx, cy+oy-rad*0.34, cx+span, cy+oy);           // barrido curvo (la ondulación de la esquiva)
      ctx.stroke();
      // punta/cabeza de la estela (hacia dónde se esquiva)
      if(here){ ctx.beginPath();
        ctx.moveTo(cx+span, cy+oy); ctx.lineTo(cx+span-rad*0.4, cy+oy-rad*0.3);
        ctx.moveTo(cx+span, cy+oy); ctx.lineTo(cx+span-rad*0.4, cy+oy+rad*0.16); ctx.stroke(); } }
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Quiebro: "+(here?("T"+tier):"—");        // etiqueta ÚNICA "Quiebro" (PLAYER-COORDINATION #12 / co-dodge / evade-sync) — 🔑 sin colisión con las 88 labels #59-#146 (verificado); distinta de "Conjuro" #146/"Tenaza" #145/"Diana" #144/"Socorro" #139/"Cuadrilla" #138
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#c8f4ea":"#8898ac"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de quiebro por kill + nº de esquivas simultáneas (dodge); si solitario, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Quiebro D"+dodge):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#8fe6d6":"#8898ac"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2849: badge de YUGO (CO_INTERRUPT_SURGE, EVO#148 — PLAYER-COORDINATION #13 / SIMULTANEIDAD-DE-CONTROL-LANDEADO / cuántos JUGADORES DISTINTOS están LANDEANDO un interrupt/stagger/poise-break sobre un enemigo EL MISMO frame = I ∈[0,P] ENTERO). El ESPEJO OFENSIVO de #147 QUIEBRO. Refleja el VM PURO (sim.coInterruptVM, autoridad en sim) ⇒ MISMO idx/players/interrupt/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). MULTIJUGADOR-NATIVO (single-player ⇒ I colapsa ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderCoInterruptBadge(){
    const k=sim.coInterruptVM&&sim.coInterruptVM(); if(!k) return;             // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+2320, sw=14, sh=14;                              // bajo el Quiebro (@+2298); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, players=k.players|0, interrupt=k.interrupt|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.7+tier*0.6))):0.55;         // pulso de yugo (chasquido del lockdown)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de YUGO (⛒): dos abrazaderas/grilletes curvos que se cierran hacia el centro (el lockdown que traba al enemigo); su cierre crece con I (idx=I/playerCap): más interrupts simultáneos = mordida más plena; solitario = trazo tenue gris
    const col=here?"#e6b84f":"rgba(150,160,175,0.36)";
    ctx.save();
    ctx.strokeStyle=col; ctx.lineWidth=here?1.8:1.1; ctx.lineJoin="round"; ctx.lineCap="round";
    const rad=sh*0.42;
    // 2 abrazaderas (izq/der) que muerden hacia el centro: su CIERRE crece con idx (mordida más apretada con más interrupts)
    const bite=Math.min(1,Math.max(idx,here?0.5:0.30)), gap=rad*(0.95-0.55*bite);   // más interrupts ⇒ menor hueco (mordida más cerrada)
    ctx.globalAlpha=pulse;
    for(let s=0;s<2;s++){ const sign=s===0?-1:1;
      ctx.beginPath();
      // arco de grillete: cuarto de círculo que abre hacia el centro
      ctx.arc(cx+sign*(rad*0.95), cy, rad*0.9, sign<0?-Math.PI*0.5:Math.PI*0.5, sign<0?Math.PI*0.5:Math.PI*1.5, false);
      ctx.stroke();
      // dientes de mordida hacia el centro (hacia el hueco que se cierra)
      if(here){ ctx.beginPath();
        ctx.moveTo(cx+sign*gap, cy-rad*0.34); ctx.lineTo(cx+sign*(gap+rad*0.34), cy);
        ctx.lineTo(cx+sign*gap, cy+rad*0.34); ctx.stroke(); } }
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Yugo: "+(here?("T"+tier):"—");            // etiqueta ÚNICA "Yugo" (PLAYER-COORDINATION #13 / co-interrupt / lockdown-sync) — 🔑 sin colisión con las 89 labels #59-#147 (verificado); distinta de "Quiebro" #147/"Conjuro" #146/"Tenaza" #145/"Diana" #144
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#f4dca0":"#8898ac"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de yugo por kill + nº de interrupts simultáneos (interrupt); si solitario, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Yugo I"+interrupt):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#e6b84f":"#8898ac"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2853: badge de ÉGIDA (CO_BUFF_UPTIME_SURGE, EVO#149 — PLAYER-COORDINATION #14 / SIMULTANEIDAD-DE-ESTADO-SOSTENIDO-DE-BUFF / cuántos JUGADORES DISTINTOS PORTAN un buff/aura activo EL MISMO frame = B ∈[0,P] ENTERO). El CONTRAPUNTO SOSTENIDO de las tres sub-facetas EVENTO (#146 cast / #147 dodge / #148 interrupt). Refleja el VM PURO (sim.coBuffVM, autoridad en sim) ⇒ MISMO idx/players/buff/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). MULTIJUGADOR-NATIVO (single-player ⇒ B colapsa ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderCoBuffBadge(){
    const k=sim.coBuffVM&&sim.coBuffVM(); if(!k) return;                     // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+2342, sw=14, sh=14;                              // bajo el Yugo (@+2320); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, players=k.players|0, buff=k.buff|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.4+tier*0.5))):0.55;         // pulso de égida (respiración del aura compartido)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de ÉGIDA (❖): un escudo/rombo protector con capas concéntricas que se llenan con B (idx=B/playerCap): más auras activos = más anillos de resguardo; solitario = contorno tenue gris
    const col=here?"#7fd8c8":"rgba(150,160,175,0.36)";
    ctx.save();
    ctx.strokeStyle=col; ctx.lineWidth=here?1.8:1.1; ctx.lineJoin="round"; ctx.lineCap="round";
    const rad=sh*0.46;
    // rombo/escudo protector: nº de anillos concéntricos = intensidad de la égida (más buffeados ⇒ más capas de resguardo)
    const rings=here?(1+Math.min(2,Math.max(0,Math.round(idx*2)))):1;       // 1..3 anillos según idx (0-float sólo cosmético)
    ctx.globalAlpha=pulse;
    for(let r=0;r<rings;r++){ const rr=rad*(1-r*0.30);
      ctx.beginPath();
      ctx.moveTo(cx, cy-rr); ctx.lineTo(cx+rr, cy); ctx.lineTo(cx, cy+rr); ctx.lineTo(cx-rr, cy); ctx.closePath();
      ctx.stroke(); }
    if(here){ ctx.beginPath(); ctx.arc(cx, cy, rad*0.14, 0, Math.PI*2); ctx.fillStyle=col; ctx.fill(); }   // núcleo del aura
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Égida: "+(here?("T"+tier):"—");           // etiqueta ÚNICA "Égida" (PLAYER-COORDINATION #14 / co-buff / shared-aegis) — 🔑 sin colisión con las 90 labels #59-#148 (verificado); distinta de "Yugo" #148/"Quiebro" #147/"Conjuro" #146/"Socorro" #139
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#a8ede0":"#8898ac"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de égida por kill + nº de auras activos simultáneos (buff); si solitario, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Égida B"+buff):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#7fd8c8":"#8898ac"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }

  // CAS-2857: badge de BROQUEL (CO_GUARD_SURGE, EVO#150 — PLAYER-COORDINATION #15 / SIMULTANEIDAD-DE-GUARDIA-ACTIVA / cuántos JUGADORES DISTINTOS en block/parry/guard-up EL MISMO frame = G ∈[0,P] ENTERO). La SEGUNDA sub-faceta DEFENSIVE REACTION-EVENT (par con #147 QUIEBRO pero OPUESTA: INTERCEPTAR ⊥ EVADIR). Refleja el VM PURO (sim.coGuardVM, autoridad en sim) ⇒ MISMO idx/players/guard/score/tier/fichas para todos los clientes con el mismo estado de sim. Cosmético puro (0 efecto de sim). MULTIJUGADOR-NATIVO (single-player ⇒ G colapsa ⇒ apagado). DARK (enabled:false) ⇒ el dispatch NO lo llama hasta el flip.
  function renderCoGuardBadge(){
    const k=sim.coGuardVM&&sim.coGuardVM(); if(!k) return;                   // sin VM ⇒ nada
    const a=badgeRowAnchor();
    const bx=a.bx, by=a.by+2364, sw=14, sh=14;                              // bajo la Égida (@+2342); gap anti-solape (CAS-2263)
    const tier=k.tier|0, here=tier>0, charge=k.charge|0, idx=+k.idx||0, players=k.players|0, guard=k.guard|0;
    const pulse=here?(0.70+0.24*Math.sin(G.t*(2.4+tier*0.5))):0.55;         // pulso de guardia (respiración del muro de escudos)
    const cx=bx+sw/2, cy=by+sh/2;
    // glifo de BROQUEL (⊓): un escudo/broquel con la TAPA levantada en GUARDIA — un arco superior (guardia alta) + faldón lateral; capas concéntricas que se llenan con G (idx=G/playerCap): más guardias activas = más escudos alzados; solitario = contorno tenue gris
    const col=here?"#9fb8d6":"rgba(150,160,175,0.36)";
    ctx.save();
    ctx.strokeStyle=col; ctx.lineWidth=here?1.8:1.1; ctx.lineJoin="round"; ctx.lineCap="round";
    const rad=sh*0.46;
    // broquel/guardia alzada: nº de capas = intensidad del co-bloqueo (más en guardia ⇒ más escudos alzados)
    const layers=here?(1+Math.min(2,Math.max(0,Math.round(idx*2)))):1;      // 1..3 capas según idx (0-float sólo cosmético)
    ctx.globalAlpha=pulse;
    for(let r=0;r<layers;r++){ const rr=rad*(1-r*0.30);
      ctx.beginPath();
      // arco superior (tapa alzada en guardia) + jambas laterales que bajan = perfil de broquel/escudo INTERCEPTANDO
      ctx.moveTo(cx-rr, cy+rr*0.85);
      ctx.lineTo(cx-rr, cy-rr*0.15);
      ctx.arc(cx, cy-rr*0.15, rr, Math.PI, 0, false);
      ctx.lineTo(cx+rr, cy+rr*0.85);
      ctx.stroke(); }
    if(here){ ctx.beginPath(); ctx.arc(cx, cy-rad*0.15, rad*0.14, 0, Math.PI*2); ctx.fillStyle=col; ctx.fill(); }   // remache central del broquel
    ctx.globalAlpha=pulse;
    // micro-label
    ctx.font="bold 11px "+FF; ctx.textAlign="left"; ctx.textBaseline="middle";
    const ty=cy, tx=bx+sw+5, lbl="Broquel: "+(here?("T"+tier):"—");         // etiqueta ÚNICA "Broquel" (PLAYER-COORDINATION #15 / co-block / co-parry) — 🔑 sin colisión con las 91 labels #59-#149 (verificado); distinta de "Égida" #149/"Yugo" #148/"Quiebro" #147
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(lbl,tx,ty);
    ctx.fillStyle=here?"#c3d4ea":"#8898ac"; ctx.fillText(lbl,tx,ty);
    // estado a la derecha: +fichas de broquel por kill + nº de guardias activas simultáneas (guard); si solitario, "—"
    ctx.font="bold 10px "+FF; ctx.textAlign="right";
    const st2c=here?("+"+charge+" Broquel G"+guard):"—";
    ctx.lineWidth=3; ctx.strokeStyle="rgba(0,0,0,0.72)"; ctx.strokeText(st2c,bx+150,ty);
    ctx.fillStyle=here?"#9fb8d6":"#8898ac"; ctx.fillText(st2c,bx+150,ty);
    ctx.restore();
  }
