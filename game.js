...
export function createGame(canvas, ctx, getView){
  ...
  if(typeof location!=="undefined" && location.search.indexOf("dev")>=0){
    window.__dev={ spawn:(type,dx,dy)=>simDev.spawn(type,dx,dy), tp:(tx,ty)=>simDev.tp(tx,ty),
      ...
      zonedom:(p)=>simDev.zonedom(p),  // CAS-2410: Bastión / Dominio de Zona OBSERVABLE hook (DARK, ZONE_DOMINANCE — canal REUSADO oocMitigation con SHARE-CAP vs Wayfarer + eje ESPACIAL/TERRITORIAL control de zona en disputa shard-wide; DISTINTO a Cadence meter / Nocturne-Tempest reloj / Trailcraft-Wayfarer exploración — rota la zona DOMINADA por hash + rampa triangular de control, mitiga mientras la sostienes; ⊥ WORLD_PULSE por canal defensa vs XP)
