...
export function createGame(canvas, ctx, getView){
  ...
  if(typeof location!=="undefined" && location.search.indexOf("dev")>=0){
    window.__dev={ spawn:(type,dx,dy)=>simDev.spawn(type,dx,dy), tp:(tx,ty)=>simDev.tp(tx,ty),
      ...
      nocturne:(p)=>simDev.nocturne(p),  // CAS-2393: Cazador Nocturno OBSERVABLE hook (DARK, NOCTURNE_HUNT — canal REUSADO goldFind con de-stack a KINSHIP/FOCUS + eje FASE TEMPORAL/caza nocturna; OPUESTO a Erudition — cuenta CUÁNDO matas, no a QUIÉN)
