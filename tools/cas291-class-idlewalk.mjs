// CAS-291 — RE-BAKE ONLY the idle + walk strips for the 4 non-warrior classes so the
// canonical hero in EVERY state is the SAME character that owns the attack animation.
//
// THE BUG (board CAS-288, mis-fixed by CAS-289): for mage/paladin/druid/priest the
// idle/walk strips (CAS-253 PixelLab-native, e.g. the BLUE mage) were a DIFFERENT
// character than the attack/death strips (CAS-238/CAS-283 healer/clarice recipe, the
// PURPLE mage with the real cast pose). On every swing the hero morphed. CAS-289 forced
// attack→idle (kept the STATIC blue character) — the OPPOSITE of what the board asked:
// "deja el sprite que TIENE la animación de ataque, no el estático."
//
// THE FIX (this tool): bake idle + walk from the EXACT SAME CAS-238 healer/clarice recipe
// (identical recolor + accent + scale) that produced the boss-approved attack/death
// (CAS-283), so all four states are one consistent character. We DO NOT touch the
// attack/death PNGs (they stay as CAS-283 baked them, wide cells), and warrior (Clarice)
// is untouched entirely. render.js restores CLASS_EXTRA_ANIM to all 5 classes so the
// dedicated attack/death strips render again instead of the procedural-lunge fallback.
// Only assets/erw/hero/classes/{druid,mage,paladin,priest}{,_walk}.png are rewritten.
//
// (Below is the original CAS-238 production recipe — SOURCE per class, unchanged — kept
//  verbatim so the idle/walk character is byte-for-byte the same family as attack/death.)
//
...
// CAS-291: write ONLY idle + walk. attack/death stay as CAS-283 baked them (boss-approved),
// so we never re-narrow the wide attack/death cells nor change their bytes.
const FILES = { idle:"", walk:"_walk" };
let total=0;
for(const cls in out){ const r=out[cls];
  ...
  console.log(`  ${cls.padEnd(8)} scale=${r.scale} figH=${r.figH}px → idle/walk (attack/death left as CAS-283)`);
}
console.log("wrote 8 idle/walk strips ("+(total/1024|0)+"KB) to", OUT);
await browser.close();
console.log("done.");
