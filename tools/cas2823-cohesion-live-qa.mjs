// CAS-2823 — POST-FLIP LIVE QA for PIÑA (CO_COHESION_SURGE.enabled:TRUE LIVE, EVO#142, 84º flag). Base master 057f076 (be3d0f6 DARK → 72f71c6 flip false→true → 057f076 version stamp 3a014051fb4f/815), served LIVE carlosdcastrosa-cloud.github.io/Mithralda-Online.
// Derivado del DARK self-verify cas2820 con: ck2 INVERTIDO LIVE-ON (enabled TRUE, solo boot ⇒ C≤1 ⇒ score/tier/charge colapsan LIMPIO), ck14 CENSUS INVERTIDO (70/70 enabled:true off=[]), + ck18 SERVED-LIVE-BUILD (root200 + version.json 3a014051fb4f/815 + served config CO_COHESION_SURGE enabled:true + census 70/70). Gameplay/determinismo corre contra checkout local NO-DRIFT (HEAD==origin==057f076, byte-idéntico a lo servido); ck18 verifica los BYTES SERVIDOS.
...
//
// Run: node tools/cas2823-cohesion-live-qa.mjs
...
const OUT = join(ROOT, "shots", "cas2823");
...
try {
  ...
  // 2 LIVE-ON fresh boot (INVERTED from DARK): CO_COHESION_SURGE.enabled TRUE; solo hero fresh boot, sin roster apiñado en radio ⇒ C≤1 < midCohesion2 ⇒ SCORE/DECISIÓN colapsan LIMPIO (tier/score/charge/preview=0). Multijugador-nativo: IMPOSIBLE formar piña en solitario. channel fresco coCohesionFind + tag vacío. 🔑 NO asertamos gExists/partyExists ni idx==0 (con enabled:true las estructuras transitorias PUEDEN materializarse y el héroe-solo tiene idx cosmético C/cap=0.25 — aserta el COLAPSO DE DECISIÓN, no la existencia/idx).
  const dark = await page.evaluate(() => window.__dev.coCohesion());
  ok("2 LIVE-ON (fresh solo boot): CO_COHESION_SURGE.enabled TRUE AND sin roster apiñado (C≤1) ⇒ score/tier/charge/preview colapsan LIMPIO a 0 AND channel coCohesionFind AND tag vacío",
     dark.enabled === true && dark.tier === 0 && dark.score === 0 && dark.cohesion <= 1 && dark.charge === 0 && dark.forageChargePreview === 0 && dark.channel === "coCohesionFind" && dark.tag === "",
  ...
  // 14 ★ 0-REGRESSION CENSUS (INVERTED LIVE-ON): served config — 70 `_SURGE` totales, 70 enabled:true, off=[] (CO_COHESION #142 LIVE + CO_FLEE #141 LIVE + los 82 priores).
  ...
  const censusOK = total === 70 && trues === 70 && falses.length === 0 && coCohesionLive && coCohesionLive[1] === "true" && coFleeLive && coFleeLive[1] === "true";
  ok("14 ★ LIVE-ON 0-REGRESIÓN CENSUS (INVERTED): served config 70 `_SURGE` totales, 70 enabled:true, off=[] (CO_COHESION_SURGE #142 LIVE + CO_FLEE_SURGE #141 LIVE + los 82 priores)",
     censusOK, `total=${total} true=${trues} false=${JSON.stringify(falses)} coCohesion=${coCohesionLive ? coCohesionLive[1] : "?"} coFlee=${coFleeLive ? coFleeLive[1] : "?"}`);
  ...
  // 18 ★ SERVED LIVE BUILD (gh-pages, no publish lag): root 200 + version.json == 3a014051fb4f/815 (NEW ≠ old 83032b54f4e8) + served sim/config.js CO_COHESION_SURGE enabled:true + census 70/70 off=[].
  try {
    ...
    const servedOK = rootRes.status === 200 && ver.build === "3a014051fb4f" && ver.files === 815 && lTotal === 70 && lTrues === 70 && lFalses.length === 0 && lCoCohesion && lCoCohesion[1] === "true" && lCoFlee && lCoFlee[1] === "true";
    ok("18 ★ SERVED LIVE BUILD (gh-pages): root 200 + version.json 3a014051fb4f/815 (NEW ≠ old 83032b54f4e8) + served sim/config.js CO_COHESION_SURGE enabled:true + census 70/70 off=[]",
       servedOK, `rootStatus=${rootRes.status} build=${ver.build} files=${ver.files} census total=${lTotal} true=${lTrues} false=${JSON.stringify(lFalses)} coCohesion=${lCoCohesion ? lCoCohesion[1] : "?"} coFlee=${lCoFlee ? lCoFlee[1] : "?"}`);
  } catch (e) {
    ok("18 ★ SERVED LIVE BUILD (gh-pages): root 200 + version.json 3a014051fb4f/815 + config CO_COHESION_SURGE enabled:true", false, `FETCH ERROR ${String(e)}`);
  }
