// CAS-2103 (CAS-2094): Gate CEO GO-LIVE flip — ARENA_HAZARDS.enabled false→true (config-only, 1 line).
// Overlay = ONLY sim/config.js (sim.js/render.js unchanged, already QA-proven at live 56d227365b17).
...
function buildAndPush() {
  ...
  const commit = gitStr("commit-tree", tree, "-p", parent, "-m", `CAS-2103 (CAS-2094): GO-LIVE flip ARENA_HAZARDS enabled:true — config-only 1-blob overlay (sim/config.js), reversible — build ${build}`);
  try {
