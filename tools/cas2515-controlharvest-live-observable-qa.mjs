// CAS-2515 — QA POST-FLIP LIVE 2-cliente (INDEP, QA-owned) para COSECHA DE SOMETIMIENTO (CONTROL_HARVEST_SURGE.enabled:TRUE), EVO mecánica #85 (27º flag del arco).
...
// EXPECT build servido = 04171c2c1c08 (== flip CAS-2513 master fc92084 == HTTP served; byte-verificado CTO 4/4 + CEO 2ª).
//
...
//   - controlHarvestScore(h)=Σ controlWeight(e) sobre los mobs VIVOS bajo CC de G.enemies cuyo CENTRO ∈ radius260 (server-auth; e.stun/e.slowT dinámicos de combate).
//   - controlWeight(e): sin-CC=0 · slow (e.slowT>0 sin stun)=1 · stun (e.stun>0)=2 (MAX, domina slow). FILTRA !e.dead.
...
//     forageChargePreview = EXACTAMENTE lo que banca el seam de kill ⇒ prueba determinista del forrajeo. Un mob aislado ralentizado (score<2) NO rinde.
//   - ANTI-AUTO-CONTEO por KILL REAL (affixSpawnKill→killEnemy): aire limpio (score 0) ⇒ 0; kill DENTRO de un pack sometido denso (score≥4) ⇒ forrajea. `_ctrlPre` muestreado en el TOP de killEnemy tras e.dead=true (el mob a rematar NO auto-cuenta su propio CC).
...
// Run: node tools/cas2515-controlharvest-live-observable-qa.mjs   [live-url]
...
const OUT = join(ROOT, "shots", "cas2515-qa");
...
const WSTUN = 2, WSLOW = 1;                                   // stun duro=2, slow blando=1, sin-CC=0
...
const errA = [], errB = [], errC = [], errM = [], net404 = [];
async function bootFresh(err, mobile = false) {
  const page = await browser.newPage();
  if (mobile) await page.setViewport({ width: 414, height: 820, deviceScaleFactor: 2, isMobile: true, hasTouch: true });
...
try {
  ...
  ok("L2 ★ 0-REGRESIÓN: 26 flags arco #59-#84 served true (intactas, incl. SKIRMISH #84) + CONTROL_HARVEST_SURGE #85 served true ⇒ 27 flags LIVE, 0 perdidas",
  ...
  ok("L3 LIVE default: __dev.controlHarvest().enabled === TRUE (flip aplicado), channel controlFind, gExists false (STATELESS, G.controlCharges nunca creado), cap 2, radius 260, score/charge 0 en aire limpio",
     st.enabled === true && st.channel === "controlFind" && st.gExists === false && st.cap === CAP && st.radius === RADIUS && st.score === 0 && st.charge === 0,
     `enabled=${st.enabled} channel=${st.channel} gExists=${st.gExists} cap=${st.cap} radius=${st.radius} score=${st.score} charge=${st.charge}`);
  ...
  // ═══ L5 — REAL SERVER-AUTH + INDEP: spawnControl inyecta mob bajo CC REAL en G.enemies; controlProbe lee score REAL ═══
  //   per-kind weight (stun2/slow1/none0) + radius-boundary gating (Δ8=256px cuenta, Δ9=288px no).
  ...
  const effect = await pageA.evaluate((h) => {
    window.__dev.controlHarvest({ clearControl: true });
    // JAULA densa: 2 aturdidos (cada uno w2) ⇒ score4 ⇒ T2/charge2
  ...
  const effOK = effect.enabled === true
    && effect.near.score === 4 && effect.near.tier === 2 && effect.near.charge === 2 && effect.near.preview === 2 && effect.near.tag === "⊗"
  ...
  ok("L6 ★ EFECTO ACTIVO LIVE (enabled:true, SIN toggle in-memory): 2 aturdidos (score 4) en radio ⇒ charge+2/T2 + forageChargePreview 2 (lo que banca el kill) + tag ⊗; héroe en aire limpio (>260px) ⇒ +0/T0/tag'' (== oráculo)",
     effOK, JSON.stringify(effect));

  // ═══ L6b — GATE-2 core ANTI-AUTO-CONTEO por KILL REAL (affixSpawnKill→killEnemy): aire limpio ⇒ 0; pack denso ⇒ 2. UN mob aislado ralentizado ⇒ 0 (score<2) ═══
  const anti = await pageA.evaluate(() => {
    ...
    // C: kill junto a UN mob aislado ralentizado (score1<2) ⇒ 0 forage (umbral)
    window.__dev.controlHarvest({ clearControl: true });
    const h2 = window.__dev.controlHarvest().hero;
    window.__dev.controlHarvest({ spawnControl: { tx: h2.tx + 2, ty: h2.ty, kind: "slow" } });
    ...
    const ctC0 = ct();
    window.__dev.affixSpawnKill("swift", "skeleton", "field");            // kill REAL junto a 1 mob ralentizado aislado
  ...
  const antiOK = anti.baseScoreA === 0 && anti.deltaA === 0 && anti.baseScoreB >= 4 && anti.deltaB === oCharge(anti.baseScoreB) && anti.deltaB === 2
    && anti.baseScoreC === 1 && anti.deltaC === 0;
  ok("L6b ★ ANTI-AUTO-CONTEO por KILL REAL (affixSpawnKill→killEnemy): aire limpio (pre=0) ⇒ Δcharge 0 (el propio mob NO auto-cuenta su CC); pack sometido denso (pre≥4) ⇒ Δcharge == my LUT=2 (refleja _ctrlPre pre-kill); UN mob ralentizado aislado (pre=1<2) ⇒ Δcharge 0 (umbral: hace falta un PACK genuino)",
  ...
  ok("L7 ★ SUB-CAP controlChargeCap: ningún score produce charge>2 (sweep score∈[0,40]); cap==2 (no stacking sin límite)",
     capChk.max <= 2 && capChk.cap === 2, JSON.stringify(capChk));

  // ═══ L7b — TABLA/UMBRAL LIVE: 1 mob slow-solo (score1)⇒charge0 ; 1 mob stun (score2)⇒charge1 (aislado con CC dura SÍ cosecha) ═══
  ...
  ok("L7b TABLA/UMBRAL LIVE: 1 slow-solo score1⇒charge0 (T0, ralentizado aislado NO cosecha); 1 stun score2⇒charge1 (CC dura = pack sometido genuino desde 1 mob)",
  ...
  ok("L8b STATELESS: saveBlob() SIN control* (h.controlCharges recurso TRANSITORIO, 100% derivado) + worldFingerprint(393) byte-estable a través de probes (las cargas NO entran al fingerprint; mobs de prueba limpiados; 0 RNG/timer drift)",
     noKey && fpBefore === fpAfter, `noKey=${noKey} fpStable=${fpBefore === fpAfter} fpLen=${fpBefore.length} saveLen=${saveBlob.length}`);

  // ═══ L9 — badge glifo "Sometimiento:" render (ctx.fillText) con pack sometido EN radio; movimiento+combate sin crash + fps ═══
  const badge = await pageA.evaluate(async (h) => {
    ...
    // (2) badge NEAR — héroe PARADO con pack sometido fresco en radio ⇒ tag + glifo dibujado
    const h2 = window.__dev.controlHarvest().hero;
    window.__dev.controlHarvest({ clearControl: true });
  ...
  ok("L9 LIVE render: con pack sometido EN radio parado ⇒ vm.tag (⊗) + glifo 'Sometimiento:' dibujado (nearGlyph>0); héroe en aire limpio ⇒ tag '' ; movimiento+combate sin crash; fps sano",
  ...
  // ═══ L9b — ⊥ DIFERENCIADOR ⊥26: inyectar pack aturdido ⇒ control→T2 MIENTRAS escaramuza#84/plaga#83 IGNORAN (mobs orc melee sin dots) + peers coexisten ═══
  const diff = await pageA.evaluate((h) => {
    window.__dev.controlHarvest({ clearControl: true });
    window.__dev.controlHarvest({ tp: { tx: h.tx, ty: h.ty } });
    const snap = () => ({
      skirmish: window.__dev.skirmishLine ? window.__dev.skirmishLine({ skirmishProbe: true }).skirmishProbe.score : null,   // #84 e.tpl.ranged
      blight: window.__dev.blightHarvest ? window.__dev.blightHarvest().score : null,                                        // #83 e.dots
    ...
    return { pre, post, skOn, blOn, maOn };
  }, await anchorHero(pageA, 170, 170));
  const peersFlat = diff.pre.skirmish === diff.post.skirmish && diff.pre.blight === diff.post.blight;
  const controlRose = diff.post.control - diff.pre.control === 4 && diff.post.control >= 4;
  const diffOK = peersFlat && controlRose && diff.skOn === true && diff.blOn === true && diff.maOn === true;
  ok("L9b ⊥ DIFERENCIADOR ⊥26: 2 aturdidos (orc melee, sin dots) ⇒ control Δ+4→T2 MIENTRAS escaramuza#84 (e.tpl.ranged) / plaga#83 (e.dots) IGNORAN (score sin cambio); vecinos LIVE enabled:true coexisten (SKIRMISH#84/BLIGHT#83/MAELSTROM#82)",
  ...
  const readNS = async (pg) => await pg.evaluate((HT) => {
    ...
    window.__dev.controlHarvest({ spawnControl: { tx: h.tx + 3, ty: h.ty, kind: "stun" } });    // w2
    window.__dev.controlHarvest({ spawnControl: { tx: h.tx, ty: h.ty + 3, kind: "stun" } });    // w2
    window.__dev.controlHarvest({ spawnControl: { tx: h.tx - 3, ty: h.ty, kind: "slow" } });    // w1
    window.__dev.controlHarvest({ tp: { tx: HT.tx, ty: HT.ty } });
    const vm = window.__dev.controlHarvest();
    const bp = window.__dev.controlHarvest({ controlProbe: true }).controlProbe;
  ...
  ok("L11 RECONNECT mid-pack STATELESS 0-drift: tras reload, enabled:true + gExists false + save sin controlCharges/controlFind + LUT idéntica + worldFingerprint idéntico + re-sync densidad (T2/tag ⊗) (0 drift, 0 persistencia indebida de cargas)",
  ...
  // ═══ L12 — build/boot MÓVIL 414x820 — nueva página, 0 error, monta __dev, juega, badge se dibuja ═══
  const pageM = await bootFresh(errM, true);
  const mobile = await pageM.evaluate((h) => {
    const play = window.__dev.scene() === "play" && !!window.__dev.controlHarvest;
    window.__dev.controlHarvest({ clearControl: true });
    window.__dev.controlHarvest({ tp: { tx: h.tx, ty: h.ty } });
    const hh = window.__dev.controlHarvest().hero;
    window.__dev.controlHarvest({ spawnControl: { tx: hh.tx + 3, ty: hh.ty, kind: "stun" } });
    ...
    window.__dev.controlHarvest({ tp: { tx: hh.tx, ty: hh.ty } });
    const vm = window.__dev.controlHarvest();
    return { play, enabled: vm.enabled, tag: vm.tag, charge: vm.charge, tier: vm.tier };
  }, await anchorHero(pageM, 90, 90));
  await sleep(200);
  await pageM.screenshot({ path: join(OUT, "mobile.png") });
  await pageM.evaluate(() => window.__dev.controlHarvest({ clearControl: true }));
  ok("L12 build/boot MÓVIL 414x820: monta __dev, 5 clases jugables, llega a play, enabled:true, pack sometido ⇒ T2/charge2/tag ⊗, 0 JS err",
     mobile.play && mobile.enabled === true && mobile.tier === 2 && mobile.charge === 2 && mobile.tag === "⊗" && errM.length === 0,
     `play=${mobile.play} enabled=${mobile.enabled} tier=${mobile.tier} charge=${mobile.charge} tag="${mobile.tag}" errM=${errM.length}`);

  // ═══ L13 — sin errores JS ni 404 (no-favicon) en toda la corrida (cliente A + B + C + móvil) ═══
  ok("L13 ★ 0 JS ERROR: sin errores JS ni 404 (no-favicon) en toda la corrida (A=0 + B=0 + C=0 + M=0)",
     errA.length === 0 && errB.length === 0 && errC.length === 0 && errM.length === 0 && net404.length === 0,
     `A=${errA.length} B=${errB.length} C=${errC.length} M=${errM.length} 404=${net404.length} ${errA.concat(errB, errC, errM).slice(0, 3).join(" | ")}`);
...
console.log(`\n${FAIL === 0 ? "ALL PASS" : "SOME FAIL"}  ${PASS}/${PASS + FAIL}  (LIVE ${LIVE}, CONTROL_HARVEST_SURGE.enabled:true, build ${EXPECT_BUILD}, 2-cliente+móvil, EVO#85 27º flag)`);
