// CAS-2193 — AD→GE hero handoff: CONFIG-DRIVEN class slicer (generalizes
// tools/cas2193-hero-slice.mjs, which was warrior-hardcoded). Slices any PixelLab
// character's walk/attack animation frames + neutral rotation into GAME-READY
// horizontal strips matching render/sprites.js CLS:
...
// so walk/attack/idle stay coherent — no per-frame jitter, feet bottom-anchored, transparent.
//
// Usage:  node tools/cas2193-class-slice.mjs tools/cas2193-cfg/<class>.json
// Config JSON shape:
// { "class":"mage",
//   "base":"https://backblaze.pixellab.ai/file/pixellab-characters/<proj>/<charId>/",
//   "anim": { "walk":   {"down":["<uuid>",6],"up":["<uuid>",6],"side":["<uuid>",6]},
//             "attack": {"down":["<uuid>",8],"up":["<uuid>",8],"side":["<uuid>",8]} } }
// (idle is auto-derived as a 2-frame breathing bob from the rotation image — 0 gens.)
...
const DIRS={ down:"south", up:"north", side:"east" }; // game dir -> pixellab dir key
...
(async()=>{
  const cfgPath=process.argv[2]; if(!cfgPath){ console.error('usage: node tools/cas2193-class-slice.mjs <config.json>'); process.exit(1); }
  const cfg=JSON.parse(fs.readFileSync(cfgPath,'utf8'));
  const CLASS=cfg.class, BASE=cfg.base, ANIM=cfg.anim;
  const animURL=(dir,uuid,i)=>`${BASE}animations/${uuid}/${DIRS[dir]}/${i}.png`;
  const rotURL=(dir)=>`${BASE}rotations/${DIRS[dir]}.png`;
  console.log(`CAS-2193 slice ${CLASS} → assets/class/  (config ${cfgPath})`);
  // 1) download every source frame once
  const store={}, rot={};
  for(const dir of Object.keys(DIRS)){
    for(const state of Object.keys(ANIM)){
      const [uuid,fc]=ANIM[state][dir]; const arr=[];
      for(let i=0;i<fc;i++){ arr.push(decodePNG(await dl(animURL(dir,uuid,i)))); }
      store[`${state}_${dir}`]=arr;
    }
    rot[dir]=decodePNG(await dl(rotURL(dir)));
  }
  // 2) Scale from the BODY (walk + rotation) so the standing silhouette fills the frame;
  //    the attack swing only dictates FW so it never clips.
