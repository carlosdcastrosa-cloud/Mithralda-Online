// ===========================================================================
// CAS-1938 — LIVE holistic regression: 21 Souls-like pillars WIRED in the served
// build (build 9b369bb51869 / 799 files). Boots the canonical gh-pages URL,
...
// the full 21-pillar kit didn't regress off the config in the deploy under test.
// PASS x2 (desktop + mobile).
//
//   node tools/cas1938-pillars-live-qa.mjs [URL]
//   default URL = https://carlosdcastrosa-cloud.github.io/Mithralda-Online/
//
// Delta vs cas1932 (20-pillar, build 7d47bd4f163f):
//   - EXPECT_BUILD -> 9b369bb51869 (Pilar 21 STATUS_BUILDUP deploy, latest gh-pages overlay)
//   - +STATUS_BUILDUP (#21; sig: enabled + decayPerSec>0 (barras decaen) + bossBuildMul<1 (jefes acumulan
//     más lento => NO one-shot) + elementMap{burn:poison,slow:frost} + bleed(procPctHp>bossProcPctHp =>
//     paridad héroe<->enemigo pero jefes reciben ráfaga MENOR) + poison(procDot DoT) + frost(procSlow+procStamDrain)).
//   - two profiles (desktop 1280x720 dsf1 + mobile 390x844 dsf2 touch) => PASS x2 wiring.
// The live served config (build 9b369bb51869) carries all 21 pillars; this run imports
...
const EXPECT_BUILD = "9b369bb51869";
...
// The 21 pillars + core knobs. `en` = the property that gates the feature (most
...
const PILLARS = [
  ...
  // #21: acumulación de estados (1er sistema de PROFUNDIDAD) — enabled + decayPerSec>0 (barras bleed/poison/frost
  //   DECAEN si no se sostienen) + bossBuildMul<1 (jefes/élites acumulan MÁS lento => umbral efectivo mayor =>
  //   NO one-shot jefes) + elementMap reconvierte {burn->poison, slow->frost} + 3 estados con procs FEEL distintos:
  //     bleed  fed físico melee, proc=ráfaga %maxHp con bossProcPctHp < procPctHp (paridad héroe<->enemigo pero
  //            los jefes reciben una ráfaga MENOR => headline observable sin one-shot);
  //     poison fed fuego/veneno reconvertido, proc=DoT (procDot.dmg>0, dur>0);
  //     frost  fed frost-buff/hielo, proc=slow (procSlow.amt>0) + drena estamina (procStamDrain>0).
