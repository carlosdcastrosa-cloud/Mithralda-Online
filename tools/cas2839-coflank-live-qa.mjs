// CAS-2839 — POST-FLIP LIVE QA for TENAZA (CO_FLANK_SURGE.enabled:TRUE LIVE, EVO#145, 87º flag). Base master bdbb009 (322b7ab DARK → 548a31c flip false→true → bdbb009 version stamp 251653b6d194/815), served LIVE carlosdcastrosa-cloud.github.io/Mithralda-Online.
// Derivado del DARK self-verify cas2835 con: ck2 INVERTIDO LIVE-ON (enabled TRUE, solo boot ⇒ F≤1 ⇒ score/tier/charge colapsan LIMPIO), ck14 CENSUS INVERTIDO (73/73 enabled:true off=[]), + ck18 SERVED-LIVE-BUILD (root200 + version.json 251653b6d194/815 + served config CO_FLANK_SURGE enabled:true + census 73/73). Gameplay/determinismo corre contra checkout local NO-DRIFT (HEAD==origin==bdbb009, byte-idéntico a lo servido); ck18 verifica los BYTES SERVIDOS.
// (A) EJE = TENAZA/CO-FLANK = F = MÁX sobre los mobs de (nº de SECTORES angulares DISTINTOS ocupados por los jugadores VIVOS que apuntan a ese mob, dentro del radio 300px) = el mayor grado de envolvimiento/pinza sobre UN objetivo. Sector por SIGNO ENTERO `(sign(dx)+1)*3+(sign(dy)+1)` ∈{0..8}; F = popcount de la máscara OR de sectores ocupados. ⊥ #144 DIANA (CONTEO — GEOMETRÍA ⊥ CONTEO), ⊥ #108 FLANK_SURGE (float MOB-side), ⊥ #142 PIÑA, ⊥ #112 CONVERGE. 🔑 DETERMINISMO (sev-1): F = popcount de máscara ENTERA (CERO atan2/float; OR CONMUTATIVO ⇒ orden-independiente) vs umbrales ENTEROS {midFlank2,hiFlank3}. 🔑 MULTIJUGADOR-NATIVO: single-player ⇒ ≤1 sector ⇒ F colapsa ⇒ 0.
// (B) CANAL = coFlankFind (fichas de tenaza por rematar con el envolvimiento ACREDITADO). Moneda FRESCA (h.coFlankBounty, TRANSITORIA, fuera del save allowlist + worldFingerprint).
//
// Run: node tools/cas2839-coflank-live-qa.mjs
...
const OUT = join(ROOT, "shots", "cas2839");
...
const FPARG = 397;
// flankProbe LUT esperada: (sectors,players)→F=clamp(sectors,0,min(players,9))→banda(F vs {midFlank2,hiFlank3,minPlayers2} ENTEROS)→{weight,rankTier}.
const EXPECT_FLK = [
  { name: "solo-P1",            sectors: 1, players: 1, F: 1, w: 0 },
  { name: "duo-P2",             sectors: 2, players: 2, F: 2, w: 1 },
  { name: "duo-of-4",           sectors: 2, players: 4, F: 2, w: 1 },
  { name: "trio-P3",            sectors: 3, players: 3, F: 3, w: 2 },
  { name: "surround-4",         sectors: 4, players: 4, F: 4, w: 2 },
  { name: "none",               sectors: 0, players: 3, F: 0, w: 0 },
  { name: "bunched-P4",         sectors: 1, players: 4, F: 1, w: 0 },
  { name: "clamp-players",      sectors: 9, players: 3, F: 3, w: 2 },
  { name: "clamp-sectors",      sectors: 12, players: 12, F: 9, w: 2 },
];
// driveFlank REAL: roster sintético (offset MOB-céntrico {dx,dy}+targetId, opcional dead) ⇒ F server-auth = MÁX popcount de máscara de sectores por targetId entre jugadores VIVOS DENTRO del radio (300px). filas: [dx, dy, targetId, dead].
const EXPECT_DRIVE = [
  { name: "opposite-duo",       players: [[50, 0, 7, false], [-50, 0, 7, false]],                       F: 2, w: 1 },
  { name: "same-sector-duo",    players: [[50, 0, 7, false], [60, 0, 7, false]],                        F: 1, w: 0 },
  { name: "solo",               players: [[50, 0, 7, false]],                                           F: 1, w: 0 },
  { name: "trio-surround",      players: [[50, 0, 7, false], [-50, 0, 7, false], [0, 50, 7, false]],    F: 3, w: 2 },
  { name: "trio-same-sector",   players: [[50, 0, 7, false], [55, 0, 7, false], [60, 0, 7, false]],     F: 1, w: 0 },
  { name: "notarget-excluded",  players: [[50, 0, 7, false], [-50, 0, 7, false], [0, 50, 0, false]],    F: 2, w: 1 },
  { name: "dead-excluded",      players: [[50, 0, 7, false], [-50, 0, 7, false], [0, 50, 7, true]],     F: 2, w: 1 },
  { name: "far-excluded",       players: [[50, 0, 7, false], [-50, 0, 7, false], [400, 0, 7, false]],   F: 2, w: 1 },
  { name: "quad-surround",      players: [[50, 0, 7, false], [-50, 0, 7, false], [0, 50, 7, false], [0, -50, 7, false]], F: 4, w: 2 },
  { name: "max-of-groups",      players: [[50, 0, 7, false], [55, 0, 7, false], [40, 40, 9, false], [-40, 40, 9, false]], F: 2, w: 1 },
];
...
try {
  ...
  // 2 LIVE-ON fresh boot (INVERTED from DARK): CO_FLANK_SURGE.enabled TRUE; solo hero fresh boot, SIN party sintética ⇒ F≤1 < midFlank2 ⇒ SCORE/DECISIÓN colapsan LIMPIO. Multijugador-nativo: IMPOSIBLE rodear en solitario. 🔑 NO asertamos gExists/partyExists false (con enabled:true PUEDEN materializarse) — aserta el COLAPSO DE DECISIÓN.
  const dark = await page.evaluate(() => window.__dev.coFlank());
  ok("2 LIVE-ON (fresh solo boot): CO_FLANK_SURGE.enabled TRUE AND sin pinza (F≤1) ⇒ score/tier/charge/preview colapsan LIMPIO a 0 AND channel coFlankFind AND tag vacío",
     dark.enabled === true && dark.tier === 0 && dark.score === 0 && dark.flank <= 1 && dark.charge === 0 && dark.forageChargePreview === 0 && dark.channel === "coFlankFind" && dark.tag === "",
  ...
  // 3 save has no coFlankFind/coFlankBounty key
  ...
  ok("3 byte-id save: sin clave 'coFlankFind'/'coFlankBounty' (fichas transitorias, fuera del save allowlist)", noFeatKey && noChargeKey, `noFeatKey=${noFeatKey} noChargeKey=${noChargeKey} len=${saveOff.length}`);
  ...
  // 6 ★ CRUX (PRIMARIO) ⊥#144 DIANA (CONTEO ⊥ GEOMETRÍA) — REAL server-auth
  ...
  ok("6 ★ CRUX (PRIMARIO) ⊥#144 DIANA (CONTEO ⊥ GEOMETRÍA, REAL server-auth): 3 sobre el MISMO mob AMONTONADOS en un flanco ⇒ DIANA G3/w2 pero TENAZA F1/w0; 3 sobre el MISMO mob desde 3 sectores ⇒ DIANA G3/w2 (IDÉNTICO) pero TENAZA F3/w2 — DIANA es CIEGA a la geometría que TENAZA resuelve (mismo G, F divergente)",
  ...
  // 6f ★ CRUX ⊥#108 FLANK_SURGE (mismo nombre-raíz, mecánica OPUESTA): canal coFlankFind ⊥ flankFind
  ...
  ok("6f ★ CRUX ⊥#108 FLANK_SURGE (canal/lado/métrica DISTINTOS): CO_FLANK_SURGE.channel='coFlankFind' (VM idem) ⊥ FLANK_SURGE.channel='flankFind' — TENAZA = # sectores ENTERO de JUGADORES ⊥ FLANK_SURGE = concentración angular float de MOBS",
  ...
  const degen = await page.evaluate((args) => {
    ...
    const mp = drive([[50, 0, 7], [-50, 0, 7], [0, 50, 7]]);
    window.__dev.coFlank({ clearFlank: true });
    window.__dev.coFlank({ enabled: false });
    return { sp, mp };
  }, { Z });
  ...
  // 9b ★ ORDER-INDEPENDENCE del OR de máscaras
  const orderInv = await page.evaluate((args) => {
    ...
    const a = g([[50, 0, 7], [-50, 0, 7], [0, 50, 7]]);
    const b = g([[0, 50, 7], [50, 0, 7], [-50, 0, 7]]);
  ...
  // 10 CANAL coFlankFind
  const forage = await page.evaluate((args) => {
    const { Z } = args;
    window.__dev.coFlank({ enabled: true });
    window.__dev.coFlank({ tp: { tx: Z.forest[0], ty: Z.forest[1] } });
    window.__dev.coFlank({ driveFlank: { players: [{ dx: 50, dy: 0, targetId: 7 }, { dx: -50, dy: 0, targetId: 7 }, { dx: 0, dy: 50, targetId: 7 }], wipe: true } });
    const actVm = window.__dev.coFlank();
    const actPrev = actVm.forageChargePreview, actCharge = actVm.charge;
    window.__dev.coFlank({ driveFlank: { players: [{ dx: 50, dy: 0, targetId: 7 }, { dx: 55, dy: 0, targetId: 7 }], wipe: true } });
  ...
  const capChk = await page.evaluate(() => {
    const vals = [];
    for (let s = 3; s <= 9; s++) vals.push(window.__dev.coFlank({ flankProbe: { sectors: s, players: s } }).flankProbe.charge);
  ...
  // 14 ★ 0-REGRESSION CENSUS (INVERTED LIVE-ON): served config — 73 `_SURGE` totales, 73 enabled:true, off=[] (CO_FLANK #145 LIVE + los 72 priores).
  ...
  const censusOK = total === 73 && trues === 73 && falses.length === 0 && coFlankLive && coFlankLive[1] === "true" && coFocusLive && coFocusLive[1] === "true";
  ok("14 ★ LIVE-ON 0-REGRESIÓN CENSUS (INVERTED): served config 73 `_SURGE` totales, 73 enabled:true, off=[] (CO_FLANK_SURGE #145 LIVE + CO_FOCUS_SURGE #144 LIVE + los 71 priores)",
     censusOK, `total=${total} true=${trues} false=${JSON.stringify(falses)} coFlank=${coFlankLive ? coFlankLive[1] : "?"} coFocus=${coFocusLive ? coFocusLive[1] : "?"}`);

  // 15 render badge "Tenaza:" drawn ON+surround / not OFF + fps
  ...
  const readVM = async (pg) => await pg.evaluate((args) => {
    ...
    window.__dev.coFlank({ enabled: true });
    window.__dev.coFlank({ tp: { tx: Z.forest[0], ty: Z.forest[1] } });
    const dv = window.__dev.coFlank({ driveFlank: { players: [{ dx: 50, dy: 0, targetId: 7 }, { dx: -50, dy: 0, targetId: 7 }, { dx: 0, dy: 50, targetId: 7 }], wipe: true } }).driveFlank;
  ...
  // 18 ★ SERVED LIVE BUILD (gh-pages, no publish lag): root 200 + version.json == 251653b6d194/815 (NEW ≠ old a210d646fa96) + served sim/config.js CO_FLANK_SURGE enabled:true + census 73/73 off=[].
  try {
    ...
    const servedOK = rootRes.status === 200 && ver.build === "251653b6d194" && ver.files === 815 && lTotal === 73 && lTrues === 73 && lFalses.length === 0 && lCoFlank && lCoFlank[1] === "true" && lCoFocus && lCoFocus[1] === "true";
    ok("18 ★ SERVED LIVE BUILD (gh-pages): root 200 + version.json 251653b6d194/815 (NEW ≠ old a210d646fa96) + served sim/config.js CO_FLANK_SURGE enabled:true + census 73/73 off=[]",
       servedOK, `rootStatus=${rootRes.status} build=${ver.build} files=${ver.files} census total=${lTotal} true=${lTrues} false=${JSON.stringify(lFalses)} coFlank=${lCoFlank ? lCoFlank[1] : "?"} coFocus=${lCoFocus ? lCoFocus[1] : "?"}`);
  } catch (e) {
    ok("18 ★ SERVED LIVE BUILD (gh-pages): root 200 + version.json 251653b6d194/815 + config CO_FLANK_SURGE enabled:true", false, `FETCH ERROR ${String(e)}`);
  }
