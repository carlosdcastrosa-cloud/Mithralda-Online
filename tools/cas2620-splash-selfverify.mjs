// CAS-2620 — self-verify for REMATE DE ESTALLIDO (DARK, SPLASH_SURGE.enabled:false). EVO mecánica #104 (serializa tras #103 GOLD_SURGE LIVE&served 3348f9ede902/815) — EJE FRESCO + CANAL FRESCO, ⊥ a las 45 LIVE #59-#103.
...
try {
  ...
  ok("2 byte-id OFF (fresh boot): SPLASH_SURGE.enabled false AND G.blastBounty NUNCA se crea (gExists false)",
     dark.enabled === false && dark.gExists === false && dark.tier === 0 && dark.score === 0 && dark.charge === 0 && dark.forageChargePreview === 0 && dark.channel === "splashFind" && dark.tag === "",
  ...
  const splashDark = flag("SPLASH_SURGE") === "false";
  ok("14 ★ 0-REGRESIÓN: 45 mecanismos del arco #59-#103 served enabled:true; SPLASH_SURGE served false (DARK #104)",
     arcAllOn && splashDark && arc.length === 45, `splash=${flag("SPLASH_SURGE")} arcAllOn=${arcAllOn} n=${arc.length} off=${JSON.stringify(arcLive.filter(([, v]) => v !== "true"))}`);
