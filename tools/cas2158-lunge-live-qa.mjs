// ---------------------------------------------------------------------------
// CAS-2158 QA OBSERVABLE — LUNGE / Estocada de Avance (mec #40) DARK.
// Live DARK build 6e32a8af63e0 (gh-pages), served blobs byte-verified == HEAD:
//   sim/config.js  f1eca9c0447cd560913c748477f20970
...
// cohesion audit v5.
//
//   AC0  [META]   served config DARK: {enabled:false, distance:118, dashMs:150,
//                 range:60, arcDeg:70, dmgMul:1.3, staminaCost:16, recoverMs:520,
//                 key:"Backslash"}.
...
//
// Run: node tools/cas2158-lunge-live-qa.mjs   (PASS×2 = invoke twice)
// ---------------------------------------------------------------------------
import puppeteer from "puppeteer-core";
import { findChromium, LAUNCH_ARGS } from "./harness.mjs";
import { mkdirSync } from "fs";
const BASE = "https://carlosdcastrosa-cloud.github.io/Mithralda-Online/";
const EXPECT_BUILD = "6e32a8af63e0";
const OUT = "shots/cas2158";
...
for (const prof of PROFILES) {
  ...
  // AC0 META: served LUNGE knob DARK with exact values
  const meta = await page.evaluate(() => JSON.parse(JSON.stringify(window.__h.cfg.LUNGE)));
  const metaOk = meta.enabled === false && meta.distance === 118 && meta.dashMs === 150 && meta.range === 60
    && meta.arcDeg === 70 && meta.dmgMul === 1.3 && meta.staminaCost === 16 && meta.recoverMs === 520 && meta.key === "Backslash";
  if (metaOk) P(`[${prof.name}] META: served LUNGE DARK — enabled=${meta.enabled}, distance=${meta.distance}, dashMs=${meta.dashMs}, range=${meta.range}, arcDeg=${meta.arcDeg}, dmgMul=${meta.dmgMul}, staminaCost=${meta.staminaCost}, recoverMs=${meta.recoverMs}, key=${meta.key}`);
