// ===========================================================================
// CAS-1754 — LIVE QA: Códice de Botín (Collection Log). Verifies the CAS-1752
...
//
//   node tools/cas1754-codex-live-qa.mjs [URL]
...
const OUT = "shots/cas1754"; fs.mkdirSync(OUT, { recursive: true });
...
async function runOnce(label, viewport, headHashes) {
  ...
  try {
    ...
    await page.evaluateOnNewDocument(() => {
      ...
      try { if (!localStorage.getItem("__cas1754_booted")) {
        ...
        localStorage.setItem("__cas1754_booted", "1"); } } catch (e) {}
      window.__frames = 0;
...
console.log(`CAS-1754 LIVE QA — Códice de Botín @ ${BASE}\nHEAD md5:`, headHashes);
...
console.log(`\n${totalFail === 0 ? "✅" : "❌"} CAS-1754 LIVE QA: ${totalPass} PASS / ${totalFail} FAIL (x2 desktop+mobile)`);
