// CAS-2202: Deploy — quitar el punto amarillo de dirección del héroe (render-only).
// Overlay = 1 blob desde HEAD (render/render.js) sobre gh-pages; resto sin tocar.
// Reversible = revertir el commit + redeploy. Mirror de cas2190-deploy.mjs (multi-blob).
...
function buildAndPush() {
  ...
  const commit = gitStr("commit-tree", tree, "-p", parent, "-m", `CAS-2202: quitar punto amarillo de dirección del héroe (render-only) — overlay 1 blob (render/render.js), reversible — build ${build}\n\nCo-Authored-By: Paperclip <noreply@paperclip.ing>`);
  try {
