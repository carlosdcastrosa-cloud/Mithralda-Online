// ---------------------------------------------------------------------------
// CAS-2109 (QA POST-FLIP for CAS-2105 / mec #33 Guard Counter) — QA HOLISTIC FULL-BUILD REGRESSION vs LIVE
// 868a3cb231ac→f38c34bb1655 after the CEO Gate flip GUARD_COUNTER.enabled false→true (config-only 1 line,
// reversible). sim/sim.js (bbae04a2a8fe1b895f006c612b60ee62) + render/render.js (837161ce…) are BYTE-IDENTICAL
// to the CAS-2108 DARK build; ONLY sim/config.js changed (md5 fbc94a734cf4d7378eea7e95d774ee7a, enabled:true).
// This bumps EXPECT_BUILD→f38c34bb1655 and flips the [GUARDCOUNTER] audit DARK→LIVE (enabled:true). Guard Counter
// is input-timing driven (0 guardCounterRng) ⇒ enabling it adds 0 draws to a normal run ⇒ 0 regression to the
// other 32 mechanics (proven by [DETERM]). LIVE-enabled OBSERVABLE proof → tools/cas2109-guard-counter-postflip.mjs.
//
// (Below banner kept from the CAS-2108 DARK regression this is derived from.)
...
const EXPECT_BUILD = "f38c34bb1655"; // CAS-2109: bumped from 868a3cb231ac after the CEO Gate flip GUARD_COUNTER.enabled false→true (config-only 1 line). Served config.js md5 fbc94a734cf4d7378eea7e95d774ee7a (enabled:true); sim bbae04a2a8fe1b895f006c612b60ee62 + render 837161ce UNCHANGED (byte-id to the DARK build) ⇒ flip is config-only, reversible.
...
function auditGuardCounter() {
  ...
  const live = gc && gc.enabled === true;   // LIVE (CAS-2109 Gate GO flip false→true, config-only, reversible→false)
  const valsOk = gc && gc.windowS === 0.6 && gc.dmgMul === 1.8 && gc.poiseMul === 2.5 && gc.staminaCost === 10;
  if (shapeOk && live && valsOk)
    pass(`[GUARDCOUNTER] GUARD_COUNTER LIVE (33rd mech, CAS-2105 Gate GO) — enabled:TRUE (config-only flip, reversible→false), full knob shape windowS=${gc.windowS}/dmgMul=${gc.dmgMul}/poiseMul=${gc.poiseMul}/staminaCost=${gc.staminaCost} — RNG-neutral (0 guardCounterRng, input-timing only) + save-neutral (h.guardCounterT transient) ⇒ enabling adds 0 draws to a normal run`);
...
function determinism() {
  ...
  if (a === b && !a.startsWith("ERR:")) pass(`[DETERM] sim cycle fingerprint identical across 2 runs (len ${a.length}) — 33 mechanics (Arena Hazards LIVE gate-closed w/o boss + Guard Counter LIVE input-timing, 0 guardCounterRng) add 0 draws to a normal run`);
...
log("");
if (ok) log(`✅ CAS-2109 HOLISTIC FULL-BUILD REGRESSION — ALL PASS (24 systems + 5 go-live + TIME-ATTACK(29) + VARIANTS(30,LIVE) + SEEDED CHALLENGE(31,LIVE) + ARENA HAZARDS(32,LIVE) + GUARD COUNTER(33,LIVE) + NG_PLUS + onboarding + 7 balance + JUICE + determinism on ${EXPECT_BUILD})`);
else { console.error("❌ CAS-2109 HOLISTIC FULL-BUILD REGRESSION — FAIL"); process.exit(1); }
