// CAS-2640 — self-verify for REMATE DE HUESTE DISPERSA (DARK, DISPERSE_SURGE.enabled:false). EVO mecánica #107 (serializa tras #106 MOTLEY_SURGE LIVE&served 08a2a853adb1/815, base HEAD 5d4d2a6) — EJE FRESCO + CANAL FRESCO, ⊥ a las 48 LIVE #59-#106.
...
const OUT = join(ROOT, "shots", "cas2640");
...
try {
  ...
  ok("2 byte-id OFF (fresh boot): DISPERSE_SURGE.enabled false AND G.disperseBounty NUNCA se crea (gExists false)",
     dark.enabled === false && dark.gExists === false && dark.tier === 0 && dark.score === 0 && dark.spread === 0 && dark.charge === 0 && dark.forageChargePreview === 0 && dark.channel === "disperseFind" && dark.tag === "",
  ...
  // 14 ★ 0-REGRESSION: 48 arc flags served true; DISPERSE_SURGE served false
  ...
  const disperseDark = flag("DISPERSE_SURGE") === "false";
  ok("14 ★ 0-REGRESIÓN: 48 mecanismos del arco #59-#106 served enabled:true; DISPERSE_SURGE served false (DARK #107)",
     arcAllOn && disperseDark && arc.length === 48, `disperse=${flag("DISPERSE_SURGE")} arcAllOn=${arcAllOn} n=${arc.length} off=${JSON.stringify(arcLive.filter(([, v]) => v !== "true"))}`);
