// CAS-233 — build a south-facing boss STRIP from PixelLab MCP animation frame URLs.
// Generalizes tools/cas209-build-strip.mjs to any state + frame size so the FOUNTAINS
// Stone Golem boss (d7f5577c) gets walk/idle/attack strips at 96px (preserves the
// cracked-stone + crimson-ember boss detail that 64px would crush).
// usage: node tools/cas233-golem-strip.mjs <state> <fw> <url0> <url1> ...
// writes assets/pixellab/fountains/anim/golem_<state>_strip.png  (N frames × fw×fw)
...
// alpha-aware nearest scale + trim of fully-transparent border; bottom-anchor feet.
// NOTE: trims per-frame bbox — fine for a strip where each frame self-centers; the
// golem's wide silhouette keeps a consistent footprint across frames.
...
(async()=>{
  const state=process.argv[2], FW=parseInt(process.argv[3],10), urls=process.argv.slice(4);
  if(!state||!FW||!urls.length){console.error('usage: node tools/cas233-golem-strip.mjs <state> <fw> <url0> ...');process.exit(1);}
  const frames=[];
  ...
  const out=`assets/pixellab/fountains/anim/golem_${state}_strip.png`;
  fs.writeFileSync(out,encodePNG(FW*N,FW,strip));
  console.log(`✓ golem ${state}: ${N} frames @ ${FW}px → ${out} (${FW*N}x${FW})`);
})();
