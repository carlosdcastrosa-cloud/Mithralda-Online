...
const EXPECT_BUILD = "cdf6e8bdcbea";
...
for (const prof of PROFILES) {
  ...
  const m = meta.m;
  const valsOk = m.enabled === false && m.dmgMul === 2.2 && m.poiseMul === 1.5 && m.essenceBonus === 2 && m.ripCapFracMaxHp === 0.25 && m.requiresMelee === true;
  if (valsOk && meta.cfgEnabled === false)
    P(`[${prof.name}] META: served RIPOSTE DARK — enabled=false, dmgMul=${m.dmgMul}, poiseMul=${m.poiseMul}, essenceBonus=${m.essenceBonus}, ripCapFracMaxHp=${m.ripCapFracMaxHp}, requiresMelee=${m.requiresMelee} (exact ticket AC1)`);
...
await browser.close();
if (anyFail) { console.error("\n✖ CAS-2131 riposte OBSERVABLE: FAIL"); process.exit(1); }
console.log("\n✅ CAS-2131 riposte (Ejecución Crítica por Aturdimiento, mec #36 DARK) OBSERVABLE: ALL PASS (desktop+mobile)");
