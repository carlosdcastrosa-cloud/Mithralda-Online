// CAS-2320 — POST-FLIP LIVE OBSERVABLE + 2-CLIENTE QA — COMPAÑEROS DE RUTA / WAYFARERS' FELLOWSHIP (FELLOWSHIP_BOND.enabled:true LIVE)
// Capa social PERSONAL: una BANDA semanal COMPARTIDA (roster pura del reloj, IDÉNTICA para todo el shard) + un VÍNCULO PERSONAL
// (kills − snapshot, aislado por héroe) que al FORJARse da un pasivo +10% al canal xpGain. EVO mecánica #47, hilo social nuevo.
//
// LIVE post-flip via CTO CAS-2317 (master fb590b9): FELLOWSHIP_BOND.enabled false→true, overlay consistente-HEAD 4-file
// {config,sim,game,render} SIN input.js, served build 63b3dee81076 (≡ master fb590b9 ≡ gh-pages 4df1977). Este harness es el
// gemelo LIVE de la DARK observable tools/cas2316-fellowship-observable-qa.mjs: MISMOS hechos, pero contra el build SERVIDO en
// gh-pages (carlosdcastrosa-cloud.github.io/Mithralda-Online), con el patrón DEFAULT-ON (el flip ya cargó: enabled:true SERVIDO +
// banda real-clock derivada del reloj) en vez del DARK-default byte-id OFF (que aquí se re-verifica vía TOGGLE).
//
// North Star (check 12, no-negociable) = CONVERGENCIA 2-CLIENTE REAL LIVE: DOS páginas puppeteer independientes (dos "jugadores"),
...
// que el VÍNCULO es PERSONAL: A profundiza su vínculo (mata 40 ⇒ bond/tier/∞ suben) ⇒ su banda queda INTACTA (pura del reloj) y NI la
// banda NI el vínculo de B se tocan (contribución per-hero AISLADA, 0 contención). Corre AL FINAL (abrir la 2ª página blurea la 1ª).
//
// Checks:
//   1  boot LIMPIO LIVE + __dev.fellowship + arc hooks + __BUILD self-consistente vs version.json + 0 err / 0 non-favicon 404.
//   2  served sim/config.js: FELLOWSHIP_BOND.enabled:true SERVIDO + arco entero enabled:true (0 regr); 3 DARK unrelated false.
//   3  DEFAULT-ON (la prueba LIVE): boot fresco 0 __dev flip ⇒ fellowship().enabled===true AND banda real-clock non-empty AND
//      bond 0 (INERTE hasta matar) AND forged false AND tag "" (el flip cargó, pero el pasivo NO se activa sin vínculo forjado).
//   4  byte-id when OFF (toggle): fellowship({enabled:false}) ⇒ saveBlob() sin clave 'fellowAt' AND forged false AND fellowMulXp 0
//      AND band [] AND tag "" AND worldFingerprint byte-stable ⇒ reversible sin drift; restaura ON. (NO se afirma gExists/hasField:
//      DEFAULT-ON el tickFellowship ya snapshoteó ⇒ transitorios true — footgun heredado de Standings CAS-2309.)
//   5  BANDA determinista + convergencia 1-página: mismo nowMs ⇒ banda de `size` miembros con nombre, IDÉNTICA en 2 lecturas (0 desync).
...
//   9  FORJAR gate + pasivo servido + KIND ÚNICO: bond<forge ⇒ forged false/mul 0; bond≥forge ⇒ forged true/mul 0.10; served sim.js aplica
//      fellowMul(h,"xpGain") en gainXP; con TODO el arco ON el xpGainMul forjado ≈ off×1.10 EXACTO (canal xpGain aislado, 0 doble-conteo).
//  10  render + nameplate ∞: render.js SERVIDO dibuja sim.wayfarerFellowship() ('Compañeros de Ruta') gated FELLOWSHIP_BOND.enabled +
//      glifo ∞ via sim.fellowshipTag (autoritativo "" OFF / "∞" forjado ON) + screenshot del nameplate forjado.
//  11  arco regr full-stack con FELLOWSHIP ON + fps no-regresión (mediana-de-5, ON ≥ OFF*0.9).
//  12  ★ NORTH STAR — CONVERGENCIA 2-CLIENTE REAL LIVE: 2 páginas, MISMO nowMs ⇒ BANDA (roster COMPARTIDO) IDÉNTICA byte-a-byte; A
//      profundiza vínculo (mata 40 ⇒ bond/∞/Jurado) ⇒ banda de A INTACTA (pura del reloj) AND banda+vínculo de B INTACTOS (aislado). (AL FINAL)
...
// Run: node tools/cas2320-fellowship-live-observable-qa.mjs   [optional LIVE base url]
...
const OUT = join(ROOT, "shots", "cas2320-live");
...
try {
  ...
  // 2 served config: FELLOWSHIP_BOND.enabled:true SERVIDO + arco entero true; 3 DARK unrelated false
  ...
  ok("2 served config: FELLOWSHIP_BOND.enabled:true SERVIDO + arco entero enabled:true (0 regr); 3 DARK unrelated (BOSS_RUSH/DOORS_INTERIORS/SEEDED_CHALLENGE) false",
     cfg.FELLOWSHIP === "true" && allArcTrue && darksFalse, JSON.stringify(cfg));

  // 3 DEFAULT-ON (la prueba LIVE): boot fresco 0 __dev flip ⇒ enabled true + banda real-clock non-empty + bond 0 (INERTE) + forged false + tag ""
  const dOn = await page.evaluate(() => { const f = window.__dev.fellowship(); return { enabled: f.enabled, bandLen: (f.band || []).length, size: f.size, bond: f.bond, forged: f.forged, mul: f.fellowMulXp, tag: f.tag }; });
  ok("3 DEFAULT-ON desde config servido: fellowship().enabled===true AND banda real-clock non-empty AND bond 0 (INERTE hasta forjar) AND forged false AND fellowMulXp 0 AND tag \"\" (flip cargó; pasivo NO activo sin vínculo)",
     dOn.enabled === true && dOn.bandLen > 0 && dOn.bandLen === dOn.size && dOn.bond === 0 && dOn.forged === false && dOn.mul === 0 && dOn.tag === "", JSON.stringify(dOn));

  // 4 byte-id when OFF (toggle): enabled:false ⇒ save sin 'fellowAt' + forged false (pasivo apagado) + mul 0 + tag "" + fp byte-stable; restaura ON.
  //   FOOTGUN DEFAULT-ON (mirror Standings CAS-2309 hasField / Ledger CAS-2302): el tickFellowship ya corrió con el reloj real ⇒ el ROSTER
  //   `band` queda POBLADO (readout transitorio, no estado); toggle-OFF NO lo limpia. La OFF-safety NO es "band []", es que NADA del canal
  //   xpGain/nameplate/save/fingerprint cambie: forged false + mul 0 + tag "" + save SIN 'fellowAt' + fingerprint byte-estable.
  const byteId = await page.evaluate(() => {
    ...
    window.__dev.fellowship({ enabled: true });                                  // restaura ON (DEFAULT-ON servido)
    return { enabled: f.enabled, forged: f.forged, mul: f.fellowMulXp, bandLen: (f.band || []).length, tag: f.tag,
      noKey: !/"fellowAt"/.test(saveOff), fpStable: fpOff === fpOff2 };
  });
  ok("4 byte-id when OFF (toggle): enabled:false ⇒ saveBlob() sin clave 'fellowAt' AND forged false AND fellowMulXp 0 AND tag \"\" AND worldFingerprint byte-stable (reversible sin drift; el roster band poblado es readout transitorio DEFAULT-ON, NO estado — footgun Standings)",
     byteId.enabled === false && byteId.forged === false && byteId.mul === 0 && byteId.tag === "" && byteId.noKey === true && byteId.fpStable === true, JSON.stringify(byteId));
  ...
  // 9 FORJAR gate + pasivo servido + KIND ÚNICO
  ...
  ok("9 FORJAR gate + pasivo servido + KIND ÚNICO: bond<forge ⇒ forged false/mul 0; bond≥forge ⇒ forged true/mul 0.10; served sim.js aplica fellowMul(h,'xpGain') en gainXP AND con TODO el arco ON xpGainMul forjado ≈ off×1.10 EXACTO (canal aislado, 0 doble-conteo)",
     forge.belowForged === false && forge.belowMul === 0 && forge.atForged === true && near(forge.atMul, 0.10) && seamWired && near(xpMul.on, xpMul.off * 1.10, 1e-3),
     `forge=${JSON.stringify(forge)} wired=${seamWired} xp=${JSON.stringify(xpMul)}`);
  ...
  await pageB.close();
  ok("12 ★ NORTH STAR CONVERGENCIA 2-CLIENTE REAL LIVE: 2 páginas, MISMO nowMs ⇒ BANDA (roster COMPARTIDO) IDÉNTICA byte-a-byte; A profundiza vínculo (mata 40 ⇒ bond/∞/Jurado) ⇒ banda de A INTACTA (pura del reloj) AND banda+vínculo de B INTACTOS (per-hero aislado, 0 desync)",
     bandShared && bondPersonal0 && aDeepened && aBandIntact && bIntact && errB.length === 0 && net404B.length === 0,
     `bandShared=${bandShared} bondPersonal0=${bondPersonal0} aDeepened=${aDeepened} aBandIntact=${aBandIntact} bIntact=${bIntact} A0=${JSON.stringify(a0)} B0=${JSON.stringify(b0)} A1=${JSON.stringify(a1)} B1=${JSON.stringify(b1)} errB=${errB.length} 404B=${net404B.length}`);

  await page.bringToFront();
  await page.evaluate(() => window.__dev.fellowship({ enabled: true }));         // deja el estado DEFAULT-ON servido
