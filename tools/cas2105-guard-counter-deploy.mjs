// CAS-2105 (CAS-2106): Deploy DARK — Guard Counter / Contragolpe de Guardia (mec #33).
// Overlay = sim/config.js + sim/sim.js (render.js unchanged). GUARD_COUNTER.enabled:false DARK/inert.
// Reversible: Gate CEO flips enabled:false→true (config-only 1 line).
...
function buildAndPush() {
  ...
  const commit = gitStr("commit-tree", tree, "-p", parent, "-m",
    `CAS-2105 (CAS-2106): Deploy DARK Guard Counter mec #33 — 2-blob overlay (sim/config.js, sim/sim.js), GUARD_COUNTER.enabled:false, reversible — build ${build}`);
  try {
