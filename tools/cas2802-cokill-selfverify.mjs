// CAS-2802 — self-verify for CUADRILLA (DARK, CO_KILL_SURGE.enabled:false). EVO mecánica #138 (base master be102e9 tras #137 COHORTE CO_PRESENCE LIVE&served d2fe18611bd2/815) — EJE FRESCO + CANAL FRESCO, ⊥ a los 79 LIVE #59-#137. El 20º eje de COMPOSICIÓN-DE-INTENCIÓN y la TERCERA faceta de la sub-familia PLAYER-COORDINATION (#136 CÓNCLAVE=ENGANCHADOS, #137 COHORTE=PRESENTES).
...
//
// Run: node tools/cas2802-cokill-selfverify.mjs
...
const OUT = join(ROOT, "shots", "cas2802");
...
try {
  ...
  ok("2 byte-id OFF (fresh boot): CO_KILL_SURGE.enabled false AND G.coKillBounty NUNCA se crea (gExists false) AND G._coKillParty NUNCA se crea (partyExists false)",
     dark.enabled === false && dark.gExists === false && dark.partyExists === false && dark.tier === 0 && dark.score === 0 && dark.idx === 0 && dark.assist === 0 && dark.charge === 0 && dark.forageChargePreview === 0 && dark.channel === "coKillFind" && dark.tag === "",
  ...
  const censusOK = total === 66 && trues === 65 && falses.length === 1 && falses[0] === "CO_KILL_SURGE" && coPresLive && coPresLive[1] === "true";
  ok("14 ★ 0-REGRESIÓN CENSUS: served config 66 `_SURGE` totales, 65 enabled:true (incl. CO_PRESENCE_SURGE #137 LIVE), sole-false = CO_KILL_SURGE (DARK #138)",
     censusOK, `total=${total} true=${trues} false=${JSON.stringify(falses)} coPresence=${coPresLive ? coPresLive[1] : "?"}`);
