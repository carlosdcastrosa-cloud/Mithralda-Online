// CAS-2648 — POST-FLIP LIVE QA for REMATE DE FALANGE (LIVE, FLANK_SURGE.enabled:true). EVO mecánica #108 = 50º flag LIVE&served 329b4a967c2d/815 (flip dedf6a5 + stamp 24b0e27). Flag-ON port of DARK harness CAS-2645 (fp 15920977 0-desync) — 2 assertions flipped (check2 enabled:true STATELESS, check14 50 flags served true off=[]). EJE FRESCO + CANAL FRESCO, ⊥ a las 49 LIVE #59-#107.
...
const OUT = join(ROOT, "shots", "cas2648-live");
...
try {
  ...
  // 2 LIVE fresh boot: enabled TRUE (50º flag LIVE) + STATELESS (flankBounty transitorio, derivado ⇒ gExists false, empty-field score 0)
  const dark = await page.evaluate(() => window.__dev.flank());
  ok("2 LIVE (fresh boot): FLANK_SURGE.enabled TRUE (50º flag LIVE) AND estado STATELESS (gExists false: flankBounty transitorio derivado; empty-field score/conc 0)",
     dark.enabled === true && dark.gExists === false && dark.tier === 0 && dark.score === 0 && dark.conc === 0 && dark.charge === 0 && dark.forageChargePreview === 0 && dark.channel === "flankFind" && dark.tag === "",
  ...
  const flankLive = flag("FLANK_SURGE") === "true";
  ok("14 ★ 0-REGRESIÓN: 49 mecanismos del arco #59-#107 served enabled:true; FLANK_SURGE served TRUE (LIVE #108, 50º flag) ⇒ 50 flags served true off=[]",
     arcAllOn && flankLive && arc.length === 49, `flank=${flag("FLANK_SURGE")} arcAllOn=${arcAllOn} n=${arc.length + 1} off=${JSON.stringify(arcLive.filter(([, v]) => v !== "true").concat(flankLive ? [] : [["FLANK_SURGE", flag("FLANK_SURGE")]]))}`);
