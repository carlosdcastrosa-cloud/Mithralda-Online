// CAS-1570 deploy: fold the Active-Abilities feature (2 drafted slots + pool of 4)
// into the canonical gh-pages build. Ships the changed runtime code + the freshly
// stamped version.json (build=6b8ee93efde2).
//
// Safety (CTO blast-radius idiom, same as CAS-1566/CAS-1567): overlay ONLY served
// files whose HEAD-COMMITTED blob differs from what origin/gh-pages serves — reading
// COMMITTED blobs (git rev-parse), never the working tree, so any uncommitted WIP can
...
// Expected deploy scope: the 6 runtime files touched by CAS-1570 + version.json.
const EXPECTED = new Set(["game.js", "input.js", "settings.js", "sim/sim.js", "sim/config.js", "render/render.js", "version.json"]);
...
const idx = "/tmp/cas1570-ghp-index";
...
const msg = `CAS-1570: deploy Active Abilities to gh-pages (build 6b8ee93efde2, HEAD ${headSha.slice(0, 7)})`;
