// CAS-2660 — self-verify for REMATE DE TROPEL DESIGUAL (DARK, SPEED_SURGE.enabled:false). EVO mecánica #111 (serializa tras #110 ORIENT_SURGE LIVE&served 449121cabca6/815, base HEAD fdd87b5) — EJE FRESCO + CANAL FRESCO, ⊥ a las 52 LIVE #59-#110.
...
//
// Run: node tools/cas2660-speed-selfverify.mjs
...
const OUT = join(ROOT, "shots", "cas2660");
...
try {
  ...
  ok("2 byte-id OFF (fresh boot): SPEED_SURGE.enabled false AND G.speedBounty NUNCA se crea (gExists false)",
     dark.enabled === false && dark.gExists === false && dark.tier === 0 && dark.score === 0 && dark.cv === 0 && dark.charge === 0 && dark.forageChargePreview === 0 && dark.channel === "speedFind" && dark.tag === "",
  ...
  // 14 ★ 0-REGRESSION: 52 arc flags served true; SPEED_SURGE served false
  ...
  const speedDark = flag("SPEED_SURGE") === "false";
  ok("14 ★ 0-REGRESIÓN: 52 mecanismos del arco #59-#110 served enabled:true; SPEED_SURGE served false (DARK #111)",
     arcAllOn && speedDark && arc.length === 52, `speed=${flag("SPEED_SURGE")} arcAllOn=${arcAllOn} n=${arc.length} off=${JSON.stringify(arcLive.filter(([, v]) => v !== "true"))}`);
