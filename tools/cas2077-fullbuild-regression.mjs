...
const EXPECT_BUILD = "c852c60d7993"; // CAS-2077: bumped from b5a1f024e42e after CAS-2075 Gate CEO GO — ENCOUNTER_VARIANTS.enabled false→true (config-only flip, config.js:1727). maybeVariant now LIVE: variants roll at chancePerZone (forest/caves/swamp 0.30, abyss 0.25) in the natural spawn loop. Prev CAS-2074 (b5a1, EV DARK deploy).
...
// CAS-2077: ENCOUNTER VARIANTS (30th mechanic) — now LIVE (enabled:true, CAS-2075 Gate CEO GO,
// config.js:1727). maybeVariant rolls at chancePerZone in the natural spawn loop (forest/caves/swamp
// 0.30, abyss 0.25). Behavioral OBSERVABLE proof (each variant EXHIBITS its hook against the running
// sim, unforced) is in tools/cas2071-variants.mjs (node AC1-6) + tools/cas2077-variants-live-natural.mjs
// (browser desktop+mobile, natural spawn path). Here we assert the LIVE go-live contract + chance table.
function auditVariants() {
  ...
  if (shapeOk && live && chanceOk)
    pass(`[VARIANTS] ENCOUNTER_VARIANTS LIVE (30th mech, CAS-2071/2075 GO) — enabled:true, markerLabel:true, rngSeed=0x${(ev.rngSeed >>> 0).toString(16)}, chancePerZone {forest/caves/swamp:0.30, abyss:0.25}, variants=[stalker,bastion,glass], byZone opted-in — natural spawn loop rolls variants`);
...
log("");
if (ok) log(`✅ CAS-2077 HOLISTIC FULL-BUILD REGRESSION — ALL PASS (24 systems + 5 go-live flags + ENCOUNTER_VARIANTS(30th,LIVE enabled:true) + NG_PLUS + onboarding deep + 7 balance + JUICE + determinism on ${EXPECT_BUILD})`);
else { console.error("❌ CAS-2074 HOLISTIC FULL-BUILD REGRESSION — FAIL"); process.exit(1); }
