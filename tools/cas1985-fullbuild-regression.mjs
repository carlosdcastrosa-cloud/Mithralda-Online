// ---------------------------------------------------------------------------
// CAS-1985 — QA FULL-BUILD REGRESSION vs LIVE c960c813843d (post-SUMMON go-live).
// Consolidated composición pass over the 23 shipped systems after the CAS-1980 go-live
// flip that made SPIRIT SUMMON (pilar 23) PLAYABLE (SUMMON.enabled false->true).
//
// DOM-free: the served blobs are md5-identical to HEAD (config·sim·render·input — verified
// out-of-band via curl, in the issue comment), so importing sim/config.js drives the EXACT
// live code. This harness is a delta of tools/cas1938-pillars-live-qa.mjs (21 pillars) that
// (a) retargets EXPECT_BUILD to the go-live build, (b) adds pilar 22 SIGNATURE_BOSS and
// pilar 23 SUMMON, and (c) asserts SUMMON now ships enabled:true (playable, not dark).
// The deep SUMMON observable (spirit fights boss + splits aggro, hero intact) lives in
// tools/cas1965-summon-live-qa.mjs and is run alongside this pass.
//
...
//   [GOLIVE]  SUMMON.enabled === true (pilar 23 no longer dark — real players can invoke it).
//   [DETERM]  a short sim cycle fingerprints identical across 2 runs (input-independent sim).
//
// Run: node tools/cas1985-fullbuild-regression.mjs
// ---------------------------------------------------------------------------
import * as cfg from "../sim/config.js";
import * as sim from "../sim/sim.js";
import { G, update } from "../sim/sim.js";

const EXPECT_BUILD = "c960c813843d";
...
const SYSTEMS = [
  ...
  // #22 SIGNATURE_BOSS — capstone multi-fase (calderatyrant) que ejerce el kit. enabled + boss + 2 fases + reward.
  ...
  // #23 SUMMON — Cenizas de Espíritu (aliado IA invocable). POST-GO-LIVE: enabled MUST be true.
...
async function checkBuild() {
  ...
  } catch (e) { log(`ℹ [BUILD] fetch skipped (${e.message}); md5 served==HEAD 4/4 verified out-of-band (issue comment)`); }
}
...
log("");
if (ok) log("✅ CAS-1985 FULL-BUILD REGRESSION — ALL PASS (23/23 systems live on c960c813843d)");
else { console.error("❌ CAS-1985 FULL-BUILD REGRESSION — FAIL"); process.exit(1); }
