// CAS-2555 — POST-FLIP QA (observable LIVE) para REMATE DE CABECILLA (ROLE_SURGE, EVO#93, flag ON) @ served d3a276a13dc0 / master 7cc64d7.
...
// en producción, 2 clientes 0-desync. El served (gh-pages d3a276a13dc0) == master HEAD 7cc64d7 (byte-verificado
// served game/render/sim/config/index blobs == HEAD) ⇒ sirvo el árbol local = el sitio servido.
//
// EJE LIVE (⊥34): ROL/ARQUETIPO DE COMBATE del mob TYPE server-auth ESTÁTICO. roleWeight(víctima) = banda de
...
//
// Cobertura (issue CAS-2555 — acceptance observable, flag ON):
//  L1 boot LIVE + build==version.json==d3a276a13dc0 (AVANZÓ de #92 c2a7ab9cf1bb) + hooks (role/spawnKill/save/fp/peers) + 0 err/404
//  L2 flag ON en prod: served config ROLE_SURGE.enabled:true + channel roleFind + params (enabler2/disruptor1/cap2/radius300/roleTier)
//  L3 LIVE default: __dev.role().enabled===TRUE + gExists false (STATELESS, G.roleBounty null) + knobs servidos == oráculos re-derivados
//  L4 LUT archProbe pura arch→band/weight/tier/charge == oráculo re-derivado (WORLD-INDEP) en cada banda + no-listado/bogus→brawler
...
//  L6b ★★ ⊥#73/⊥champion CRUX: e.tpl.arch sobrescrito en el CLON (healer→'brute', orc→'summoner') ⇒ roleOf lee ETPL[type].arch BASE ⇒ healer SIGUE 2, orco SIGUE 0
//  L7 ★★ ⊥#92 CRUX LIVE: SUMMONER(enabler rol2, sz20⇒mole1) vs MOOSE(brute rol0, sz26⇒mole2) — DISJUNTOS (rol=arch/FUNCIÓN vs mole=size)
//  L7b ★★ ⊥#84 CRUX LIVE: SPEARMAN(caster ranged rol0/skirmish>0) vs HEALER(enabler NO-ranged rol2/skirmish0) — OPUESTOS
//  L8 DIFERENCIADOR ⊥ peers: SUMMONER enabler SANO SUELTO OCIOSO a quemarropa ⇒ rol T2 mientras mole=1(size)/escaramuza/manada/siega/control/interrupt/reach/embestida/zona=0
//  L9 ★ REAL GRANT (flag ON LIVE default): spawnKill ⇒ REAL killEnemy seam ⇒ Δh.roleBounty == oGrant(type) por banda (summoner/healer+2, volatile/revenant/demon+1, orc/wolf/moose+0, adv+0); sub-cap 2; flag OFF ⇒ Δ0
...
//
// Run: CHROME_PATH=/usr/bin/chromium node tools/cas2555-role-live-observable-qa.mjs
...
const OUT = join(ROOT, "shots", "cas2555-qa");
...
try {
  ...
  const rBlock = cfgSrc.match(/export const ROLE_SURGE\s*=\s*\{[\s\S]*?\n\};/);
  const rt = rBlock ? rBlock[0] : "";
  const enabledOn = /enabled:\s*true/.test(rt);
  const chOk = /channel:\s*"roleFind"/.test(rt);
  const paramsOk = /enabler:\s*2/.test(rt) && /disruptor:\s*1/.test(rt) && /roleBountyCap:\s*2/.test(rt) &&
    /radius:\s*300/.test(rt) && /summoner:\s*"enabler"/.test(rt) && /healer:\s*"enabler"/.test(rt) &&
    /warlock:\s*"disruptor"/.test(rt) && /volatile:\s*"disruptor"/.test(rt) && /punisher:\s*"disruptor"/.test(rt);
  ok("L2 flag ON en prod: served config ROLE_SURGE.enabled:true + channel roleFind + params (enabler2/disruptor1/cap2/radius300/roleTier summoner·healer=enabler, warlock·volatile·punisher=disruptor)",
  ...
  ok("L4 LUT archProbe arch→band/weight/tier/charge == oráculo re-derivado (WORLD-INDEP) en cada banda + no-listado/bogus→brawler(0)",
  ...
  ok("L5 LUT scoreProbe score→tier→charge == oráculo oRank/oCharge(score) + sub-cap (max charge ≤ cap=2): 0→T0/0, 1→T1/1, 2/3/5/12/100→T2/2",
  ...
  // ---- L7 ⊥#92 CRUX LIVE: summoner(enabler rol2, sz20⇒mole1) vs moose(brute rol0, sz26⇒mole2) — DISJOINT ----
  ...
  ok("L7 ★★ ⊥#92 CRUX LIVE: SUMMONER (enabler rol2, sz20⇒mole1) vs MOOSE (brute rol0, sz26⇒mole2) — DISJUNTOS (rol lee arch/FUNCIÓN de IA, mole lee size/TAMAÑO físico)",
    crux92OK, JSON.stringify(crux92));

  // ---- L7b ⊥#84 CRUX LIVE: healer(enabler rol2, ranged:false skirmish0) vs spearman(caster rol0, ranged:true skirmish>0) ----
  ...
  ok("L7b ★★ ⊥#84 CRUX LIVE: SPEARMAN (caster ranged) ⇒ rol0/skirmish>0; HEALER (enabler NO-ranged) ⇒ rol2/skirmish0 — OPUESTOS (rol lee arch/FUNCIÓN, escaramuza lee ranged/ALCANCE)",
  ...
  // NOTE: rp.mobs[]/rpCount mirror the AMBIENT G.enemies within radius, which drift by session-age (page A booted first) —
  // a session-age artifact, NOT a product desync. The server-auth signal is rpScore (MAX weight) + score/tier/charge
  ...
  ok("L12 ★ NORTH STAR 2-CLIENTE 0-desync LIVE: MISMO summoner+héroe ⇒ score/tier/charge + rpScore(MAX) + LUT arch/score + worldFingerprint + terrHash IDÉNTICOS byte-a-byte, Y ambos == QA oracle (rp.mobs[] ordering=artefacto ambiente por edad de sesión, ⊥ señal)",
...
console.log(`\n=== CAS-2555 QA LIVE observable (ROLE_SURGE #93): ${PASS}/${PASS + FAIL} PASS ===`);
