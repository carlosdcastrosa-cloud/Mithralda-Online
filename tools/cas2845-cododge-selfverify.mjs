// CAS-2845 — self-verify for QUIEBRO (DARK, CO_DODGE_SURGE.enabled:false). EVO mecánica #147 (base master 80a5208 tras #146 CONJURO CO_CAST LIVE&served b8c147402d1f/815, census 74) — EJE FRESCO + CANAL FRESCO, ⊥ a los 88 LIVE #59-#146. El 29º eje de COMPOSICIÓN-DE-INTENCIÓN y la DOCEAVA faceta de la sub-familia PLAYER-COORDINATION (#136 CÓNCLAVE=ENGANCHADOS, #137 COHORTE=PRESENTES, #138 CUADRILLA=REMATAN, #139 SOCORRO=SOSTIENEN/heal, #140 MURALLA=ABSORBEN, #141 REPLIEGUE=SE ALEJAN/velocidad, #142 PIÑA=APIÑADOS, #143 ENVITE=EMPUJAN, #144 DIANA=CONVERGEN-el-aim/CONTEO, #145 TENAZA=GEOMETRÍA, #146 CONJURO=ACTIVAN-habilidad/cast); es la sub-faceta DEFENSIVE REACTION-EVENT (evento-de-reacción-defensiva).
// (A) EJE FRESCO = QUIEBRO/CO-DODGE = D = nº de jugadores VIVOS DISTINTOS con un flag de esquiva/rodada/i-frame (`dodge`) ACTIVO este frame (co-dodge/evade-sync), ⊥ #146 CONJURO (ANY-CAST — activar habilidad ⊥ estado-de-esquiva), ⊥ #141 REPLIEGUE (evento-dodge discreto ⊥ velocidad-away), ⊥ #140 MURALLA (esquivar/i-frame EVITA ⊥ tank ABSORBE), ⊥ #138 CUADRILLA (esquivar ⊥ rematar). Tally BOOLEANO ENTERO. 🔑 DETERMINISMO (sev-1): D = conteo ENTERO de banderas (CERO float/atan2 en el score/decisión; el conteo es CONMUTATIVO ⇒ orden-independiente) vs umbrales ENTEROS {midDodge2,hiDodge3} ⇒ 0-float en el score/decisión. Bandas: D≥hiDodge(3) ⇒ sync-pleno ⇒ 2; D≥midDodge(2) ⇒ sync-parcial ⇒ 1; <2 (solitario) ⇒ 0. 🔑 MULTIJUGADOR-NATIVO: single-player ⇒ ≤1 dodger ⇒ D colapsa ⇒ 0 (colapso LIMPIO).
...
//     CRUX (LA CRÍTICA) — QUIEBRO = SIMULTANEIDAD-DE-ESQUIVA (evento dodge/roll/i-frame) ⊥ TODOS los priores. PRIMARIO ⊥#146 CONJURO (DODGE-STATE ⊥ ANY-CAST): 3 rodando un AoE ⇒ QUIEBRO D3/w2 pero CONJURO U0/w0 (ninguno casteando); 3 mid-cast ⇒ CONJURO U3/w2 pero QUIEBRO D0/w0 (ninguno rodando) — los conteos se mueven INDEPENDIENTEMENTE. ⊥#141 REPLIEGUE (esquiva-en-el-sitio ⊥ velocidad-away). ⊥#140 MURALLA (EVITAR ⊥ ABSORBER). ⊥#138 CUADRILLA (esquivar ⊥ rematar).
//
// Run: node tools/cas2845-cododge-selfverify.mjs
...
const OUT = join(ROOT, "shots", "cas2845");
...
const FPARG = 397;
// dodgeProbe LUT esperada: (dodgers=nº de jugadores en esquiva/i-frame, players=P gate)→D=clamp(dodgers,0,players)→banda(D vs {midDodge2,hiDodge3,minPlayers2} ENTEROS)→{weight,rankTier}. UMBRALES hiDodge 3 / midDodge 2 + colapso single-player (D<minPlayers2 ⇒ 0).
const EXPECT_DODGE = [
  { name: "solo-P1",       dodgers: 1, players: 1, D: 1, w: 0 },   // single-player: D1<midDodge ⇒ 0 (colapso LIMPIO)
  { name: "duo-P2",        dodgers: 2, players: 2, D: 2, w: 1 },   // 2 dodgers ⇒ D2 ⇒ 1 (sync parcial)
  { name: "duo-of-4",      dodgers: 2, players: 4, D: 2, w: 1 },   // 2 dodgers DE un roster de 4 ⇒ D2 ⇒ 1
  { name: "trio-P3",       dodgers: 3, players: 3, D: 3, w: 2 },   // 3 dodgers ⇒ D3 ⇒ 2 (sync pleno)
  { name: "quad-4",        dodgers: 4, players: 4, D: 4, w: 2 },   // 4 dodgers ⇒ D4 ⇒ 2
  { name: "none",          dodgers: 0, players: 3, D: 0, w: 0 },   // 🔑 0 dodgers (nadie esquiva) ⇒ D0 ⇒ 0
...
const EXPECT_DRIVE = [
  ...
  { name: "all-idle",          players: [[false, false], [false, false], [false, false]],       D: 0, w: 0 },   // 🔑 nadie esquiva ⇒ D0 ⇒ 0
];
...
try {
  ...
  ok("5c ★ REAL SERVER-AUTH QUIEBRO (tally de banderas dodge entre VIVOS): duo-dodge(D2)=w1, one-dodge(D1)=w0, solo(D1)=w0, trio-dodge(D3)=w2, nondodge-excl(D2)=w1, dead-excl(D2)=w1, quad-dodge(D4)=w2, all-idle(D0)=w0 (filtra muertos + no-dodgers)",
  ...
  const cruxCast = await page.evaluate(() => {
    ...
    // 3 rodando un AoE (D3) pero NINGUNO casteando (U0)
    const roll = { ddg: ddg(3, 3), cst: cst(0, 3) };
    // 3 casteando (U3) pero NINGUNO rodando (D0) — conteos independientes
    const cast = { ddg: ddg(0, 3), cst: cst(3, 3) };
    return { roll, cast };
  });
  const cruxCastOK =
    cruxCast.roll.ddg.dodge === 3 && cruxCast.roll.ddg.weight === 2 && cruxCast.roll.cst.cast === 0 && cruxCast.roll.cst.weight === 0
    && cruxCast.cast.ddg.dodge === 0 && cruxCast.cast.ddg.weight === 0 && cruxCast.cast.cst.cast === 3 && cruxCast.cast.cst.weight === 2;
  ok("6 ★ CRUX (LA CRÍTICA, PRIMARIO) ⊥#146 CONJURO (DODGE-STATE ⊥ ANY-CAST): {3 rodando AoE, 0 casteando} ⇒ QUIEBRO D3/w2 pero CONJURO U0/w0; {0 rodando, 3 casteando} ⇒ QUIEBRO D0/w0 pero CONJURO U3/w2 ⇒ estado-de-esquiva ⊥ activar-cualquier-habilidad (conteos divergen en AMBOS sentidos)",
     cruxCastOK, JSON.stringify(cruxCast));

  // 6b ★ CRUX ⊥#141 REPLIEGUE (esquiva-en-el-sitio/i-frame ⊥ velocidad-away): QUIEBRO cuenta el flag DISCRETO de esquiva; REPLIEGUE cuenta kiters con velocidad-alejándose. Conteos independientes.
  const cruxFlee = await page.evaluate(() => {
    const ddg = (dodgers, P) => window.__dev.coDodge({ dodgeProbe: { dodgers, players: P } }).dodgeProbe;
    const flee = (breaking, P) => window.__dev.coFlee({ fleeProbe: { breaking, players: P } }).fleeProbe;
    // 3 esquivando EN EL SITIO (D3) pero 0 replegándose/velocidad-away (F0)
    const A = { ddg: ddg(3, 3), flee: flee(0, 3) };
    // 3 replegándose (F3) pero 0 con i-frame de esquiva (D0)
    const B = { ddg: ddg(0, 3), flee: flee(3, 3) };
    return { A, B };
  });
  const cfA = cruxFlee.A, cfB = cruxFlee.B;
  const cruxFleeOK = cfA.ddg.dodge === 3 && cfA.ddg.weight === 2 && cfA.flee && cfA.flee.weight === 0
    && cfB.ddg.dodge === 0 && cfB.ddg.weight === 0 && cfB.flee && cfB.flee.weight === 2;
  ok("6b ★ CRUX ⊥#141 REPLIEGUE (evento-dodge discreto ⊥ velocidad-away): {3 esquivando en el sitio, 0 replegándose} ⇒ QUIEBRO D3/w2 pero REPLIEGUE F0/w0; {0 esquivando, 3 replegándose} ⇒ QUIEBRO D0/w0 pero REPLIEGUE w2 ⇒ i-frame ⊥ velocidad-alejándose",
     cruxFleeOK, JSON.stringify(cruxFlee));

  // 6c ★ CRUX ⊥#140 MURALLA (esquivar/i-frame EVITA ⊥ tank ABSORBE — OPUESTO): QUIEBRO cuenta quién EVITA el golpe; MURALLA cuenta quién lo ABSORBE. Conteos independientes.
  const cruxTank = await page.evaluate(() => {
    const ddg = (dodgers, P) => window.__dev.coDodge({ dodgeProbe: { dodgers, players: P } }).dodgeProbe;
    const tank = (taking, P) => window.__dev.coTank({ bruntProbe: { taking, players: P } }).bruntProbe;
    // 3 esquivando/i-frame (D3, EVITAN) pero 0 absorbiendo (T0)
    const A = { ddg: ddg(3, 3), tank: tank(0, 3) };
    // 3 absorbiendo (T3) pero 0 esquivando (D0)
    const B = { ddg: ddg(0, 3), tank: tank(3, 3) };
    return { A, B };
  });
  const ctA = cruxTank.A, ctB = cruxTank.B;
  const cruxTankOK = ctA.ddg.dodge === 3 && ctA.ddg.weight === 2 && ctA.tank && ctA.tank.weight === 0
    && ctB.ddg.dodge === 0 && ctB.ddg.weight === 0 && ctB.tank && ctB.tank.weight === 2;
  ok("6c ★ CRUX ⊥#140 MURALLA (EVITAR/i-frame ⊥ ABSORBER — OPUESTO): {3 esquivando, 0 absorbiendo} ⇒ QUIEBRO D3/w2 pero MURALLA T0/w0; {0 esquivando, 3 absorbiendo} ⇒ QUIEBRO D0/w0 pero MURALLA w2 ⇒ evitar-el-golpe ⊥ comer-el-golpe",
     cruxTankOK, JSON.stringify(cruxTank));

  // 6f ★ CRUX ⊥#146/#141/#140/#138 CANAL DISTINTO: coDodgeFind ⊥ coCastFind/coFleeFind/coTankFind/coKillFind.
  const cfgSrc = await page.evaluate(async () => { const r = await fetch("sim/config.js"); return await r.text(); });
  const grab = (n) => (cfgSrc.match(new RegExp("export const " + n + "\\s*=\\s*\\{[\\s\\S]*?channel:\\s*\"([^\"]+)\"")) || [])[1];
  const coDodgeCh = grab("CO_DODGE_SURGE"), coCastCh = grab("CO_CAST_SURGE"), coFleeCh = grab("CO_FLEE_SURGE"), coTankCh = grab("CO_TANK_SURGE"), coKillCh = grab("CO_KILL_SURGE");
  const vmCh = await page.evaluate(() => window.__dev.coDodge().channel);
  const chOK = coDodgeCh === "coDodgeFind" && vmCh === "coDodgeFind" && coCastCh === "coCastFind" && coFleeCh === "coFleeFind" && coTankCh === "coTankFind" && coKillCh === "coKillFind"
    && [coCastCh, coFleeCh, coTankCh, coKillCh].every(c => c !== coDodgeCh);
  ok("6f ★ CRUX CANAL FRESCO coDodgeFind ⊥ coCastFind #146 / coFleeFind #141 / coTankFind #140 / coKillFind #138 (canal DISTINTO por construcción; VM idem)",
     chOK, `coDodge=${coDodgeCh} vm=${vmCh} coCast=${coCastCh} coFlee=${coFleeCh} coTank=${coTankCh} coKill=${coKillCh}`);
  ...
  // 15 render badge "Quiebro:" drawn ON+salvo / not OFF + fps. 🔑 label ÚNICO "Quiebro:" (⊥ #146 'Conjuro:'/#145 'Tenaza:'/#144 'Diana:'/#139 'Socorro:'/#138 'Cuadrilla:').
  ...
  ok("15 render badge \"Quiebro:\" se DIBUJA ON+salvo (D>0, re-driven cada frame) y NO OFF (D 0) + fps sano (label ÚNICO ⊥ #146 'Conjuro:'/#145 'Tenaza:'/#144 'Diana:'/#139 'Socorro:'/#138 'Cuadrilla:')",
  ...
  ok("16 ★ NORTH STAR 2-CLIENTE (MANDATORY sev-1): MISMA salva D=3 ⇒ score/idx/dodge/players/tier/charge + dodgeProbeLive(field,dodge,players,score) + dodgeProbe LUT (D enteros) + worldFingerprint IDÉNTICOS byte-a-byte (0 desync)",
