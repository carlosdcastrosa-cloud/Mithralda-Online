// ---------------------------------------------------------------------------
// CAS-2144 QA OBSERVABLE — Ataque Cargado con Híper-Armadura (mec #37) DARK.
...
const EXPECT_BUILD = "7847befbb4a7";
const OUT = "shots/cas2144";
...
for (const prof of PROFILES) {
  ...
  const m = meta.m;
  const metaOk = m.enabled === true && m.chargeThresholdMs === 350 && m.maxChargeMs === 900
  ...
  if (metaOk && meta.cfgEnabled === true)
    P(`[${prof.name}] META: served CHARGED_ATTACK LIVE — enabled=true (post-flip Gate CAS-2139), chargeThresholdMs=${m.chargeThresholdMs}, dmgMul=${m.dmgMul}, poiseMul=${m.poiseMul}, hyper=${m.hyperArmorGrant}, capIn=${m.incomingDmgCapFracMaxHp}, capOut=${m.releaseCapFracMaxHp}`);
