// CAS-2589 — EVO#99 WINDUP_SURGE (Remate de Presagio) DARK QA (Gate 2/2 input) — 41º flag candidate. Base master HEAD 6168eea.
// INDEPENDENT QA harness: re-derives EVERY oracle at the Node level from the imported knobs (WINDUP_SURGE + ETPL + peer
// surges) — it does NOT copy the GE seam math nor the GE cas2585 selfverify file. It then cross-checks the browser
// server-auth probes (__dev.wind / __dev.spawnKill) against those re-derived oracles.
//
// AXIS (⊥40): BASE WIND-UP / TELEGRAPH TIME of the mob TYPE, server-auth STATIC (how long its FACTORY type winds up
//   before it hits — its intrinsic anticipation cadence). windWeight(victim) = band of windOf(e)=ETPL[e.type].windup
//   (immutable base row): windup≥hiWind(0.85) ⇒ deliberate/long-telegraph ⇒ 2; windup≥midWind(0.62) ⇒ measured ⇒ 1;
//   windup<midWind ⇒ snappy/sudden ⇒ 0. Fresh channel windFind → h.windBounty (transient, STATELESS), sub-cap
//   windBountyCap:2, badge "Remate de Presagio" (⌛). Score sampled at TOP of killEnemy (_windPre). PRE-FLIGHT PASSES
//   with NO pivot: the ONLY readers of .windup are (a) the AI state machine (e.st=tpl.windup, DURATION of the state)
//   and (b) the TELL render — never a kill reward; no *Weight of the 40 live flags #59-#98 reads windup as SCORE.
//   ⊥ override/#74/champion/élite: windOf reads ETPL[type].windup BASE, NOT the clone e.tpl.windup (an elite/scaled
//   spawn scales the CLONE); applyZoneScale never scales windup ⇒ telegraph zone-independent (⊥#91).
//   CRUX ⊥#89 INTERRUPT: #89 scores e.state DYNAMIC (executing NOW?); windup is the STATIC tell-duration of the TYPE.
//
// GATES (map to the 5 task QA gates):
//   1  boot + wind + peer hooks + spawnKill + __BUILD, 0 err
//   2  byte-neutral OFF (fresh boot): enabled false, gExists false (G.windBounty never created), all-zero, tag "" (gate 1)
//   3  STATELESS: save blob has NO windBounty/windFind key (task gate 1/5)
//   4  worldFingerprint byte-stable across enabled toggle (tokens NOT in fp; 0 RNG drift) (task gate 5)
//   5  windupProbe LUT: windup→band→weight→tier→charge == oracle across 0.62/0.85 boundaries (task gate 2)
//   6  scoreProbe LUT: score→tier→charge == oracle table + sub-cap headroom (task gate 2)
//   7  REAL spawnWind over ALL ETPL windup rows: browser windWeight == oWeightType(ETPL[type].windup) BASE (task gate 2)
//   8  ⊥ DECOUPLING (⊥override/⊥#74/⊥champion/⊥élite): spawnWind overrideWindup scales the CLONE ⇒ windOf reads BASE,
//        IGNORES clone: orc base0.82 clone0.99 stays wind1; golem base0.95 clone0.1 stays wind2 (task gate 3)
//   9  CRUX ⊥#98 ram / ⊥#97 sentinel / ⊥#96 tough / ⊥#95 menace / ⊥#94 swift / ⊥#92 bulk / ⊥#93 role / ⊥#84 skirmish (gate 3)
//   10 REAL GRANT (flag ON): spawnKill drives the REAL killEnemy seam ⇒ Δh.windBounty == oGrant(type); sub-cap 2 (task gate 2)
//   11 REAL GRANT byte-neutral OFF: same spawnKill with flag OFF ⇒ Δ == 0 (task gate 1)
//   12 0-REGRESSION: 40 arc flags #59-#98 served enabled:true; WINDUP_SURGE served false (DARK #99), off=[] (task gate 4)
//   13 NORTH STAR 2-client: A==B (score/tier/charge + windProbe MAX + LUTs + worldFingerprint + terrHash) + oracle (task gate 5)
...
import { WINDUP_SURGE, ETPL, RAM_SURGE, SENTINEL_SURGE, TOUGH_SURGE, MENACE_SURGE, SWIFT_SURGE, BULK_SURGE } from "../sim/config.js";
...
// oWind/oWeightType: BASE windup of a real ETPL type → weight.
const oWind = (type) => { const t = ETPL[type]; return t && t.windup != null ? +t.windup : 0; };
const oWeightType = (type) => oWeight(oWind(type));
...
// peer axis bands (ram/sentinel/tough/menace/swift/bulk) re-derived independently from THEIR imported knobs to prove orthogonality.
...
const toughBand = (hp) => band3(hp, +TOUGH_SURGE.hiHp, +TOUGH_SURGE.midHp);
const menaceBand = (d) => band3(d, +MENACE_SURGE.hiDmg, +MENACE_SURGE.midDmg);
const swiftBand = (s) => band3(s, +SWIFT_SURGE.hiSpd, +SWIFT_SURGE.midSpd);
const bulkBand = (sz) => band3(sz, +BULK_SURGE.hiSize, +BULK_SURGE.midSize);

const exe = findChromium();
if (!exe) { console.error("no chromium"); process.exit(1); }
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
const OUT = join(ROOT, "shots", "cas2589-qa");
...
// windup sweep around both thresholds (independent of any real type). Covers 0.62/0.85 exact boundaries.
const WIND_SWEEP = [0, 0.28, 0.5, 0.61, 0.62, 0.63, 0.7, 0.82, 0.84, 0.85, 0.86, 0.9, 0.95, 1.0, 9];
// score sweep for the tier/charge LUT + sub-cap headroom.
const SCORES = [0, 1, 2, 3, 5, 99];
// EVERY windup row in ETPL — REAL server-auth spawn coverage (QA enumerates ETPL itself: ≥ the 15 required).
...
try {
  ...
  const wn = await page.evaluate((sweep) => sweep.map(w => { const p = window.__dev.wind({ windupProbe: { windup: w } }).windupProbe; return { w, band: p.band, weight: p.weight, tier: p.tier, charge: p.charge }; }), WIND_SWEEP);
  const wnBad = wn.filter(r => r.band !== oBand(r.w) || r.weight !== oWeight(r.w) || (r.tier != null && r.tier !== oRank(oWeight(r.w))) || (r.charge != null && r.charge !== oCharge(oWeight(r.w), true)));
  ok(`5 windupProbe LUT: windup→band→weight == oracle en el sweep de umbral (${WIND_SWEEP.length} pts: bordes exactos 0.61/0.62 y 0.84/0.85)`,
     wnBad.length === 0, wnBad.length ? JSON.stringify(wnBad.map(r => ({ w: r.w, got: [r.band, r.weight], exp: [oBand(r.w), oWeight(r.w)] }))) : `all ${WIND_SWEEP.length} match`);
  ...
  const spawn7 = await page.evaluate((args) => {
    ...
    for (const t of types) {
      ...
      const wp = window.__dev.wind({ windProbe: true }).windProbe;
      const mine = wp.mobs.find(m => m.type === t) || null;
      out[t] = { windup: r.windup, weight: r.weight, valid: r.valid, wpW: mine ? mine.weight : -1, wpWind: mine ? mine.windup : -1 };
    }
    window.__dev.wind({ clearWind: true }); window.__dev.wind({ enabled: false });
    return out;
  }, { types: ALL_TYPES, Z });
  const s7bad = ALL_TYPES.filter(t => !spawn7[t].valid || spawn7[t].windup !== oWind(t) || spawn7[t].weight !== oWeightType(t) || spawn7[t].wpW !== oWeightType(t) || spawn7[t].wpWind !== oWind(t));
  ok(`7 REAL spawnWind sobre LAS ${ALL_TYPES.length} filas windup de ETPL: browser windWeight == oWeightType(ETPL[type].windup) BASE (server-auth, ≥15 requeridas)`,
     s7bad.length === 0, s7bad.length ? JSON.stringify(s7bad.map(t => ({ t, windup: oWind(t), exp: oWeightType(t), got: spawn7[t] }))) : `all ${ALL_TYPES.length} types match base band (golem=${spawn7.golem.weight} charger=${spawn7.charger.weight} orc=${spawn7.orc.weight} bat=${spawn7.bat.weight} mage=${spawn7.mage.weight} summoner=${spawn7.summoner.weight})`);
  ...
  const dec = await page.evaluate((Z) => {
    ...
    const run = (type, ov) => { window.__dev.wind({ clearWind: true });
      ...
      const wp = window.__dev.wind({ windProbe: true }).windProbe;
      const mine = wp.mobs.find(m => m.type === type) || null;
      return { base: r.windup, cloneWind: r.tplWindup, weight: r.weight, wpW: mine ? mine.weight : -99 }; };
    const orcElite = run("orc", 0.99);       // base 0.82, elite inflates clone to 0.99 — wind must stay 1
    const golemDeflate = run("golem", 0.1);  // base 0.95, deflated clone to 0.1 — wind must stay 2
    window.__dev.wind({ clearWind: true }); window.__dev.wind({ enabled: false });
    return { orcElite, golemDeflate };
  }, Z);
  const decOK = dec.orcElite.base === 0.82 && dec.orcElite.cloneWind === 0.99 && dec.orcElite.weight === WM && dec.orcElite.wpW === WM &&
    dec.golemDeflate.base === 0.95 && dec.golemDeflate.cloneWind === 0.1 && dec.golemDeflate.weight === WD && dec.golemDeflate.wpW === WD;
  ok("8 ⊥ DESACOPLE (⊥override/⊥#74/⊥campeón/⊥élite): overrideWindup escala el CLON e.tpl.windup mimetizando un spawn escalado/élite — windOf lee BASE ETPL[type].windup ⇒ orco base0.82 élite(clon0.99) SIGUE wind1; golem base0.95 deflactado(clon0.1) SIGUE wind2 ⇒ PRESAGIO (base estático) ⊥ élite/campeón (clon dinámico)",
  ...
  const D = (t) => ({ wind: oWeightType(t), ram: ramBand(+ETPL[t].knock), sentinel: sentinelBand(+ETPL[t].aggro), tough: toughBand(+ETPL[t].hp), menace: menaceBand(+ETPL[t].dmg), swift: swiftBand(+ETPL[t].spd), bulk: bulkBand(+ETPL[t].size), arch: ETPL[t].arch, ranged: !!ETPL[t].ranged, windup: +ETPL[t].windup });
  const golem = D("golem"), charger = D("charger"), bat = D("bat"), ironback = D("ironback"), mage = D("mage"), moose = D("moose"), orc = D("orc"), mudlurker = D("mudlurker"), summoner = D("summoner"), volatile = D("volatile"), spearman = D("spearman");
  const cruxOK =
    // ⊥#98 RAM (base knock): golem knock60 ram0 / windup0.95 wind2 vs charger knock235 ram2 / windup0.66 wind1 — ORDER OPPOSED
    golem.ram === 0 && golem.wind === 2 && charger.ram === 2 && charger.wind === 1 &&
    // ⊥#97 SENTINEL (base aggro): ironback aggro220 sentinel0 / windup0.86 wind2 vs mudlurker aggro250 sentinel1 / windup0.48 wind0 — OPPOSED
    ironback.sentinel === 0 && ironback.wind === 2 && mudlurker.sentinel === 1 && mudlurker.wind === 0 &&
    // ⊥#96 TOUGH (base hp): charger hp140 tough2 / wind1 vs mage hp56 tough1 / windup0.9 wind2 — OPPOSED
    charger.tough === 2 && charger.wind === 1 && mage.tough === 1 && mage.wind === 2 &&
    // ⊥#95 MENACE (base dmg): summoner dmg0 menace0 / windup0.95 wind2 vs volatile dmg23 menace2 / windup0.7 wind1 — OPPOSED
    summoner.menace === 0 && summoner.wind === 2 && volatile.menace === 2 && volatile.wind === 1 &&
    // ⊥#94 SWIFT (base spd): bat spd158 swift2 / windup0.28 wind0 vs golem spd46 swift0 / windup0.95 wind2 — DIAMETRICALLY OPPOSED
    bat.swift === 2 && bat.wind === 0 && golem.swift === 0 && golem.wind === 2 &&
    // ⊥#92 BULK (base size): charger sz26 bulk2 / wind1 vs summoner sz20 bulk1 / windup0.95 wind2 — OPPOSED
    charger.bulk === 2 && charger.wind === 1 && summoner.bulk === 1 && summoner.wind === 2 &&
    // ⊥#93 ROLE (arch): orc brute / windup0.82 wind1 vs moose brute / windup0.88 wind2 — SAME arch DIFFERENT band ⇒ NOT a re-map
  ...
  ok("9 ⊥ CRUX cross-axis: golem ram0/wind2 vs charger ram2/wind1 ORDEN OPUESTO (⊥#98); ironback sentinel0/wind2 vs mudlurker sentinel1/wind0 OPUESTO (⊥#97); charger tough2/wind1 vs mage tough1/wind2 OPUESTO (⊥#96); summoner menace0/wind2 vs volatile menace2/wind1 OPUESTO (⊥#95); bat swift2/wind0 vs golem swift0/wind2 DIAMÉTRICAMENTE OPUESTOS (⊥#94); charger bulk2/wind1 vs summoner bulk1/wind2 OPUESTO (⊥#92); orc brute/wind1 vs moose brute/wind2 MISMO arch DISTINTA banda (⊥#93); golem melee/wind2 vs spearman ranged/wind1 OPUESTO (⊥#84)",
     cruxOK, JSON.stringify({ golem, charger, bat, ironback, mage, moose, orc, mudlurker, summoner, volatile, spearman }));

  // 10 REAL GRANT (flag ON): spawnKill → REAL killEnemy seam → Δh.windBounty
  const GRANT_TYPES = ["golem", "summoner", "mage", "moose", "ironback", "wendigo", "magmabrute", "emberkin", "revenant", "charger", "toadbrute", "spearman", "volatile", "orc", "healer", "demon", "bat", "rat", "wolf", "mudlurker", "bandit", "skeleton", "quillback"];
  ...
  ok("10 REAL GRANT (flag ON): spawnKill ⇒ REAL killEnemy seam ⇒ Δh.windBounty == oGrant(type) por banda (ponderoso golem/summoner/mage/moose/ironback/wendigo/magmabrute/emberkin+2; medido revenant/charger/toadbrute/spearman/volatile/orc/healer/demon+1; súbito bat/rat/wolf/mudlurker/bandit/skeleton/quillback+0); sub-cap 2",
  ...
  const offGrant = await page.evaluate((Z) => {
    ...
    window.__dev.spawnKill("mage");
    const after = window.__dev.wind().hero.windBounty | 0;
    return { before, after, delta: after - before };
  }, Z);
  ok("11 REAL GRANT byte-neutral OFF: spawnKill(golem)+spawnKill(mage) con flag OFF ⇒ Δh.windBounty == 0 (rama muerta, 0 fichas al seam)",
  ...
  const readVM = async (pg) => await pg.evaluate((Z) => {
    ...
    window.__dev.wind({ clearWind: true }); window.__dev.wind({ enabled: false });
    return { score: vm.score, tier: vm.tier, charge: vm.charge, wpScore: wp.score,
  ...
  await pageB.screenshot({ path: join(OUT, "client-b-windup.png") });
  await page.evaluate(() => window.__dev.wind({ enabled: false }));
