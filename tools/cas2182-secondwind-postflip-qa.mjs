// ---------------------------------------------------------------------------
// CAS-2182 QA POST-FLIP LIVE — SEGUNDO ALIENTO / SECOND_WIND (mec #41).
// CEO gate CAS-2165 flipped SECOND_WIND.enabled false→true (config-only, 1 line,
// commit 9687d87). Live build c9a575374a81/799 (supersede DARK bf8a811b3dc0).
// GUARDRAIL 0-leak: the flip touched ONLY sim/config.js; the 4 non-config blobs
// are byte-verified == HEAD (master 9687d87) == the DARK QA-proven set (CAS-2167):
//   sim/sim.js       d7b985b662262458569e426228ce00b7   (byte-id DARK QA-proven)
//   strings.js       3e5913ccaf9d44f6bba8577539b8be71   (byte-id DARK QA-proven)
...
//   sim/config.js    9718113569d385d09bcc672a39f6ac98   (ONLY changed blob: enabled:true)
// ⇒ live behaviour == the DARK QA-observable proof; only the meta.enabled default flipped.
//
...
//
//   AC0  [META]   served SECOND_WIND knob LIVE POST-FLIP: {enabled:TRUE (was false
//                 in DARK), surviveHpFrac:0.15, novaRadius:120, novaKnockback:180,
//                 novaPoiseDmg:0, iframesMs:600, chargesPerRest:1}.
//   AC-BLOB [0-LEAK] served config/sim/render/strings/input md5 == HEAD 9687d87;
//                 4 non-config blobs byte-id DARK QA-proven ⇒ flip config-only.
//   AC1  [HEADLINE] with enabled (default true now), a LETHAL hit does NOT kill:
...
//   AC6  [OFF]    force-toggle enabled=false ⇒ lethal hit KILLS normally, no nova, no
//                 floater ⇒ reversibility / byte-id baseline pre-mec41 (dead branch).
...
//   60fps DPR desk+mob.
//
// Run: node tools/cas2182-secondwind-postflip-qa.mjs   (PASS×2 = invoke twice)
// ---------------------------------------------------------------------------
...
const EXPECT_BUILD = "c9a575374a81";
const OUT = "shots/cas2182";
// HEAD 9687d87 md5s. config CHANGED by flip; the 4 others byte-id DARK QA-proven (CAS-2167).
const EXPECT_MD5 = {
  "sim/config.js":    "9718113569d385d09bcc672a39f6ac98",  // ONLY changed blob (enabled:true)
  "sim/sim.js":       "d7b985b662262458569e426228ce00b7",  // byte-id DARK QA-proven
  "strings.js":       "3e5913ccaf9d44f6bba8577539b8be71",  // byte-id DARK QA-proven
  "render/render.js": "7eec7d36ba870e327e54a0c22cf40e75",  // untouched
  "input.js":         "877899def9c3f51e8ea3a232ce0d2cc4",  // untouched (AUTOMÁTICO)
};
...
// AC-BLOB [0-LEAK] — fetch the served blobs and md5-check vs HEAD 9687d87.
// The flip is config-only: config CHANGED, the other 4 byte-id the DARK QA-proven set.
{
  const page = await browser.newPage();
  for (const [path, want] of Object.entries(EXPECT_MD5)) {
    const body = await page.evaluate(async (u) => {
      const r = await fetch(u + "?cb=" + Date.now(), { cache: "no-store" });
      return r.ok ? await r.text() : null;
    }, BASE + path).catch(() => null);
    if (body == null) { F(`[blob] ${path} fetch failed`); continue; }
    const got = createHash("md5").update(body).digest("hex");
    if (got === want) P(`[blob] served ${path} md5 == HEAD ${want}`);
    else F(`[blob] served ${path} md5=${got} != HEAD ${want}`);
  }
  // config must literally serve enabled:true post-flip
  const cfgTxt = await page.evaluate(async (u) => {
    const r = await fetch(u + "?cb=" + Date.now(), { cache: "no-store" });
    return await r.text();
  }, BASE + "sim/config.js").catch(() => "");
  const m = cfgTxt.match(/export const SECOND_WIND\s*=\s*\{\s*enabled\s*:\s*(true|false)/);
  if (m && m[1] === "true") P(`[blob] served config.js SECOND_WIND.enabled:true (flip landed)`);
  else F(`[blob] served config.js SECOND_WIND.enabled not true: ${m && m[1]}`);
  await page.close();
}
...
for (const prof of PROFILES) {
  ...
  // AC0 META — served SECOND_WIND knob LIVE POST-FLIP (enabled:TRUE is the shipped state now)
  const meta = await page.evaluate(() => JSON.parse(JSON.stringify(window.__h.cfg.SECOND_WIND)));
  const metaOk = meta.enabled === true && meta.surviveHpFrac === 0.15 && meta.novaRadius === 120
    && meta.novaKnockback === 180 && meta.novaPoiseDmg === 0 && meta.iframesMs === 600 && meta.chargesPerRest === 1;
  if (metaOk) P(`[${prof.name}] META: served SECOND_WIND LIVE POST-FLIP — enabled=${meta.enabled} (was false DARK), surviveHpFrac=${meta.surviveHpFrac}, novaRadius=${meta.novaRadius}, novaKnockback=${meta.novaKnockback}, novaPoiseDmg=${meta.novaPoiseDmg}, iframesMs=${meta.iframesMs}, chargesPerRest=${meta.chargesPerRest}`);
