// CAS-356 — build single-row enemy strips for the Quillback Stalker mob from
// PixelLab MCP animation frame URLs. Generalizes tools/cas209-build-strip.mjs to
// emit any {mob}_{state}_strip.png at NxN frames (autoTrim + feet-anchor).
//
// usage: node tools/cas356-build-strips.mjs <manifest.json>
// manifest: { "mob":"quillback", "fw":64, "strips": { "idle":[url,...], "walk":[...], ... } }
...
// autoTrim non-transparent bbox, scale into NxN keeping aspect, feet bottom-anchored.
...
(async()=>{
  ...
  const mob=man.mob, FW=man.fw||64;
  fs.mkdirSync('assets/pixellab/fountains/anim',{recursive:true});
  for(const [state,urls] of Object.entries(man.strips)){
    const frames=[];
    ...
    console.log(`✓ ${mob}_${state}: ${N} frames @ ${FW}px → ${out} (${FW*N}x${FW})`);
  }
})();
