...
const EXPECT_BUILD = "37521ab19b4c"; // CAS-2110: bumped from 868a3cb231ac after CAS-2110 Build+Deploy DARK Dodge Counter (mec#34). Served blobs config d133cd838e0547d5472a10f0ecc9eda4 / sim 2524c04f07b8b5f0961f587071d4250c / render 837161ce (UNCHANGED — no art). DODGE_COUNTER.enabled=false ⇒ dead branches ⇒ byte-id to mec#33 in every system.
...
function auditDodgeCounter() {
  ...
  const dark = dc && dc.enabled === false;   // SHIP DARK; Gate CEO CAS-2111 flips false→true (config-only, reversible)
  ...
  if (shapeOk && dark && valsOk && parity)
    pass(`[DODGECOUNTER] DODGE_COUNTER DARK (34th mech, CAS-2110) — enabled:FALSE (dead branches ⇒ byte-id to mec#33 build), full knob shape windowS=${dc.windowS}/dmgMul=${dc.dmgMul}/poiseMul=${dc.poiseMul}/staminaCost=${dc.staminaCost}/perfectWindowMs=${dc.perfectWindowMs}, UNIVERSAL requiresShield=false, parity<GuardCounter — RNG-neutral (0 dodgeCounterRng) + save-neutral (h.dodgeCounterT/_rollAge transient) ⇒ srand ON==OFF & save.v1 byte-id`);
  else fail(`[DODGECOUNTER] shapeOk=${shapeOk} dark=${dark} valsOk=${valsOk} parity=${parity} dc=${JSON.stringify(dc)}`);
}
...
function determinism() {
  ...
  if (a === b && !a.startsWith("ERR:")) pass(`[DETERM] sim cycle fingerprint identical across 2 runs (len ${a.length}) — 34 mechanics (Arena Hazards LIVE gate-closed w/o boss + Guard Counter DARK + Dodge Counter DARK) add 0 draws to a normal run`);
...
log("");
if (ok) log(`✅ CAS-2110 HOLISTIC FULL-BUILD REGRESSION — ALL PASS (24 systems + 5 go-live + TIME-ATTACK(29) + VARIANTS(30,LIVE) + SEEDED CHALLENGE(31,LIVE) + ARENA HAZARDS(32,LIVE) + GUARD COUNTER(33,LIVE) + DODGE COUNTER(34,DARK) + NG_PLUS + onboarding + 7 balance + JUICE + determinism on ${EXPECT_BUILD})`);
else { console.error("❌ CAS-2110 HOLISTIC FULL-BUILD REGRESSION — FAIL"); process.exit(1); }
