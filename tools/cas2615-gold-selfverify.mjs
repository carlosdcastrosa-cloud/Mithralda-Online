...
try {
  ...
  ok("2 byte-id OFF (fresh boot): GOLD_SURGE.enabled false AND G.coinBounty NUNCA se crea (gExists false)",
     dark.enabled === false && dark.gExists === false && dark.tier === 0 && dark.score === 0 && dark.charge === 0 && dark.forageChargePreview === 0 && dark.channel === "coinFind" && dark.tag === "",
  ...
  const goldDark = flag("GOLD_SURGE") === "false";
  ok("14 ★ 0-REGRESIÓN: 44 mecanismos del arco #59-#102 served enabled:true; GOLD_SURGE served false (DARK #103)",
     arcAllOn && goldDark && arc.length === 44, `gold=${flag("GOLD_SURGE")} arcAllOn=${arcAllOn} n=${arc.length} off=${JSON.stringify(arcLive.filter(([, v]) => v !== "true"))}`);
