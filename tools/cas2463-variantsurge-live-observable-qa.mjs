// CAS-2463 — QA POST-FLIP LIVE 2-cliente (INDEP, QA-owned) para VARIANTE DE ENCUENTRO ACTIVA (ENCOUNTER_VARIANT_SURGE.enabled:TRUE), EVO mecánica #76.
// Mirror LIVE de la DARK QA CAS-2458/CAS-2457 (tools/cas2456-variantsurge-dark-observable-qa.mjs). Patrón LIVE = cas2455.
// URL oficial = gh-pages https://carlosdcastrosa-cloud.github.io/Mithralda-Online/ (el build que juegan los jugadores).
// EXPECT build servido = 4ffd21ead509 (== master flip ac1b910 == deploy tools/cas2460-deploy.mjs == gh-pages f0040be == HTTP served; byte-verificado CTO 6/6).
//
// Diferencia clave vs DARK (cas2456): enabled:TRUE ⇒ el efecto ENCOUNTER_VARIANT_SURGE está ACTIVO en el build servido, SIN flip in-memory:
//   - variantSurgeScore(h)=Σ variantWeights[e.variant] sobre mobs VIVOS con variante dentro de radius260 (server-auth: G.enemies con e.variant, subsistema ENCOUNTER_VARIANTS/CAS-2071).
//   - TABLA por score: score≥3→T2/sockets2, [1,3)→T1/sockets1, 0→T0/sockets0 (sub-cap variantSocketCap=2).
//   - canal FRESCO socketFind: forrajeo DENTRO de un encuentro de variante en radio ⇒ +esquirlas de engarce por kill (grantSocket a h.socketShards, recurso TRANSITORIO fuera del save+fingerprint).
//   - mob muerto o sin variante en radio (>260px) ⇒ peso 0 ⇒ T0/+0 (control).
...
// Run: node tools/cas2463-variantsurge-live-observable-qa.mjs   [live-url]
...
const OUT = join(ROOT, "shots", "cas2463-qa");
...
const DARK_BUILD = "f7b79c60d831";     // build DARK previo (CAS-2456/2458) — served version.json avanza en deploy; el LIVE DEBE haber avanzado de aquí (o del #75 3a834d06862e)
const PREV_LIVE = "3a834d06862e";      // build LIVE previo (#75 ZONE_EVENT_SURGE) — el served DEBE haber avanzado de aquí
...
const ORACLE_WEIGHTS = { stalker: 1, bastion: 2, glass: 1 };   // id ausente → 1 (fallback defensivo)
...
async function toPlay(page) {
  ...
  await page.evaluate(() => { const ni = document.getElementById("nameInput"); if (ni) ni.value = "QAVariant";
...
try {
  ...
  // ═══ L1 — Boot LIMPIO LIVE + build self-consistent (== version.json == EXPECT, AVANZÓ del #75) + hooks ═══
  ...
  ok(`L1 boots LIVE a 'play'; build==version.json==EXPECT ${EXPECT_BUILD} (AVANZÓ del #75 ${PREV_LIVE}); __dev.variantSurge/ward/saveBlob/worldFingerprint; 0 err/404 (sin black-screen)`,
     hooks && build === verBuild && build === EXPECT_BUILD && build !== PREV_LIVE && errA.length === 0 && net404.length === 0,
     `build=${build} version.json=${verBuild} expect=${EXPECT_BUILD} prevLive=${PREV_LIVE} err=${errA.length} 404=${net404.length}`);
  ...
  //   per-tipo weight (stalker=+1 vs bastion=+2 vs glass=+1) + radius-boundary gating (~256px cuenta, ~384px no).
  ...
  ok("L5 ★ REAL SERVER-AUTH: spawnVariant→G.enemies, variantProbe lee score REAL; INDEP per-tipo stalker=+1 bastion=+2 glass=+1; radius-boundary ~256px=+1 / ~384px=+0",
     s5OK, JSON.stringify(server5));

  // ═══ L6 — EFECTO ACTIVO LIVE (enabled:true SIN flip in-memory): mob-variante de alto valor en radio ⇒ +sockets por tier; lejos ⇒ +0 ═══
  const effect = await pageA.evaluate(() => {
    ...
    // LEJOS: teleport héroe fuera del radio260 (>10 tiles) ⇒ sin variante en radio ⇒ T0/+0 control
  ...
  ok("L8b STATELESS: saveBlob() SIN socketShards/socketFind/variantSurge (recurso TRANSITORIO, 100% derivado) + worldFingerprint(393) byte-estable a través de probes (las esquirlas NO entran al fingerprint; 0 RNG/timer drift)",
  ...
  const badge = await pageA.evaluate(async () => {
    ...
    window.__dev.variantSurge({ tp: { tx: h.tx + 40, ty: h.ty } });            // variante fuera de radio ⇒ badge "Variante: —"
  ...
  // Tiles ABSOLUTOS fijos: ambos clientes inyectan el MISMO mob-variante + tp héroe a las MISMAS coords (variantSurgeScore = fn PURA de mobs-variante vivos en radio).
  const A_TILE = { tx: 70, ty: 44 }, B_TILE = { tx: 71, ty: 44 }, HERO_TILE = { tx: 74, ty: 44 };  // variante 3-4 tiles del héroe (~96-128px → en radio260)
  ...
  // variantMobCount NO se compara: nº AMBIENTAL de mobs-variante (sembrados por ENCOUNTER_VARIANTS). La señal determinista per-snapshot
  ...
  await sleep(200);
  await pageB.screenshot({ path: join(OUT, "client-b-variantsurge.png") });

  // ═══ L11 — reconnect STATELESS 0-drift: reload B ⇒ enabled true + gExists false + save sin socketShards + LUT/fp idénticos ═══
  const fpB_pre = NB.fp;
  ...
  await pageB.waitForFunction("window.__dev.variantSurge", { timeout: 8000 });
  const reconn = await pageB.evaluate((AT, BT, HT) => {
    const s = window.__dev.variantSurge();
    const save = JSON.stringify(window.__dev.saveBlob());
    const lut = [0, 1, 3].map(sc => { const p = window.__dev.variantSurge({ scoreProbe: { score: sc } }).scoreProbe; return { s: sc, tier: p.tier, sockets: p.sockets }; });
    window.__dev.variantSurge({ spawnVariant: { tx: AT.tx, ty: AT.ty, variant: "bastion" } });
    window.__dev.variantSurge({ spawnVariant: { tx: BT.tx, ty: BT.ty, variant: "stalker" } });
    window.__dev.variantSurge({ tp: { tx: HT.tx, ty: HT.ty } });
    const fp = JSON.stringify(window.__dev.worldFingerprint(393));
    return { enabled: s.enabled, gExists: s.gExists, hasKey: /"socketShards"\s*:|"(variantSurge|socketFind)[A-Za-z]*"\s*:/.test(save), lut, fp };
  }, A_TILE, B_TILE, HERO_TILE);
  ...
  ok("L11 RECONNECT STATELESS 0-drift: tras reload, enabled:true + gExists false + save sin socketShards/socketFind + LUT idéntica + worldFingerprint idéntico (0 drift, 0 persistencia indebida de esquirlas)",
