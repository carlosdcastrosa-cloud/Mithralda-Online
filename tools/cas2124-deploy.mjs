// CAS-2124: Deploy — Tibia-style enemy HP bars + nameplates (RENDER-ONLY).
// Overlay = render/render.js ONLY. sim/config.js + sim/sim.js are byte-identical to LIVE
// (0 behaviour change ⇒ srand ON==OFF preserved). NAMEPLATE knob is render-local (enabled:true).
...
function buildAndPush() {
  ...
  const commit = gitStr("commit-tree", tree, "-p", parent, "-m",
    `CAS-2124: Deploy Tibia enemy HP bars + nameplates — render-only 1-blob overlay (render/render.js), sim byte-id — build ${build}`);
  try {
