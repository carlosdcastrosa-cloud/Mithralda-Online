// Deploy: overlay the warrior walk8 blade-stub fix onto origin/gh-pages, stamp
// version.json from the EXACT deployed fileset, commit onto the CURRENT gh-pages tip
// via a temp index, push. Same build-id algorithm as tools/build-id.mjs. Retries once
// on a diverged push. Mirror of tools/cas447-deploy.mjs.
...
const CHANGED = [
  "assets/erw/hero/classes/warrior_walk8.png",
];
...
function buildAndPush() {
  ...
  const idx = "/tmp/warrior-fix-index";
  ...
  const msg = `Fix warrior walk sprite: quitar trozo suelto de la hoja (E/W/NW/NE) — build ${build}`;
