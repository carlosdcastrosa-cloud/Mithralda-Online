// CAS-2346 — POST-FLIP LIVE OBSERVABLE + 2-CLIENTE QA — VIGILIA / LONG WATCH (LONG_WATCH.enabled:true LIVE). EVO mecánica #54.
// Gemelo LIVE de la DARK observable `tools/cas2341-longwatch-observable-qa.mjs` (14/14 ×2 PASS): MISMOS hechos, contra el
// build SERVIDO en gh-pages tras el flip CAS-2345/CAS-2343 (`LONG_WATCH.enabled:false→true`, deploy consistent-HEAD).
// URL oficial de verificación = gh-pages `https://carlosdcastrosa-cloud.github.io/Mithralda-Online/` (NO el mirror
// `.higgsfield.gg`, congelado por outage — ver CAS-412). El config servido DEBE mostrar LONG_WATCH.enabled:true; si aún
...
// gh-pages con el MISMO snapshot {streak,atMs,present} + MISMO reloj (nowMs) ⇒ streak/tier/buff IDÉNTICOS byte-a-byte.
// Subir y decaer convergen. A SALE físicamente ⇒ Δ_A=0 PERO streak/tier server-authoritative + Δ_B INTACTOS. Desync=sev-1.
//
// Difs vs la DARK (post-flip LIVE): (1) check 1 = build servido AVANZÓ de pre-flip `2ee8e57d9807` + version.json
// self-consistent + LONG_WATCH.enabled:true served + 5 flags del arco served true (0-regr) + 0 err/404; (2) check 2 =
// DEFAULT-ON (boot fresco 0 __dev ⇒ longWatch().enabled===true cargó del config servido) + byte-id OFF vía TOGGLE
// (DEFAULT-ON ya pudo instanciar G.longWatch ⇒ el byte-id REAL es save-SIN-clave + fingerprint estable, footgun
// heredado CAS-2337/2340); resto = mismos hechos server-authoritative que la DARK.
// Run: node tools/cas2346-longwatch-live-observable-qa.mjs   [optional LIVE base url]
...
const OUT = join(ROOT, "shots", "cas2346-live");
try { mkdirSync(OUT, { recursive: true }); } catch (e) {}

const PREFLIP = "2ee8e57d9807";          // build servido ANTES del flip (DARK LONG_WATCH, deploy CAS-2342) — el LIVE debe AVANZAR de este
...
async function installQA(page) {
  await page.evaluate((NOW) => {
    window.__NOW = NOW;
    // flippa OFF in-memory a TODOS los peers DEFAULT-ON del canal restedMult (arco LIVE) — para medir Vigilia AISLADA
...
try {
  ...
  // config servido (una vez) — flags served + LONG_WATCH.enabled:true
  ...
  const servedTrue = en("LONG_WATCH") === "true";
  const arc = { CONGREGATION: en("CONGREGATION"), WAYFARER: en("WAYFARER_TRAIL"), PULSE: en("WORLD_PULSE"), SOUL: en("SOUL_RECOVERY"), DIVERSE: en("DIVERSE_COMPANY") };
  const arcTrue = Object.values(arc).every((v) => v === "true");

  // 1 boot LIMPIO LIVE + hooks + build self-consistent vs version.json + AVANZÓ de pre-flip + served LONG_WATCH.enabled:true + arco 0-regr + 0 err/404
  ...
  ok("1 boots LIVE; build self-consistent vs version.json + AVANZÓ de pre-flip; __dev.longWatch + arc hooks; served LONG_WATCH.enabled:true + 5 flags del arco true (0 regr); 0 err/404",
     hooks && build === verBuild && !!build && build !== PREFLIP && servedTrue && arcTrue && errors.length === 0 && net404.length === 0,
     `build=${build} version.json=${verBuild} preflip=${PREFLIP} LONG_WATCH=${en("LONG_WATCH")} arc=${JSON.stringify(arc)} err=${errors.length} 404=${net404.length}`);

  // 2 DEFAULT-ON desde config servido + byte-id OFF vía TOGGLE
  const dOn = await page.evaluate(() => { const s = window.__dev.longWatch(); return { enabled: s.enabled, mul: s.longWatchMulRested, tier: s.tier, streak: s.streak }; });
  const byteId = await page.evaluate(() => {
    ...
    return { enabled: s.enabled, mul: s.longWatchMulRested, tag: s.tag, tier: s.tier, saveNoKey: !/longWatch(Server)?"/.test(saveOff), fpMatch: fp1 === fp2 };
  });
  ok("2 DEFAULT-ON desde config servido: longWatch().enabled===true (flip cargó) + passive 0 sin streak; byte-id OFF (toggle): enabled false ⇒ mul 0 + tag \"\" + save SIN clave longWatch + fingerprint byte-estable",
     dOn.enabled === true && dOn.mul === 0 && dOn.tier === 0 && dOn.streak === 0 &&
  ...
  ok("3 tabla tiers = fn PURA del streak (tier/boost DERIVADOS de constantes de config servido, 0 hardcode QA)",
  ...
  ok("4 server-authoritative reflect+validate LIVE: zona válida refleja; zona fuera de zones DESCARTADA; streak neg ⇒ 0",
  ...
  ok("5 ★ SUBIDA LIVE: presencia continua sube streak (10s⇒T0 breve NO abre; 30s⇒T1; 90s⇒T2; 180s⇒T3)",
  ...
  ok("6 ★ DECAY+RUPTURA LIVE: vacío decae vida-media (+halfLife⇒50) dentro de ventana; hueco > gapBreakSec ⇒ ROMPE→0/T0",
  ...
  ok("7 ★ RELEVO vs ABANDONO LIVE: hueco>gap ⇒ ROTO(0); hueco<gap ⇒ PRESERVADO(>0) y el RELEVO reconstruye ≥T1",
     !relay.bad && relay.abandonS === 0 && relay.abandonT === 0 && relay.heldS > 0 && relay.reliefT >= 1, JSON.stringify(relay));

  // 8 passive server-side deterministic (isolated)
  ...
  ok("8 passive server-side determinista LIVE (aislado): héroe EN zona streak90⇒T2 mul==0.10; leave ⇒ 0 + tier0",
     !pass.bad && near(pass.inMul, 0.10) && pass.inTier === 2 && near(pass.inStreak, 90) && pass.outMul === 0 && pass.outTier === 0, JSON.stringify(pass));

  // 9 seam gainXP servido (LIVE) + passive activo con streak
  ...
  const on = await page.evaluate(() => {
    ...
    window.__dev.longWatch({ enabled: true });
    return { onMul, enabledOff: s.enabled, mulOff: s.longWatchMulRested, tagOff: s.tag };
  });
  ok("9 seam gainXP servido LIVE (longWatchMul en gainXP) + passive ON con streak30⇒0.05; OFF ⇒ mul 0/tag \"\"",
     seam && !on.bad && near(on.onMul, 0.05) && on.enabledOff === false && on.mulOff === 0 && on.tagOff === "", `seam=${seam} ${JSON.stringify(on)}`);
  ...
  ok("10 precedencia MÁXIMO ÚNICO LIVE: VIGILIA CEDE a STANDINGS/CONGREGATION/WAYFARER/CONFLUENCIA ⇒ 0; COEXISTE TERRITORY(⊥)",
  ...
  ok("11 ★ COBERTURA 6 zonas LIVE: cada LONG_WATCH.zones hospeda Vigilia observable (streak≥180⇒T3) broken=[]",
     cov.n === 6 && cov.broken.length === 0, `n=${cov.n} broken=${JSON.stringify(cov.broken)}`);

  // 12 render badge — instrumentación fillText/strokeText DETERMINISTA (ON dibuja "Vigilia: <zona>"; OFF gated ⇒ 0) + fps
  ...
  ok("12 render badge 'Vigilia' DIBUJA con ON LIVE (fillText 'Vigilia: <zona>' >0, etiqueta correcta) + byte-clean OFF (0 dibujos) + fps≥55",
  ...
  // 13 AC-3 0-REGRESIÓN — 5 flags del arco served true; LONG_WATCH served true (reusa cfgSrc)
  ok("13 AC-3 0-REGRESIÓN LIVE: 5 flags del arco (CONGREGATION/WAYFARER/WORLD_PULSE/SOUL/DIVERSE) served enabled:true intactos; LONG_WATCH served true",
     arcTrue && servedTrue, `arc=${JSON.stringify(arc)} LONG_WATCH=${en("LONG_WATCH")}`);

  // 14 ★ NORTH STAR — 2-client convergence LIVE (open page2 last: blurs page1 ⇒ pausa loop de page1, ok — proyección determinista)
  ...
  ok("14 ★ NORTH STAR LIVE — CONVERGENCIA 2-CLIENTE: 2 páginas gh-pages mismo snapshot+reloj ⇒ streak/tier/buff IDÉNTICOS; subir(T3)/decaer(T1) converge; A sale ⇒ Δ_A=0 pero compartido + Δ_B INTACTO (0 desync)",
     northOk, JSON.stringify(north));
  await page2.screenshot({ path: join(OUT, "client-b.png") });

  console.log(`\n${FAIL === 0 ? "✅" : "❌"} cas2346 LIVE QA: ${PASS} PASS / ${FAIL} FAIL  (build ${build}, err ${errors.length}, 404 ${net404.length})`);
