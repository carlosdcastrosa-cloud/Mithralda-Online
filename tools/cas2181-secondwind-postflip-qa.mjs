// ---------------------------------------------------------------------------
// CAS-2181 QA POST-FLIP LIVE — SEGUNDO ALIENTO / SECOND_WIND (mec #41) enabled:true.
// Gate CEO CAS-2165 GO ejecutado: flip config-only SECOND_WIND.enabled false→true
// LIVE en build c9a575374a81/799. The 4 BEHAVIOUR blobs are BYTE-ID to the ones
// QA drove DARK in CAS-2164/CAS-2167 (0-leak — only config toggled):
//   sim/sim.js       d7b985b662262458569e426228ce00b7   (== DARK)
//   strings.js       3e5913ccaf9d44f6bba8577539b8be71   (== DARK)
...
//   sim/config.js    9718113569d385d09bcc672a39f6ac98   (ONLY blob changed: enabled:true)
// Copy of cas2167 — probes force-toggle enabled ⇒ flip-agnostic; only AC0 asserts the
// SERVED default is now enabled===true (players get SECOND_WIND without a test patch).
//
...
//
//   AC0  [META]   served SECOND_WIND knob LIVE POST-FLIP: {enabled:true,
...
//   AC6  [OFF]    force enabled=false ⇒ lethal hit KILLS normally, no nova, no
...
//   60fps DPR desk+mob.
//
// Run: node tools/cas2181-secondwind-postflip-qa.mjs   (PASS×2 = invoke twice)
// ---------------------------------------------------------------------------
...
for (const prof of PROFILES) {
  ...
  // AC0 META — served SECOND_WIND knob LIVE POST-FLIP (enabled:true is now the shipped state)
  ...
  if (metaOk) P(`[${prof.name}] META: served SECOND_WIND LIVE POST-FLIP — enabled=${meta.enabled} (TRUE), surviveHpFrac=${meta.surviveHpFrac}, novaRadius=${meta.novaRadius}, novaKnockback=${meta.novaKnockback}, novaPoiseDmg=${meta.novaPoiseDmg}, iframesMs=${meta.iframesMs}, chargesPerRest=${meta.chargesPerRest}`);
  else F(`[${prof.name}] META unexpected (expected enabled:true): ${JSON.stringify(meta)}`);
