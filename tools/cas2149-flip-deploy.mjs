// CAS-2149 (CAS-2146): Gate CEO GO-LIVE flip — GUARD_BREAK.enabled false→true (config-only, 1 line).
// Overlay = ONLY sim/config.js (sim.js/render.js unchanged, already QA-proven at DARK 9f219a9bb567 (CAS-2148)).
// version.json regenerated over full tree. Reversible = flip knob back to false + redeploy.
// Mirror de cas2139-flip-deploy.mjs.
...
function buildAndPush() {
  ...
  const commit = gitStr("commit-tree", tree, "-p", parent, "-m", `CAS-2149 (CAS-2146): GO-LIVE flip GUARD_BREAK enabled:true (mec #38 Empujón/Rompe-Guardia anti-turtle) — config-only 1-blob overlay (sim/config.js), reversible — build ${build}`);
  try {
