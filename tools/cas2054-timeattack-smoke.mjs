...
for (const prof of PROFILES) {
  ...
  if (dark.ta === false) okp(`[${prof.name}] served DARK reaches browser: BOSS_RUSH.timeAttack === false`);
  else badp(`[${prof.name}] served timeAttack wrong (expected false DARK): ${dark.ta}`);
  if (dark.offKeys === "bestRound,v") okp(`[${prof.name}] OFF save shape = HEAD {v,bestRound} exactly (no record keys) ⇒ save byte-id: ${dark.offSerialize}`);
  else badp(`[${prof.name}] OFF save shape leaked record keys: ${dark.offSerialize}`);
  ...
  if (afterReload.ta === false && afterReload.bestTimeMs === savedBlob.bestTimeMs && afterReload.bestScore === savedBlob.bestScore)
    okp(`[${prof.name}] bootBossRush loads records forward even DARK (timeAttack:false): best time ${afterReload.bestTimeMs} / score ${afterReload.bestScore}`);
...
await browser.close();
console.log(anyFail ? "\nCAS-2054 BOSS RUSH TIME-ATTACK OBSERVABLE SMOKE — FAIL ✖" : "\nCAS-2054 BOSS RUSH TIME-ATTACK OBSERVABLE SMOKE — PASS ✓");
