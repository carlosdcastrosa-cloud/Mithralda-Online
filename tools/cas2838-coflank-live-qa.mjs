// CAS-2838 — POST-FLIP LIVE QA for TENAZA (CO_FLANK_SURGE.enabled:TRUE LIVE, EVO#145, 87º flag). Base master bdbb009 (322b7ab DARK → 548a31c flip false→true → bdbb009 version stamp 251653b6d194/815), served LIVE carlosdcastrosa-cloud.github.io/Mithralda-Online.
...
//
// Run: node tools/cas2838-coflank-live-qa.mjs
...
const OUT = join(ROOT, "shots", "cas2838");
...
try {
  ...
  // 2 LIVE-ON fresh boot (INVERTED from DARK): CO_FLANK_SURGE.enabled TRUE; solo hero fresh boot, SIN party sintética ⇒ F≤1 < midFlank2 ⇒ SCORE/DECISIÓN colapsan LIMPIO (tier/score/charge/preview=0). Multijugador-nativo: IMPOSIBLE pinzar en solitario. channel fresco coFlankFind + tag vacío. 🔑 NO asertamos gExists/partyExists false (con enabled:true las estructuras transitorias PUEDEN materializarse) — aserta el COLAPSO DE DECISIÓN.
  const dark = await page.evaluate(() => window.__dev.coFlank());
  ok("2 LIVE-ON (fresh solo boot): CO_FLANK_SURGE.enabled TRUE AND sin envolvimiento (F≤1) ⇒ score/tier/charge/preview colapsan LIMPIO a 0 AND channel coFlankFind AND tag vacío",
  ...
  // 6 ★ CRUX (LA CRÍTICA, PRIMARIO) ⊥#144 DIANA (CONTEO ⊥ GEOMETRÍA) — REAL server-auth
  ...
  ok("6f ★ CRUX ⊥#108 FLANK_SURGE (canal/lado/métrica DISTINTOS): CO_FLANK_SURGE.channel='coFlankFind' (VM idem) ⊥ FLANK_SURGE.channel='flankFind' — TENAZA = # sectores ENTERO de los JUGADORES alrededor de un mob ⊥ FLANK_SURGE = concentración angular float (resultant-length) de los RUMBOS de los MOBS",
  ...
  // 14 ★ 0-REGRESSION CENSUS (INVERTED LIVE-ON): served config — 73 `_SURGE` totales, 73 enabled:true, off=[] (CO_FLANK #145 LIVE + los 86 priores incl. CO_FOCUS #144).
  ...
  ok("14 ★ LIVE-ON 0-REGRESIÓN CENSUS (INVERTED): served config 73 `_SURGE` totales, 73 enabled:true, off=[] (CO_FLANK_SURGE #145 LIVE + CO_FOCUS_SURGE #144 LIVE + los 85 priores)",
  ...
  ok("16 ★ NORTH STAR 2-CLIENTE (MANDATORY sev-1): MISMO envolvimiento F=3 ⇒ score/idx/flank/players/tier/charge + flankProbeLive(field,flank,players,score) + flankProbe LUT (F enteros) + worldFingerprint IDÉNTICOS byte-a-byte (0 desync)",
     conv, `A={score:${A.score},idx:${A.idx},flank:${A.flank},players:${A.players},tier:${A.tier},charge:${A.charge},liveField:${A.liveField},liveFlank:${A.liveFlank},fpLen:${A.fp.length}} B={score:${B.score},idx:${B.idx},flank:${B.flank},players:${B.players},tier:${B.tier},charge:${B.charge},liveField:${B.liveField},liveFlank:${B.liveFlank},fpLen:${B.fp.length}} fpMatch=${A.fp === B.fp} fp=${A.fp}`);
  await page.evaluate(() => window.__dev.coFlank({ enabled: false }));
