...
const EXPECT_BUILD = "2b2cfd0d2313";   // CAS-2150 POST-FLIP LIVE (GUARD_BREAK.enabled:true, Gate CEO CAS-2149)
const OUT = "shots/cas2150";
const exe = findChromium(); if (!exe) { console.error("no chromium"); process.exit(1); }
mkdirSync(OUT, { recursive: true });
const browser = await puppeteer.launch({ executablePath: exe, headless: true, args: LAUNCH_ARGS });

const PROFILES = [
  { name: "desktop", vp: { width: 960, height: 600, deviceScaleFactor: 1 }, ua: null },   // CAS-2150 spec desk 960x600
  { name: "mobile",  vp: { width: 414, height: 820, deviceScaleFactor: 2, isMobile: true, hasTouch: true },   // CAS-2150 spec mob 414x820 DPR2
...
for (const prof of PROFILES) {
  ...
  const m = meta.m;
  // CAS-2150 POST-FLIP: served config must now be enabled:TRUE (mec #38 LIVE); other knobs byte-identical to QA-proven DARK.
  const metaOk = m.enabled === true && m.key === "Period" && m.range === 52 && m.arcDeg === 130
    && m.dmg === 5 && m.poiseMul === 3.2 && m.staminaCost === 20 && m.recoverMs === 600
    && m.knock === 1.7 && m.cracksShield === true && m.requiresMelee === false;
  if (metaOk && meta.cfgEnabled === true)
    P(`[${prof.name}] META: served GUARD_BREAK LIVE — enabled=TRUE, key=${m.key}, range=${m.range}, poiseMul=${m.poiseMul}, dmg=${m.dmg}, stam=${m.staminaCost}, recoverMs=${m.recoverMs}, knock=${m.knock}, cracksShield=${m.cracksShield}, requiresMelee=${m.requiresMelee}`);
