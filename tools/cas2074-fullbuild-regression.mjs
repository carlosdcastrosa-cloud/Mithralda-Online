...
const EXPECT_BUILD = "b5a1f024e42e"; // CAS-2074: bumped from d1043faa3064 after CAS-2073 Encounter Variants deploy (config/sim/render 3-blob byte-change, ENCOUNTER_VARIANTS enabled:false DARK ⇒ maybeVariant no-ops ⇒ runtime byte-identical to prior live). Prev CAS-2064 (d1043, a11y hitStop+flash toggles).
...
// CAS-2074: ENCOUNTER VARIANTS (30th mechanic) — ships DARK (enabled:false). Knob shape present +
// wired but 0-draw: maybeVariant returns on line 1 ⇒ spawns byte-identical to prior live. Behavioral
// OBSERVABLE proof (each variant EXHIBITS its hook) is in tools/cas2071-variants.mjs (node AC0-6) +
// tools/cas2071-variants-live-smoke.mjs (browser desktop+mobile). Here we assert the DARK contract only.
function auditVariants() {
  ...
  const dark = ev && ev.enabled === false;
  if (shapeOk && dark)
    pass(`[VARIANTS] ENCOUNTER_VARIANTS wired (30th mech, CAS-2071) — enabled:false DARK (0-draw), rngSeed=0x${(ev.rngSeed >>> 0).toString(16)}, variants=[stalker,bastion,glass], byZone opted-in — Gate CEO flips live (mirror CAS-2043/2055)`);
  else fail(`[VARIANTS] shapeOk=${shapeOk} dark=${dark} ev=${JSON.stringify(ev && { enabled: ev.enabled, seed: ev.rngSeed })}`);
}
...
log("");
if (ok) log(`✅ CAS-2074 HOLISTIC FULL-BUILD REGRESSION — ALL PASS (24 systems + 5 go-live flags + ENCOUNTER_VARIANTS(30th,DARK) + NG_PLUS + onboarding deep + 7 balance + JUICE + determinism on ${EXPECT_BUILD})`);
