// CAS-2307 — QA POST-FLIP LIVE OBSERVABLE for CLASIFICACIÓN DE ÓRDENES / ORDER STANDINGS (ORDER_STANDINGS.enabled:true LIVE via CTO/GE flip CAS-2306).
...
//
// Blocked-by CAS-2306 (flip, done). Differentiator vs the DARK observable (CAS-2305, 14/14 PASS build c4a549ae2fa1):
//   * served sim/config.js ORDER_STANDINGS.enabled:true (the flip shipped) + build self-consistent vs version.json (new build id).
//   * DEFAULT-ON — a FRESH boot with ZERO __dev flip has standings().enabled===true AND leader!==null (real players get the
//     competitive ranking layer off the real wall-clock, no injection).
//   * full-stack regression — the whole arc stack (LEDGER / OATH / EMISSARY / REWARDS / REP / BOUNTY_BOARD / WORLD_EVENT / RECALL, +
//     SAFEZONE/TEMPLE/RESTED) all still enabled:true, 0 regression; only the 3 known unrelated DARKs (BOSS_RUSH / DOORS / SEEDED) stay false.
//   * served-source presence — sim.js carries tickStandings/standingsRank/standingsMul and render.js carries renderStandingsRow +
//     sanctuaryStandingsTag ♛ draw (the DARK subsystem shipped across the consistent-HEAD 4-file overlay config+sim+game+render, NO input.js).
...
//   2  served config: ORDER_STANDINGS.enabled:true + full arc stack all true (0 regr); only 3 known DARKs false.
//   3  DEFAULT-ON: fresh boot ⇒ standings().enabled===true AND leader!==null (real-clock ranking), heroOrder null pre-pledge (0 __dev flip).
//   4  served-source presence: sim.js tickStandings+standingsRank+standingsMul; render.js renderStandingsRow+sanctuaryStandingsTag ♛.
//   5  RANKING PURO+válido: mismo nowMs ⇒ mismo leader/order (determinista) AND rangos 1..3 desc AND leader==rango1.
//   6  PLEDGE requerido: ON + sin juramento ⇒ heroOrder null AND mineLeading false AND standingsMul 0 (la clasificación existe igual).
//   7  LEADER passive por KNOB EFECTIVO: jurar la orden LÍDER ⇒ mineLeading true AND standingsMul 0.15 AND restedXpMult 1.5→1.65 (seam real de Descanso).
//   8  LEADER passive gateado a LIDERAR: jurar orden NO-líder ⇒ mineLeading false AND standingsMul 0 AND restedXpMult 1.5 (no basta jurar).
//   9  ROTACIÓN semanal: los 3 líderes rotan a lo largo de las semanas (rivalidad/identidad social cambiante, 0 RNG).
//  10  render nameplate ♛: NAV REAL + Date.now pin, orden del héroe LIDERANDO ⇒ se dibuja el ♛ (Δ px STANDINGS on vs off).
...
//  13  fps NO-REGRESIÓN (mediana-de-3) STANDINGS ON vs OFF (relativo, ON ≥ OFF*0.9).
//  14  *** CONVERGENCIA 2-CLIENTE REAL LIVE *** (North Star del ticket): 2 páginas LIVE, MISMO nowMs ⇒ `order`/leader IDÉNTICO
//      byte-a-byte (0 desync); heroOrder DIVERGE por cliente; A mata 500 ⇒ el `order` compartido de B NO cambia (sin contención). (AL FINAL)
//   0  no JS errors + no non-favicon 404 durante toda la corrida.
//
// LIVE wiring (mirror CAS-2304/2294): gh-pages ?dev=1; build compared to served version.json (NOT hardcoded — a later flip advances
// LIVE build); gh-pages favicon 404 filtered; both pages SHARE the localStorage origin ⇒ clear mithralda.* keys per page; the tick
// reads the REAL wall-clock every frame ⇒ inject nowMs SYNCHRONOUSLY in the same __dev.standings call (schedule pin, no sim freeze).
// Run: node tools/cas2307-standings-live-observable-qa.mjs   [optional LIVE base url]
...
const OUT = join(ROOT, "shots", "cas2307-live");
try { mkdirSync(OUT, { recursive: true }); } catch (e) {}

// SEMANAL: periodSec=604800 ⇒ periodMs=604800000. Semana con frac alto ⇒ baselines separados ⇒ ranking claro (misma ventana que la DARK).
const PERIOD_MS = 604800 * 1000;
const T_LATE = 5000 * PERIOD_MS + Math.floor(PERIOD_MS * 0.90);   // frac ≈ 0.90 (misma semana que la self-verify/DARK: leader='dawn')
...
try {
  ...
  const cfg = await page.evaluate(async (live) => {
    ...
    return { STANDINGS: en("ORDER_STANDINGS"), LEDGER: en("SANCTUARY_LEDGER"), OATH: en("SANCTUARY_OATH"), EMISSARY: en("SANCTUARY_EMISSARY"), REWARDS: en("SANCTUARY_REWARDS"),
  ...
  ok("2 served config: ORDER_STANDINGS.enabled:true + full arc stack all enabled:true (0 regr); only 3 known DARKs false",
     cfg.STANDINGS === "true" && allArcTrue && darksFalse, JSON.stringify(cfg));

  // 3 DEFAULT-ON (the LIVE proof): fresh boot ⇒ standings().enabled===true AND leader!==null, ZERO __dev flip, heroOrder null pre-pledge
  const dOn = await page.evaluate(() => { const d = window.__dev.standings(); return { enabled: d.enabled, leader: d.leader, heroOrder: d.heroOrder, mineLeading: d.mineLeading, n: d.order ? d.order.length : -1 }; });
  ok("3 DEFAULT-ON from served config: standings().enabled===true AND leader!==null at fresh boot (real-clock ranking), 0 __dev flip (heroOrder null pre-pledge)",
     dOn.enabled === true && dOn.leader !== null && dOn.heroOrder === null && dOn.mineLeading === false && dOn.n === 3, JSON.stringify(dOn));
  ...
  const src = await page.evaluate(async (live) => {
    ...
    return { simTick: /tickStandings/.test(sim), simRank: /standingsRank/.test(sim), simMul: /standingsMul/.test(sim),
      rndRow: /renderStandingsRow/.test(rnd), rndTag: /sanctuaryStandingsTag/.test(rnd) };
  }, LIVE);
  ok("4 served-source presence: sim.js has tickStandings+standingsRank+standingsMul; render.js has renderStandingsRow+sanctuaryStandingsTag ♛ (consistent-HEAD 4-file overlay live)",
     src.simTick && src.simRank && src.simMul && src.rndRow && src.rndTag, JSON.stringify(src));

  // 5 ranking puro + válido
  const valid = await page.evaluate((T) => {
    const a = window.__dev.standings({ enabled: true, nowMs: T }), b = window.__dev.standings({ nowMs: T });
  ...
  ok("5 RANKING PURO+válido: mismo nowMs ⇒ mismo leader/order (determinista) AND rangos 1..3 desc AND leader==rango1",
  ...
  // 6 pledge required: ON + no oath ⇒ heroOrder null, mineLeading false, standingsMul 0 (community ranking still exists)
  ...
  ok("6 PLEDGE requerido: ON + sin juramento ⇒ heroOrder null AND mineLeading false AND standingsMul 0 (clasificación existe igual)",
     noOath.heroOrder === null && noOath.mineLeading === false && noOath.mul === 0 && noOath.leaderExists === true, JSON.stringify(noOath));

  // 7 leader passive por KNOB EFECTIVO real: jurar la orden LÍDER ⇒ mineLeading true AND restedXpMult == base(1.5)+0.15
  await armOath(page);
  ...
  ok("7 LEADER passive por KNOB EFECTIVO: jurar la orden LÍDER ⇒ mineLeading true AND standingsMul 0.15 AND restedXpMult 1.5→1.65 (seam real de Descanso)",
  ...
  // 8 leader passive gateado a LIDERAR: jurar una orden NO-líder ⇒ mineLeading false, standingsMul 0, restedXpMult base
  await pledge(page, nonLeader);
  const gated = await page.evaluate((T) => {
    const s = window.__dev.standings({ enabled: true, nowMs: T });
    return { mineLeading: s.mineLeading, mul: s.standingsMulRestedMult, restedXpMult: s.restedXpMult, heroOrder: s.heroOrder };
  }, T_LATE);
  ok("8 LEADER passive gateado a LIDERAR: jurar orden NO-líder ⇒ mineLeading false AND standingsMul 0 AND restedXpMult 1.5 (no basta jurar)",
  ...
  // 9 rotación semanal: barre 60 semanas y cuenta líderes distintos
  ...
  ok("9 ROTACIÓN semanal: los 3 líderes rotan a lo largo de las semanas (rivalidad/identidad social cambiante, 0 RNG)",
  ...
  // 14 *** CONVERGENCIA 2-CLIENTE REAL LIVE *** (AL FINAL: abrir un 2º page blurea `page`, pero ya no quedan render-probes) — DOS páginas
  // LIVE independientes (2 clientes reales del shard), MISMO nowMs inyectado ⇒ el `order` (totales+rangos+leader) es IDÉNTICO byte-a-byte;
  ...
  ok("14 CONVERGENCIA 2-CLIENTE REAL LIVE: 2 páginas, mismo nowMs ⇒ `order`/leader IDÉNTICO (0 desync) AND heroOrder diverge AND sin contención",
