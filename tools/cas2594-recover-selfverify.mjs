// CAS-2594 — self-verify for REMATE DE RECOBRO (DARK, RECOVER_SURGE.enabled:false). EVO mecánica #100 (serializa tras #99 WINDUP_SURGE LIVE&served aca8e44656c4) — EJE FRESCO + CANAL FRESCO, ⊥ a las 41 LIVE #59-#99.
...
const OUT = join(ROOT, "shots", "cas2594");
...
try {
  ...
  ok("2 byte-id OFF (fresh boot): RECOVER_SURGE.enabled false AND G.recoverBounty NUNCA se crea (gExists false)",
     dark.enabled === false && dark.gExists === false && dark.tier === 0 && dark.score === 0 && dark.charge === 0 && dark.forageChargePreview === 0 && dark.channel === "recoverFind" && dark.tag === "",
  ...
  // 14 ★ 0-REGRESSION: 41 arc flags served true; RECOVER_SURGE served false
  ...
  const recoverDark = flag("RECOVER_SURGE") === "false";
  ok("14 ★ 0-REGRESIÓN: 41 mecanismos del arco #59-#99 served enabled:true; RECOVER_SURGE served false (DARK #100)",
     arcAllOn && recoverDark && arc.length === 41, `recover=${flag("RECOVER_SURGE")} arcAllOn=${arcAllOn} n=${arc.length} off=${JSON.stringify(arcLive.filter(([, v]) => v !== "true"))}`);
