// CAS-355 — bake a south-facing mob anim STRIP from PixelLab MCP frame URLs.
// usage: node tools/cas355-build-strip.mjs <mob> <state> <FW> <url0> <url1> ...
// writes assets/pixellab/fountains/anim/<mob>_<state>_strip.png  (N frames × FW×FW)
// Reuses the proven CAS-209 pure-JS PNG decode/encode + alpha-trim/center/foot-anchor.
...
// Trim transparent border, fit subject into N keeping aspect, anchor feet to bottom, center horizontally.
// SHARED bbox across all frames keeps the body baseline stable so the loop doesn't jitter in scale.
function fitFrame(src,sw,sh,N,bb){
  const {x0,y0,x1,y1}=bb; const bw=x1-x0+1, bh=y1-y0+1;
  const pad=Math.round(N*0.06), avail=N-pad*2;
  const s=Math.min(avail/bw, avail/bh);
  const dw=Math.round(bw*s), dh=Math.round(bh*s);
  const offx=Math.round((N-dw)/2), offy=N-pad-dh;
  const out=Buffer.alloc(N*N*4);
  for(let y=0;y<dh;y++)for(let x=0;x<dw;x++){
    const sx=x0+Math.floor(x*bw/dw), sy=y0+Math.floor(y*bh/dh);
    const si=(sy*sw+sx)*4, dx=offx+x, dy=offy+y; if(dx<0||dy<0||dx>=N||dy>=N)continue;
    const di=(dy*N+dx)*4; out[di]=src[si];out[di+1]=src[si+1];out[di+2]=src[si+2];out[di+3]=src[si+3];
  }
  return out;
}
function bbox(src,sw,sh){let x0=sw,y0=sh,x1=0,y1=0;for(let y=0;y<sh;y++)for(let x=0;x<sw;x++){if(src[(y*sw+x)*4+3]>16){if(x<x0)x0=x;if(x>x1)x1=x;if(y<y0)y0=y;if(y>y1)y1=y;}}if(x1<x0){x0=0;y0=0;x1=sw-1;y1=sh-1;}return{x0,y0,x1,y1};}
async function dl(u){const r=await fetch(u);if(!r.ok)throw new Error('dl '+r.status+' '+u);return Buffer.from(await r.arrayBuffer());}
(async()=>{
  const mob=process.argv[2], state=process.argv[3], FW=parseInt(process.argv[4],10), urls=process.argv.slice(5);
  const decoded=[];
  for(const u of urls){const b=await dl(u);decoded.push(decodePNG(b));}
  // union bbox across frames → stable scale/baseline across the whole anim
  let X0=1e9,Y0=1e9,X1=-1,Y1=-1;
  for(const d of decoded){const bb=bbox(d.rgba,d.W,d.H);X0=Math.min(X0,bb.x0);Y0=Math.min(Y0,bb.y0);X1=Math.max(X1,bb.x1);Y1=Math.max(Y1,bb.y1);}
  const bb={x0:X0,y0:Y0,x1:X1,y1:Y1};
  const frames=decoded.map(d=>fitFrame(d.rgba,d.W,d.H,FW,bb));
  ...
  const out=`assets/pixellab/fountains/anim/${mob}_${state}_strip.png`;
  fs.writeFileSync(out,encodePNG(FW*N,FW,strip));
  console.log(`OK ${mob}_${state}: ${N} frames @ ${FW}px -> ${out} (${FW*N}x${FW})`);
})();
