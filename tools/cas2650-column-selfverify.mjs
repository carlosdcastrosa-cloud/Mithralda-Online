// CAS-2650 — self-verify for REMATE DE COLUMNA (DARK, COLUMN_SURGE.enabled:false). EVO mecánica #109 (serializa tras #108 FLANK_SURGE LIVE&served 329b4a967c2d/815, base HEAD 0679723) — EJE FRESCO + CANAL FRESCO, ⊥ a las 50 LIVE #59-#108.
...
const OUT = join(ROOT, "shots", "cas2650");
...
try {
  ...
  ok("2 byte-id OFF (fresh boot): COLUMN_SURGE.enabled false AND G.columnBounty NUNCA se crea (gExists false)",
     dark.enabled === false && dark.gExists === false && dark.tier === 0 && dark.score === 0 && dark.elong === 0 && dark.charge === 0 && dark.forageChargePreview === 0 && dark.channel === "columnFind" && dark.tag === "",
  ...
  // 14 ★ 0-REGRESSION: 50 arc flags served true; COLUMN_SURGE served false
  ...
  const columnDark = flag("COLUMN_SURGE") === "false";
  ok("14 ★ 0-REGRESIÓN: 50 mecanismos del arco #59-#108 served enabled:true; COLUMN_SURGE served false (DARK #109)",
     arcAllOn && columnDark && arc.length === 50, `column=${flag("COLUMN_SURGE")} arcAllOn=${arcAllOn} n=${arc.length} off=${JSON.stringify(arcLive.filter(([, v]) => v !== "true"))}`);
