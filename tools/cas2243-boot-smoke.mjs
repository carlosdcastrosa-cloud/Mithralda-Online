// CAS-2243 — LIVE boot-smoke of the FLIPPED City Safe Zone deploy (build 4380e9b3aa6a). Proves the new module
// graph (render.js imports SAFEZONE from config.js) BOOTS on the live CDN with 0 JS errors (anti-CAS-2220),
// the __dev.safeZone hook mounts, SAFEZONE.enabled is TRUE (LIVE — sanctuary regen active), and every currently-LIVE
...
try {
  ...
  A.push(["SAFEZONE.enabled === true (LIVE — sanctuary active)", sz.enabled === true]);
  ...
  console.log(`\nCAS-2243 LIVE boot-smoke (build ${build || EXPECT_BUILD})\n`);
