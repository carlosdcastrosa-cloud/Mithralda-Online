// Screenshot the fixed left sidebar in-engine (desktop width) + one narrow width.
//   node tools/sidebar-shot.mjs
...
const errs=[];
async function run(w,h,name){
  const page=await (await browser.createBrowserContext()).newPage();
  page.on("pageerror",e=>errs.push(name+": "+e));
  page.on("console",m=>{ if(m.type()==="error" && !/Failed to load resource/.test(m.text())) errs.push(name+": "+m.text()); });
  ...
  await fr.evaluate(()=>{ document.getElementById("nameInput").value="Tibia"; window.dispatchEvent(new KeyboardEvent("keydown",{code:"Enter",key:"Enter",bubbles:true})); });
  ...
  // also open inventory via a sidebar button click (desktop) to prove buttons work
  await page.close();
}
await run(1280,800,"sidebar-desktop.png");
await run(1024,720,"sidebar-1024.png");
await run(600,900,"sidebar-mobile.png");
await browser.close(); await srv.close();
console.log(errs.length?("ERRORS:\n"+errs.slice(0,8).join("\n")):"no console errors");
console.log("wrote sidebar-desktop.png / sidebar-1024.png / sidebar-mobile.png");
