// CAS-2210 — hero anim pipeline continuation: slice PixelLab class anim frames into
// GAME-READY horizontal strips, uniform with CAS-2193 warrior. WALK + IDLE only
// (attack is BUDGET-BLOCKED: generations_remaining=0). idle is derived from the static
// rotation (2f breathing bob) at ZERO generation cost, same as the warrior.
// Config-driven: pass a JSON `tools/cas2210-<class>.json`:
//   { "class":"mage", "base":"https://.../<charId>/",
//     "walk":{ "down":[uuid,fc], "up":[uuid,fc], "side":[uuid,fc] } }
// Output: assets/class/{class}_{walk,idle}_{dir}.png + shots/cas2210/{class}_framedata.json
// Same GLOBAL transform (one scale/foot-line/center-x across all frames) as CAS-2193/2196.
...
const CLASS=CFG.class, BASE=CFG.base, WALK=CFG.walk;
...
(async()=>{
  console.log(`CAS-2210 slice ${CLASS} (${BASE.split('/').slice(-2,-1)[0]}) -> assets/class/  [walk+idle]`);
  const walk={}, rot={};
  const hasWalk = WALK && Object.keys(WALK).length===3;
  for(const dir of Object.keys(DIRS)){
    if(hasWalk){ const [uuid,fc]=WALK[dir]; const arr=[];
      for(let i=0;i<fc;i++){ const d=decodePNG(await dl(animURL(dir,uuid,i))); arr.push(d); }
      walk[dir]=arr; }
    rot[dir]=decodePNG(await dl(rotURL(dir)));
  }
  const bodyFrames=[...(hasWalk?[...walk.down,...walk.up,...walk.side]:[]),...Object.values(rot)];
  ...
  const fd={ [CLASS]:{ fc:{idle:ic,walk:wc,attack:0}, fw:FW, fh:FH, note:"attack BUDGET-BLOCKED (gen=0); idle=breathing bob from rotation" } };
  fs.mkdirSync('shots/cas2210',{recursive:true});
  fs.writeFileSync(`shots/cas2210/${CLASS}_framedata.json`, JSON.stringify(fd,null,2));
  console.log(`  ok frame-data -> shots/cas2210/${CLASS}_framedata.json  ${JSON.stringify(fd)}`);
