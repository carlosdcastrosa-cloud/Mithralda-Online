// CAS-2638 — POST-FLIP LIVE QA for REMATE DE RALEA ABIGARRADA (LIVE, MOTLEY_SURGE.enabled:true). EVO mecánica #106 = 48º flag LIVE&served 08a2a853adb1/815 (flip cf0cf76 + stamp 27323ff). Flag-ON port of DARK harness CAS-2634/CAS-2635/CAS-2636 (fp 15920977 0-desync) — 2 assertions flipped (check2 enabled:true STATELESS, check14 48 flags served true off=[]). EJE FRESCO + CANAL FRESCO, ⊥ a las 47 LIVE #59-#105.
...
//
// Run: node tools/cas2638-motley-live-qa.mjs
...
const OUT = join(ROOT, "shots", "cas2638");
...
try {
  ...
  // 2 LIVE fresh boot: enabled TRUE (48º flag LIVE) + STATELESS (motleyBounty transitorio, derivado ⇒ gExists false, empty-field score 0)
  const dark = await page.evaluate(() => window.__dev.motley());
  ok("2 LIVE (fresh boot): MOTLEY_SURGE.enabled TRUE (48º flag LIVE) AND estado STATELESS (gExists false: motleyBounty transitorio derivado; empty-field score/count 0)",
     dark.enabled === true && dark.gExists === false && dark.tier === 0 && dark.score === 0 && dark.count === 0 && dark.charge === 0 && dark.forageChargePreview === 0 && dark.channel === "motleyFind" && dark.tag === "",
  ...
  // 14 ★ 0-REGRESSION: 47 arc flags served true; MOTLEY_SURGE served true ⇒ 48 flags served true off=[]
  ...
  const motleyLive = flag("MOTLEY_SURGE") === "true";
  ok("14 ★ 0-REGRESIÓN: 47 mecanismos del arco #59-#105 served enabled:true; MOTLEY_SURGE served TRUE (LIVE #106, 48º flag) ⇒ 48 flags served true off=[]",
     arcAllOn && motleyLive && arc.length === 47, `motley=${flag("MOTLEY_SURGE")} arcAllOn=${arcAllOn} n=${arc.length + 1} off=${JSON.stringify(arcLive.filter(([, v]) => v !== "true").concat(motleyLive ? [] : [["MOTLEY_SURGE", flag("MOTLEY_SURGE")]]))}`);
