...
function buildAndPush() {
  ...
  const idx = "/tmp/cas2082fix-index";
  ...
  const commit = gitStr("commit-tree", tree, "-p", parent, "-m", `CAS-2082 (fix CAS-2080): Deploy render.js PACTS-import fix — 1 blob (render.js), restores minimap/XP/vitals + D1 badge, reversible via revert — build ${build}`);
  try {
