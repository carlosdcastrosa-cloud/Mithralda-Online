...
const OUT = join(ROOT, "shots", "cas2792");
...
try {
  ...
  ok("2 byte-id OFF (fresh boot): CO_STRIKE_SURGE.enabled false AND G.coStrikeBounty NUNCA se crea (gExists false) AND G._coStrikeParty NUNCA se crea (partyExists false)",
     dark.enabled === false && dark.gExists === false && dark.partyExists === false && dark.tier === 0 && dark.score === 0 && dark.idx === 0 && dark.players === 0 && dark.engaged === 0 && dark.muster === 0 && dark.charge === 0 && dark.forageChargePreview === 0 && dark.channel === "coStrikeFind" && dark.tag === "",
  ...
  const censusOK = total === 64 && trues === 63 && falses.length === 1 && falses[0] === "CO_STRIKE_SURGE" && rankSpreadLive && rankSpreadLive[1] === "true";
  ok("15 ★ 0-REGRESIÓN CENSUS: served config 64 `_SURGE` totales, 63 enabled:true (incl. RANK_SPREAD_SURGE #135 LIVE), sole-false = CO_STRIKE_SURGE (DARK #136)",
     censusOK, `total=${total} true=${trues} false=${JSON.stringify(falses)} rankSpread=${rankSpreadLive ? rankSpreadLive[1] : "?"}`);
