// CAS-2655 — self-verify for REMATE DE DESBANDADA (DARK, ORIENT_SURGE.enabled:false). EVO mecánica #110 (serializa tras #109 COLUMN_SURGE LIVE&served bc21f1f7ac8a/815, base HEAD c1e1ee9) — EJE FRESCO + CANAL FRESCO, ⊥ a las 51 LIVE #59-#109.
...
const OUT = join(ROOT, "shots", "cas2655");
...
try {
  ...
  ok("2 byte-id OFF (fresh boot): ORIENT_SURGE.enabled false AND G.orientBounty NUNCA se crea (gExists false)",
     dark.enabled === false && dark.gExists === false && dark.tier === 0 && dark.score === 0 && dark.spread === 0 && dark.charge === 0 && dark.forageChargePreview === 0 && dark.channel === "orientFind" && dark.tag === "",
  ...
  // 14 ★ 0-REGRESSION: 51 arc flags served true; ORIENT_SURGE served false
  ...
  const orientDark = flag("ORIENT_SURGE") === "false";
  ok("14 ★ 0-REGRESIÓN: 51 mecanismos del arco #59-#109 served enabled:true; ORIENT_SURGE served false (DARK #110)",
     arcAllOn && orientDark && arc.length === 51, `orient=${flag("ORIENT_SURGE")} arcAllOn=${arcAllOn} n=${arc.length} off=${JSON.stringify(arcLive.filter(([, v]) => v !== "true"))}`);
