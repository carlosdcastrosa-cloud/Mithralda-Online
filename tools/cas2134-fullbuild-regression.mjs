...
const EXPECT_BUILD = "272d22870026"; // CAS-2134: bumped from cdf6e8bdcbea after CAS-2132 Gate CEO GO flip RIPOSTE.enabled false→true (mec#36 LIVE, config-only 1-line, reversible). Live config md5 6bfa3bb9c4d49a41371c935cb57f4da7 == HEAD (commit 27b96dc). Only sim/config.js changed vs DARK build (sim/strings byte-id). RIPOSTE now enabled:true ⇒ live counter-execution seams active. First fresh full-build regression against the post-flip #36 live.
...
function auditRiposte() {
  ...
  const live = r && r.enabled === true;                   // CAS-2132 Gate CEO GO — flipped false→true. Now LIVE.
  const valsOk = r && r.dmgMul === 2.2 && r.poiseMul === 1.5 && r.essenceBonus === 2
    && r.ripCapFracMaxHp === 0.25 && r.requiresMelee === true;
  if (shapeOk && live && valsOk)
    pass(`[RIPOSTE] RIPOSTE LIVE (36th mech, CAS-2132 Gate flip) — enabled:true (Ejecución Crítica por Aturdimiento — 1st melee on a broken target = execution, config-only 1-line flip vs DARK, sim/strings byte-id), full knob shape dmgMul=${r.dmgMul}/poiseMul=${r.poiseMul}/essenceBonus=${r.essenceBonus}/ripCapFracMaxHp=${r.ripCapFracMaxHp} (Riposte≤${r.ripCapFracMaxHp}×maxHp on boss/elite), requiresMelee=${r.requiresMelee} — RNG-neutral (0 riposteRng, keyed by enemy e.staggerT/e._ripArm state) + save-neutral (e._ripArm transient) ⇒ srand ON==OFF & save.v1 byte-id; DISJOINT from CAS-210 h.riposte/CFG.riposteMult (hero perfect-dodge crit)`);
...
function determinism() {
  ...
  if (a === b && !a.startsWith("ERR:")) pass(`[DETERM] sim cycle fingerprint identical across 2 runs (len ${a.length}) — 36 mechanics (Arena Hazards LIVE gate-closed w/o boss + Guard/Dodge Counter LIVE arm-only-on-perfect-input + Rally LIVE damage-taken pool + Riposte LIVE enemy-broken-only, keyed by e.staggerT/e._ripArm) add 0 draws to a normal run`);
...
log("");
if (ok) log(`✅ CAS-2134 HOLISTIC FULL-BUILD REGRESSION — ALL PASS (24 systems + 5 go-live + TIME-ATTACK(29) + VARIANTS(30,LIVE) + SEEDED CHALLENGE(31,LIVE) + ARENA HAZARDS(32,LIVE) + GUARD COUNTER(33,LIVE) + DODGE COUNTER(34,LIVE) + RALLY(35,LIVE) + RIPOSTE(36,LIVE) + NG_PLUS + onboarding + 7 balance + JUICE + determinism on ${EXPECT_BUILD})`);
