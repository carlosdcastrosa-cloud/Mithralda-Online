// CAS-2218 — hero anim pipeline COMPLETION: slice PixelLab class anim frames into
// GAME-READY horizontal strips, uniform with CAS-2193 warrior / CAS-2210 walk+idle.
// Adds ATTACK (budget now approved, CAS-2213) alongside WALK + derived IDLE.
//
// KEY INVARIANT: the locomotion transform (scale / center-x / foot-line) is computed
// from WALK + ROTATION frames ONLY — identical basis to CAS-2210 — so re-slicing walk/idle
// here reproduces the committed strips. Attack frames are placed with that SAME transform
// (feet planted on the same baseline, body centered), so attack lines up with walk/idle.
// Only the frame WIDTH (FW) considers attack extent, and grows (mult of 8, cap 88) if an
// attack pose is wider than the locomotion body; walk/idle re-slice at that FW for consistency.
//
// Config JSON `tools/cas2218-<class>.json`:
//   { "class":"mage", "base":"https://.../<userId>/<charId>/",
//     "walk":{ "down":[uuid,fc], "up":[uuid,fc], "side":[uuid,fc] },
//     "attack":{ "down":[uuid,fc], "up":[uuid,fc], "side":[uuid,fc] } }
//   (attack optional — omit to reproduce CAS-2210 walk+idle only.)
// Output: assets/class/{class}_{walk,idle,attack}_{dir}.png + shots/cas2218/{class}_framedata.json
...
let FW=64; const FH=64, PAD=5;
const CFG=JSON.parse(fs.readFileSync(process.argv[2],'utf8'));
const CLASS=CFG.class, BASE=CFG.base, WALK=CFG.walk, ATK=CFG.attack;
...
(async()=>{
  console.log(`CAS-2218 slice ${CLASS} (${BASE.split('/').slice(-2,-1)[0]}) -> assets/class/  [walk+idle+attack]`);
  const walk={}, atk={}, rot={};
  const hasWalk = WALK && Object.keys(WALK).length===3;
  const hasAtk  = ATK  && Object.keys(ATK).length===3;
  for(const dir of Object.keys(DIRS)){
    if(hasWalk){ const [uuid,fc]=WALK[dir]; const arr=[]; for(let i=0;i<fc;i++) arr.push(decodePNG(await dl(animURL(dir,uuid,i)))); walk[dir]=arr; }
    if(hasAtk){ const [uuid,fc]=ATK[dir]; const arr=[]; for(let i=0;i<fc;i++) arr.push(decodePNG(await dl(animURL(dir,uuid,i)))); atk[dir]=arr; }
    rot[dir]=decodePNG(await dl(rotURL(dir)));
  }
  // Locomotion basis: WALK + ROTATION only (identical to CAS-2210) → protects the committed
  // scale / center-x / foot baseline from attack-pose distortion (raised arms, weapon dip).
  const locoFrames=[...(hasWalk?[...walk.down,...walk.up,...walk.side]:[]),...Object.values(rot)];
  let BX0=1e9,BY0=1e9,BX1=-1,BY1=-1;
  for(const d of locoFrames){ const b=bbox(d.rgba,d.W,d.H); if(b.x1<0)continue;
  ...
  // FW basis: locomotion + attack width, so a wide attack pose is not clipped.
  const widthFrames=[...locoFrames,...(hasAtk?[...atk.down,...atk.up,...atk.side]:[])];
  let maxHalf=0;
  for(const d of widthFrames){ const b=bbox(d.rgba,d.W,d.H); if(b.x1<0)continue;
    maxHalf=Math.max(maxHalf,(b.x1-gcx)*scale,(gcx-b.x0)*scale); }
  FW=Math.min(88, Math.max(64, Math.ceil((maxHalf+PAD)*2/8)*8));
  console.log(`  loco bbox ${BX1-BX0+1}x${bodyH} center-x ${gcx.toFixed(1)} scale=${scale.toFixed(3)} -> char h~${Math.round(bodyH*scale)}px; FW=${FW}`);
  let wc=0,ic=0,ac=0;
  for(const dir of Object.keys(DIRS)){
    if(hasWalk) wc=writeStrip(`${CLASS}_walk_${dir}`, walk[dir].map(d=>place(d.rgba,d.W,d.H,scale,gcx,gfeet,0)));
    const r=rot[dir]; ic=writeStrip(`${CLASS}_idle_${dir}`,[ place(r.rgba,r.W,r.H,scale,gcx,gfeet,0), place(r.rgba,r.W,r.H,scale,gcx,gfeet,-1) ]);
    if(hasAtk) ac=writeStrip(`${CLASS}_attack_${dir}`, atk[dir].map(d=>place(d.rgba,d.W,d.H,scale,gcx,gfeet,0)));
  }
  const fc={idle:ic}; if(hasWalk)fc.walk=wc; if(hasAtk)fc.attack=ac;
  const fd={ [CLASS]:{ fc, fw:FW, fh:FH, note:"CAS-2218: walk+idle+attack, one loco transform (walk+rot basis); idle=breathing bob" } };
  fs.mkdirSync('shots/cas2218',{recursive:true});
  fs.writeFileSync(`shots/cas2218/${CLASS}_framedata.json`, JSON.stringify(fd,null,2));
  console.log(`  ok frame-data -> shots/cas2218/${CLASS}_framedata.json  ${JSON.stringify(fd)}`);
