// CAS-2632 — POST-FLIP LIVE QA for REMATE DE PONZOÑA (LIVE, BANE_SURGE.enabled:true). EVO mecánica #105 = 47º flag LIVE&served 8e1b7d472e89/815 (flip 13ffed2/ba394fc + stamp 9b39af1). Flag-ON port of DARK harness CAS-2627/CAS-2628 (fp 15920977 0-desync) — 2 assertions flipped (check2 enabled:true, check14 47 flags served true off=[]). EJE FRESCO + CANAL FRESCO, ⊥ a las 46 LIVE #59-#104.
...
//
// Run: node tools/cas2632-bane-live-qa.mjs
...
const OUT = join(ROOT, "shots", "cas2632");
...
try {
  ...
  // 2 LIVE fresh boot: enabled TRUE (47º flag LIVE) + STATELESS (baneBounty transitorio, derivado ⇒ gExists false)
  const dark = await page.evaluate(() => window.__dev.bane());
  ok("2 LIVE (fresh boot): BANE_SURGE.enabled TRUE (47º flag LIVE) AND estado STATELESS (gExists false: baneBounty transitorio, derivado)",
     dark.enabled === true && dark.gExists === false && dark.tier === 0 && dark.score === 0 && dark.charge === 0 && dark.forageChargePreview === 0 && dark.channel === "baneFind" && dark.tag === "",
  ...
  // 14 ★ 0-REGRESSION: 46 arc flags served true; BANE_SURGE served true ⇒ 47 flags served true off=[]
  ...
  const baneLive = flag("BANE_SURGE") === "true";
  ok("14 ★ 0-REGRESIÓN: 46 mecanismos del arco #59-#104 served enabled:true; BANE_SURGE served TRUE (LIVE #105, 47º flag) ⇒ 47 flags served true off=[]",
     arcAllOn && baneLive && arc.length === 46, `bane=${flag("BANE_SURGE")} arcAllOn=${arcAllOn} n=${arc.length + 1} off=${JSON.stringify(arcLive.filter(([, v]) => v !== "true").concat(baneLive ? [] : [["BANE_SURGE", flag("BANE_SURGE")]]))}`);
