// CAS-2807 — self-verify for SOCORRO (DARK, CO_SUPPORT_SURGE.enabled:false). EVO mecánica #139 (base master d58d6736 tras #138 CUADRILLA CO_KILL LIVE&served 9527556851d4/815) — EJE FRESCO + CANAL FRESCO, ⊥ a los 80 LIVE #59-#138. El 21º eje de COMPOSICIÓN-DE-INTENCIÓN y la CUARTA faceta de la sub-familia PLAYER-COORDINATION (#136 CÓNCLAVE=ENGANCHADOS, #137 COHORTE=PRESENTES, #138 CUADRILLA=REMATAN).
...
// (B) CANAL FRESCO = coSupportFind (fichas de socorro por rematar con el socorro ACREDITADO — NINGUNO de los 80 flags lo usa). Moneda FRESCA (h.coSupportBounty, TRANSITORIA, fuera del save allowlist + worldFingerprint).
...
//
// Run: node tools/cas2807-cosupport-selfverify.mjs
...
const OUT = join(ROOT, "shots", "cas2807");
...
try {
  ...
  ok("2 byte-id OFF (fresh boot): CO_SUPPORT_SURGE.enabled false AND G.coSupportBounty NUNCA se crea (gExists false) AND G._coSupportParty NUNCA se crea (partyExists false)",
     dark.enabled === false && dark.gExists === false && dark.partyExists === false && dark.tier === 0 && dark.score === 0 && dark.idx === 0 && dark.aid === 0 && dark.charge === 0 && dark.forageChargePreview === 0 && dark.channel === "coSupportFind" && dark.tag === "",
  ...
  ok("3 byte-id save OFF: sin clave 'coSupportFind'/'coSupportBounty' (fichas transitorias, fuera del save allowlist)", noFeatKey && noChargeKey, `noFeatKey=${noFeatKey} noChargeKey=${noChargeKey} len=${saveOff.length}`);
  ...
  const cruxKill = await page.evaluate(() => {
    ...
    // {3 jugadores: 2 sanando, 1 rematando}: SOCORRO S2/w1 pero CUADRILLA K1/w0 (sanadores landean 0 kills)
    const A = { aid: aid(2, 3), kill: kill(1, 3) };
    // {mismos 3: 1 sanando, 3 rematando}: SOCORRO S1/w0 pero CUADRILLA K3/w2 (DPS soleando, 0 socorro)
  ...
  const cruxPres = await page.evaluate(() => {
    ...
    // {4 presentes, 0 sanando}: COHORTE R4/w2 vs SOCORRO S0/w0 (party llena presente pero nadie socorre)
    const A = { aid: aid(0, 4), pres: pres(4, 4) };
    // {2 presentes, 2 sanando}: COHORTE R2/w1 y SOCORRO S2/w1
  ...
  const cruxCon = await page.evaluate(() => {
    ...
    // {2 enganchados por 2 mobs, 0 sanando}: CÓNCLAVE F2/w1 pero SOCORRO S0/w0 (ambos tanqueando, nadie socorre)
    const A = { aid: aid(0, 2), muster: muster([0, 1], 2) };
    // {refriega mínima/sin muster (1 mob ⇒ CÓNCLAVE gated 0), 2 sanando}: CÓNCLAVE w0 pero SOCORRO S2/w1 (2 sanadores no-enganchados sostienen)
  ...
  const cruxVital = await page.evaluate(() => {
    ...
    // {party full-HP, NADIE sanando}: TEMPLE W1.0/w2 (sana) pero SOCORRO S0/w0 (nadie ACTÚA)
    const A = { aid: aid(0, 3), vital: vital([1, 1, 1], 3) };
    // {party maltrecha 20%, 3 sanando}: TEMPLE W0.2/w0 (moribunda) pero SOCORRO S3/w2 (3 ACTÚAN para sostenerla)
  ...
  // 14 ★ 0-REGRESSION CENSUS: served config — 67 `_SURGE` totales, sole-false = CO_SUPPORT (66 true, incl. CO_KILL #138 LIVE).
  ...
  const censusOK = total === 67 && trues === 66 && falses.length === 1 && falses[0] === "CO_SUPPORT_SURGE" && coKillLive && coKillLive[1] === "true";
  ok("14 ★ 0-REGRESIÓN CENSUS: served config 67 `_SURGE` totales, 66 enabled:true (incl. CO_KILL_SURGE #138 LIVE), sole-false = CO_SUPPORT_SURGE (DARK #139)",
     censusOK, `total=${total} true=${trues} false=${JSON.stringify(falses)} coKill=${coKillLive ? coKillLive[1] : "?"}`);
