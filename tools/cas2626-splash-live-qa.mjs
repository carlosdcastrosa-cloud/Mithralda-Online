// CAS-2626 — POST-FLIP LIVE QA for REMATE DE ESTALLIDO (LIVE, SPLASH_SURGE.enabled:true). EVO mecánica #104 = 46º flag LIVE&served 7cf773339112/815 (flip 53228a3 + stamp fbdb24b). Flag-ON port of DARK harness CAS-2620/CAS-2621 (fp 15920977 0-desync) — 2 assertions flipped (check2 enabled:true, check14 46 flags served true off=[]). EJE FRESCO + CANAL FRESCO, ⊥ a las 45 LIVE #59-#103.
...
// ★ 0-REGRESIÓN (check 14): las 45 mecánicas del arco #59-#103 siguen served enabled:true; SPLASH_SURGE served TRUE (LIVE #104, 46º flag) ⇒ 46 flags served true off=[].
...
//
// Run: node tools/cas2626-splash-live-qa.mjs
...
const OUT = join(ROOT, "shots", "cas2620");
...
try {
  ...
  ok("2 LIVE (fresh boot): SPLASH_SURGE.enabled TRUE (46º flag LIVE) AND estado STATELESS (gExists false: blastBounty transitorio, derivado)",
     dark.enabled === true && dark.gExists === false && dark.tier === 0 && dark.score === 0 && dark.charge === 0 && dark.forageChargePreview === 0 && dark.channel === "splashFind" && dark.tag === "",
  ...
  ok("14 ★ 0-REGRESIÓN: 45 mecanismos del arco #59-#103 served enabled:true; SPLASH_SURGE served TRUE (LIVE #104, 46º flag) ⇒ 46 flags served true off=[]",
