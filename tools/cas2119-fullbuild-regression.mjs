...
const EXPECT_BUILD = "f2305f6b890d"; // CAS-2119: bumped from 37521ab19b4c after CAS-2111 flip DODGE_COUNTER→LIVE (mec#34) + CAS-2114 Build+Deploy DARK Rally/Regain (mec#35). Served blobs (byte-id to HEAD): config 39cffe4dfcd0f17660f60a6d2653b1a3 / sim 336ee2289e23e28162e336bd732abc72 / render ef21e533b90b912e859da36c47004077 / game 4192c674 / input 01491366 / persist 4fb4e887. RALLY.enabled=false ⇒ dead branches ⇒ byte-id to mec#34 in behavior.
...
function auditRally() {
  ...
  const dark = r && r.enabled === false;   // SHIP DARK; Gate CEO flips false→true (config-only, reversible)
  ...
  if (shapeOk && dark && valsOk)
    pass(`[RALLY] RALLY DARK (35th mech, CAS-2114) — enabled:FALSE (dead branches ⇒ byte-id to mec#34 build), full knob shape recoverFrac=${r.recoverFrac}/windowS=${r.windowS}/healPerHitFrac=${r.healPerHitFrac}/decayPerSec=${r.decayPerSec}/capFracMaxHp=${r.capFracMaxHp} (pool≤${r.capFracMaxHp}×maxHp), requiresMelee=${r.requiresMelee} — RNG-neutral (0 rallyRng) + save-neutral (h.rallyPool/h.rallyT transient) ⇒ srand ON==OFF & save.v1 byte-id; orthogonal to dodge/guard-counter (heals a damage-taken pool, no swing amplify)`);
  else fail(`[RALLY] shapeOk=${shapeOk} dark=${dark} valsOk=${valsOk} r=${JSON.stringify(r)}`);
}
...
function determinism() {
  ...
  if (a === b && !a.startsWith("ERR:")) pass(`[DETERM] sim cycle fingerprint identical across 2 runs (len ${a.length}) — 35 mechanics (Arena Hazards LIVE gate-closed w/o boss + Guard Counter LIVE + Dodge Counter LIVE arm-only-on-perfect-roll + Rally DARK) add 0 draws to a normal run`);
...
log("");
if (ok) log(`✅ CAS-2119 HOLISTIC FULL-BUILD REGRESSION — ALL PASS (24 systems + 5 go-live + TIME-ATTACK(29) + VARIANTS(30,LIVE) + SEEDED CHALLENGE(31,LIVE) + ARENA HAZARDS(32,LIVE) + GUARD COUNTER(33,LIVE) + DODGE COUNTER(34,LIVE) + RALLY(35,DARK) + NG_PLUS + onboarding + 7 balance + JUICE + determinism on ${EXPECT_BUILD})`);
else { console.error("❌ CAS-2119 HOLISTIC FULL-BUILD REGRESSION — FAIL"); process.exit(1); }
