// CAS-2624 — POST-FLIP LIVE QA for REMATE DE ESTALLIDO (LIVE, SPLASH_SURGE.enabled:TRUE, 46º flag). EVO mecánica #104 SHIPPED (served 7cf773339112/815 == HEAD fbdb24b). Flag-ON port of GE DARK cas2620-splash-selfverify.mjs — flips check2 (fresh-boot enabled:true) + check14 (SPLASH served true, 46 arc flags true off=[]). All other checks toggle the flag via __dev so they hold in both states. Byte-neutrality: local tree == served build proven by CEO 7/7 byte-verify (served sim/config.js sha256==HEAD blob) ⇒ runtime harness on local server is behaviorally identical to LIVE served build.
// Run: node tools/cas2624-splash-live-qa.mjs
...
//
// Run: node tools/cas2620-splash-selfverify.mjs
...
const OUT = join(ROOT, "shots", "cas2624");
...
try {
  ...
  // 2 LIVE fresh boot: SPLASH_SURGE.enabled TRUE (46º flag LIVE); state STATELESS (gExists false: blastBounty transient, 100% derived)
  ...
  const allLive = arcLive.concat([["SPLASH_SURGE", flag("SPLASH_SURGE")]]);
  ok("14 ★ 0-REGRESIÓN: 46 mecanismos del arco #59-#104 served enabled:true; SPLASH_SURGE served TRUE (LIVE #104, 46º flag) off=[]",
     arcAllOn && splashLive && arc.length === 45, `splash=${flag("SPLASH_SURGE")} arcAllOn=${arcAllOn} n=${allLive.length} off=${JSON.stringify(allLive.filter(([, v]) => v !== "true"))}`);
